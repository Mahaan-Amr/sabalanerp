...
The audited Partner profile lifecycle `فعال`, `معلق`, or `خاتمه‌یافته`, distinct from the User account's global login state. Direct user-profile conversion creates Active immediately after Price Responder selection; legacy `در انتظار تکمیل` profiles retain their identity and become Active through that same action rather than being recreated. Suspended preserves login and read-only history while freezing mutation, and Terminated blocks login and new activity while preserving committed Accounting, correction, and fulfillment.
_Avoid_: creating a new multi-stage Pending onboarding flow, discarding a legacy Pending profile, treating the profile as an ordinary Role or `isActive` toggle, restoring old internal grants through generic reactivation, letting a responder decide suspended work, deleting history at termination, stopping committed downstream work, or changing lifecycle without accountable audit evidence
...
The system-wide availability boundary for the Partner Seller capability after its accepted release. It has no named cohort or per-User enrollment: every directly activated Partner Seller may begin work immediately, while a separate operational emergency pause may freeze all uncommitted Partner and responder mutation without disturbing authorized reads, remediation, or committed Accounting and fulfillment.
_Avoid_: requiring cohort selection or enrollment during activation, delaying an activated Partner behind a per-User rollout gate, conflating system availability with an operational emergency pause, turning off committed downstream obligations, auto-converting existing Users or Contracts, backfilling invented Partner evidence, or restricting ordinary Sales through Partner policies
...
**توقف اضطراری عملیاتی فروشنده همکار**:
The automatic containment of uncommitted Partner and Price Responder work after a confirmed cross-owner disclosure, wholesale disclosure to a Customer, incomplete document pair, duplicate commitment, or unexplained financial or quantity divergence. It preserves evidence, authorized reads, support cancellation, internal remediation, and committed Accounting and fulfillment under their own integrity controls; ordinary delays or correctly rejected requests do not alone trigger it.
_Avoid_: confusing emergency containment with enrollment pause, erasing accepted business facts, stopping all committed obligations indiscriminately, allowing downstream work on invalid evidence, or resuming before the cause is corrected and acceptance is re-established
