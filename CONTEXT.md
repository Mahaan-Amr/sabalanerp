...
ترکیب آغازین الگوی Job فروشنده است که سی درصد امتیاز نهایی را از معیارهای رفتاری و هفتاد درصد را از معیارهای عملکردی می‌سازد. از کل نتیجه، داده کمی عملکردی سامانه شصت‌وسه درصد، قضاوت عملکردی Supervisor هفت درصد، قضاوت رفتاری Supervisor دوازده درصد، نظرسنجی رفتاری همکاران نه درصد و داده رفتاری معتبر سامانه نه درصد سهم دارند؛ بنابراین داده سامانه در مجموع هفتادودو درصد نتیجه را می‌سازد.
...
کمپین زمان‌دار و نسخه‌داری است که User دارای مجوز مستقل مدیریت نظرسنجی برای جامعه مشخصی از Personnelهای هدف و پاسخ‌دهندگان دارای ارتباط کاری معتبر فعال می‌کند. هر گزاره رفتاری مقیاس ثابت `کاملاً مخالفم`، `مخالفم`، `نه موافقم نه مخالف`، `موافقم` و `کاملاً موافقم` دارد؛ `اطلاعات کافی برای ارزیابی ندارم` گزینه بی‌امتیاز جداگانه است. پرسش، گزینه، ترتیب و قواعد امتیاز در نسخه فعال قفل می‌شوند و تغییر بعد از آغاز پاسخ‌گویی فقط با نسخه یا کمپین تازه ممکن است.
_Avoid_: رأی مستقیم به Badge، نظرسنجی همه درباره همه بدون ارتباط کاری، ویرایش معنای سؤال یا گزینه پس از دریافت پاسخ، پاسخ به خود، یا استنتاج اختیار مدیریت نظرسنجی از عنوان سازمانی

**پاسخ محرمانه نظرسنجی رفتاری**:
پاسخ هویت‌داری است که Personnel هدف و سایر همکاران، هویت پاسخ‌دهنده یا پاسخ فردی را نمی‌بینند و نتیجه فقط پس از رسیدن به حداقل جمعیت مصوب به‌صورت تجمیعی قابل‌استفاده است. هویت برای جلوگیری از تکرار و رسیدگی محدود به تخلف حفظ می‌شود؛ متن آزاد امتیاز مستقل ندارد و پیش از هر تحویل انسانی نیازمند رسیدگی مجاز است.
_Avoid_: وعده ناشناس‌بودن مطلق، نمایش پاسخ خام یا هویت به Personnel هدف، امتیازسازی از گروه کوچک، رأی تکراری، افشای متن آزاد بدون رسیدگی، یا دسترسی عمومی مدیران
...
یکی از دو بازه ثابت شش‌ماهه فروردین تا پایان شهریور و مهر تا پایان اسفند است که نتیجه نهایی و Badge رسمی فروشنده از شواهد ثابت همان دوره ساخته می‌شود. پیشرفت و نقص داده می‌تواند ماهانه نمایش داده شود، اما Badge رسمی فقط پس از پایان دوره، بازبینی نتیجه و پایان فرصت اعتراض تغییر می‌کند و نظرسنجی رفتاری می‌تواند بدون سقف تعداد در طول دوره اجرا شود. ارزیابی رسمی دست‌کم نود روز مأموریت مؤثر و نمونه معتبر هر نرخ را می‌خواهد؛ هدف‌های مبلغی و تعدادی با روزهای مؤثر تعدیل می‌شوند و تغییر Job یا Position بخش جدا می‌سازد.
_Avoid_: تغییر ماهانه Badge رسمی، استفاده از داده بیرون دوره، نهایی‌سازی پیش از بازبینی و اعتراض، محدودکردن تعداد نظرسنجی، امتیازسازی زیر نود روز، هدف کامل برای حضور بخشی، نرخ بدون حداقل نمونه، نسبت‌دادن کل دوره به آخرین مأموریت، یا ترکیب دو رابطه استخدامی
...
هفت گروه نسخه‌دار `آورده فروش` با وزن بیست‌ودو، `وصول و پیگیری مالی` با وزن پانزده، `سرعت گردش قرارداد` با وزن هشت، `کیفیت و دقت قرارداد` با وزن هفت، `لغو، مرجوعی و برگشت سفارش` با وزن پنج، `مدیریت مشتری و فرصت فروش` با وزن هشت و `کیفیت تعهد تحویل` با وزن پنج درصد از نتیجه کل هستند. شصت‌وسه درصد از داده معتبر سامانه و هفت درصد از قضاوت زمینه‌ای Supervisor می‌آید؛ هر عامل فقط با مخرج، بازه، انتساب و دلیل قابل‌اعتماد امتیاز می‌سازد و رخداد خارج از کنترل فروشنده زمینه است نه جریمه.
_Avoid_: غالب‌کردن مبلغ خام فروش، شمارش بدون مخرج، جریمه تأخیر واحد دیگر، انتساب لغو بدون دلیل، امتیازسازی از داده متعارض، تکرار عدد سامانه در قضاوت Supervisor، یا دوباره‌شماری یک رویداد در چند عامل
...
مسیر رسمی پایان دوره است که پس از تثبیت و کنترل داده، تکمیل ارزیابی انسانی و نظرسنجی، محاسبه قابل‌بازسازی سامانه و بررسی نقص داده، نتیجه را به Personnel نشان می‌دهد و هفت روز تقویمی برای اعتراض می‌دهد. HR Manager اعتراض را بررسی می‌کند؛ پس از پایان این مسیر نتیجه قفل، Badge جاری جایگزین و استفاده رسمی مجاز می‌شود، بی‌آنکه HR Processor یا Company Manager امتیاز را مستقیم بازنویسی کند.
_Avoid_: اثر رسمی پیش از فرصت اعتراض، ویرایش مستقیم امتیاز، تغییر بی‌دلیل Company Manager، پنهان‌کردن مسیر محاسبه از Personnel، یا بازنویسی نتیجه قفل‌شده
...
ارزش قرارداد و وجه وصول‌شده برای آورده مالی به فروشنده قطعی هنگام تحقق فروش منتسب می‌شوند، درحالی‌که کیفیت و موعد پیگیری وصول به Personnel مسئول واقعی پیگیری در زمان سررسید تعلق دارد. انتقال مسئولیت تاریخ اثر و دلیل دارد و اعتبار یا مسئولیت گذشته را بازنویسی نمی‌کند.
_Avoid_: دادن اعتبار قرارداد به مسئول تازه مشتری، جریمه فروشنده قبلی برای پیگیری پس از انتقال، یکی‌گرفتن اعتبار وجه با کیفیت پیگیری، یا انتقال بدون تاریخ اثر
...
تفکیک `لغو قرارداد` پیش از تحویل نهایی، `مرجوعی` کالای تحویل‌شده، `برگشت سفارش` به مرحله قبلی برای اصلاح و `اصلاح مالی` سند یا مبلغ است. واحد ثبت‌کننده دلیل و مسئولیت اولیه را از فروشنده، مشتری، موجودی یا تولید، Accounting، Logistics یا Guard، مدیریت، مشترک یا در حال بررسی انتخاب می‌کند؛ مورد اختلاف تا تصمیم HR Manager بر Badge اثر ندارد.
_Avoid_: یکی‌گرفتن لغو و مرجوعی، شمردن اصلاح مالی به‌عنوان لغو، رویداد بی‌دلیل یا بی‌مسئولیت، اثر مورد اختلاف، یا جریمه فروشنده برای علت واحد دیگر
...
**فرصت فروش قابل‌سنجش**:
فرصتی است که در دوره واقعاً برنده یا ازدست‌رفته شده و در مخرج تبدیل قرار می‌گیرد؛ فرصت باز وارد مخرج نمی‌شود، اما بازماندن بدون پیگیری پس از مهلت مصوب در عامل رکود اثر دارد. علت خارج از کنترل فروشنده علیه او محاسبه نمی‌شود و باز نگه‌داشتن فرصت، راه بهبود مصنوعی نرخ نیست.
_Avoid_: ورود فرصت باز به نرخ تبدیل، حذف فرصت ازدست‌رفته، پاداش بازماندن عمدی، جریمه نبود موجودی یا تصمیم مدیریت، یا فرصت بی‌دلیل و بدون وضعیت

**اصلاح ورود دستی شاهد عملکرد**:
نسخه دلیل‌دار مقدار کمی دارای منبع است که HR Processor تا پیش از نهایی‌شدن اصلاح می‌کند و مقدار قبلی و تازه را نگه می‌دارد. پس از نهایی‌شدن فقط نسخه اصلاحی نتیجه مجاز است؛ HR Manager منابع و هشدار اختلاف را هنگام نهایی‌سازی می‌بیند و خودکارسازی آینده رکورد مبدأ را جایگزین ورود دستی می‌کند.
_Avoid_: عدد بی‌منبع، حذف مقدار قبلی، ویرایش مستقیم نتیجه نهایی، نهایی‌سازی بدون دیدن اختلاف، یا ازبین‌بردن مسیر اصلاح هنگام خودکارسازی
...
مجموعه قابل‌ویرایش دوازده گزاره مشاهده‌پذیر در شش عامل مصوب احترام، همکاری، مسئولیت‌پذیری، ارتباط، یادگیری و آراستگی محیط کار است که از قضاوت شخصیت و ویژگی محافظت‌شده پرهیز می‌کند. هر نسخه فعال متن همان زمان را قفل می‌کند و `اطلاعات کافی ندارم` پاسخ معتبر بی‌امتیاز باقی می‌ماند.
...
سه قضاوت محدود درباره مدیریت حرفه‌ای پرونده پیچیده، ارتباط و پیشبرد مسئولانه فرصت مشتری و واقع‌بینانه‌بودن تعهد تحویل است که به‌ترتیب دو، سه و دو درصد از نتیجه کل را می‌سازند. این قضاوت عدد موجود سامانه را تکرار یا جایگزین نمی‌کند.
_Avoid_: امتیاز دوباره مبلغ فروش، قضاوت کلی و بی‌موضوع، جایگزینی داده سیستمی، یا وزن بیش از هفت درصد
...
Personnel فعال Badge جاری دارد؛ Personnel معلق آخرین Badge معتبر را بدون ارزیابی تازه نگه می‌دارد و Personnel پایان‌یافته فقط تاریخچه مجاز دارد. استخدام مجدد با Badge دارای نام `همراه` و وضعیت بدون نتیجه رسمی آغاز می‌شود و نتیجه رابطه قبلی منتقل نمی‌گردد؛ نتیجه دوره ناقص فقط با دست‌کم نود روز مأموریت و نهایی‌شدن معتبر، شاهد تاریخی است نه Badge جاری پس از خروج.
_Avoid_: ارزیابی تازه در تعلیق، Badge جاری برای رابطه پایان‌یافته، انتقال Badge به استخدام مجدد، حذف تاریخچه، یا ساخت نتیجه از دوره کوتاه
...
ترکیب همه نظرسنجی‌های معتبر و نامحدود یک دوره است که پاسخ‌های هر ترکیب پاسخ‌دهنده، Personnel هدف و عامل رفتاری را ابتدا میانگین می‌گیرد و سپس میان پاسخ‌دهندگان یکتا با وزن برابر ترکیب می‌کند. تکرار پاسخ یک شخص قدرت رأی او را افزایش نمی‌دهد، متن آزاد امتیاز ندارد و اثر امتیازی دست‌کم سه پاسخ‌دهنده یکتا می‌خواهد.
_Avoid_: محدودکردن تعداد نظرسنجی، وزن‌دادن به تعداد فرم‌های یک پاسخ‌دهنده، شمارش پاسخ تکراری، امتیاز متن آزاد، یا نتیجه از کمتر از سه همکار یکتا

**دامنه انتخاب نظرسنجی رفتاری**:
انتخاب صریح سؤال‌ها، Personnelهای هدف، Personnelهای پاسخ‌دهنده و زمان شروع و پایان به دست HR Manager یا Company Manager است. پاسخ به خود و پاسخ تکراری همان فرم ممنوع است؛ هر دو طرف باید فعال باشند و `اطلاعات کافی ندارم` پاسخ بی‌امتیاز معتبر است، بی‌آنکه تعداد کمپین، هدف یا اجرا محدود شود.
_Avoid_: خودارزیابی، پاسخ تکراری، اجبار به قضاوت بدون شناخت، هدف یا پاسخ‌دهنده غیرفعال، یا سقف تعداد نظرسنجی

**دسترسی متن نظرسنجی رفتاری**:
مرز محرمانگی‌ای است که Personnel هدف، Supervisor و Company Manager فقط خلاصه بی‌نام و بررسی‌شده و HR Processor فقط امتیاز تجمیعی و وضعیت تکمیل را می‌بینند. فقط HR Manager متن خام و هویت پاسخ‌دهنده را برای رسیدگی می‌بیند و هر مشاهده آن حسابرسی می‌شود؛ محتوای توهین‌آمیز، حساس یا شخصی وارد خلاصه تحویلی نمی‌شود.
_Avoid_: افشای متن خام یا هویت به هدف، استفاده HR Manager از اختیار برای پاسخ‌های مربوط به خودش، دسترسی HR Processor یا Company Manager به هویت پاسخ‌دهنده، خلاصه‌سازی بدون رسیدگی، مشاهده بی‌حسابرسی، یا انتقال ادعای حساس به Personnel
...
حداقل شاهد لازم برای نرخ معتبر است: سه پاسخ‌دهنده یکتای همکار، سه قرارداد قطعی برای کیفیت قرارداد، سه تعهد سررسیدشده برای وصول، پنج فرصت تعیین‌تکلیف‌شده برای تبدیل و نود روز مأموریت مؤثر برای نتیجه رسمی. زیرعامل ناکافی صفر نمی‌شود و وزن آن فقط طبق قاعده ثابت در همان خانواده توزیع می‌شود؛ نبود داده معتبر کل یک خانواده اصلی، صدور نتیجه تازه را متوقف می‌کند.
_Avoid_: صفرکردن کمبود داده، انتقال وزن به خانواده دیگر، نرخ از نمونه کوچک، صدور نتیجه بدون خانواده اصلی، یا جایگزینی Badge معتبر با خطای پوشش

**زیرعامل‌های عملکردی فروشنده**:
تقسیم مصوب هفتاد درصد عملکردی است: آورده فروش شامل ارزش خالص قرارداد نسبت به هدف با وزن چهارده و وجه وصول‌شده نسبت به هدف با وزن هشت؛ وصول شامل نرخ وصول با وزن پنج، رعایت موعد پیگیری با وزن شش و مطالبات بدون پیگیری با وزن چهار؛ سرعت شامل نخستین ارسال با وزن سه، اصلاح فعال با وزن دو و ارسال به Accounting با وزن سه؛ کیفیت شامل پذیرش نخستین ارسال با وزن سه، اصلاح منتسب با وزن دو و قضاوت Supervisor با وزن دو؛ لغو و برگشت شامل لغو منتسب با وزن سه و مرجوعی منتسب با وزن دو؛ مدیریت مشتری شامل مشتری جدید با وزن دو، تبدیل فرصت با وزن دو، فرصت راکد با وزن یک و قضاوت Supervisor با وزن سه؛ تعهد تحویل شامل دقت وعده با وزن دو، مشکل منتسب با وزن یک و قضاوت Supervisor با وزن دو درصد از نتیجه کل است.
_Avoid_: جمعی غیر از هفتاد درصد، دوباره‌شماری وجه وصول‌شده در نرخ وصول، وزن پنهان، شمار خام بدون هدف یا مخرج، یا قضاوت انسانی به‌جای داده موجود
...
ارزیابی نهایی‌نشده‌ای است که فقط پس از ورود دست‌کم یک مقدار و اقدام صریح `ذخیره پیش‌نویس` ساخته می‌شود و فقط سازنده دارای مجوز زنده می‌تواند ادامه دهد یا نهایی کند. بازکردن یا بستن فرم خالی هیچ رکوردی نمی‌سازد. لغو مجوز، دسترسی و نهایی‌سازی را متوقف می‌کند، اما پیش‌نویس داخلی و بی‌اثر را برای بازگشت احتمالی همان مجوز نگه می‌دارد. ارزیاب مجاز دیگر می‌تواند ارزیابی تازه خودش را آغاز کند و هیچ پیش‌نویسی بر امتیاز یا نشان جاری اثر ندارد. اگر پس از آغاز پیش‌نویس نتیجه نهایی جدیدتری ثبت شده باشد، سامانه پیش از تأیید نهایی هشدار می‌دهد و ارزیاب می‌تواند بازگردد یا نتیجه خود را به‌عنوان ارزیابی تازه نهایی کند؛ آخرین نتیجه نهایی نشان جاری را تعیین می‌کند.
_Avoid_: ساخت پیش‌نویس از کلیک یا بازدید، ذخیره پیش‌نویس کاملاً خالی، ادامه کار پس از لغو مجوز، انتقال پیش‌نویس به کاربر دیگر، ویرایش پیش‌نویس دیگری، حذف ارزیابی هم‌زمان، نهایی‌سازی بی‌هشدار روی نتیجه جدیدتر، قفل‌کردن ارزیابی تازه به‌علت وجود پیش‌نویس، نمایش پیش‌نویس به‌عنوان نتیجه، یا تغییر نشان از روی داده نهایی‌نشده
...
عدد ثبت‌شده به‌دست ارزیاب برای عملکرد همان Personnel در ماه انتخابی است. در نسخه نخست این مقدار دستی است و منبع عملیاتی خودکار ندارد.
_Avoid_: مقدار فرضی، استفاده از عدد ماه دیگر، جایگزینی داده مفقود با صفر، یا نمایش مقدار خودکار تأییدنشده به‌عنوان ورودی نهایی
...
زبان کوتاه و فارسیِ عنوان، توضیح، راهنما، دکمه و خطای مسیر روزمره ارزیابی است که اقدام بعدی را بدون اصطلاح فنی بیان می‌کند.
_Avoid_: هش، Snapshot، بازسازی آمادگی، تصاحب پرونده، شناسه فنی، جمله طولانی، یا توضیحی که برای انجام کار لازم نیست
