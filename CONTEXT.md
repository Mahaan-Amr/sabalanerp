...
The audited removal of an Employment Assignment from current or future organizational effect through one explicit operation: cancelling a not-yet-effective assignment, voiding an assignment that was never true, or ending a real current assignment on an effective date. A voided assignment is labelled as never valid and contributes to no current or historical capacity, supervision, schedule, or operational calculation; ending a Primary assignment may leave an Active Employment Relationship visibly flagged as lacking a current Primary assignment until Human Resources records a replacement, with no Position or employment end inferred automatically.
_Avoid_: one ambiguous delete action for every assignment state, rewriting an assignment that was once true, describing an erroneous assignment as valid ended service, counting a voided assignment as organizational truth, choosing a replacement Position automatically, ending employment implicitly, or hiding the missing-Primary condition
