...
**ارزیاب عملکرد**:
User فعالی است که یک ارزیابی عملکرد را به‌عنوان سرپرست مسئول همان Personnel یا مدیر منابع انسانی دارای مجوز مستقل انجام می‌دهد. ارزیابی و نتیجه نهایی همیشه هویت ارزیاب و مبنای اختیار او را ثبت می‌کنند و عنوان مدیر منابع انسانی، بدون مجوز مؤثر، اختیار ارزیابی ایجاد نمی‌کند.
_Avoid_: ارزیاب بی‌هویت، استنتاج اختیار از عنوان شغلی، پنهان‌کردن اینکه ارزیابی به‌دست HR انجام شده، یا نسبت‌دادن ارزیابی HR به سرپرست

**اختیار ثبت ارزیابی عملکرد**:
مجوز زنده یک User برای ذخیره پیش‌نویس و نهایی‌کردن ارزیابی است. سرپرست فقط Personnelهای تحت مسئولیت خود را ارزیابی می‌کند؛ مدیر منابع انسانی دارای مجوز مستقل می‌تواند هر Personnel مجاز در دامنه منابع انسانی را ارزیابی کند و اقدام او با مبنای `مدیر منابع انسانی` حسابرسی می‌شود.
_Avoid_: یکی‌گرفتن دسترسی مشاهده با اختیار ثبت، ارزیابی عمومی بدون دامنه، ثبت نیابتی بی‌نشان، یا تبدیل مدیر منابع انسانی به سرپرست تاریخی

**مجوزهای ساده عملکرد**:
پنج مجوز مستقل `مشاهده ارزیابی‌های عملکرد`، `ارزیابی افراد تحت سرپرستی`، `ارزیابی همه پرسنل`، `مدیریت الگوهای ارزیابی` و `مشاهده نشان در فهرست پرسنل` هستند. مجوز ارزیابی همه پرسنل در سطح ویرایش، قابل انقضا و قابل حسابرسی است و عنوان سازمانی به‌تنهایی آن را ایجاد نمی‌کند.
_Avoid_: یک مجوز کلی برای همه عملیات عملکرد، استنتاج مجوز از عنوان HR، اعطای بدون سابقه، یا یکی‌گرفتن مشاهده با ارزیابی

**الگوی ارزیابی عملکرد**:
مجموعه نسخه‌دار شاخص‌های وزن‌دار، هدف‌ها، واحدها و جهت مطلوب است که مستقل از واحد و عنوان شغلی نام‌گذاری می‌شود و هر Personnel دقیقاً یک نسخه فعال از آن دارد. ارزیابی تازه همان الگو را پیشنهاد می‌دهد، ارزیابی باز نسخه ثابت انتخاب‌شده را نگه می‌دارد و تغییر الگو فقط بر ارزیابی‌های آینده اثر می‌گذارد.
_Avoid_: یکی‌گرفتن الگو با واحد سازمانی یا Job، انتخاب چند الگوی فعال هم‌زمان، تغییر ارزیابی باز با ویرایش الگو، یا معیار شخصی بدون الگوی نسخه‌دار
...
نسخه قفل‌شده تاریخ ارزیابی، مقادیر واقعی، امتیاز محاسبه‌شده، سطح پنج‌گانه، الگوی ارزیابی، هویت ارزیاب و زمان نهایی‌سازی است که پس از اقدام صریح `نهایی‌کردن` ایجاد می‌شود. هر ارزیابی تازه نتیجه مستقل خود را دارد؛ همان ارزیاب می‌تواند پیش‌نویس را نهایی کند و صف بررسی جداگانه‌ای وجود ندارد.
_Avoid_: نهایی‌شدن با ذخیره خودکار، بازنویسی نتیجه قبلی با ارزیابی تازه، ویرایش درجا پس از نهایی‌شدن، نتیجه بدون الگوی ثابت، یا الزام تأییدکننده دوم
...
نتیجه نهایی تازه‌ای است که مقادیر نتیجه قبلی را برای اصلاح آماده می‌کند، دلیل اصلاح را ثبت می‌کند و با حفظ پیوند به نسخه قبلی جای آن را به‌عنوان نتیجه مؤثر می‌گیرد.
_Avoid_: ویرایش نتیجه نهایی درجا، حذف نسخه قبلی، اصلاح بدون دلیل، یا مؤثر ماندن هم‌زمان نتیجه اصلی و نسخه اصلاحی آن
...
سطح پنج‌گانه آخرین نتیجه نهایی مؤثر Personnel است. هر نتیجه نهایی تازه، حتی در همان روز، مبنای نشان جاری می‌شود؛ پیش‌نویس‌ها و میانگین ماهانه اثری بر آن ندارند و نسخه اصلاحی مؤثر به‌جای نتیجه اصلی خودش استفاده می‌شود.
_Avoid_: میانگین‌گیری از چند نتیجه، انتخاب بهترین نتیجه ماه، اثر دادن به پیش‌نویس، یا نگه‌داشتن نشان قدیمی پس از نتیجه نهایی تازه
