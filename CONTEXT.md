...
The Case-revision and per-row lifecycle `در انتظار پاسخ`, `تأییدشده`, `ردشده`, `منقضی‌شده`, `جایگزین‌شده`, or `لغوشده`, with immutable decision evidence and no cross-Case reuse. For each row, `ثبت قیمت سبلان` requires a positive responder rate and shows no explanation field; `رد جهت اصلاح` requires no rate and reveals a mandatory rejection-reason field. A rejection returns the same numbered Case to the Partner with only the rejected row and its affected dependencies open for correction; unchanged accepted rows remain retained. Resubmission creates successor evidence for the changed revision rather than erasing prior outcomes. The Case can be finalized only while every required row is approved and the 48-hour package window measured from the final required approval remains open. When that package expires, `درخواست مجدد قیمت` preserves the same Case number, expires every row in the package and creates one linked successor duty for all required rows; the responder may see their own prior rates for comparison but must explicitly enter and decide every new rate.
_Avoid_: describing the action as approval of the Partner's price, creating a new Case merely because one row was rejected or the package expired, editing or erasing decision history, unlocking unrelated accepted rows after a rejection, reusing a decision in another Case, silently carrying an expired rate forward, displaying an explanation field for acceptance, accepting a non-positive rate, losing the rejection reason, or finalizing with a rejected, pending, or expired required row
...
The rule that Partner suspension blocks pricing submission, response use and finalization without pausing, extending, shortening or revoking the Case package's natural 48-hour clock measured from its final required approval. If the profile returns to Active, only a package whose server-recorded expiry remains in the future may be finalized; an expired package requires `درخواست مجدد قیمت` for every required row.
_Avoid_: freezing expiry during suspension, granting a fresh 48 hours after reactivation, invalidating a package early solely because of suspension, carrying some expired rows forward, or allowing a suspended Partner to create, submit or finalize a Case
