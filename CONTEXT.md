...
The destination-workspace collection shared by every active User with the duty type's required action permission, from which one eligible User claims accountability while an authorized destination manager may reassign it. An unassigned duty may appear both as Available to a User who can claim it and as Unassigned to a destination manager who can allocate it; claiming moves it to that User's My Duties without granting ownership merely because the User has broad workspace access. The source actor chooses the required action and workspace, never a named recipient or organizational title.
_Avoid_: source-selected recipients, title-derived authority, auto-assigning shared work to every broadly authorized User, treating Available and Unassigned as mutually exclusive lifecycle states, duplicating one duty per eligible User, leaving a claimed duty jointly accountable, or making personnel absence block the workflow

**Cross-Workspace Duty Attention Count**:
The positive count displayed for each duty-queue view when that view contains work requiring attention. My Duties, Available, and Unassigned count their current records; History counts only closed duties the User has not yet seen, and opening History marks those visible records seen without changing their immutable workflow or audit history.
_Avoid_: displaying a zero badge, using the entire permanent History size as an alert, treating a view as a duty status, or changing duty evidence when acknowledging that its history was seen

**Sales Contract Correction Request**:
The Accounting-originated, contract-scoped request for a controlled correction of a locked sales Contract. A User with the explicit Accounting creation permission opens it, an Accounting manager approves or rejects it, and approval assigns one Sales correction opportunity to the Responsible Seller; ADMIN may perform every stage through a separately confirmed Admin Override whose mandatory reason and actual actor are audited, while each non-admin stage remains independently permissioned.
_Avoid_: relying on a verbal handoff, requiring the Responsible Seller to originate an Accounting finding, letting ordinary Accounting viewers create requests, deriving authority from a title, silently self-approving as ADMIN, collapsing creation and manager decision into one permission or event, bypassing the controlled correction chain, or losing rejected and approved request evidence
...
The required Accounting review of the complete before/after difference after a Seller or ADMIN uses a Sales Contract Correction Opportunity, offered through the permission-based shared queue with the Accounting initiator preferred when still active and independently authorized; an Accounting manager may reassign it and ADMIN may always act through an audited override. The review ends the request only when Accounting accepts the corrected Contract; return for further correction is a structured result that requires a new Accounting manager decision and a distinct three-working-day opportunity, even when one User performs multiple separately recorded stages.
_Avoid_: completing the request automatically on Sales save, assigning verification permanently to an unavailable or unauthorized User, reviewing without the recorded difference, treating return as a technical failure, reopening Sales from return alone, silently reusing a prior opportunity, or leaving the Contract unlocked during Accounting review
...
The Finance-owned review of required collateral and obligations that begins only after Candidate selection and Accepted Offer. Through task-scoped work in the Accounting destination, one Finance recorder registers receipt, custody details, and scans, while a distinct Finance verifier approves or returns the evidence and clearance required before Hire Conversion; Candidates who are rejected, withdrawn, or still under assessment are not asked to provide collateral.
_Avoid_: collecting collateral from every applicant, self-verification by the recorder, performing Finance work through general Human Resources access, Human Resources or a generic Admin approving Finance evidence, converting to Personnel before clearance, or treating offer acceptance as collateral approval
...
The Finance-recorded handover of a collateral original after a pre-activation cancellation or another authorized release, identifying the recipient and return date and preserving a description plus uploaded proof of handover. The return controls exist only when an original is held and a return is required, and a distinct Finance verifier confirms the completed handover.
_Avoid_: showing return fields during ordinary receipt, recording return without a recipient or proof file, confusing original return with rejection for correction, self-confirmation by the recorder, or losing the instrument's custody history
