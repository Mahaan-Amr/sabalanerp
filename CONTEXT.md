...
**تخصیص منبع لایه پله**:
A deterministic allocation from one explicitly seller-selected source policy for one layer row: `سنگ والد`, one exact `باقی‌مانده قرارداد`, or `سنگ جدید`. Nothing is auto-selected or consumed, including compatible remainder owned by the exact parent. `سنگ والد` explicitly authorizes use of already-paid material and remainders belonging to that parent. Contract-remainder selection stays inside the same central modal with same-parent/same-catalog candidates suggested first but never selected. New stone is explicitly procured and material-priced. Preview may calculate fit without mutating inventory; only the atomic parent-and-layer save commits stable physical allocations. Cancel/back consumes nothing. If the chosen source is insufficient, the layer shows only its local shortage and save fails without falling through to another remainder or new stone. Edit/delete atomically rebuilds the exact allocations. Parent/paid remainder material carries zero second material charge while new stone charges both material and new operations.
_Avoid_: automatically consuming parent remainder, falling through between source policies, mutating inventory during preview, identifying a source by stair type/name/index, partially saving an insufficient allocation, double-charging already-paid material, or silently replacing an invalidated source with new stone

**چرخه عمر لایه پله وابسته**:
An attached layer belongs to one exact parent stair row through stable row identity. Parent edits atomically preview and recalculate attached layers, source allocations, remainders, cuts, add-ons, and totals while preserving independent layer selections and requiring renewed confirmation for affected manual overrides; any allocation conflict rejects the whole edit. Parent deletion opens no alert/modal and performs nothing on first click: its row becomes an inline `این ردیف … لایه وابسته دارد` confirmation with compact layer summaries and `انصراف | حذف والد و لایه‌ها`. Explicit confirmation atomically deletes only that parent and all structural layers, releases/replays their source allocations and remainders, and removes each deleted layer's owned tools, finishing, cuts, and snapshots. Any rebuild failure rolls back everything and pending state stays local to that row. A layer may instead be viewed or deleted independently, restoring/replaying only its allocations; sibling stair parts remain untouched. Finalized contracts require explicit edit flow.
...
**طول مادر سنگ پله**:
Source length is an explicit required inventory/catalog fact parallel to mother width and is never seller-editable in contract configuration. The compact catalog line renders facts such as `گرانیت نطنز · مادر ۳m × ۴۰cm · ضخامت ۲cm` rather than a disabled input. A stair part's entered طول is always finished-piece length. Equality with source length creates no cross cut or longitudinal remainder; a shorter piece creates the real cross cut and a full-mother-width leftover length; a longer value is preserved, blocks save at that exact field, and shows `حداکثر طول این سنگ … متر است`. The hard-coded `300cm` fallback is removed. A catalog product missing source length yields no guessed geometry, shows only `طول مادر در موجودی ثبت نشده است` in that section, and cannot be committed while the rest of the draft remains usable. Source length is snapshotted so later inventory edits cannot rewrite saved consumption or remainders. A remaining-stone child uses its source remainder's exact physical length instead of the catalog standard.
...
**نرخ مفقود برش**:
An explicit zero inventory rate is valid and preserves the physical cut at zero charge; null, a missing/inactive unresolved role, or no stable semantic rate match is missing data and is never converted to zero or recovered by name matching. A missing required longitudinal or cross rate preserves the complete seller draft, leaves valid cut directions visible, shows only `نرخ برش طولی در موجودی ثبت نشده است` or its cross equivalent in the flat cut summary, and blocks atomic product save when the still-clickable action focuses that section. Other form sections remain usable and a valid direction continues to show its own amount. After inventory correction, the section refetches/recalculates without losing the draft; valid cached rates show no repeat skeleton. An old saved row with a rate snapshot remains viewable/editable from that snapshot even after catalog removal, while only a newly selected or newly introduced operation requires a current resolvable inventory rate.
