...
The positive total linear meters of a longitudinal Contract Item whose raw zero quantity records that the optimizer derived the piece count rather than that nothing was contracted. Accounting may seal this quantity only when the frozen optimizer plan and canonical Product Graph agree with the exact scale-three sum of every persisted frozen Delivery/Product quantity for the same stable Product row; a wizard-era Delivery copy is corroborating evidence when present and any disagreement blocks approval, but its absence does not invalidate complete persisted evidence. A zero invoice-item quantity shares the sentinel meaning only after that full reconciliation succeeds, while any positive invoice quantity must equal the sealed meters exactly; every operational view and document then displays canonical meters, raw zero remains visible only as audit evidence, and an unresolved row displays an explicit quantity-review state.
_Avoid_: treating a raw Contract Item or invoice-item zero sentinel as a financial or delivery quantity, normalizing an invoice sentinel before full reconciliation, accepting a positive invoice quantity that differs from sealed meters, deriving meters from current product rules, accepting agreement by catalog identity instead of stable Product row identity, treating the wizard-era Delivery copy as authoritative, requiring an absent redundant wizard copy when persisted evidence is complete, ignoring a conflicting wizard copy, modifying the original Contract or its snapshots, displaying zero operationally when canonical witnesses agree, hiding the raw sentinel from audit evidence, displaying an invented quantity while reconciliation is unresolved, accepting a partial Delivery sum, mixing units, double-counting a duplicate Delivery row, hiding over-allocation, or silently choosing one witness when quantities conflict
