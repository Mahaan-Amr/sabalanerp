...
**Prepared Product Commercial Evidence**:
A prepared Contract Product freezes its explicit unit-bearing quantity and explicit unit rate as the independent witnesses that determine its canonical base amount under the Contract's versioned pricing and rounding policy. Count is integral while square-meter and ton quantities use their declared measured-unit precision; complete agreeing witnesses are normalized automatically, while missing, unsupported, or contradictory witnesses fail closed without deriving quantity from money or changing the commercial agreement.
_Avoid_: treating the saved row total as sufficient pricing evidence, deriving prepared quantity from total divided by rate, accepting a fractional count, losing the prepared unit, silently choosing among contradictory quantity fields, or requiring Sales or Accounting to repair deterministic evidence
...
_Avoid_: پیام انگلیسی برای کاربر عملیاتی، نمایش مستقیم exception، اصطلاحات برنامه‌نویسی، افشای شناسه یا داده‌ی حساس، پیام مبهمی مانند «خطایی رخ داد»، درخواست از کاربر برای حل ناسازگاری داخلی، حواله‌دادن کلی کاربر به پشتیبانی به‌جای ارائه اقدام واقعی یا شناسه پیگیری امن، موفق‌نمایی عملیات ناموفق، هشدار بدون اقدام بعدی، یا ترجمه‌ای که معنای کسب‌وکاری خطا را تغییر دهد
...
**Organizational Foundation Detail**:
The permission-scoped, directly addressable detail page opened from the main content of every active or inactive Organizational Unit, Job, or Position card, while the card's embedded edit, lifecycle, and deletion actions remain independent. It separates current dependencies, scheduled dependencies, ended or corrected historical relationships, and lifecycle or structural history, showing names, states, effective dates, counts, and authorized drilldown links while preserving the originating list context.
_Avoid_: a temporary detail view that cannot be linked or restored, showing only current blockers, exposing internal identifiers instead of organizational meaning, hiding corrected or ended history, bypassing record-level permissions, or letting an embedded action open the detail

**Organizational Foundation Deletion Preview**:
The required impact view shown before any irreversible deletion challenge for an Organizational Unit, Job, or Position, separating resolvable current dependencies, historical references, and legacy dependencies with direct authorized paths to each available resolution. Password and exact-confirmation challenges appear only after the requested deletion is eligible under the selected resolution policy.
_Avoid_: asking for credentials before checking eligibility, reducing every blocker to one generic error, omitting inactive or legacy dependencies, presenting historical evidence as a current assignment, or offering a resolution the acting User is not authorized to perform

**Organizational Foundation Dependency Resolution**:
The administrator-directed preparation of an Organizational Unit, Job, or Position for permanent deletion by explicitly moving, reparenting, cancelling, voiding, ending, or separately deleting each live dependency instead of applying an implicit cascade. Historical operational or legal records retain a version-specific snapshot of the removed definition and never cause attendance, scheduling, recruitment, contract, payroll, or other evidence to disappear merely because the live foundation definition is deleted.
_Avoid_: an automatic graph cascade, making the system guess whether data was real or test data, silently choosing a replacement definition, deleting legal or operational evidence, or requiring direct database intervention

**Organizational Foundation Permanent Deletion**:
The irreversible removal of an Organizational Unit, Job, or Position from its live foundation table after Organizational Foundation Deletion Preview and Dependency Resolution, authorized through a dedicated permanent-deletion permission with a fresh credential, mandatory reason, exact-code challenge, and final confirmation. A non-content deletion receipt remains with the entity type, versioned business identity, time, actor, reason, and dependency-resolution counts, while deleted descriptive content is not recoverable from the live definition.
_Avoid_: treating ordinary edit access as deletion authority, deleting before dependencies are resolved, retaining a recoverable copy of the deleted definition in the receipt, reporting completion before the transaction commits, or leaving no accountable evidence that deletion occurred

**Versioned Organizational Foundation Code**:
The reusable business code of one Organizational Unit, Job, or Position, identified in historical evidence by entity type, code, and a monotonically increasing occurrence number such as `Position 22 · Version 2`. The current collection normally shows the code with a version badge only after reuse, while details, deletion evidence, historical snapshots, and prior-version search always expose the complete versioned identity; a minimal identity ledger survives deletion so an occurrence number is never reused.
_Avoid_: treating the code alone as permanent identity, sharing one version sequence across different foundation entity types, resetting a sequence after deletion, mixing snapshots from separate occurrences, showing every historical occurrence in ordinary current search, or retaining the deleted definition's descriptive content merely to allocate the next version

**Organizational Foundation Code Change**:
The immediate, single-form audited operation for changing the current business code of an Organizational Unit, Job, or Position while preserving the definition's internal identity, regardless of whether the User's reason is typo correction or organizational renumbering. It requires ordinary organizational-structure edit authority, a mandatory reason and inline impact summary in the existing edit form, allocates the next occurrence of the requested code, releases the prior code for its next occurrence, preserves every earlier snapshot unchanged, and rejects a conflict when another current definition of the same type owns the requested code; saving the form is the only confirmation and the system never asks the User to classify the change.
_Avoid_: two separate code-change workflows, a separate confirmation form or step, silently editing identity without a reason or visible impact, rewriting historical snapshots, scheduling a future code change, resetting or reusing an occurrence number, accepting a current-code collision, creating a replacement definition, requiring permanent-deletion authority for a code change, or inferring the User's business reason
...
**کاتالوگ مجوزهای افشای عملکرد**:
مجموعه قابلیت‌های مؤثر و مستقلی است که بررسی ارسال Supervisor، مشاهده سابقه محرمانه، نمایش Badge در فهرست Personnel، تحلیل تجمیعی، تحلیل یا رتبه‌بندی نام‌دار، مشاهده کالیبراسیون ارزیاب، خروجی PDF/Excel و مشاهده حسابرسی عملکرد را جداگانه مجاز می‌کند. پروفایل‌ها فقط پیش‌فرض اعطای این مجوزها هستند و عنوان سازمانی، نقش Company Manager یا Human Resources Manager و مسئولیت Supervisor به‌خودی‌خود اجازه افشا نمی‌سازند؛ نمایش شخصی سطح عملکرد از پیوند هویت Personnel–User می‌آید.
_Avoid_: یک مجوز عمومی برای همه اطلاعات عملکرد، استفاده مجدد از مجوزهای ارزیابی استخدام، افشای کالیبراسیون با مجوز تحلیل عادی، استنتاج دسترسی از عنوان شغلی، یکی‌گرفتن افشا با اختیار ارسال Supervisor، یا بازسازی منطق مجوز در رابط کاربری

**مرز عدم افشای عملکرد**:
قاعده‌ای است که مجموعه‌ها، شمارش‌ها و فیلترها را فقط از رکوردهای مجاز می‌سازد و درخواست مستقیم غیرمجاز را بدون تأیید وجود Personnel، ارزیابی، نتیجه یا سطح پاسخ می‌دهد. پاسخ محرمانه قابل نگهداری در حافظه مشترک نیست و خطای ردشده هیچ هویت، مقدار، وضعیت یا دلیل محرمانه‌ای را آشکار نمی‌کند.
_Avoid_: شمارش رکورد پنهان، تفاوت قابل تشخیص میان شناسه موجود و ناموجود برای درخواست‌کننده غیرمجاز، بازگرداندن داده و پنهان‌کردن آن در frontend، یا قراردادن جزئیات محرمانه در پیام خطا

**دسترسی بررسی عملکرد**:
مجوز وظیفه‌محوری است که صف تصمیم و جزئیات کامل لازمِ فقط همان پرونده‌های آماده بررسی را در اختیار بررسی‌کننده Human Resources می‌گذارد و پس از تصمیم، سابقه محدود همان کار را نگه می‌دارد. اگر عامل هم‌زمان سرپرست مسئول همان بخش و دارنده مجوز مؤثر ارسال و بررسی باشد، می‌تواند ارسال نوشته‌شده به دست خودش را نیز بررسی کند؛ سامانه عامل واحد و خودبررسی را صریحاً حسابرسی می‌کند و جدایی اجباری وظایف وجود ندارد. این هم‌زمانی مسئولیت سرپرستی، اختیار ارسال یا امکان خودارزیابی Personnel را ایجاد نمی‌کند. جست‌وجوی آزاد در Personnelهای دیگر، دوره‌های گذشته یا پرونده‌های بسته به مجوز مستقل مشاهده سوابق محرمانه نیاز دارد.
_Avoid_: تبدیل وظیفه بررسی به دسترسی عمومی تاریخچه، نمایش پرونده نامرتبط، تصمیم بدون شواهد و مسیر محاسبه لازم، اعطای ضمنی تحلیل مدیریتی، منع خودبررسی برای عامل دارای هر دو اختیار، یا استفاده از مجوز بررسی برای جعل مسئولیت سرپرستی و ارسال

**تصویر حداقلی شاهد عملکرد**:
نمای تغییرناپذیر و حداقل‌ضروری از یک شاهد متعلق به سامانه دیگر است که فقط واقعیت لازم برای قضاوت عملکرد را آشکار می‌کند و دسترسی به رکورد اصلی را همچنان تابع مجوز مستقل سامانه مالک نگه می‌دارد. داده پزشکی، مالی، انضباطی و سایر اطلاعات بسیار حساس فقط در حد نتیجه عملیاتی ضروری وارد این تصویر می‌شوند.
_Avoid_: کپی کامل رکورد اصلی، افشای پیوست یا جزئیات نامرتبط، گسترش مجوز سامانه مالک از راه پرونده عملکرد، یا ارجاعی که بدون مجوز مستقل رکورد اصلی را باز کند

**مشاهده حسابرسی عملکرد**:
مجوز دیدن عامل، زمان، نوع اقدام، دامنه، دلیل، نسخه، نتیجه و شناسه پیگیری رخدادهای عملکرد است. مقدارهای محرمانه پیش و پس از تغییر، شواهد، روایت Supervisor و محتوای خروجی فقط هنگامی آشکار می‌شوند که بیننده علاوه بر حسابرسی، مجوز مشاهده همان نوع داده را نیز داشته باشد.
_Avoid_: استفاده از حسابرسی برای دورزدن محرمانگی، نمایش محتوای حساس با مجوز فراداده، حذف عامل واقعی، یا پنهان‌کردن استفاده از دسترسی مدیریتی
...
**محدوده مشتری فروشنده همکار**:
The owner-only CRM boundary applied specifically to a Partner Seller: a newly created Customer belongs to that Partner Seller, and the Partner Seller may view and use only their own Customers and related work. Partner Customer creation uses the ordinary Sales customer workflow, fields, numeric normalization, duplicate detection and validation, while fixing ownership to the Partner Seller; its permitted Customer types are Individual, Company and Government, while Collaborative Customer creation remains an internal managed capability. A supplied primary phone is a valid Iranian mobile; supplied identity evidence is a ten-digit personal national code for an Individual or an eleven-digit legal national identifier for a Company or Government Customer. Historical Customers with older identifier shapes remain selectable without silent rewriting, while any later explicit edit must correct fields that do not satisfy the current shared validation. A duplicate already owned by the Partner Seller is not recreated: the shared workflow offers that existing Customer for explicit selection and returns to the same Contract Draft. A possible duplicate owned by someone else reveals only the matched name, person type, city, and a masked match witness such as the final four phone digits, then offers a managed transfer request; the Contract Draft remains recoverable while that request is pending, another owned Customer may be selected, and approval later offers an explicit return-and-select action rather than attaching the transferred Customer silently. Full identifiers, address, owner, Projects, Contracts, and timeline remain hidden before authorized transfer. Ordinary internal Sabalan sellers retain their explicitly scoped CRM access and are not restricted by this Partner-only boundary.
_Avoid_: a reduced Partner-only customer form, validating a legal identifier as a personal national code, allowing a Partner Seller to create a Collaborative Customer, creating a second Customer when an owned duplicate exists, silently selecting a duplicate or newly transferred Customer, blocking or discarding the Contract Draft while transfer is pending, rewriting or blocking selection of historical Customer evidence merely because an identifier predates current validation, preserving an invalid identifier through an explicit edit, applying the Partner restriction to ordinary internal sellers, exposing another owner's complete match witness or related activity, treating a missing department as company scope, silently transferring ownership, allowing a Partner Seller to use another owner's Customer in a Contract, weakening ordinary phone or identity validation for new or edited data, or resolving a foreign match by creating an unrestricted duplicate

**پروژه مشتری**:
The contract-selectable CRM Project owned by one Customer, recording the ordinary project name, address, city, project type, Project Manager identity and contact, and marketer evidence. Ordinary Sales and Partner Sales create, validate, select and return from the same Customer Project workflow; Partner access additionally requires ownership of the Customer. When Customer creation includes its first Project, both records commit atomically and idempotently or neither exists; adding a Project to an existing Customer is likewise retry-safe. A Potential Project is pre-sale opportunity evidence and never substitutes for this contract Project. Existing numbered Partner Sale Cases retain their frozen historical Project evidence. If the one retained legacy unnumbered Partner Draft references a Potential Project, migration creates one traced Customer Project from its known name and address, leaves genuinely unknown optional facts unset, preserves the Potential Project, and rebinds only that Draft; other Potential Projects remain pre-sale evidence and are not made contract-selectable automatically.
_Avoid_: a Customer without its submitted first Project after partial failure, duplicate Customer or Project creation on retry, rewriting a numbered Case, deleting or reclassifying a Potential Project, bulk-promoting Potential Projects into Contract choices, inventing unknown migrated fields, using a Potential Project as the Contract's selected Project, a reduced Partner-only Project form, losing the in-progress Contract or its step when returning from Project creation, attaching a Project to another Partner Seller's Customer, or storing fields that only imitate the ordinary layout without preserving their business meaning

**پاسخ‌دهنده قیمت فروشنده همکار**:
The one active internal Sabalan seller with effective edit-capable Sales access whom management assigns to a Partner Seller account and whom the system automatically uses for every newly submitted Case pricing duty from that Partner. The Partner cannot choose or shop among responders per Case. A global ADMIN or authorized Sales manager may transfer a pending duty to another eligible responder only with a recorded reason and preserved assignment history. Because pricing happens before Delivery planning, the responder sees only the shared technical Product Graph; they never see the Partner's Customer identity, telephone, Delivery plan, exact address, customer-facing price, payment plan, margin or other retail economics. The authority never makes the responder the Contract Creator, Responsible Seller, Customer owner, or recipient of sales-performance credit.
_Avoid_: letting the Partner select a responder per Case, asking the responder to approve the Partner's retail price, exposing Customer identity, contact details, Delivery facts, exact address or margin data, silently reassigning pending work, deriving the responder from Contract responsibility, allowing an unassigned seller to quote, or crediting the responder for the Partner Seller's sale

**استعلام قیمت فروشنده همکار**:
The mandatory Case-scoped request created when a Partner Seller completes Product selection and continues into the pricing gate, before Delivery and payment planning. Each price-bearing Product row has an independent decision in that Case revision; every required row must be approved and explicitly accepted by the Partner before the wizard unlocks `برنامه تحویل`, and no approval is reusable in another Case. The approved quote package remains usable for 48 hours from the final required row approval; finalization within that window permanently snapshots the approved Sabalan prices for the resulting internal Sale Record.
_Avoid_: approving the Partner's retail price, reusing one Case's quote in another Case, exposing Customer identity or retail economics to the responder, collapsing partial row outcomes into one unexplained result, or allowing customer confirmation before all required rows are approved

**نرمال‌سازی سراسری ورودی عددی**:
The platform-wide boundary for every structured numeric or digit-shaped field in every workspace. Persian, Arabic, Latin or mixed-script digits are converted immediately to canonical Latin digits in field state, validation, API transfer and persistence. Arithmetic amounts and decimals also canonicalize supported Persian or Latin decimal and grouping separators to an ungrouped Latin-decimal value, while identifier-shaped values such as mobile, National Code, postal code and business numbers remain strings and preserve leading zeroes. The server independently applies the same normalization and validation boundary.
...
_Avoid_: mutable approval evidence, anonymous decisions, accepting a changed Case revision under stale evidence, relying on current assignment to explain history, losing a row outcome inside a bulk command, or recording only the latest state

**قیمت فروش سبلان به فروشنده همکار**:
The Partner Seller's final purchase amount from Sabalan for one Contract Product row, calculated by the ordinary Contract pricing engine exactly once. The Case-scoped responder rate is multiplied by the canonical family quantity: square meters for Longitudinal, finished tread count for `کف پله`, finished riser count for `خیز`, finished landing count for `پاگرد`, consumed mother-stone count for Slab, and catalog-unit quantity for Prepared Product. The same canonical system-calculated mandatory charges, cutting, adhesive, tools, finishing, and other Sabalan-supplied components used in the customer projection are then added. Generated consumption rows present the calculation and are neither separate quote subjects nor second charges. The successful Case snapshots the result without constraining or revealing the Partner's retail rate.
_Avoid_: using the wrong family quantity, attaching a second responder rate to a generated consumption row, charging main material twice, carrying one Case's quote to another, letting either actor manually price ancillary components, diverging ancillary calculations between the two projections, or repricing a finalized Case after quote expiry

**مصرف تأیید قیمت فروشنده همکار**:
The append-only evidence that one Partner Sale Case revision finalized with the approved responder quote for each matching stable `productRowId`. The Case snapshots the approved Sabalan rate and unit, canonical material or catalog-unit calculation, system-calculated ancillary components and final Sabalan amount without exposing wholesale evidence to the Customer.
_Avoid_: applying approval to another Case or revision, matching rows by position or label, changing the catalog-owned unit, omitting canonical calculation evidence, pricing a consumed-stone display row independently, or exposing wholesale evidence to the Customer

**قیمت فروش فروشنده همکار به مشتری**:
The customer-facing amount calculated from the Partner Seller's freely chosen family rate using the same fixed basis as the hidden Sabalan rate: square meters for Longitudinal, per tread, riser, or landing for Stair sections, per consumed mother-stone unit for Slab, and catalog unit for Prepared Product. The same system-calculated cutting, adhesive, tool, finishing and other ancillary components used in the Sabalan projection are added. Only the base Product-family rate differs between the two projections. The responder cannot see or approve this retail rate; both rates remain preserved for restricted dual-price reporting, and their difference is not Sabalan income.
_Avoid_: letting the Partner manually price system-owned ancillary components, using a different unit or ancillary calculation in the two projections, forcing equality with the responder quote, exposing the retail rate to the responder, treating a higher Partner price as Sabalan revenue, overwriting either price with the other, or storing only the difference

**بازفروش مستقل فروشنده همکار**:
The commercial relationship in which a Partner Seller buys from Sabalan at the Case-scoped approved Sabalan price and independently resells to their own Customer at the Partner Customer Sale Price. The Partner Seller, not the end Customer, owes Sabalan; any resale profit or loss belongs to the Partner Seller and is not Sabalan revenue or loss. The end Customer remains the Partner Seller's customer and does not become Sabalan's accounting debtor merely because the sale is recorded in Sabalan ERP.
_Avoid_: treating the Partner Seller as a commission agent selling on Sabalan's behalf, making the end Customer owe Sabalan, recognizing the resale difference as Sabalan income, or merging Sabalan's receivable with the Partner Seller's customer receivable

**اختلاف قیمت بازفروش فروشنده همکار**:
The stored difference between the Partner Customer Sale Price and Sabalan's approved selling price for the same configured Product and quantity. A negative difference produces a clear non-blocking warning with the amount and requires simple confirmation, but the Partner Seller remains free to complete the below-cost resale. Both source prices are preserved so the difference remains derivable and auditable.
_Avoid_: blocking a below-cost resale, silently hiding a loss, requiring Sales price approval for the Partner Seller's chosen resale price, treating the difference as Sabalan revenue, or persisting only a mutable calculated difference

**محرمانگی قیمت بازفروش فروشنده همکار**:
The access boundary for Partner resale economics: the Partner Seller sees their customer-facing calculation and the approved Sabalan purchase calculation for their own Cases, while the assigned Price Responder sees only Case-scoped technical evidence and the Sabalan rates they quote. Dual-price comparison and margin are restricted to system Administrators and explicitly authorized company-management reporting; Sabalan Accounting uses only the Sabalan receivable, and unrelated sellers see neither price.
_Avoid_: exposing retail economics or Customer detail to the Price Responder, exposing internal policy or cost evidence to the Partner, using retail value as an Accounting receivable, deriving dual-price visibility from an ordinary Sales role, or showing another Partner Seller's economics

**پرونده فروش همکار**:
The aggregate root created through one Partner Seller workflow that atomically owns one shared Product Graph and exactly two distinct commercial records: one Sabalan-to-Partner Sale Record and one Partner-to-Customer Sales Contract. It is the sole owner of their canonical lifecycle, while the Partner Seller completes one wizard and the system preserves the different parties, prices, visibility, accounting meaning, outputs, and evidence without asking for two contracts.
_Avoid_: putting two debtors or two monetary truths into one ordinary Sales Contract, modelling the internal record as a second ordinary Sales Contract, giving either linked record an independent lifecycle, allowing only one linked record to be created, asking the Partner Seller to duplicate the Product Graph manually, losing the link between the records, or treating the pair as two unrelated sales

**پرونده شماره‌دار در انتظار استعلام فروش همکار**:
A saved Partner Sale Case created when the Partner continues from Product selection into the Case-scoped pricing gate. It owns one immutable Case number and current revision so every duty, decision, rejection and correction remains traceable, but it does not yet own a final customer-Contract number or internal Sabalan-record number and cannot be shared with the Customer. It creates no Sabalan debt, Accounting eligibility, final artifact, reservation, fulfillment authority or realized-sale effect until the Partner accepts the complete price package, finishes Delivery and payment planning, and finalization succeeds with every required approval still valid.
_Avoid_: unnumbered pricing work, treating the Case number as commercial commitment, allocating final linked-record numbers before Partner finalization, customer exposure before pricing completes, provisional Sabalan receivable, final PDF, or deleting and reusing the Case number

**سند فروش سبلان به فروشنده همکار**:
The purpose-specific internal commercial record created inside a Partner Sale Case only by successful Partner finalization. Its creation atomically freezes every Product amount from valid Case-scoped Sabalan pricing evidence, makes the Partner Seller Sabalan's debtor, and establishes the sole basis for Sabalan Accounting, receivable and collection. It is not an ordinary Sales Contract, has no pre-finalization shell, and is never exposed to the end Customer.
_Avoid_: modelling it as a second ordinary Sales Contract, creating an unpriced pre-finalization shell, naming the end Customer as Sabalan's debtor, using the Partner Customer Sale Price for Sabalan Accounting, exposing the record or pricing evidence to the end Customer, or creating it without the linked Partner Customer Sales Contract

**قرارداد فروش فروشنده همکار به مشتری**:
The customer-facing Sales Contract kind inside a Partner Sale Case between the Partner Seller and one Customer within that Partner Seller's Customer Scope. It reuses ordinary customer-Contract capabilities but is explicitly distinct from both Standard and Collaboration Sales Contracts, uses Partner Customer Sale Prices, and can exist only as the customer view of one complete Partner Sale Case.
_Avoid_: treating it as a Collaboration Sales Contract, inferring its kind from free-form snapshot data, making Sabalan the creditor for the end Customer's retail amount, revealing the Partner Seller's buying price or margin, selecting another seller's Customer, or separating it from its internal Sabalan-to-Partner record

**حساب تجاری فروشنده همکار**:
The debtor account linked one-to-one to a Partner Seller User and created automatically from that User's current profile when the exclusive Partner persona is activated. It requires no separate commercial-identity form or verification gate; missing optional identity details may be completed later without delaying activation, and the account remains distinct from every end Customer owned by the Partner Seller.
_Avoid_: asking the activating manager to duplicate identity data, blocking Partner activation for a separate commercial-identity workflow, treating an owned end Customer as the Partner Seller's own debtor account, or changing the debtor to the delivery recipient

**قطعی‌شدن پرونده فروش همکار**:
The synchronized commercial commitment of an already numbered and fully priced Partner Sale Case. After every required responder quote is approved and still valid, the explicit `نهایی‌سازی خرید از سبلان و ایجاد قرارداد مشتری` action shows the Partner their retail total, Sabalan purchase total and resulting Sabalan debt and requires deliberate confirmation. Success atomically allocates the customer-Contract and internal Sabalan-record numbers, creates and seals both linked records and their evidence, fixes the Partner Seller's commercial debt to Sabalan, and enters the Accounting queue exactly once. Only after that succeeds may the Partner start the ordinary customer confirmation, signature and delivery of the retail-only Contract; Accounting financial approval later creates the ordinary official Accounting Receivable.
_Avoid_: hiding the debt consequence behind a generic button, creating commercial debt or either final linked record from pricing submission, responder approval, preview, or retry, exposing the customer Contract before Partner finalization, creating the official Accounting Receivable before financial approval, delaying Partner commercial commitment until customer action, creating duplicate cases or record numbers on retry, committing only one linked record, or losing the commitment transition evidence

**تحویل مستقیم مشتری پرونده فروش همکار**:
The only Partner Sale Case fulfillment mode in the initial version: Sabalan delivers the shared configured Products directly to the Partner Seller's end Customer at the Contract delivery destination, while the Partner Seller remains Sabalan's debtor. All authoritative inventory, loading, and delivery operations belong to the Sabalan-to-Partner Sale Record and their customer-readable status is reflected in the linked Partner-to-Customer Sales Contract.
_Avoid_: offering Partner pickup in the initial version, making the delivery recipient Sabalan's debtor, maintaining two independent delivery quantities or schedules, or allowing the customer-facing record to authorize physical fulfillment independently

**Product Graph مشترک پرونده فروش همکار**:
The single canonical Product, configuration, quantity, processing, tool, and eventual Delivery plan owned by a Partner Sale Case and shared by both records. Before pricing submission, the ordinary Contract Product Graph and pricing engine handle Product edits. After a row is approved, any edit that changes its Sabalan base-rate subject, canonical quantity, or system-calculated Sabalan amount invalidates only that row and its price-dependent descendants and requires a new responder decision; retail-rate-only, Customer-identity, retail-payment, or the later Delivery-plan changes do not invalidate Sabalan pricing. A child made from previously paid remainder carries zero new material charge, while its new cutting, tools, and finishing remain billable exactly once. Correction preserves stable row identity, replays source and remaining-stone children atomically, and cannot reduce quantity below effective reserved plus physically dispatched evidence.
_Avoid_: reducing the Product step to a simple catalog picker, duplicating the graph, matching rows by position, retaining an approval after its Sabalan amount changes, invalidating unrelated accepted rows, invalidating a quote for a retail-only change, charging previously paid remainder material again, accepting partial child replay, or allowing linked records to drift

**محصول تاریخی غیرفعال در پیش‌نویس همکار**:
A Partner draft row whose catalog product later becomes inactive remains readable from its saved product evidence, but cannot be used as a new technical or pricing subject or finalized even if its earlier price approval has time remaining. Continuing requires an active replacement as a new contract row with a new price inquiry; the former row remains in the Case history.
_Avoid_: deleting the row automatically, calling an active product inactive because its catalog failed to load, silently reactivating it, finalizing a newly inactive product, reusing its row identity for a different product, or carrying its price approval to that product

**نسخه پرونده فروش همکار**:
The one coherent revision of a Partner Sale Case that aligns its shared Product Graph, party snapshots, wholesale and retail price envelopes, payment evidence, customer-visible content, and both linked-record views. Any change to a party, commercial amount, physical obligation, or customer output advances the case revision with append-only history; internal notes, notifications, access logs, and operational metadata remain separate evidence. A stale competing change cannot replace the current revision, commitment freezes it, and a later formal correction creates a traceable successor rather than an overwrite.
_Avoid_: versioning either linked record or price envelope independently, mutating committed evidence, letting a view point at a different graph revision, accepting a stale competing change, losing material Draft change history, advancing the commercial revision for unrelated operational metadata, or treating a correction as an overwrite

**ویرایش پیش‌نویس پرونده فروش همکار**:
The continued editing of an already-numbered but unfinalized Partner Sale Case. A retail-rate-only, Customer-identity, retail-payment, or non-priced Delivery-detail edit advances the Case revision without invalidating a responder quote. An edit that changes one row's Sabalan pricing subject, canonical quantity, system-calculated ancillary amount or final Sabalan amount invalidates only that row and its price-dependent descendants, retains all other unchanged accepted rows, and creates successor pricing evidence on resubmission. Quote-package expiry blocks Partner finalization but never silently reprices or deletes the Case.
_Avoid_: treating recoverable browser state as Case evidence, silently replacing a quoted rate, retaining stale approval after its Sabalan calculation changes, invalidating unrelated accepted rows, invalidating Sabalan pricing for a retail-only change, carrying approval to another pricing subject, or deleting an expired Case

**تصویر سندهای پرونده فروش همکار**:
The purpose-limited view of one case revision used by the internal Sabalan record, customer Contract, Accounting, fulfillment, PDF, or public confirmation. Product rows and Deliveries remain authoritative only in the case revision; every view preserves the same stable row identities, and an immutable customer-output snapshot proves what was shown without becoming a second editable Product Graph.
_Avoid_: owning writable Item or Delivery copies in either linked record, rebuilding a view from current catalog facts, using an output snapshot as graph authority, exposing wholesale facts through a customer view, or allowing views of different revisions to coexist as current

**چرخه پرونده فروش همکار**:
The monotonic Case-owned commercial lifecycle `در انتظار قیمت سبلان`, `نیازمند اصلاح قیمت`, `آماده نهایی‌سازی`, `قطعی‌شده`, or pre-commit `لغوشده`, followed only where the reviewed post-commit voiding workflow succeeds by terminal `باطل‌شده`. Customer send, confirmation, rejection, signature and print become independent append-only facts only after `قطعی‌شده`; none creates or erases Sabalan debt. Financial and physical settlement also remain independent facts, so `باطل‌شده` never erases or rewrites what was previously committed.
_Avoid_: exposing a Customer Contract before commitment, giving either linked record an independent commercial lifecycle, treating Customer status as the Case commitment trigger, moving a committed Case into a pre-commit state, treating voiding as deletion, creating a second commitment or receivable from replay, or cancelling only one linked record

**یکپارچگی جفت پرونده فروش همکار**:
The commit-time invariant that one Partner Sale Case has exactly one immutable reciprocal link to one Partner Customer Sales Contract and one purpose-specific internal Sabalan Sale Record, and neither record may exist durably as a partial or orphaned pair. The customer record must carry the Partner Customer Contract kind, and every authorized writer preserves the complete triangle of identities.
_Avoid_: relying only on route ordering, permitting nullable or replaceable post-creation links, linking either record to a different case, leaving a partial pair after failure, or repairing an orphan by guessing its counterpart

**ثبت اتمیک پرونده فروش همکار**:
The staged all-or-nothing persistence boundary. Product-boundary pricing submission rechecks the active Partner Seller, Customer scope, shared Product Graph and Partner retail inputs before creating exactly one Case identity, first revision, Case number, pricing duty and audit evidence. Partner acceptance of the complete quote package advances that numbered Draft and unlocks Delivery; later Partner finalization rechecks the current Case revision, completed Delivery and payment plans, and every required still-valid row decision before atomically allocating both final linked-record identities and numbers, creating their snapshots and bindings, fixing the Sabalan debt and making the customer-safe Contract available. Replaying any command with the same identity returns its recorded result instead of creating another Case or record pair.
_Avoid_: partial Case or linked-record creation, trusting client-time eligibility, allocating final record numbers during pricing submission, accepting one invalid row while finalizing the rest, reusing a quote in another Case, creating duplicate Cases or pairs on retry, or exposing a customer number before finalization commits

**هویت نسخه‌شده طرفین پرونده فروش همکار**:
The pairing of live Partner Commercial Account and Customer identities for current authority with their Case-revision snapshots for commercial evidence. Suspension blocks mutation and commitment without erasing the Case, price-package expiry blocks finalization without deleting or silently repricing it, Customer transfer never moves the Case automatically, and changed party facts enter only through an explicit pre-commit revision or formal post-commit correction.
_Avoid_: authorizing from a stale snapshot, silently refreshing commercial evidence from live records, mutating while the Partner is suspended, deleting an expired pricing Case, transferring a Case merely because Customer ownership changed, or overwriting committed party evidence

**مالک تجاری پرونده فروش همکار**:
The immutable Partner Seller who creates the Partner Sale Case and remains its Creator, Responsible Seller, and sales-credit owner. Admin support and lifecycle actions retain the Admin's own actor identity, while neither an Admin nor the Price Responder impersonates, replaces, or receives credit from the Partner Seller in the initial version.
_Avoid_: proxy creation presented as Partner action, reassigning case ownership, crediting the Price Responder, confusing support authority with commercial ownership, or rewriting creator evidence after suspension or termination

**شاهد یکپارچگی پرونده فروش همکار**:
The versioned canonical fingerprint binding one Case revision's Product Graph, party snapshots, row-price decision bindings, price envelopes, payment evidence and Delivery plan to its internal, customer, public, Accounting and fulfillment views. A mismatch blocks sensitive progression and becomes an explicit evidence-review case; no view is silently chosen as truth over the canonical revision.
_Avoid_: accepting a mismatched view, rebuilding historical evidence from current catalog data, allowing commitment or downstream work after a failed integrity check, hiding schema version, or guessing which conflicting copy is correct

**خروجی مشتری پرونده فروش همکار**:
The existing standard customer-facing Sales Contract template and flows rendered without structural, visual, or interaction divergence for a Partner Sale Case. It identifies the Partner Seller by snapshotted trade name or fallback legal name, commercial phone and address without a custom logo or sensitive registration identifiers, names Sabalan only as supplier and deliverer, substitutes retail prices, and exposes only allowlisted customer Contract, Customer, Product, retail total, customer payment, Delivery, public legal, signature, and confirmation facts.
_Avoid_: inventing a separate Partner template or flow, using the Partner User's personal identity, identifying Sabalan as the retail seller or Customer creditor, exposing a case or internal-record number, wholesale inquiry or usage evidence, resale difference, Partner-to-Sabalan terms, Accounting or Sepidar evidence, raw `contractData`, a raw Product Graph, or a shared serialized entity

**تصویر تأیید مشتری پرونده فروش همکار**:
The immutable schema-versioned customer-output evidence minted by the ordinary send-for-confirmation action only after Partner finalization and bound to one committed Case revision, customer Contract number, normalized recipient, expiry, allowlisted content and canonical hash. Customer approval or rejection is independent post-commit evidence and never creates, blocks or automatically erases the Partner's Sabalan debt; a material customer-visible or recipient correction invalidates the prior confirmation and requires the ordinary send action again.
_Avoid_: contacting the Customer before Partner finalization, treating approval as the source of Sabalan debt, automatically voiding on Customer rejection, minting on first public view, rebuilding from live relations, changing the recipient under an existing session, preserving a pending session after material correction, or making the snapshot an editable Product Graph

**نهایی‌سازی فروشنده همکار**:
The authenticated, explicit and audited decision by the Partner Seller that shows the Sabalan obligation, atomically revalidates every required Case-scoped price decision, freezes the Sabalan purchase amount and Partner-to-Customer commercial revision, records any required below-cost acceptance, creates both linked records, and creates the Partner's debt and downstream Accounting and fulfillment eligibility. Customer output, SMS, confirmation, rejection and signature occur only afterward and do not change that commitment; later correction, cancellation and voiding use the formal Partner Case lifecycle.
_Avoid_: Case save, autosave, responder approval, SMS acceptance, customer confirmation, finalization with an invalid or expired price package, silent below-cost acceptance, creating debt before finalization, or directly editing finalized commercial evidence

**برنامه‌های پرداخت پرونده فروش همکار**:
The two independent payment truths in a Partner Sale Case. The Partner Seller records the Partner-to-Customer plan in the sales wizard, while Accounting determines and manages the Partner-to-Sabalan plan after commercial commitment from the approved Sabalan amount. Absence of the Sabalan plan never blocks pricing submission, Partner finalization or later Customer confirmation but blocks final financial approval; corrected Customer plans preserve historical receipt and allocation evidence.
_Avoid_: requiring Partner credit terms before activation, blocking Customer workflow on a missing Sabalan plan, financially approving without that plan, copying one plan into both relationships, rewriting historical Customer receipts, treating a private Customer receipt as a Sabalan receipt, or making the end Customer's schedule a Sabalan receivable

**ریسک وصول مشتری فروشنده همکار**:
The Partner Seller's sole responsibility for collecting the Partner-to-Customer amount. End-Customer delay, default, partial payment, refund, or other collection failure never reduces, suspends, or rewrites the Partner Seller's committed debt to Sabalan.
_Avoid_: conditioning Sabalan's receivable on end-Customer collection, transferring the Partner Seller's resale risk to Sabalan, netting private retail payments against Sabalan Accounting without an actual Partner payment, or presenting customer default as a Sabalan accounting status

**مرز حسابداری پرونده فروش همکار**:
The rule that commercial commitment immediately fixes the Partner Seller's debt and queues only the Sabalan-to-Partner Sale Record for Sabalan Accounting at Case-scoped approved Sabalan prices, while financial approval creates its official Accounting Receivable under the ordinary Sales controls. Accounting's reviewable-contract view may find that internal record by its Case, customer-Contract, or internal-record number and shows only its Sabalan-side financial projection; the Partner-to-Customer Sales Contract, retail prices, payment plan, and actual Customer collections remain outside Sabalan's financial workflow and are available only in authorized Sales and management reporting.
_Avoid_: delaying the Partner Seller's commercial debt until financial approval, creating an official receivable before financial approval, financially approving the retail Contract in Sabalan Accounting, recognizing retail value or margin as Sabalan revenue, letting Accounting edit the Partner's customer price or collection history, or omitting the internal Sabalan record from the ordinary financial controls that apply to Sabalan receivables

**لغو پرونده فروش همکار**:
The coordinated cancellation boundary for both linked records. An active Partner may cancel a numbered pre-commit Case only with a required reason; cancellation atomically closes every pending pricing duty, creates no debt, retains all numbers, revisions and decisions, and never reuses the Case number. Post-commit voiding is requested by the active Partner or a named management remediation actor, separately reviewed by Accounting after every mutable dependency is released, and becomes `باطل‌شده` only when customer cancellation, internal voiding, dated adjustments, audit, and customer-safe notice are recorded together.
_Avoid_: hard-deleting a numbered Case, cancelling without a reason, leaving a pricing duty open, reusing a cancelled Case number, cancelling only one linked record, creating debt from a cancelled Draft, moving a committed Case into its pre-commit cancelled state, letting the requester approve their own Accounting stages, giving the Customer an operational veto, automatically erasing a committed debt or receivable, bypassing dependency disposal, deleting physical or financial evidence, or silently voiding a customer-visible Contract

**حساب من با سبلان**:
The Partner Seller's private read-only financial view of their own Sabalan-to-Partner Sale Records, showing purchase amount, Partner-to-Sabalan payment plan, recorded payments, balance, and customer-safe Accounting status. It excludes internal Accounting notes, approval mechanics, other Partner Sellers, and every unrelated Customer or financial record.
_Avoid_: exposing internal notes or workflow details, allowing the Partner Seller to mutate Accounting truth, mixing end-Customer collections into the balance, showing another Partner Seller's account, or reducing the view to a stale Sales-owned copy

**مانده بدهی فروشنده همکار به سبلان**:
The current owner-scoped balance of committed Sabalan-to-Partner commercial amounts less valid Sabalan Accounting payments, credits, reversals and adjustments attributable to that Partner. It is derived from Sabalan-side authoritative evidence and remains distinct from the Partner's private Customer collections.
_Avoid_: using retail Contract amounts, subtracting a Customer receipt as a Sabalan payment, counting Draft or unfinalized Cases, trusting a Partner-entered balance, or recomputing from a reporting cache as authority

**مانده مطالبات مشتریان فروشنده همکار**:
The current private retail balance owed to one Partner Seller across their committed customer Contracts, derived from each Customer's net retail obligation less valid Partner-recorded Customer receipts, refunds, reversals and retail adjustments. It never becomes a Sabalan receivable and Customer collection risk remains with the Partner.
_Avoid_: exposing another Partner's Customers, importing the balance into Sabalan Accounting, subtracting Partner-to-Sabalan payments, counting a scheduled installment as received cash, or treating Customer default as reduced Sabalan debt

**دریافتی خصوصی مشتری فروشنده همکار**:
The append-only evidence that the Partner Seller actually received money from one of their own Customers for one customer Contract, recording positive amount, effective date, method, optional reference and optional note under the actual actor. It updates only the Partner's private Customer collection truth. A mistake is corrected by a linked reversal or adjustment rather than deletion or overwrite, and neither the original nor correction is a Sabalan Accounting receipt.
_Avoid_: recording a planned installment as a receipt, allowing a receipt for another Partner's Contract, editing or deleting posted evidence, netting it against Sabalan debt, exposing it to a Price Responder or ordinary Accounting user, or presenting it as Sabalan revenue

**چرخه استعلام قیمت فروشنده همکار**:
The Case-revision and per-row lifecycle `در انتظار پاسخ`, `تأییدشده`, `ردشده`, `منقضی‌شده`, `جایگزین‌شده`, or `لغوشده`, with immutable decision evidence and no cross-Case reuse. For each row, `ثبت قیمت سبلان` requires a positive responder rate and shows no explanation field; `رد جهت اصلاح` requires no rate and reveals a mandatory rejection-reason field. A rejection returns the same numbered Case to the Partner with only the rejected row and its affected dependencies open for correction; unchanged accepted rows remain retained. Resubmission creates successor evidence for the changed revision rather than erasing prior outcomes. Each approval has its own 48-hour validity; the complete price package remains usable only until the earliest expiry among its required approved rows. `درخواست مجدد قیمت` preserves the same Case number and creates successor duties only for expired, changed, or otherwise invalid rows and their price-dependent descendants; still-valid unchanged approvals retain their original expiry.
_Avoid_: describing the action as approval of the Partner's price, creating a new Case merely because one row was rejected or expired, editing or erasing decision history, unlocking unrelated accepted rows, reusing a decision in another Case, extending an older approval when a successor row is decided, silently carrying an expired rate forward, displaying an explanation field for acceptance, accepting a non-positive rate, losing the rejection reason, or finalizing with a rejected, pending, or expired required row

**مانع قیمت‌گذاری ردیف فروش همکار**:
The actionable, row-specific reason that prevents an unfinalized Partner Case from accepting or finalizing its current price package when a required quote is expired, changed, rejected, or otherwise incompatible. The Partner retains the numbered Draft and unaffected valid approvals, sees the affected row and reason, and can request a successor inquiry for that row; unresolved evidence-integrity conflicts remain blocked for review rather than being treated as a valid price.
_Avoid_: a generic refresh-only error, discarding the Draft or unrelated approvals, silently accepting a conflicting witness, or making the Partner diagnose internal evidence fields

**ویرایش محصول برگشتی فروشنده همکار**:
A rejected Product row remains the same Case-owned row when its catalog version has changed but the Product is still available; the Partner may correct its frozen configuration while its historical catalog identity and decision evidence remain traceable. A Product no longer available in the current catalog requires explicit replacement, and neither path silently rewrites earlier evidence.
_Avoid_: blocking correction solely because an available Product has a newer catalog version, treating an unavailable Product as currently sellable, silently rebasing historical evidence, or losing the rejected row's history

**نماهای پاسخ استعلام فروشنده همکار**:
`استعلام شده` contains current-revision rows with a recorded approval or rejection, while `در انتظار استعلام` contains sent current-revision rows still awaiting a response; a Case with rows in both states appears in both views. Expired, superseded, and cancelled outcomes remain in Case history rather than either active view.
_Avoid_: classifying a whole mixed Case by a single status, treating an old response as current, hiding a rejection from answered work, or calling an unsent row pending with the responder

**مرز مهاجرت استعلام قدیمی فروشنده همکار**:
Finalized historical Partner records and their pricing evidence remain immutable. A legacy inquiry remains readable history but cannot authorize a new Case-scoped Contract. An open legacy draft is migrated only when its Customer, Product Graph, stable rows, retail inputs and system-calculated components map deterministically to the new Case model; otherwise it remains recoverable and visibly requires Partner review and fresh pricing submission. No historical total is divided, decomposed, guessed, deleted or silently rewritten.
_Avoid_: using a legacy approval for a new Case, guessing a rate from a historical total, deleting an ambiguous draft, silently changing Product or price meaning during migration, repricing a finalized record, or hiding the required Partner review

**وظیفه بین‌واحدی بررسی قیمت فروش همکار**:
The accountable work item created for the assigned Price Responder when a Partner Seller sends one completed but unfinalized Case for Sabalan pricing. It carries the Case revision and technical Product rows, returns independent approval or reasoned rejection outcomes to the Partner dashboard, and never carries Customer identity, retail price, margin or Customer payment evidence.
_Avoid_: using an untracked notification as the queue, exposing retail economics in the duty, approving another Case revision, crediting the responder for the sale, or requiring the Partner to recreate the Contract after a partial rejection

**اعلان استعلام و تأیید مشتری فروشنده همکار**:
The separation between internal Case-pricing notifications and the existing customer Contract confirmation channel. Pricing submission and each responder decision notify only the accountable internal and Partner actors; no Customer SMS, link, confirmation or signature action becomes available until every required Case row is approved, after which the Partner deliberately starts the ordinary customer-confirmation channel from the retail-only snapshot.
_Avoid_: sending inquiry SMS, contacting the Customer before Sabalan pricing completes, adding a Partner-specific customer template or confirmation page, exposing wholesale evidence in the customer channel, or advancing Sabalan debt on a responder decision

**انتقال مالکیت مشتری به فروشنده همکار**:
...
**نبود پاسخ‌دهنده قیمت فعال**:
The fail-closed state when a Partner Seller has no assigned active and authorized Price Responder. New inquiry submission stops with the Persian operational message `برای حساب شما پاسخ‌دهنده قیمت فعال تعیین نشده است.` and creates a trackable Admin support request. The former responder immediately loses ordinary mutation authority, existing approved rows remain valid until normal expiry, and pending rows wait for formal assignment; they remain available to global ADMIN and effective Sales workspace administrators for an audited takeover and response.
_Avoid_: routing sensitive pricing to an unassigned seller or open queue, allowing an inactive former responder to mutate pending rows, invalidating an already approved price early, silently dropping pending rows, allowing inquiry submission without accountable response ownership, or showing an internal permission error instead of the actionable Persian message

**وضعیت پروفایل فروشنده همکار**:
The audited, per-seller Partner profile lifecycle `فعال`, `معلق`, or `غیرفعال`, distinct from the User account's global login state. Direct user-profile conversion creates Active immediately after Price Responder selection; legacy `در انتظار تکمیل` profiles retain their identity and become Active through that same action rather than being recreated. Suspended and Inactive both preserve login and owner-scoped read-only history while freezing new Partner mutations; an authorized company manager or ADMIN may reactivate a Suspended profile or resume an Inactive collaboration without repeating initial onboarding. Inactivation preserves committed Accounting, correction, fulfillment, and audit evidence and may first transfer or close pending work under the lifecycle lock.
_Avoid_: creating a new multi-stage Pending onboarding flow, discarding a legacy Pending profile, treating the profile as an ordinary Role or `isActive` toggle, disabling the global User account when collaboration is inactivated, restoring old internal grants through generic reactivation, requiring initial onboarding or release evidence again to resume collaboration, letting a responder decide suspended work, deleting history at inactivation, stopping committed downstream work, or changing lifecycle without accountable audit evidence

**فعال‌سازی مستقیم فروشنده همکار**:
The single audited management action `تبدیل به فروشنده همکار` that atomically creates or completes the Partner profile, resets incompatible internal Role and grants with recorded dispositions, creates the Partner commercial account and fixed capability bundle, assigns the selected eligible Price Responder, and makes the account Active immediately. It is available to ADMIN and actors with the explicit Partner onboarding and management authorities; it does not wait for a release cohort, readiness witness, global operational opening, separate commercial-identity form, or a later activation action.
_Avoid_: asking the manager to select individual permissions, requiring cohort enrollment, readiness evidence or a second activation click, leaving a successfully converted account in Pending, retaining incompatible internal mutation grants, treating a stale global pause flag as authority, or reporting successful conversion before every atomic effect commits

**مانع اقدام فروشنده همکار**:
A current, complete, action-specific reason that prevents activation, reactivation, or safe conversion reversal. Management sees every blocker in simple Persian with what is blocked, the business reason, the responsible role, and the exact next step; the Partner Seller sees only safe owner-relevant details without internal access, security, or third-party information. Multiple simultaneous blockers are listed independently, and resolving one never hides the others.
_Avoid_: a generic conflict message when an actionable reason is known, exposing enum names or internal identifiers, showing only the first blocker, hiding who must act, suggesting an action the current user cannot perform, or leaking another User's permissions or sensitive evidence

**پاسخ‌گویی پاسخ‌دهنده منتسب**:
The rule that Case-price response requires edit-capable effective Sales access and current assignment. The responder workspace supports searchable concurrent duties, Case-grouped technical rows, independent per-row rates in the fixed Product-family unit, `ثبت قیمت سبلان` without an explanation field, `رد جهت اصلاح` with a mandatory reason, and immutable history. A global ADMIN or authorized Sales manager may take over pending rows atomically with reasoned assignment evidence.
_Avoid_: an ownerless price decision, response by a view-only User, changing the pricing unit, showing the Partner retail price, forcing one rate across unrelated rows, showing or storing an acceptance explanation, overwriting a decided row, changing assignment without history, or granting mutation through reporting access

**محدوده امکانات فروشنده همکار**:
The full ordinary Sales and CRM creation experience constrained to the Partner Seller's own Customers, required owned Customer Projects, recoverable Drafts, numbered Partner Sale Cases, Case-scoped pricing duties, customer payment plans, post-finalization customer confirmation SMS and output, fulfillment status, reporting, private Customer receipts, and حساب من با سبلان. Contract date, Draft entry and recovery, Customer creation and selection, and Customer Project creation and selection use the same workflow, Design System compositions, fields, validation and return behavior as ordinary Sales. Contract creation otherwise retains the ordinary seven-stage vocabulary, searchable selection, complete Product configuration, the ordinary Delivery-list behavior and quantity allocation, multi-method and multi-installment Customer payment entry, validation, recovery, and digital confirmation while substituting the Partner Case-scoped pricing, two-price confidentiality, numbering and two-record finalization boundaries. The Contract list and detail share the ordinary Sales structure, sections, and actions applicable to the Partner's authority. It excludes standalone reusable inquiry creation, Cubic Product, Product-catalog or base-price administration, other sellers' records, price response, Accounting mutation, and authoritative inventory, loading, or Delivery mutation.
_Avoid_: copying ordinary retail presentation while duplicating its behavior, replacing the workflow with a reduced Partner form, weakening Partner pricing or confidentiality boundaries in the name of parity, removing ordinary Product, Delivery, Payment, recovery, or customer-confirmation capability merely because pricing is Case-scoped, allowing a Contract without an owned Customer Project, granting catalog administration, granting cross-seller visibility, exposing internal prices, or replacing action-specific restrictions with one broad Role check

**داشبورد فروشنده همکار**:
The concise operational home for the Partner Seller, showing Cases awaiting Sabalan pricing or Partner finalization, Contract Drafts, Cases awaiting Customer confirmation, upcoming Deliveries, prior internal work awaiting transfer, and the owner-scoped financial overview of Sabalan debt, Customer receivables and actual Customer receipts, with direct actions for a new sale and permitted receipt recording. It follows the ordinary Sales workspace hierarchy and Sabalan Design System, keeps balances and receipt flows visibly distinct, and uses only short guidance that prevents a likely mistake.
_Avoid_: a setup guide after activation, a standalone reusable-price inquiry action, mixing Customer receipts with Sabalan payments, repeating visible information in explanatory paragraphs, exposing internal policy or unrelated seller data, hiding urgent work behind decorative summaries, or diverging visually from the Sabalan Design System

**پیش‌نویس ایجاد پرونده فروش همکار**:
The single creator-private recoverable Partner Contract attempt before Case-number allocation, governed by the ordinary Contract Creation Draft recovery and entry decisions. When it exists, entering new Contract creation offers the same explicit resume or confirmed discard-and-start-empty choice as ordinary Sales; confirmed discard is authoritative across the Partner Seller's editing locations. One editing location owns the writable lease: another active location offers explicit takeover, restores the latest committed recovery there, and makes the previous location read-only before another protected save. Continuing past the valid Product-selection boundary promotes the attempt into its own numbered Partner Sale Case and Case-scoped pricing duty, after which it is no longer this disposable Draft and remains an independently resumable or cancellable numbered Case. A Partner Seller may therefore have at most one unnumbered creation Draft alongside multiple numbered Cases. When legacy data contains multiple unnumbered Drafts, migration retains only the Draft with the latest meaningful business change and automatically discards every older Draft, preserving only the same minimal discard audit fact; page entry, navigation, search, filter text, or another transient interaction never changes which Draft is latest.
_Avoid_: two writable locations, field-by-field merging after takeover, multiple named unnumbered Partner Contract Drafts, a separate Draft-management entry page, silently choosing an older Draft over the meaningfully latest Draft, treating page entry or transient interaction as recency, retaining discarded legacy contents, silently restoring the retained Draft, starting a second unnumbered attempt without confirmed discard, treating a numbered Case as disposable recovery, sharing a Draft across Partner Sellers, loading another Case's quote, or allowing one Draft's recovery state to leak into another

**تصمیم مجوز منبع فروشنده همکار**:
The single authoritative fail-closed decision made by the backend at action time from the Actor, explicit Action permission, Resource, Actor-to-Resource relationship, business purpose, Partner lifecycle, grant expiry, and current assignment. Every mutation re-evaluates this decision inside the same transaction immediately before its write, so current authority defeats stale UI state; a workspace, feature, Role, managerial title, frontend decision, or route-local check may establish eligibility but never removes the Partner Seller's owner-only boundary or grants a more revealing projection.
_Avoid_: authorizing from Role alone, treating list filtering as complete authorization, trusting a client-side gate, caching authority across a write, allowing an expired, revoked, or superseded grant or assignment, skipping nested-resource checks, or implementing inconsistent route-specific ownership rules

**ریشه مجوز فروشنده همکار**:
The authorization root that every nested Partner resource resolves before access: Customer uses its owner User, and every Case revision, pricing duty, row decision, linked record, receipt and report uses its Partner Sale Case's Partner Seller, while every child inherits that root and a direct child identifier grants nothing by itself. A Potential Project additionally preserves its independent Responsible Seller, so Partner access requires both an owned Customer and Partner responsibility without rewriting transferred historical Projects.
_Avoid_: authorizing a child row independently, trusting a submitted parent id, treating Customer ownership as Project responsibility, rewriting historical Project ownership during Customer transfer, or attaching an owned child to a foreign root

**بسته قابلیت فروشنده همکار**:
The fixed non-extensible set of ordinary owner-scoped Partner actions granted automatically when the Partner profile becomes Active; the activating manager does not select individual permissions. The bundle is re-evaluated with lifecycle and resource ownership on every action, while prior internal responsibilities remain available separately only for viewing and requesting transfer.
_Avoid_: asking the activating manager to configure Partner permissions individually, manually widening one Partner Seller's capabilities, extending the bundle through a generic grant, using prior-responsibility access to edit or complete internal work, treating that access as a continuing internal persona, or allowing a suspended or terminated profile to retain Partner mutation authority

**هم‌ارزی کانال مجوز فروشنده همکار**:
The rule that list, detail, search, count, export, notification, PDF, public output, and direct-link navigation enforce the same resource predicate while returning only their purpose-specific allowlisted projection; every target is re-authorized when consumed. No channel may reveal a raw entity, a broader count or existence signal, or data that its corresponding interactive view would hide.
_Avoid_: export-only leakage, notification text that reveals a hidden resource, authorizing from possession of a link, returning raw `contractData`, inconsistent list and detail scopes, or exposing hidden population counts

**گستره مدیریت فروشنده همکار**:
The explicit scope of authorized Partner management: a global ADMIN has company-wide oversight and effective Sales workspace administration grants company-wide inquiry management and audited response takeover. A `MANAGER` title without effective Sales administration, Customer visibility, Price Responder assignment, or reporting access alone never grants Partner resale economics, cross-Partner comparison, or management reporting.
_Avoid_: inferring company-wide Partner access from a title, treating a missing department as global scope, exposing margin through ordinary Sales access, or allowing Customer access to imply Partner Sale Case access

**مالکیت اقدام‌های فروشنده همکار**:
The separation in which authorized User administration owns direct exclusive-persona activation, suspension, and termination; Sales owns Price Responder assignment and commercial case management; CRM owns duplicate Customer identity and ownership transfer; Accounting determines and manages the Partner-to-Sabalan payment plan after sale creation; and Logistics owns fulfillment of the internal Sabalan record. Activation automatically creates the debtor account and Partner capability bundle, without separate commercial-identity, commercial-terms, or credit-terms decisions.
_Avoid_: turning activation into a multi-workspace onboarding form, granting User administration pricing authority, granting Accounting retail mutation, granting Logistics commercial visibility, or treating administrative authority as permission to impersonate Partner-authored evidence

**انحصار persona فروشنده همکار**:
The rule that an Active Partner Seller User cannot simultaneously hold a general internal operational persona or create, edit, approve, or complete internal work. Conversion removes incompatible access but preserves an immutable pre-conversion role-and-access snapshot plus narrow read and transfer-request access to prior responsibilities. Before the first Partner Customer or Partner Sale Case, an audited reversal restores that exact snapshot; afterward the Partner persona is irreversible on that User and a later internal hire receives a separately linked identity.
_Avoid_: combining Partner ownership with general internal authority, using prior-responsibility access to mutate or discover new work, restoring guessed or current role defaults instead of the exact snapshot, silently deleting or transferring prior work, reversing after Partner evidence exists, or deleting grant history
...
_Avoid_: blocking conversion solely because prior work remains open, silently cancelling, completing, or transferring prior work, treating the proposed destination as approval, granting broad internal workspace access, permitting direct mutation through the Partner persona, rewriting historical credit, or exposing unrelated internal records

**عدم نیابت مدیریتی فروشنده همکار**:
The boundary that Sales management and Admin authority permit only named management actions and never impersonation or raw editing of a Partner Seller's Draft, retail price, customer payment plan, or other Partner-authored evidence. Correction, transfer, assignment, lifecycle, and reporting actions retain their own actor and audit meaning instead of being performed as the Partner.
_Avoid_: editing on behalf of a Partner, hiding the true actor, granting broad record mutation through management scope, or bypassing a correction workflow with direct data access

**تقدم قواعد اختصاصی فروشنده همکار**:
The Partner-only exception to general administrative override: even an Admin must have an eligible audited Price Responder assignment, may not author retail evidence as the Partner, and may not process or manager-approve the same Partner financial chain they requested. Partner correction retains its own request, Sales approval, Accounting, and remediation paths rather than inheriting ordinary Accounting-originated correction authority; ordinary internal Sales authority and workflows remain unchanged.
_Avoid_: removing Partner safeguards through a global Admin override, treating audit alone as permission to bypass assignment or separation of duties, importing ordinary correction initiation into a Partner Case, or restricting ordinary internal Sales through this Partner-only exception

**پاسخ امن مجوز فروشنده همکار**:
The non-disclosing outcome contract in which a missing, cross-owner, or out-of-scope resource is indistinguishable as `404`; a visible resource with a forbidden action returns `403`; and an authorized action blocked by lifecycle or business dependencies returns `409` with a Persian operational message. Duplicate detection is the sole owner-existence exception and returns only its approved masked projection.
_Avoid_: revealing another owner's resource through `403`, returning different timing or metadata for hidden existence, reporting a lifecycle conflict as missing permission, exposing internal policy language, or expanding the duplicate projection

**ممیزی مجوز فروشنده همکار**:
The protected evidence of each sensitive authorization decision, recording the Actor, Action, authorization root, effective scope, lifecycle and assignment versions, outcome, reason, and correlation identity without copying unnecessary business secrets. Company-wide Admins and actors with explicit scoped `PARTNER_AUDIT_VIEW` may read it; a Partner Seller sees only their own business history and never internal grants, policy mechanics, or unrelated actors.
_Avoid_: exposing policy internals to a Partner, using ordinary management access as audit access, omitting denied sensitive attempts, storing secret payloads in authorization evidence, or losing the policy inputs that explain a historical decision

**تعلیق و اعتبار قیمت فروشنده همکار**:
The rule that Partner suspension blocks pricing submission, response use and finalization without pausing, extending, shortening or revoking any row approval's own 48-hour clock. If the profile returns to Active, finalization requires every necessary row approval to remain valid; `درخواست مجدد قیمت` renews only expired or otherwise invalid rows and their price-dependent descendants.
_Avoid_: freezing expiry during suspension, granting a fresh 48 hours after reactivation, invalidating a valid row early solely because of suspension, carrying an expired row forward, renewing unchanged valid rows solely because another row expired, or allowing a suspended Partner to create, submit or finalize a Case

**مرز هم‌زمانی استعلام قیمت فروشنده همکار**:
The first-valid-commit rule for competing row response, duty reassignment, Case correction, cancellation, suspension, package expiry, re-request and Partner finalization. Every contender rechecks the current Case revision, row state, assignment, actor eligibility, Partner status and package time boundary at commitment; the first valid commitment is retained and later contenders fail with an explicit retryable conflict, while a successfully finalized Case is never undone by a later lifecycle change. Retrying the same command identity with the same intent returns its recorded whole or per-row result, while reusing that identity for different intent is a conflict and never creates duplicate decisions, duties, Cases or linked records.
_Avoid_: last-write-wins mutation, validating only before the commitment boundary, allowing two terminal outcomes for one pending row, letting stale assignment authorize a response, treating retry as a fresh decision, accepting changed intent under an old command identity, or retroactively invalidating a finalized Case

**گیت فعال‌سازی فروشنده همکار**:
The single user-profile action through which an authorized administrator selects one active eligible Price Responder and confirms exclusive-persona conversion; the system records the standard reason, creates or reuses the Partner profile and automatic debtor account, replaces incompatible general access with the complete Partner capability bundle, preserves read-and-transfer-request access for prior responsibilities, and activates immediately. The target must be active and not already a Partner Seller; self-conversion and conversion of the last Admin are forbidden, and another Admin must remove an Admin role before that User can be converted.
_Avoid_: a separate onboarding workspace, a free-text activation reason, manually selected permissions, duplicated User data, commercial-identity or terms prerequisites, cohort or release input, hidden access changes, lost prior-responsibility scope, recreated legacy Pending profiles, protected Admin conversion, or activation without an eligible Price Responder

**بسته فعال‌سازی فروشنده همکار**:
The immutable, versioned audit evidence created by the server for one direct Partner activation. It binds the User, selected Price Responder, removed incompatible access, automatically granted Partner capability bundle, automatically created debtor account, preserved pre-conversion responsibility scope, actor, reason, command identity, and exact revisions; it contains no commercial-identity, commercial-terms, credit-terms, cohort, or release-readiness prerequisite selected by the browser.
_Avoid_: trusting client-supplied permission or conversion results, reusing activation evidence across Users or revisions, omitting removed or preserved access from the audit, requiring the administrator to assemble gate evidence, or mutating activation evidence after issuance

**انتشار آمادگی فروش همکار**:
The append-only binding of one independently verified `GO` release package to the deployed Partner release, schema, immutable image set, recovery and telemetry evidence, and accountable approvals. Once accepted, the capability is available system-wide for every directly activated Partner Seller without cohort preparation, while every operational pause remains fail-closed; the binding is neither deployment authority nor Partner activation by itself.
_Avoid_: publishing self-reported booleans, treating a test fixture or stale candidate as production evidence, opening traffic or SMS during publication, allowing a signature to cover a failed gate, or carrying readiness across a different release or schema

**نمای عمومی تأیید قرارداد فروشنده همکار**:
The existing public card-and-table confirmation experience reached through the ordinary customer SMS, token or lookup route, and OTP, rendered from the immutable customer-output snapshot while a separate allowlisted live projection may show customer-safe Delivery status. An unverified session is invalidated by cancellation or material revision; a verified session remains read-only until expiry with a visible cancelled or superseded state, never redirects into a successor, and remains distinct from the structurally unchanged full print/PDF template.
_Avoid_: replacing the public surface with the PDF, changing ordinary navigation, rendering current mutable relations, redirecting an old session into a successor, mixing live Delivery status into approved commercial evidence, disclosing wholesale, margin, Partner-to-Sabalan or Accounting facts, or committing Sabalan debt at OTP approval

**تخفیف و سود خالص بازفروش فروشنده همکار**:
The Partner Seller's freedom to apply the ordinary customer-Contract discount only to the Partner-to-Customer Sales Contract without changing the linked Sabalan receivable. Resale profit or loss equals the end Customer's net payable commercial amount after retail discount minus the Partner Seller's net commercial debt to Sabalan; a negative result produces the existing non-blocking below-cost warning and confirmation.
_Avoid_: reducing Sabalan debt with a retail discount, calculating margin from pre-discount retail value, blocking a negative result, applying the Partner's discount policy to the internal Sabalan record, or treating margin as Sabalan income

**مرز مالیات و هزینه پرونده فروش همکار**:
The separation of each linked record's own financial components. Pass-through tax and statutory charges are excluded from resale profit or loss, which compares net commercial amounts. A Sabalan-supplied cost appears through the shared Product Graph in both records, while an end-Customer tax, charge, or fee never enters Sabalan revenue or receivable unless the same component explicitly exists in the Sabalan-to-Partner Sale Record.
_Avoid_: calculating profit from incomparable gross totals, importing retail tax into Sabalan Accounting, dropping a Sabalan-supplied component from one linked record, assuming same-named fees share ownership, or netting unrelated customer charges against Partner debt

**اصلاح پرونده فروش همکار**:
The formal audited successor boundary after Partner Sale Case commitment. Retail-only correction is Sales-approved and becomes effective after one Partner save and fresh Customer confirmation without changing the Sabalan rate, internal record, debt or Accounting projection. Shared-graph, Sabalan-amount, Partner-to-Sabalan-term, or voiding correction additionally requires fresh pricing for every affected row plus the ordinary Accounting processing and manager gates, freezes affected mutation while unresolved, and switches the effective revision atomically only after every required verification and confirmation succeeds. End-Customer rejection does not automatically erase the Partner's committed Sabalan debt; ending that purchase requires the formal reasoned voiding workflow.
_Avoid_: creating an independent customer-only revision, routing retail-only correction through Accounting, changing internal truth for a retail-only correction, automatically cancelling Sabalan debt after Customer rejection, making a successor effective before all gates pass, retaining old customer confirmation for changed visible content, changing the shared Product Graph in only one projection, reusing stale approval after the Sabalan calculation changes, letting the Partner edit Sabalan terms, or rewriting conclusive physical or financial evidence

**فرصت اصلاح پرونده فروش همکار**:
The three-Tehran-working-day, scope-bound opportunity for the active Partner Seller to make exactly one successful complete successor save after the required Sales or Accounting decision. Invalid attempts do not consume it, a successful save immediately relocks editing, and expiry, scope change, or another save requires a fresh request and decision; a suspended or terminated Partner cannot receive the opportunity and management may initiate remediation but never edit as the Partner.
_Avoid_: open-ended correction, multiple successful saves, consuming the opportunity on validation failure, silently extending or widening scope, editing on behalf of an inactive Partner, or making the predecessor ineffective before its successor completes every gate

**شماره‌های پرونده فروش همکار**:
The three immutable, unique, never-reused searchable business identifiers allocated at two explicit boundaries. Sending a valid completed wizard for Sabalan pricing allocates only the Partner Sale Case number. Successful Partner finalization after all required quotes are valid atomically allocates the customer Contract number used in customer output, SMS and public confirmation and the internal Sabalan record number used by Accounting. The end Customer searches and sees only the customer Contract number; the owning Partner may search its Case and customer Contract numbers, Accounting may cross-search all three through its purpose-bound view, and scoped management or Admin may search all three without widening returned projections.
_Avoid_: requiring gapless allocation, exposing the Case or internal number to the end Customer, treating Case-number allocation as debt creation, allocating final record numbers before Partner finalization, letting a Partner search the internal number, widening a projection because an identifier matched, reusing a cancelled number, or allocating any number for a failed command

**عرضه افزایشی فروشنده همکار**:
The system-wide availability boundary for the Partner Seller capability after its accepted release. It has no named cohort, per-User enrollment, or global operational stop: every directly activated Partner Seller may begin work immediately, while suspension and inactivation apply only to the selected Partner profile and integrity incidents remain scoped to their affected resource.
_Avoid_: requiring cohort selection or enrollment during activation, delaying an activated Partner behind a per-User rollout gate, adding a global emergency-pause option, turning off unrelated Partner Sellers or committed downstream obligations, auto-converting existing Users or Contracts, backfilling invented Partner evidence, or restricting ordinary Sales through Partner policies

**پذیرش عرضه فروشنده همکار**:
The evidence-backed readiness of the complete Partner Seller workflow and preserved internal operations, accepted by the release owner with accountable Sales, Accounting, technical/security, and affected HR and Logistics responsibilities. Eligible Users may begin real work immediately after acceptance without a mandatory pilot duration or case-count threshold, while unresolved release-relevant defects prevent acceptance.
_Avoid_: treating code completion alone as operational readiness, imposing an unapproved pilot waiting period, waiving a mandatory safety gate, activating an incomplete workflow, or claiming the absence of all possible defects

**رخداد یکپارچگی فروشنده همکار**:
The tracked, deduplicated and evidence-backed incident created after a confirmed cross-owner disclosure, wholesale disclosure to a Customer, incomplete document pair, duplicate commitment, or unexplained financial or quantity divergence. It blocks only the unsafe affected resource or action through its existing integrity rule and remains available for accountable remediation; it never toggles a global Partner Seller stop or interrupts unrelated profiles and healthy committed obligations.
_Avoid_: turning an incident into a global pause, erasing accepted business facts, stopping unrelated sellers or committed obligations, allowing the affected invalid action to continue, resolving without corrected evidence, or treating ordinary delays and correctly rejected requests as integrity incidents
...
**سنگ لایه وابسته**:
سنگ لایه‌ای که در مسیر پله برای یک کف پله، خیز یا پاگرد مشخص سفارش داده می‌شود، جزء تجاری همان بخش پله است و در قرارداد مشتری محصول مستقل یا ردیف شماره‌دار جداگانه محسوب نمی‌شود؛ فروش مستقل سنگ لایه فقط با ثبت صریح آن به‌عنوان محصول مستقل معنا دارد.
_Avoid_: شماره‌گذاری مستقل لایه وابسته، جداکردن آن از بخش پله مالک، یا مستقل فرض‌کردن آن صرفاً به دلیل داشتن محاسبات ماده و عملیات جداگانه

**نمایش سنگ لایه در قرارداد مشتری**:
نمایش ساده و تجاری سنگ لایه وابسته در گروه بخش پله مالک که با همان ستون‌ها و دستور دیداری سایر گروه‌های محصول، مشخصات سفارش و اقلام قابل‌پرداخت را نشان می‌دهد و جزئیات تولیدی مانند تعداد سنگ‌های منبع، چیدمان برش و ساختار داخلی تخصیص باقی‌مانده را به خروجی‌های تخصصی واگذار می‌کند.
_Avoid_: ساخت جدول یا زبان دیداری اختصاصی برای لایه، نمایش جزئیات بسته‌بندی و منبع تولید در PDF مشتری، تبدیل توضیح فنی به ردیف فروش، یا پنهان‌کردن مقدار و هزینه‌ای که مشتری واقعاً بابت آن متعهد است

**نمایش سنگ لایه در خروجی حسابداری**:
نمایش تفصیلی و قابل‌رسیدگی سنگ لایه وابسته که با همان ستون‌ها و دستور دیداری سایر گروه‌های محصول و ضمن حفظ رابطه با بخش پله مالک، مقدار و ابعاد تجاری، تفکیک سنگ باقی‌مانده بدون هزینه مجدد از سنگ جدید قابل‌پرداخت، و مبنای مقدار، نرخ و مبلغ هزینه نوع لایه، ماده، برش و عملیات را نشان می‌دهد تا هر مبلغ مستقیماً قابل بازبینی باشد.
_Avoid_: جدول اختصاصی لایه، استفاده از نمایش ساده مشتری برای حسابداری، ترکیب منابع رایگان و قابل‌پرداخت در یک مقدار، پنهان‌کردن صفر واقعی ماده باقی‌مانده، نمایش تعداد یا مساحتی که با نرخ و مبلغ همان ردیف هم‌دامنه نیست، یا جداکردن لایه از بخش پله مالک

**تجمیع منبع سنگ لایه در حسابداری**:
منابع سنگ لایه با نوع تأمین، ابعاد و نرخ یکسان در یک ردیف حسابداری تجمیع می‌شوند و منابعی که هر یک از این حقایق در آن‌ها متفاوت است ردیف جدا دارند؛ باقی‌مانده قبلاً پرداخت‌شده نرخ و مبلغ ماده صفر را همراه با علت محاسبه در محصول والد صریحاً نشان می‌دهد.
_Avoid_: نمایش شناسه فنی تک‌تک قطعات، تجمیع ابعاد یا نرخ‌های متفاوت، خالی‌گذاشتن صفر واقعی، یا نسبت‌دادن هزینه عملیات جدید به ماده قبلاً پرداخت‌شده

**تجمیع عملیات سنگ لایه در حسابداری**:
هر عملیات لایه با نوع، نرخ و مبنای محاسبه یکسان در یک ردیف حسابداری تجمیع می‌شود و تفاوت در هر یک از این حقایق ردیف جدا می‌سازد؛ عملیات قابل‌پرداخت روی سنگ باقی‌مانده مستقل از صفر بودن هزینه خود ماده باقی می‌ماند.
_Avoid_: ترکیب عملیات با نرخ یا مبنای متفاوت، تفکیک بی‌دلیل عملیات همسان براساس منبع ماده، یا رایگان فرض‌کردن برش و ابزار صرفاً به دلیل قبلاً پرداخت‌شدن سنگ

**کد ردیف حسابداری سنگ لایه**:
هر ردیف گروه لایه کد مفهوم قیمت‌گذاری‌شده خودش را نشان می‌دهد: ردیف اصلی کد نوع لایه، ردیف ماده کد سنگ همان منبع، و ردیف عملیات کد همان برش یا ابزار؛ نبود کد نوع لایه با خط تیره نمایش داده می‌شود.
_Avoid_: تکرار کد سنگ والد در همه زیردیف‌ها، نمایش کد منبع دیگر، یا اختراع کد برای نوع لایه بدون کد کاتالوگ

**هندسه حسابداری سنگ لایه چندسمتی**:
ردیف اصلی لایه تعداد ست و مساحت نهایی را نشان می‌دهد و هر گروه نوار با طول و عرض متفاوت در زیردیف بدون قیمت جداگانه ثبت می‌شود؛ هندسه‌های کاملاً یکسان می‌توانند تجمیع شوند و این تفکیک تولیدی در قرارداد مشتری نمایش داده نمی‌شود.
_Avoid_: نسبت‌دادن یک طول مشترک نادرست به لایه چندسمتی، قیمت‌گذاری دوباره زیردیف هندسه، مخلوط‌کردن هندسه‌های متفاوت، یا نمایش نوارهای فیزیکی در PDF مشتری

**تطبیق دیداری جمع قرارداد**:
مجموع دقیق تمام مبالغ قابل‌مشاهده و یکتای گروه‌های محصول و خدمت پس از تخفیف با جمع کل فاکتور برابر است و هر ردیف قیمت‌دار مقدار، نرخ و مبلغ هم‌دامنه و قابل‌بازبینی دارد.
_Avoid_: هزینه پنهان فقط در جمع کل، نمایش دوباره یک مبلغ، مقدار گرد‌شده‌ای که مبلغ را توضیح نمی‌دهد، یا ترکیب نرخ و مبلغ مربوط به مبناهای متفاوت

**مقدار قیمت‌گذاری نوع لایه در قرارداد مشتری**:
مقدار و واحدی که هزینه نوع لایه واقعاً از آن محاسبه شده است—ست، نوار، متر طول یا مترمربع—همیشه در ردیف قیمت‌دار مشتری نمایش داده می‌شود؛ تعداد نوار فقط وقتی ظاهر می‌شود که واحد تجاری قیمت‌گذاری است، نه برای توضیح تولید.
_Avoid_: پنهان‌کردن مبنای مبلغ، نمایش نوار فیزیکی غیرقیمت‌گذار در PDF مشتری، یا استفاده از مقدار و واحدی متفاوت از شواهد مالی ردیف

**حکمی سنگ جدید لایه**:
افزایش حکمی فقط بر ماده جدید قابل‌پرداخت لایه در ردیف مستقل اعمال می‌شود و از مبلغ پایه سنگ، هزینه نوع لایه و ماده باقی‌مانده قبلاً پرداخت‌شده جدا می‌ماند.
_Avoid_: ادغام حکمی در مبلغ یا نرخ پایه سنگ، اعمال دوباره آن بر باقی‌مانده پرداخت‌شده، اعمال آن بر هزینه نوع لایه، یا پنهان‌کردن مبلغ حکمی در جمع کل

**دقت دیداری ردیف قیمت‌دار لایه**:
مقدار نمایش‌داده‌شده همان مقدار تجاری مبنای محاسبه مبلغ با حداکثر چهار رقم اعشار و بدون صفرهای پایانی غیرضروری است، به‌گونه‌ای که مقدار ضربدر نرخ دقیقاً مبلغ ردیف را تحت سیاست گردکردن ذخیره‌شده توضیح دهد.
_Avoid_: گردکردن ظاهری ناسازگار با مبلغ، نمایش مقدار فیزیکی به‌جای مقدار قیمت‌گذاری، یا تغییر شواهد مالی ذخیره‌شده برای زیباترشدن خروجی

**عنوان ردیف اصلی سنگ لایه وابسته**:
عنوان کوتاه ردیف اصلی، بخش پله، نوع لایه و سمت‌های اجرا را بیان می‌کند و نام و کد سنگ را به ردیف ماده متعلق به همان منبع واگذار می‌کند؛ سنگ متفاوت فقط در ردیف سنگ جدید خودش نام‌گذاری می‌شود.
_Avoid_: تکرار نام طولانی سنگ در عنوان لایه، پنهان‌کردن هویت سنگ متفاوت، یا مخلوط‌کردن هویت نوع لایه با هویت ماده
...
**بازسازی سازگاری محصولات باقی‌مانده در ثبت و ویرایش**:
بازسازی خودکار و قطعیِ شواهد ناقص محصولات ساخته‌شده از باقی‌مانده، در پیش‌نویس ثبت یا ویرایش مجاز قرارداد، با حفظ هویت و ترتیب تخصیص‌ها، مصرف واقعی سنگ، قیمت صفر مواد اولیه و تمام مبالغ توافق‌شده؛ نتیجه فقط همراه ذخیره موفق قرارداد ماندگار می‌شود. مشاهده قرارداد نهایی‌شده سابقه آن را بازنویسی نمی‌کند و اگر بازسازی یکتا و بدون تغییر مبلغ ممکن نباشد، پیش‌نویس حفظ و ثبت با راهنمای فارسیِ مشخص برای فروشنده و کد پیگیری متوقف می‌شود.
_Avoid_: بازنویسی خودکار قرارداد نهایی‌شده هنگام مشاهده، دورزدن مجوز یا فرایند اصلاح قرارداد، حدس قیمت یا منبع سنگ، تبدیل مبلغ نامعلوم به صفر، حذف خودکار محصول یا تخصیص، موجودشدن دوباره سنگ مصرف‌شده، تغییر پنهانی مبلغ، ثبت بخشی از بازسازی، یا ازبین‌رفتن پیش‌نویس هنگام خطا

**اعتبارسنجی هنگام ویرایش محصول قرارداد**:
هر مقداری که با ابعاد، تعداد، منبع سنگ، ظرفیت باقی‌مانده یا محصول وابسته ناسازگار شود، همان هنگام ویرایش با علت و اقدام اصلاحی کنار فیلد مربوط نشان داده می‌شود؛ ذخیره محصول و ثبت قرارداد فقط لایه‌های اطمینان نهایی هستند، نه نخستین محل کشف خطای قابل‌تشخیص.
_Avoid_: منتظرماندن تا ثبت قرارداد، نشان‌دادن خطای زنجیره‌ای برای مقدار نامعتبر محلی، تغییر خودکار مقدار، یا ذخیره‌کردن بخشی از محصول نامعتبر

راهنمای بازسازی، ردیف مشکل‌دار، مالک اصلی سنگ، تولیدکنندهٔ باقی‌ماندهٔ مصرف‌شده و فرزندان وابسته را با هویت پایدار مشخص می‌کند و ترتیب ساخت مجدد را از منبع به مصرف‌کننده نشان می‌دهد. بازسازی محصول مستقل نیازمند حذف سایر محصولات نیست؛ ترتیب حذف دستیِ لازم برعکس ترتیب وابستگی است و هیچ حذفی خودکار انجام نمی‌شود. اگر خودِ زنجیره قابل اثبات نباشد، ترتیب حدسی پیشنهاد نمی‌شود. موفقیت ثبت وابسته به اعتبارسنجی دوبارهٔ کل زنجیره و حفظ مبلغ تک‌تک ردیف‌هاست، نه فقط برابری جمع قرارداد.
...
گردش عمومی ابطال رکورد مالی در سبلان به شاهد یا مرجع خارجی وابسته نیست؛ دلیل، تاریخ مؤثر، عامل مجاز و در ابطال ناشی از صدور تکراری پیوند به رکورد معتبر باقی‌مانده، شواهد ممیزی ابطال هستند. مرجع خارجیِ موجود در سوابق قدیمی حفظ می‌شود، اما نبود آن مانع ابطال نیست.
_Avoid_: اجبار کاربر به ساختن مرجع خارجی، متوقف‌کردن ابطال داخلی به‌دلیل نبود سند بیرونی، یا حذف مرجع تاریخی موجود
...
**ابطال مرحله‌ای زنجیره مالی**:
ابطال رکورد مالی تأییدشده هیچ دریافت واقعی یا ابزار وصول را خودکار برگشت نمی‌زند. حسابداری ابتدا هر دریافت و چک وابسته را با رویداد معتبر خودش برگشت، عودت یا تعیین‌تکلیف می‌کند؛ سپس دریافتنیِ بدون وصول را باطل می‌کند و تنها پس از آن رکورد مالی قابل ابطال است. هر مرحله سابقه، دلیل، تاریخ مؤثر و عامل خود را نگه می‌دارد.
_Avoid_: ابطال آبشاری دریافت‌ها با یک کلیک، باطل‌کردن دریافتنی دارای اثر وصول، یا ابطال فاکتور پیش از تعیین‌تکلیف وابستگی‌های پایین‌دستی

**برگشت دریافت حسابداری**:
رویداد ممیزی‌شده‌ای که اثر یک دریافت واقعی را در تاریخ مؤثر برگشت خنثی می‌کند، بدون حذف یا بازنویسی دریافت اصلی. دریافت برگشت‌خورده پایانی است و مبلغ پرداخت‌شده و مانده دریافتنی از جمع خالص رویدادهای معتبر وصول و برگشت دوباره محاسبه می‌شوند.
_Avoid_: حذف دریافت، تغییر مبلغ دریافت اصلی، ثبت مبلغ منفیِ بدون پیوند، یا نمایش برگشت به‌عنوان ابطال دریافتنی

**ابطال دریافتنی حسابداری**:
پایان ممیزی‌شده یک مطالبه‌ای که هیچ اثر وصول فعال ندارد. دریافتنیِ وابسته به فاکتور معتبر به‌تنهایی باطل نمی‌شود؛ ابتدا باید علت مالی بالادستی تعیین‌تکلیف شود، و هر دریافت یا چک وابسته نیز پیش از ابطال با رویداد معتبر خودش بسته شود.
_Avoid_: باطل‌کردن دریافتنی دارای مبلغ پرداخت‌شده، پنهان‌کردن دریافت فعال، یا حذف فیزیکی دریافتنی از تاریخچه

**پرونده ابطال زنجیره مالی**:
گردش ممیزی‌شده‌ای که با انتخاب رکورد مالی مبدأ، دلیل، تاریخ مؤثر و عامل مجاز آغاز می‌شود، زنجیره مرتبط را در برابر عملیات مالی جدید قفل می‌کند و ابطال مرحله‌ای دریافت‌ها، چک‌ها، دریافتنی، مالیات و در پایان رکورد مالی را هدایت می‌کند. تا تکمیل مرحله نهایی، رکورد مالی در محاسبات معتبر باقی می‌ماند؛ پرونده هرگز مجوز حذف یا بازنویسی شواهد قبلی نیست.
_Avoid_: ایجاد ناسازگاری موقت با ابطال مستقل دریافتنیِ متصل به فاکتور معتبر، خارج‌کردن زودهنگام فاکتور از محاسبات، یا ادامه ثبت وصول روی زنجیره‌ای که در حال ابطال است

**رکورد معتبرِ باقی‌مانده برای صدور تکراری**:
هنگام ابطال رکورد مالی به دلیل صدور تکراری، یک رکورد صادرشده و باطل‌نشده از همان قرارداد به‌عنوان رکورد معتبر باقی‌مانده انتخاب می‌شود. این پیوند دلیل حذف اثر رکورد تکراری را روشن می‌کند و به معنای انتقال خودکار دریافت، چک، مالیات یا سایر وابستگی‌ها نیست.
...
_Avoid_: حذف پرونده پایان‌یافته، نگه‌داشتن آن در صف اقدام باز، یا نمایش نتیجه بدون شواهد مرحله‌ای

**راهنمای رفع مانع عملیاتی حسابداری**:
نمایش ساده و مرحله‌ای هر مانع کسب‌وکاری یا خطای عملیاتی که به زبان فارسی توضیح می‌دهد چه چیزی متوقف شده، چرا متوقف شده، چه اقدام مشخصی باید بعد انجام شود و کدام نقش مسئول آن است. هرجا اقدام در سامانه ممکن باشد، راهنما کاربر را مستقیماً به همان رکورد و اقدام می‌برد؛ چند مانع به ترتیب وابستگی نمایش داده می‌شوند و مرحله بعدی برجسته است.
_Avoid_: نمایش exception یا نام فیلد داخلی، پیام مبهمی مانند «امکان عملیات وجود ندارد»، ارائه همه جزئیات فنی، مخفی‌کردن نقش مسئول، یا راهنمایی بدون اقدام قابل دسترس

**لغو پرونده ابطال زنجیره مالی**:
مدیر مجاز فقط تا پیش از نخستین تغییر مالی پایین‌دستی می‌تواند پرونده ابطال را با دلیل لغو کند. پس از برگشت دریافت یا چک، ابطال دریافتنی یا هر تغییر مالی دیگر، پرونده باید تکمیل شود و بازگرداندن آثار تنها از گردش اصلاحی مستقل عبور می‌کند؛ لغو و دلیل آن در تاریخچه باقی می‌ماند.
_Avoid_: پاک‌کردن پرونده ابطال، لغو پس از ایجاد اثر مالی، یا بازگرداندن پنهانی رویدادهای انجام‌شده

**مسیر ابطال دریافتنی**:
دریافتنی متصل به فاکتور فقط از پرونده ابطال همان فاکتور تعیین‌تکلیف می‌شود. دریافتنی مستقل تنها وقتی مستقیم باطل می‌شود که هیچ اثر وصول یا چک فعال نداشته باشد. صفحه دریافتنی‌ها مسیر صحیح را بر اساس این پیوند نشان می‌دهد و کاربر را به اقدام بالادستی هدایت می‌کند.
_Avoid_: دکمه ابطال مستقل برای دریافتنی متصل به فاکتور، ایجاد بن‌بست میان فاکتور و دریافتنی، یا نمایش اقدامی که کاربر مجوز اجرای آن را ندارد بدون معرفی نقش مسئول

**بستن اصلاح مبلغ پس از تایید مالی**:
A price-changing correction request after financial approval is closed only after the old approved invoice is voided or reversed with its required Sabalan audit evidence, a replacement invoice candidate is created from the corrected contract amount, that replacement is financially approved, and downstream accounting dependencies are handled or documented.
...
The effective-dated relationship that places Personnel in a Position for a defined period. Personnel may hold concurrent assignments with at most one active Primary assignment and every other active assignment explicitly Secondary or Acting; an Active Employment Relationship without a Primary assignment is a visible condition requiring Human Resources attention rather than an invalid hidden state. Transfers, promotions, and other placement changes close or supersede assignments while preserving the organizational history that applied on each date.
_Avoid_: a mutable Position field on Personnel, multiple primary assignments, unclassified concurrent duties, hiding an active relationship that lacks a Primary assignment, overwriting earlier placements, or reconstructing historical reporting or cost attribution from today's organization

**Employment Assignment Withdrawal**:
The audited removal of an Employment Assignment from organizational effect through one state-specific operation: cancelling a not-yet-effective assignment or ending a current assignment on an effective date with a reason, including when the assignment was created for testing. New void operations are not permitted; legacy void records remain readable audit evidence and contribute to no current or historical organizational calculation. Ending a Primary assignment may leave an Active Employment Relationship visibly flagged as lacking a current Primary assignment until Human Resources records a replacement, with no Position or employment end inferred automatically.
_Avoid_: exposing void as an available operation, permanently deleting an assignment, one ambiguous delete action for every assignment state, rewriting an assignment that was once true, choosing a replacement Position automatically, ending employment implicitly, rewriting legacy void evidence, or hiding the missing-Primary condition
...
The one accountable Personnel selected for an Employment Assignment from active occupants of its supervising Position. A sole eligible occupant is suggested or selected automatically; when several are eligible, saving the subordinate assignment requires an explicit selection whose effective period is fully covered by the supervisor's active assignment. Supervisor responsibility has its own effective-dated history within the subordinate assignment, so a supervisor change preserves the Personnel's unchanged placement while ending the earlier responsibility and beginning the new one. A Primary, Secondary, or Acting assignment in the supervising Position may carry responsibility while its Employment Relationship is Active; Planned and Suspended periods do not. Exactly one Personnel is accountable in each uninterrupted responsibility period, and a Personnel can never be their own Responsible Supervisor.
_Avoid_: arbitrary selection among multiple occupants, a supervisor outside the supervising Position, responsibility extending beyond the supervisor's active assignment, splitting or falsifying the subordinate's placement merely because the supervisor changed, simultaneous co-responsibility for one period, self-supervision, treating Planned or Suspended occupancy as active responsibility, or deleting previous supervisor history
...
The candidate-facing questionnaire contains the applicable personal and contact information, address and postal code, education, employment history, skills, languages, work preferences, requested Position, desired compensation, declaration, and configured written questions. Full name, alias response, birth date and place, military-service status, father's name and occupation, marital status, residential address, mobile and home-phone responses, latest education level, field of study, graduation year, social-media response, and Iranian National Code are required for Human Resources record completeness; number of children and spouse's occupation appear and become required only when the Candidate is married, while a Candidate to whom National Code does not apply supplies a classified foreign identity type and number instead, and none of the family or social details may drive automated scoring, filtering, or rejection. Latest education means the latest completed level and is selected from ابتدایی، متوسطه اول، دیپلم، کاردانی، کارشناسی، کارشناسی ارشد، دکتری، حوزوی، or سایر; selecting سایر requires its separate custom title, while current incomplete study is not represented as a completed level. A legacy education value that exactly matches no standard option is preserved as the custom title under سایر rather than deleted or rewritten. Graduation year uses the canonical Jalali year-only selector, accepts 1300 through the current Jalali year without month or day, and persists one four-digit Latin year. An ambiguous legacy year—such as free text, a future year, a Gregorian-looking year, or a value outside the valid range—is preserved for audit and reported for review rather than guessed; the next edit requires an explicit valid selection. Completely empty optional repeatable rows are ignored, while any partially completed row must be completed or removed.
_Avoid_: exposing internal assessments to the Candidate, silently using family or social details as screening criteria, automatic rejection from record-completeness fields, an undocumented reviewer decision based on sensitive details, a free-text education level outside سایر, deleting or silently standardizing an unmatched legacy education value, saving سایر without its title, presenting an incomplete current course as a completed qualification, accepting a future or non-four-digit graduation year, guessing a conversion for an ambiguous legacy year, inventing required fields beyond the documented scope, or persisting a partial optional row
...
Private, versioned recruitment evidence recorded only by an HR Processor after identifying whether the original was inspected or a copy was received. An original-seen record keeps the category, inspector, server time, version, and optional note without requiring a file; a copy-received record requires an uploaded, malware-checked file available only through authorized, audited delivery rather than public links. Every identity-document presentation labels `ORIGINAL_SEEN` as `مشاهده‌شده` and `COPY_RECEIVED` as `دریافت‌شده`, without redefining the generic `RECEIVED` state in other domains. The Candidate portal exposes safe missing or replacement requests but not internal comparison notes.
_Avoid_: Candidate-uploaded identity evidence in the first version, requiring an upload for original-seen evidence, accepting copy-received evidence without its file, missing inspection-source metadata, presenting an inspected original as a received copy, globally relabeling unrelated received evidence, public file URLs, replacing a rejected version, or exposing internal reviewer notes

**Candidate Identity Verification**:
The field-level Human Resources comparison of an Application Form Submission with received identity evidence: a User with the document-review action permission records documents and marks each check as matching, mismatched, or not applicable only where the domain permits, while a User with the final-identity-clearance permission grants clearance. Each comparison presents a compact title and explicit `مطابق`, `مغایرت`, and conditionally `نامرتبط` actions; the current choice is repeated as a textual, colored badge so meaning never depends on color alone. Selecting mismatch reveals and requires a field-specific discrepancy explanation, while matching and not-applicable choices show no discrepancy field. Document receipt and comparison are one permission-shared HR Work Item; once every required document and check is evidence-complete with no unresolved discrepancy, the system closes that item and creates the separate final-clearance item without requiring a manual completion action. A mismatch blocks clearance until Human Resources returns the identified Candidate fields for resubmission or requests replacement evidence; ordinary Users remain subject to separation of duties, while a Human Resources Broad-Manager Override may deliberately complete both items with automatic self-approval audit.
_Avoid_: one unexplained overall checkbox, color-only comparison state, accepting an unexplained mismatch, showing a discrepancy field before mismatch is chosen, offering not-applicable for an inapplicable field, requiring an HR Manager responsibility assignment, requiring a redundant completion click after the evidence is complete, allowing an ordinary reviewer to approve their own check, approving unresolved discrepancies, changing Candidate answers on their behalf, or replacing evidence without version history

**Candidate Correction Request**:
A single candidate-facing request issued after Human Resources finishes reviewing an Application Form Submission and identifies one or more fields that the Candidate must correct. The request preserves every prior draft and submitted revision, reopens only the identified fields, and sends one SMS directing the Candidate to the fixed `/apply` page. `ذخیره پیش‌نویس` preserves editable correction work and keeps the request visible; `ذخیره و ارسال اصلاحات` validates and submits the corrected revision and closes the correction section after success. A still-valid Application-specific OTP remains usable without being repeated in the correction SMS; if access has expired or been revoked, Human Resources issues a replacement OTP for the same Job Application without deleting or resetting any Candidate information, form revision, correction history, or recruitment evidence.
_Avoid_: sending one SMS for every mismatch click, calling final correction submission `ارسال مجدد`, closing the correction section after draft save, exposing internal field keys, reviewer notes, or sensitive values in SMS, resending an unrecoverable raw OTP, reopening the whole form, creating a new Application for correction, or discarding saved Candidate data when access is replaced

**Deferred Candidate Postal-Code Verification**:
The required exactly 10-digit postal-code response whose basic format is validated while external verification remains explicitly not performed. Its localized digits are immediately normalized to Latin, an invalid format is reported beside the field, and final submission remains blocked until the format is valid. Its deferred external-verification status is visible but blocks no recruitment, conversion, contract, or activation transition; a future verification service may queue historical values without retroactively invalidating completed hires.
_Avoid_: accepting fewer or more than 10 digits, labeling format validation as external verification, blocking today's workflow on an unavailable service, dropping the field, treating a later failed check as automatic retroactive termination

**Candidate Mobile Invitation**:
Application-scoped access issued by Human Resources as a six-digit OTP sent to a normalized snapshot of the Candidate's recorded mobile number, which must contain exactly 11 Latin digits beginning with `09`; Persian, Arabic, or mixed digits normalize immediately, invalid format is reported beside the field, and final submission is blocked. When an authorized creator atomically creates an Application and its required initial Formal Assessment Plan, successful persistence automatically proceeds to invitation issuance and SMS sending and reports the actual send outcome instead of requiring a separate first-send action. A failed or indeterminate SMS outcome preserves the committed Application and plan, distinguishes persistence success from delivery status, and offers authorized resend without pretending delivery or deleting valid recruitment evidence. The Candidate enters that same mobile number and OTP at the fixed public `/apply` entry page, and the pair opens only the associated open Job Application; separate active Applications for the same mobile receive distinct OTPs and are never listed or disclosed by the public page. Editing the Candidate's current mobile does not transfer access: Human Resources must issue a replacement invitation to the new number. The pair remains reusable across devices for seven days. On HR resend, the previous code remains valid until the replacement is successfully used, its original expiry, or a thirty-minute overlap expires; replacement never changes Application data. Five incorrect attempts revoke every active code for the Application and require Candidate contact with HR, while any successful verification resets the failure count. A successful verification creates an Application-only browser session and separately proves Applicant access without rewriting the SMS provider's delivery report. Closing revokes access; formal reopening requires a fresh HR-issued invitation and never restores an old credential. Saved drafts and submitted revisions remain recruitment evidence. Final submission makes the form read-only unless Human Resources returns specified fields for correction, in which case `/apply` shows and edits only those fields with their prior value and field-specific explanation. Public verification gives indistinguishable responses for unknown mobiles and invalid, expired, revoked, or closed-Application invitations, and reveals Candidate or Application details only after success.
_Avoid_: accepting a mobile other than exactly 11 digits beginning with `09`, a dynamic or personal SMS link, Candidate self-service OTP issuance, treating the invitation as a User login or two-factor authentication, opening or revealing another Candidate's Application, reusing one OTP across Applications, revoking the still-valid previous code before the safe overlap ends, granting HR workspace access, applicant enumeration through error details, discarding drafts when access is reissued, or displaying unrelated fields during correction

**Candidate SMS Delivery Evidence**:
The immutable sequence of SMS.ir delivery attempts for an initial Application invitation, Candidate Correction Request, or Candidate Offer Notification. Every attempt retains its own provider message identifier, purpose, time, initiating User or automatic source, immediate provider outcome, separately refreshed delivery state, and retry lineage; provider rejection, provider acceptance, provider-reported delivery, provider-reported failure, unknown status, and successful `/apply` verification are distinct facts. The logical notification is Delivered as soon as any of its attempts is provider-reported delivered, disabling further retry even if a newer or older sibling attempt later reports differently; the newest attempt appears in the primary view and every attempt remains available in expandable history. SabalanERP polls every unresolved attempt at five-minute intervals for at most twenty-four hours and offers authorized Human Resources a manual `به‌روزرسانی وضعیت تحویل` action. `تلاش مجدد ارسال پیامک` requires the recruitment-case management action permission and is available only after immediate provider rejection, provider-reported failure, or an unresolved unknown state after the report window; a provider-accepted attempt still awaiting a report is never resent automatically. A retry is atomically guarded against duplicate submission and begins a two-minute per-notification cooldown, records the actual User and provider result in audit, reuses the current Application OTP while it remains usable, and issues a replacement only after expiry, revocation, or lockout; replacement preserves Application evidence and gives the prior usable code at most the established thirty-minute overlap. The newest currently usable OTP is never included in the ordinary case payload; it may be explicitly revealed inside Formation and Applicant Form only through a separate no-store endpoint requiring both recruitment-case management and full-applicant-information permissions, with a dedicated audit event on every reveal. Issuing a replacement immediately clears any already displayed secret even while the prior code remains temporarily usable under overlap rules.
_Avoid_: treating API acceptance as handset delivery, deriving aggregate state only from the newest attempt, enabling retry after any sibling attempt is delivered, discarding a provider message identifier, overwriting an earlier attempt during retry, accepting concurrent or cooldown-period retries, retrying an accepted attempt still inside its report window, automatic duplicate resend, retry without recruitment-case management permission or actor audit, rewriting unknown delivery as delivered after login, replacing a still-usable OTP merely because delivery failed, exposing an OTP through the ordinary case response, revealing it without both permissions or without audit, leaving a superseded displayed code labeled as current, or polling indefinitely
...
The Candidate's explicit truthfulness attestation made by accepting the declaration and submitting from an OTP-verified Application session. Initial submission, corrected resubmission, and the separately consequential Accepted Offer do not ask the Candidate to retype their name; offer evidence instead binds the decision to the latest submitted Application Form name and blocks while a correction draft is open. Submission audit evidence retains the version, timestamp, masked mobile number, and available IP and device metadata, but it is not the employment contract signature.
_Avoid_: an implicit declaration, requiring a redundant typed-name confirmation for form submission, correction, or offer acceptance, reading identity from a draft correction, treating OTP access alone as acceptance, replacing the later paper contract signature, or losing declaration evidence when the form is corrected
...
The versioned Internal Candidate Assessment before management requirements are finalized. Every interview snapshots the current single company-wide set of default HR criteria, may add case-specific criteria, records the Candidate's answers separately from the interviewer's analysis, and ends with an explicit positive or negative decision and reason; a permitted preliminary decision-maker reads the completed report before preliminary approval, and a permitted management decision-maker reads that report plus the preliminary decision before the later management decision. Selecting any numeric score or Not Assessed completes a scored criterion without requiring its optional note. Every current and historical completed-report version displays an informational arithmetic mean, rounded to one decimal, across all numeric 1–5 criteria in that frozen version, including case-specific numeric criteria while excluding Not Assessed and non-numeric criteria, together with the scored-criterion count; no numeric score displays `میانگین قابل محاسبه نیست`. Criterion scores, their mean, and contextual judgments inform but never automatically calculate the interviewer's decision, and a completed interview is corrected only through a reasoned new version.
_Avoid_: exposing the interview in `/apply`, rendering one criterion description twice, requiring a note only for a numeric stability score, hiding the completed report or its scored-count mean from a permitted decision-maker, averaging Not Assessed or non-numeric criteria, omitting case-specific numeric criteria from the mean, persisting a cross-version average that can drift from frozen evidence, asking for the management decision before requested evaluation results are complete, silently changing an active interview when defaults change, mixing the Candidate's self-description with interviewer analysis, calculating the outcome automatically from scores or their mean, or editing a completed version in place

**Initial HR Interview Criteria Set**:
The published, company-wide version of criteria snapshotted when a new Initial HR Interview begins and managed outside any Applicant case through explicit view and manage action permissions. If an environment has no published criteria yet, the canonical defaults are atomically materialized as version 1 before the first interview draft begins; version 0 is never a valid persisted criteria version. A new ordinary criterion is required and uses Descriptive Answer, Score 1–5, or Yes/No; Yes/No also requires a positive, neutral, or negative decision effect and a negative effect requires a reason. The system-owned eighteenth criterion is the single composite Applicant-Reported Personality Test Summary, whose title, answer shape, required DISC response, active state, and position are fixed while its optional guidance may be changed. Deactivation and reordering of ordinary criteria publish a new version without changing active drafts or completed interviews.
_Avoid_: assigning an HR role to manage criteria, changing an in-progress interview after publication, deleting historical criteria, treating version 0 as a persistable fallback, requiring manual publication before the first interview in a fresh environment, allowing a new ordinary criterion without its required answer, adding Not Assessed to a new 1–5 criterion, recording a negative Yes/No effect without a reason, splitting the three personality-test summaries into separate criteria, or allowing the required composite criterion to be deactivated, reshaped, or moved from eighteenth position
...
**Accepted Offer**:
The Candidate's recorded acceptance of the latest fully approved offer, performed directly through the Application portal or captured as an audited Offline Candidate Offer Acceptance by an authorized User. The portal first requires the Candidate to choose Acceptance rather than rendering acceptance and decline controls together; Acceptance then requires a dedicated unchecked confirmation checkbox followed by an explicit green acceptance action, without asking the OTP-verified Candidate to retype their first name, family name, or full name. Successful recording presents an unambiguous portal confirmation and notifies the responsible Human Resources and Company Management participants internally without sending a second Candidate SMS. It is distinct from the earlier Candidate Submission Declaration. The Job Application remains in the Offer stage pending successful employment conversion; acceptance does not by itself create Personnel employment, reserve capacity, or mean Hired.
_Avoid_: accepting an internally incomplete or obsolete offer version, reusing hidden or stale declaration state, prechecking or omitting deliberate acceptance, requiring redundant name re-entry in the portal, leaving the Candidate unsure whether the action succeeded, omitting the internal acceptance notification, sending a redundant Candidate confirmation SMS, obscuring whether the Candidate acted directly or HR recorded offline consent, treating verbal or recorded acceptance as completed hiring, closing the Application before employment records exist, partially creating employment after acceptance

**Declined Offer**:
The Candidate's direct portal decision, or an authorized User's documented offline record, that the latest fully approved offer is not accepted. The portal first requires the Candidate to choose Decline rather than rendering acceptance and decline controls together; Decline then requires the same structured Persian reason category used by the offline path, may include an explanatory note, and ends with an explicit red decline action. Successful recording presents an unambiguous portal confirmation, blocks Hire Conversion, and notifies the responsible Human Resources and Company Management participants internally with the structured reason category and Candidate note, without sending a second Candidate SMS or automatically closing the Job Application. Company Management may respond with a new offer version or explicitly close the Application with the appropriate outcome; every earlier offer and decline remains preserved and audited.
_Avoid_: treating silence as decline, declining an obsolete or internally incomplete offer, leaving the Candidate unsure whether the action succeeded, converting after decline, automatically closing the Application without a Company Management decision, overwriting the declined offer, losing or omitting the Candidate's reason from the internal notification, sending a redundant Candidate confirmation SMS, or omitting offline declines from the audit history

**Offline Candidate Offer Acceptance**:
An authorized User holding the recruitment-case action permission records that the Candidate accepted the latest fully approved Offer Compensation Summary through a documented phone call, in-person meeting, or other approved offline channel when SMS delivery fails or the Candidate cannot use `/apply`. Default HR Processor and HR Manager profiles may exercise it only while they retain that permission. The recorder selects only the Candidate's decision date, which may be today or a past date but never a future date; no manual time input is requested, while the server separately preserves the exact recording timestamp in audit. The Candidate name comes only from the latest submitted Application Form and is displayed read-only rather than retyped or edited. The record also preserves the communication method, the reason for using the offline path, and an explanatory note; it remains visibly distinct from direct portal acceptance and cannot be silently edited or replaced. Offline decline uses the same authority and communication evidence, requires the same structured decline-reason category as the portal, and may include an optional note.
_Avoid_: deriving authority from a title without permission, describing the recording User as the accepting party, using an offline decision before all internal approvals, deciding an older offer version, accepting a future decision date, asking the recorder to invent a decision time, conflating the declared decision date with the server audit timestamp, asking Human Resources to retype or alter the Candidate name, reading identity from a draft correction, recording consent or decline without communication evidence, hiding the recording User, editing the decision in place, or requiring a second HR approver when the authorized recorder has captured the required evidence
...
The idempotent SMS notification sent automatically after the latest Offer Compensation Summary passes payroll verification, directing the Candidate to the fixed `/apply` page and supplying the current Application-specific OTP required by the approved SMS.ir offer template without another Human Resources release step. A still-valid OTP is reused on an authorized retry; if access has expired, been revoked, or been locked, the system issues a replacement OTP for the same Job Application without changing Candidate information or recruitment history. SMS failure does not reverse the completed verification, remains visibly failed in the Human Resources case view with its Candidate SMS Delivery Evidence, and can be retried explicitly under the shared delivery policy without producing duplicate successful notifications.
_Avoid_: presenting an unverified offer, requiring a manual Human Resources release after verification, omitting the OTP required by the approved offer template, creating a new Application when access expires, replacing a still-usable OTP on retry, discarding Candidate data during access replacement, rolling back verification because an external SMS provider failed, silently losing notification failure or provider message identity, or sending duplicate SMS messages on page refresh
...
The versioned requirement bundle proposed by Company Management for one Application before offer acceptance, containing one or more stably identified, explicitly ordered, required collateral lines such as promissory notes, cheques, guarantees, undertakings, or an explicitly classified other item. Repeated types are allowed when they represent distinct instruments. Promissory-note and cheque lines require a positive whole-Rial amount; guarantor and undertaking lines may omit amount; Other requires its own title and may omit amount. Before the bundle is saved, management may freely add, remove, edit, and reorder draft lines. The Candidate sees every saved line's type and applicable amount with a system-generated statement that Finance will coordinate receipt after offer acceptance; management supplies neither a separate delivery-time value nor free-text Candidate instruction. Adding, removing, reordering, or changing any saved line creates a new bundle version, preserves the prior version, and requires renewed Candidate acceptance; Explicit No Pre-Hire Collateral Requirement is mutually exclusive with every line. Only after acceptance does a Finance recorder add each line's identifier, issuer or guarantor, receipt date, original custody location, and versioned scans, followed by independent Finance verification of every required current line.
_Avoid_: one hardcoded checklist for every Job, representing simultaneous collateral as successive singular requirement versions, merging repeated instruments into one ambiguous line, unstable row identity or order, saving a promissory note or cheque without a positive amount, saving Other without its title, forcing an invented amount for guarantor or undertaking, a second ambiguous obligation field, manually authored Candidate delivery timing or instruction, collecting confidential instrument, guarantor, issuer, or custody details before acceptance or outside Finance, hiding a requirement from the Candidate, combining no-collateral with requirement lines, editing a saved bundle in place, preserving acceptance after the bundle changes, clearing the gate before every required current line is verified, an unclassified attachment, a scan without original-custody evidence, or losing an instrument's return history
...
The Finance-recorded explanation of why a received collateral item is incomplete, mismatched, or otherwise requires correction or coordination. For collateral receipt verification and original-return verification, Finance first chooses Approve or Return for Follow-Up; the reason exists and becomes required only after Return for Follow-Up is chosen, and neither choice commits until Finance explicitly sends the decision. It is the limited coordination explanation Human Resources may see.
_Avoid_: requesting or showing a follow-up reason during approval, submitting immediately when a choice is selected, treating the reason as custody evidence, exposing protected instrument details, or showing an unexplained free-text field before follow-up is selected
...
The Accounting-destination Shared Cross-Workspace Decision Duty for the one latest submitted and unresolved contract version, exposing only the person name and safe case reference, contract number and dates, version and submission evidence, recorder identity, scan, and approve or return actions. Human Resources must withdraw a still-pending submission with a reason before recording its immutable successor; approved and withdrawn versions remain in History, while only the latest valid unresolved version is actionable. Human Resources Users with the independent recording permission retain access to the submitted version, status, and return reason; return closes that review and creates one claimable Human Resources execution duty for a new immutable version whose Accounting review receives a fresh three-Tehran-working-day deadline.
_Avoid_: exposing the Application form, assessments, compensation, collateral, identity documents, or an HR case link; granting contract evidence from general Recruitment Case access; editing a submitted version; submitting a successor before reasoned withdrawal of pending work; showing creation and withdrawal as simultaneous alternatives; showing two actionable versions for one Application; deleting superseded history; reopening a returned review; hiding correction in a notification; inheriting an expired deadline; approving a stale version; or granting source-workspace access through the duty

**Signed Employment Contract Correction Duty**:
The Individual Execution duty in Human Resources created by an Accounting return or a reasoned pending-submission withdrawal, available to active Users with the current contract-recording permission and exposing the prior submitted version plus its structured correction reason. An Accounting return creates a claimable unassigned duty; a permitted User who deliberately withdraws the pending version is atomically assigned that correction duty without a second claim step. The accountable User first saves the immutable successor, prefilled with the prior contract number and validity dates and reusing the prior immutable scan unless an explicit replacement is uploaded, and then explicitly submits it to complete the duty and create a fresh Accounting review, while its independent three-Tehran-working-day deadline may warn but never closes the work.
_Avoid_: a notification-only return, permanent assignment to the prior recorder after an Accounting return, a redundant claim step immediately after self-initiated withdrawal, editing before assignment or claim, allowing another User to record against an assigned duty, editing the prior version, parallel successor drafts, combining save and submission without a reviewable saved state, showing both steps as simultaneous primary actions, inheriting the Accounting deadline, or completing correction without submitting the successor for review

**Signed Employment Contract Submission Withdrawal**:
The reasoned withdrawal by any active User holding the independent contract-recording permission of the latest unresolved submitted contract version before Accounting commits a result and while the Employment Relationship remains Planned, preserving that version while cancelling its review duty with a system reason and atomically assigning the withdrawing User a Contract Correction Duty for an immutable successor. The original recorder does not receive exclusive withdrawal authority, while a concurrently committed Accounting result wins and makes withdrawal unavailable.
_Avoid_: deleting or editing the submitted version, deriving withdrawal authority from title or original-recorder identity, withdrawal without contract-recording permission and a reason, withdrawing an approved version, using this flow after Employment activation, silently removing Accounting work, reopening the cancelled review, or overwriting a committed Accounting result
...
**برنامه ارزیابی عملکرد**:
تعریف نسخه‌دار و قابل‌تکرار هدف، جامعه مشمول و قواعد آغاز ارزیابی‌های عملکرد مانند ارزیابی فصلی یا پایان دوره آزمایشی است. تغییر برنامه فقط ارزیابی‌ها و نوبت‌های آینده را تحت‌تأثیر قرار می‌دهد و خود برنامه سطح عملکرد جاری تولید نمی‌کند.
_Avoid_: تغییر ارزیابی باز با ویرایش برنامه، استفاده از برنامه به‌عنوان نتیجه، یا وابسته‌کردن Badge به تقویم برنامه

**نوبت ارزیابی عملکرد**:
گروه اختیاری ارزیابی‌هایی است که منابع انسانی با هدف مشترک و در یک اقدام مشخص آغاز می‌کند، مانند ارزیابی سالانه ۱۴۰۵. نوبت برای پیگیری و گزارش است و پایان یا تغییر آن سطح عملکرد جاری را خودکار محاسبه یا پاک نمی‌کند.
_Avoid_: الزام هر ارزیابی موردی به عضویت در نوبت، یکی‌گرفتن نوبت با بازه سنجش هر فرد، یا محاسبه Badge از نوبت

**بازه سنجش عملکرد**:
در مدل میراثی فاصله صریح «از تاریخ» تا «تا تاریخ» بود. در جریان ساده ADR-0057، `تاریخ ارزیابی` زمان مشاهده مستقل را مشخص می‌کند و ماه شمسی همان تاریخ فقط گروه گزارش است؛ بازه جداگانه‌ای ساخته نمی‌شود.
_Avoid_: افزودن دوباره فیلدهای از/تا به جریان ساده، سرایت نتیجه به استخدام مجدد، یا تبدیل ماه گزارش به محدودیت ثبت

**بازه گزارش عملکرد**:
بازه انتخابی برای گروه‌بندی، نمودار، رتبه‌بندی، مقایسه و خروجی ارزیابی‌ها و نتایج مصوب است. تغییر بازه گزارش هیچ ارزیابی، امتیاز مصوب یا سطح عملکرد جاری را تغییر نمی‌دهد.
_Avoid_: استفاده از فیلتر گزارش به‌عنوان ورودی Badge، ایجاد نتیجه تازه با تغییر نمودار، یا بازنویسی تاریخچه برای گزارش

**ارزیابی عملکرد پرسنل**:
پرونده ارزیابی یک Personnel در دقیقاً یک Employment Relationship و یک بازه سنجش عملکرد است که یک نسخه ثابت از الگوی ارزیابی، مقادیر واقعی شاخص‌ها، امتیاز محاسبه‌شده و هویت ارزیاب را نگه می‌دارد و در نهایت حداکثر یک نتیجه نهایی دارد. ارزیابی می‌تواند مستقل یا عضو یک نوبت ارزیابی باشد و به‌دست سرپرست مسئول یا مدیر منابع انسانی دارای مجوز انجام شود؛ استخدام مجدد همیشه ارزیابی تازه‌ای می‌سازد و غیرفعال یا بایگانی‌شدن Personnel تاریخچه را حذف نمی‌کند.
_Avoid_: ارزیابی User account، چند الگوی هم‌زمان برای یک ارزیابی، ترکیب دو دوره استخدامی، نتیجه متعارض برای یک بازه، پنهان‌کردن هویت یا مبنای اختیار ارزیاب، یا حذف تاریخچه Personnel غیرفعال و بایگانی‌شده

**بخش ارزیابی مأموریت شغلی**:
بخش مؤثر یک ارزیابی عملکرد پرسنل است که قضاوت مربوط به یک ترکیب پیوسته و بدون تغییر از Employment Assignment، Job، Position، زمینه سازمانی و سرپرست مسئول را در بازه هم‌پوشان با بازه سنجش نگه می‌دارد. هر تغییر مؤثر بخش تازه‌ای می‌سازد و مأموریت‌های اصلی، فرعی و سرپرستی موقت هم‌زمان یا متوالی، در صورت اثر واقعی در بازه، بخش‌های جداگانه‌اند. هر هم‌پوشانی مثبت ابتدا به‌عنوان حقیقت تاریخی حفظ می‌شود؛ سیاست نسخه‌دار ارزیابی تعیین می‌کند آیا مدت و شواهد آن برای قضاوت کافی است. سرپرستان می‌توانند بخش‌های سالم خود را مستقل تکمیل کنند، اما نتیجه مصوب تا زمانی که هر بخش لازم ارسال معتبر نداشته یا دلیل‌دار غیرقابل‌ارزیابی نشده باشد صادر نمی‌شود.
_Avoid_: نادیده‌گرفتن مأموریت فرعی، موقت، کوتاه یا مؤثر، نسبت‌دادن کل بازه به آخرین Position یا سرپرست، کشیدن یک بخش از روی مرز تغییر سازمانی، متوقف‌کردن بخش‌های سالم به‌علت مانع بخش دیگر، حذف یا صفرکردن بخش ناکافی، یا ساخت چند نتیجه مصوب برای یک ارزیابی

**مانع ساختار سازمانی ارزیابی**:
وضعیت آشکاری است که ارزیابی عملکرد پرسنل را هنگام نبود مأموریت اصلی، Position، سرپرست مسئول یا زمینه تاریخی معتبر متوقف می‌کند تا منابع انسانی ساختار را اصلاح کند یا با دلیل ثبت‌شده دوره را غیرقابل‌ارزیابی اعلام نماید.
_Avoid_: حذف بی‌صدای Personnel از دوره، حدس‌زدن سرپرست، تعمیم ساختار امروز به گذشته، یا تولید نتیجه از زمینه ناقص

**ارزیاب عملکرد**:
User فعالی است که یک ارزیابی عملکرد را به‌عنوان سرپرست مستقیم جاری همان Personnel یا مدیر منابع انسانی دارای مجوز مستقل انجام می‌دهد. در جریان ساده، سرپرست فقط از ساختار سازمانی و تخصیص شغلی جاری شناخته می‌شود و هیچ زمینه عملکرد یا مسئول ارزیابی جداگانه‌ای تنظیم نمی‌شود. ارزیابی و نتیجه نهایی همیشه هویت ارزیاب و مبنای اختیار او را ثبت می‌کنند و عنوان مدیر منابع انسانی، بدون مجوز مؤثر، اختیار ارزیابی ایجاد نمی‌کند.
_Avoid_: ارزیاب بی‌هویت، تنظیم سرپرست جداگانه برای ارزیابی ساده، الزام زمینه عملکرد، استنتاج اختیار از عنوان شغلی، پنهان‌کردن اینکه ارزیابی به‌دست HR انجام شده، یا نسبت‌دادن ارزیابی HR به سرپرست

**اختیار ثبت ارزیابی عملکرد**:
مجوز زنده یک User برای ذخیره پیش‌نویس و نهایی‌کردن ارزیابی Personnel دارای رابطه استخدامی فعال است. سرپرست فقط Personnelهایی را ارزیابی می‌کند که ساختار سازمانی و تخصیص شغلی در لحظه ثبت مستقیماً زیر نظر او نشان می‌دهد؛ جایگاه سرپرستی خالی یا دارای چند متصدی به هیچ‌کدام اختیار خودکار نمی‌دهد. پس از تغییر ساختار، سرپرست قبلی دسترسی آینده به پرونده و سابقه Personnel را از دست می‌دهد و فقط هویت او در نتیجه‌هایی که قبلاً ثبت کرده است برای منابع انسانی مجاز باقی می‌ماند؛ سرپرست جدید و مدیر منابع انسانی مجاز می‌توانند ارزیابی تازه ثبت کنند. مدیر منابع انسانی دارای مجوز مستقل می‌تواند هر Personnel فعال و مجاز در دامنه منابع انسانی، از جمله فرد بدون سرپرست یا دارای ساختار مبهم، را ارزیابی کند، اما هیچ ارزیابی‌کننده‌ای نمی‌تواند ارزیابی خودش را ثبت یا نهایی کند. Personnel معلق نشان قبلی خود را حفظ می‌کند اما ارزیابی تازه نمی‌گیرد؛ Personnel پایان‌یافته فقط سابقه دارد و نشان جاری ندارد.
_Avoid_: ارزیابی تازه برای رابطه معلق یا پایان‌یافته، ادامه دسترسی سرپرست قبلی پس از انتقال Personnel، یکی‌گرفتن دسترسی مشاهده با اختیار ثبت، اعطای دسترسی به همه متصدیان یک جایگاه سرپرستی مبهم، تنظیم رابطه سرپرستی مخصوص ارزیابی، مسدودکردن فرد بدون سرپرست برای مدیر منابع انسانی مجاز، خودارزیابی، ارزیابی عمومی بدون دامنه، ثبت نیابتی بی‌نشان، یا تبدیل مدیر منابع انسانی به سرپرست تاریخی

**مجوزهای ساده عملکرد**:
پنج مجوز مستقل `مشاهده ارزیابی‌های عملکرد`، `ارزیابی افراد تحت سرپرستی`، `ارزیابی همه پرسنل`، `مدیریت الگوهای ارزیابی` و `مشاهده نشان در فهرست پرسنل` هستند. مجوزهای مشاهده همراه با `ارزیابی افراد تحت سرپرستی` فقط زیرمجموعه‌های مستقیم جاری را پوشش می‌دهند و همراه با `ارزیابی همه پرسنل` دامنه مجاز منابع انسانی را پوشش می‌دهند. مجوز ارزیابی همه پرسنل در سطح ویرایش، فردبه‌فرد، قابل انقضا و قابل حسابرسی است و عنوان مدیر یا کارشناس منابع انسانی به‌تنهایی آن را ایجاد نمی‌کند.
_Avoid_: یک مجوز کلی برای همه عملیات عملکرد، مشاهده سابقه یا نشان افراد خارج از دامنه سرپرست، استنتاج مجوز از عنوان HR، اعطای گروهی بی‌سابقه، یا یکی‌گرفتن مشاهده با ارزیابی

**مسیر مستقیم ارزیابی عملکرد**:
اقدام یکسان `ثبت ارزیابی` کنار نشان هر Personnel مجاز است که همان فرد و الگوی اصلی را از پیش انتخاب می‌کند، تاریخ را روی امروز می‌گذارد و ارزیاب را مستقیماً به ورود مقادیر واقعی می‌برد. مدیر منابع انسانی، کارشناس منابع انسانی و سرپرست از همین فرم استفاده می‌کنند و فقط دامنه Personnelهای مجازشان متفاوت است. بستن فرم خالی بی‌درنگ انجام می‌شود؛ خروج پس از ورود داده ذخیره‌نشده، پیام `تغییرات ذخیره نشده است` و دو اقدام `ادامه ارزیابی` و `خروج بدون ذخیره` دارد. اگر الگوی اصلی آماده نباشد، اقدام غیرفعال و پیام `فرم ارزیابی آماده نیست` دیده می‌شود؛ فقط دارنده مجوز مدیریت الگو اقدام `آماده‌سازی فرم` را می‌بیند.
_Avoid_: مسیر جدا برای هر عنوان سازمانی، انتخاب دوباره Personnel یا الگو، ازبین‌بردن داده واردشده بدون هشدار، هشدار برای فرم خالی، نمایش اقدام برای فرد غیرمجاز، فرم ارزیابی بدون الگوی اصلی معتبر، نمایش مدیریت الگو به ارزیاب عادی، یا وابسته‌کردن شروع ارزیابی به بازکردن جزئیات ساختار سازمانی

**الگوی ارزیابی عملکرد**:
مجموعه نسخه‌دار معیارهای وزن‌دار مشترک یک Job است که سامانه آن را از مأموریت شغلی مؤثر Personnel انتخاب می‌کند و در صورت وجود با افزوده نسخه‌دار Position همان مأموریت ترکیب می‌نماید. ارزیاب در مسیر روزمره الگو انتخاب یا تخصیص نمی‌دهد؛ هر تغییر آینده نسخه تازه می‌سازد و ارزیابی باز و نتیجه تاریخی نسخه ثابت خود را حفظ می‌کنند.
_Avoid_: انتخاب الگو از نام واحد سازمانی یا شباهت متنی عنوان، یکی‌گرفتن Job با Position، تخصیص دستی الگوی اصلی به هر Personnel، انتخاب الگو به‌دست ارزیاب، انتقال نتیجه مأموریت قبلی به جایگاه تازه، تغییر ارزیابی باز یا نتیجه تاریخی با نسخه تازه، یا معیار شخصی بدون الگوی نسخه‌دار

**افزوده ارزیابی جایگاه**:
مجموعه نسخه‌دار و اختیاری معیارهایی است که فقط انتظارهای واقعاً ویژه یک Position را به الگوی مشترک Job می‌افزاید و همراه با همان مأموریت شغلی در نتیجه ثابت می‌شود. افزوده جایگزین معیارهای پایه Job نیست و تغییر Position نتیجه یا Badge مأموریت قبلی را به جایگاه تازه منتقل نمی‌کند.
_Avoid_: ساخت الگوی کامل تکراری برای هر Position، حذف یا بازنویسی معیار پایه Job، افزودن ویژگی شخصی Personnel، یا ادامه خودکار سطح مأموریت قبلی پس از جابه‌جایی

**ترکیب رفتاری و عملکردی فروشنده**:
ترکیب آغازین الگوی Job فروشنده است که سی درصد امتیاز نهایی را از معیارهای رفتاری و هفتاد درصد را از معیارهای عملکردی می‌سازد. از کل نتیجه، داده کمی عملکردی سامانه شصت‌وپنج درصد، قضاوت عملکردی Supervisor پنج درصد، قضاوت رفتاری Supervisor دوازده درصد، نظرسنجی رفتاری همکاران نه درصد و داده رفتاری معتبر سامانه نه درصد سهم دارند؛ بنابراین داده سامانه در مجموع هفتادوچهار درصد نتیجه را می‌سازد.
_Avoid_: یکی‌گرفتن «هفتاد درصد کیفی» با وزن دسته رفتاری، یکی‌گرفتن «نود درصد کمی» با وزن دسته عملکردی، تغییر پنهان نسبت سی‌به‌هفتاد یا سهم منبع‌ها، جمع وزنی غیر از صد درصد، یا امتیازسازی از داده سیستمی نامعتبر

**رقابت سطح عملکرد**:
مقایسه انگیزشی Personnelهای همتای شغلی است که پس از سنجش هر فرد نسبت به انتظارهای ثابت و نسخه‌دار Job انجام می‌شود. هیچ سهمیه، منحنی اجباری یا تعداد ازپیش‌تعیین‌شده‌ای برای سطح‌ها وجود ندارد و همه افراد واجد شرایط می‌توانند هم‌زمان به بالاترین سطح برسند.
_Avoid_: پایین‌آوردن سطح یک فرد برای ساخت برنده، مقایسه Jobهای ناهمگون، تبدیل رتبه نسبی به حقیقت عملکرد، یا پنهان‌کردن جمعیت و دوره مقایسه

**نظرسنجی رفتاری همکاران**:
کمپین دارای تاریخ شروع و پایان و نسخه‌داری است که User دارای مجوز مستقل مدیریت نظرسنجی برای جامعه صریحی از Personnelهای هدف و پاسخ‌دهندگان فعال می‌کند. سامانه وجود یا نبود ارتباط کاری را اطلاع می‌دهد، اما انتخاب فرد بدون ارتباط کاری را مسدود نمی‌کند و دلیل اجباری نمی‌خواهد. هر گزاره مثبت و مشاهده‌پذیر مقیاس ثابت `هرگز`، `به‌ندرت`، `گاهی`، `اغلب` و `همیشه` با امتیازهای صفر، بیست‌وپنج، پنجاه، هفتادوپنج و صد دارد؛ `اطلاعات کافی برای پاسخ ندارم` گزینه بی‌امتیاز جداگانه است. هر سؤال باید یکی از این پاسخ‌ها را داشته باشد، توضیح متنی اختیاری است و فقط فرم کامل نهایی می‌شود. پرسش، گزینه، ترتیب و قواعد امتیاز در نسخه فعال قفل می‌شوند و تغییر بعد از آغاز پاسخ‌گویی فقط با نسخه یا کمپین تازه ممکن است؛ تعداد و مدت انتخابی کمپین‌ها سقف ندارد.
_Avoid_: الزام رابطه کاری یا دلیل برای انتخاب، کمپین بی‌تاریخ پایان، مقیاس موافقت، واژه `تقریباً`، سؤال منفی یا معکوس، رأی مستقیم به Badge، ویرایش معنای سؤال یا گزینه پس از دریافت پاسخ، پاسخ به خود، یا استنتاج اختیار مدیریت نظرسنجی از عنوان سازمانی

**پاسخ محرمانه نظرسنجی رفتاری**:
پاسخ هویت‌داری است که Personnel هدف و سایر همکاران، هویت پاسخ‌دهنده، پاسخ فردی یا متن خام را نمی‌بینند و نتیجه فقط پس از رسیدن به حداقل جمعیت مصوب به‌صورت تجمیعی قابل‌استفاده است. هویت برای جلوگیری از تکرار و رسیدگی محدود به تخلف حفظ می‌شود؛ متن آزاد امتیاز مستقل ندارد و مشاهده آن فقط برای HR Manager و Company Manager دارای مجوز مخصوص و با حسابرسی مجاز است.
_Avoid_: وعده ناشناس‌بودن مطلق، نمایش پاسخ خام یا هویت به Personnel هدف، امتیازسازی از گروه کوچک، رأی تکراری، افشای متن آزاد بدون مجوز و حسابرسی، یا دسترسی ناشی از عنوان مدیریتی بدون مجوز مؤثر

**زمان تحت کنترل فروشنده**:
مدت برنامه کاری قابل‌انتسابی است که یک قرارداد یا اصلاح آن واقعاً در اختیار فروشنده مسئول قرار دارد؛ تعطیلی، مرخصی و مأموریت ثبت‌شده و انتظار مشتری، Accounting، مدیریت، موجودی، تولید، حمل‌ونقل و سایر واحدها شمرده نمی‌شوند. زمان از واگذاری معتبر آغاز و با ارسال معتبر متوقف می‌شود؛ انتظار امضای مشتری امتیاز زمان نمی‌سازد و فقط رعایت برنامه پیگیری امضا سنجیده می‌شود.
_Avoid_: زمان تقویمی کلی قرارداد، جریمه فروشنده برای انتظار یا تعطیلی خارج از کنترل، توقف ساعت به‌علت بازنکردن پرونده، سنجش مستقیم تأخیر امضای مشتری، حدس مرحله از وضعیت فعلی، یا ترکیب زمان اصلاح با انتظار واحد دیگر

**آورده مالی فروشنده**:
دو شاهد مستقل عملکردی است: ارزش خالص قراردادهای قطعی منتسب به فروشنده پس از لغو و اصلاح، و وجه واقعاً وصول‌شده از تعهدهای سررسیدشده همان فروش‌ها. این دو مبلغ جداگانه سنجیده می‌شوند و جمع‌کردن یا دوباره‌شماری آن‌ها یک شاخص مالی سوم نمی‌سازد.
_Avoid_: یکی‌گرفتن مبلغ قرارداد با وجه وصول‌شده، امتیاز کامل برای قرارداد لغوشده، شمردن یک مبلغ در هر دو شاهد به‌عنوان دو برابر ارزش، استفاده از مبلغ ناخالص پیش از اصلاح، یا نسبت‌دادن وصول بدون انتساب معتبر فروش

**عملکرد وصول فروشنده**:
ترکیب جداگانه نرخ وصول تعهدهای سررسیدشده، رعایت موعد پیگیری، مطالبات بدون پیگیری معتبر و تأخیر پس از پیگیری کامل است. کوتاهی یا تأخیر قابل‌انتساب فروشنده در پیگیری اثر بیشتری از نپرداختن مشتری پس از پیگیری کامل دارد و استثنای مستند خارج از کنترل فروشنده از اثر مستقیم کنار گذاشته می‌شود.
_Avoid_: جریمه یکسان برای نبود پیگیری و نپرداختن مشتری، استفاده از کل مانده بدون توجه به سررسید، امتیازدادن به ثبت صوری پیگیری، یا نسبت‌دادن تأخیر Accounting و اختلاف تأییدشده به فروشنده

**دوره رسمی عملکرد فروشنده**:
یکی از دو بازه ثابت شش‌ماهه فروردین تا پایان شهریور و مهر تا پایان اسفند است که نتیجه رسمی فروشنده از شواهد همین دوره ساخته می‌شود. در هر زمان از دوره و بدون کف زمانی یا سقف تعداد، نتیجه رسمی میان‌دوره‌ای می‌تواند شواهد از آغاز دوره تا تاریخ برش و هدف‌های مبلغی و تعدادی تعدیل‌شده تا همان روز را قفل کند، مسیر بازبینی و اعتراض را بگذراند و Badge را تعیین کند؛ در هر لحظه فقط یک نتیجه همان دوره می‌تواند در وضعیت پیشنهادشده، بازبینی یا اعتراض باشد و تازه‌ترین نتیجه منتشرشده، از جمله نتیجه پایان دوره، جای Badge قبلی را می‌گیرد، اما همه نسخه‌ها در سابقه می‌مانند. نظرسنجی رفتاری نیز بدون سقف تعداد در طول دوره اجرا می‌شود و تغییر Job یا Position بخش جدا می‌سازد.
_Avoid_: کف زمانی یا سقف تعداد برای نتیجه میان‌دوره‌ای، چند نتیجه باز هم‌زمان، استفاده از داده پس از تاریخ برش، هدف کامل شش‌ماهه برای نتیجه میان‌دوره‌ای، تغییر Badge پیش از بازبینی و اعتراض، حذف نتیجه قدیمی از سابقه، محدودکردن تعداد نظرسنجی، نرخ بدون حداقل شاهد، نسبت‌دادن کل دوره به آخرین مأموریت، یا ترکیب دو رابطه استخدامی

**نتیجه رسمی میان‌دوره‌ای عملکرد**:
نتیجه رسمی آینده‌اثر یک دوره باز است که بدون کف زمانی، شواهد را از آغاز دوره تا تاریخ برش، هدف‌های مبلغی و تعدادی را به نسبت روزهای مؤثر و نرخ‌ها را از رخدادهای واقعاً سررسیده همان بازه می‌سنجد. پس از بازبینی و اعتراض، این نتیجه Badge جاری را تعیین می‌کند تا نتیجه رسمی تازه‌تر همان دوره، به‌ویژه نتیجه پایان دوره، جایگزین آن شود؛ هیچ نسخه‌ای از سابقه حذف نمی‌شود.
_Avoid_: پیش‌بینی آینده دوره، شمردن رخداد سررسیدنرسیده، مقایسه هدف کامل با دوره ناقص، ساخت چند نتیجه باز هم‌زمان، حذف هنگام نتیجه پایان دوره، یا چند Badge جاری هم‌زمان

**اثر رسمی نتیجه عملکرد**:
استفاده مستند از نتیجه معتبر عملکرد به‌عنوان یکی از ورودی‌های تصمیم انسانی مستقل درباره جبران خدمت، پاداش، ترفیع، برنامه بهبود یا اقدام انضباطی است. هر پیامد سیاست، اختیار، دلیل و مسیر اعتراض خودش را دارد؛ نتیجه به‌تنهایی پیامد را اجرا نمی‌کند و اصلاح نتیجه، تصمیم وابسته را نیازمند بازبینی می‌سازد.
_Avoid_: تغییر خودکار حقوق یا رابطه استخدامی از Badge، تنبیه با یک نتیجه منفرد، تصمیم بدون دیدن شواهد و عوامل خارج از کنترل، حذف حق اعتراض، یا بی‌اثر دانستن اصلاح نتیجه بر تصمیم وابسته

**گذار هفت‌سطحی عملکرد**:
مرزی آینده‌اثر است که نتیجه‌های پنج‌سطحی قبلی را با همان نام، امتیاز، نشان و نسخه سیاست فقط در تاریخچه حفظ می‌کند، Badge جاری همه Personnelهای فعال را با نام صرفاً `همراه` و وضعیت بدون نتیجه رسمی آغاز می‌کند و فقط ارزیابی‌های دوره‌های تازه را با سیاست هفت‌سطحی می‌سنجد. نخستین نتیجه معتبر هفت‌سطحی Badge جاری را جایگزین می‌کند و هیچ نتیجه تاریخی دوباره طبقه‌بندی نمی‌شود.
...
_Avoid_: نمایش واژه آغازین در نام Badge، ساخت امتیاز صفر، فرض عملکرد ضعیف، ورود به رتبه یا پیامد رسمی، ایجاد نتیجه جعلی، یا نمایش Badge جداگانه ارزیابی‌نشده

**دسترسی سطح عملکرد Personnel**:
Personnel فقط Badge و جزئیات مجاز خودش را می‌بیند؛ Supervisor فقط Badge، عوامل و سابقه زیرمجموعه مستقیم جاری خود را می‌بیند. پروفایل‌های پیش‌فرض HR Processor، HR Manager و Company Manager امکان مشاهده Badge همه Personnelها را می‌دهند، اما جزئیات حساس و هویت پاسخ‌دهنده همچنان مجوز مستقل می‌خواهند و سامانه اختیار را از مجوز مؤثر می‌سنجد نه از شباهت عنوان.
_Avoid_: نمایش Badge همکاران به Personnel عادی، دسترسی Supervisor به خارج از زیرمجموعه مستقیم، افشای خودکار جزئیات حساس با مشاهده Badge، استنتاج اختیار از متن عنوان، یا بازسازی منطق مجوز در رابط کاربری

**جزئیات شخصی عملکرد**:
نمای متعلق به خود Personnel از نام و تصویر Badge، امتیاز کل، سهم رفتاری و عملکردی، امتیاز هر عامل، مقدار و منبع داده سیستمی، نتیجه تجمیعی واجد حداقل جمعیت نظرسنجی، توضیح استاندارد قوت و زمینه بهبود، دوره، ارزیابی بعدی و مسیر اعتراض است. این نما هویت یا پاسخ فردی همکار، متن خام نظر، داده سایر Personnelها و یادداشت محرمانه رسیدگی را آشکار نمی‌کند.
_Avoid_: Badge بدون مسیر اعتراض، عدد بدون منبع، افشای پاسخ‌دهنده یا متن خام، مقایسه نام‌دار با همکار، یا پنهان‌کردن عامل مؤثر از خود Personnel

**خانواده عوامل عملکردی فروشنده**:
هفت گروه نسخه‌دار `آورده فروش` با وزن بیست‌ودو، `وصول و پیگیری مالی` با وزن پانزده، `سرعت گردش قرارداد` با وزن هشت، `کیفیت و دقت قرارداد` با وزن هفت، `لغو، مرجوعی و برگشت سفارش` با وزن پنج، `مدیریت مشتری و پروژه‌های در حال پیگیری فروش` با وزن هشت و `کیفیت تعهد تحویل` با وزن پنج درصد از نتیجه کل هستند. شصت‌وپنج درصد از داده معتبر سامانه و پنج درصد از قضاوت زمینه‌ای Supervisor می‌آید؛ هر عامل فقط با مخرج، بازه، انتساب و دلیل قابل‌اعتماد امتیاز می‌سازد و رخداد خارج از کنترل فروشنده زمینه است نه جریمه.
_Avoid_: غالب‌کردن مبلغ خام فروش، شمارش بدون مخرج، جریمه تأخیر واحد دیگر، انتساب لغو بدون دلیل، امتیازسازی از داده متعارض، تکرار عدد سامانه در قضاوت Supervisor، قضاوت مبهم درباره `پرونده پیچیده`، یا دوباره‌شماری یک رویداد در چند عامل

**عوامل رفتاری مشترک Personnel**:
شش رفتار کیفی `احترام و خوش‌برخوردی حرفه‌ای` با وزن چهار، `همکاری و کار تیمی` با وزن چهار، `مسئولیت‌پذیری و پیگیری تعهدات` با وزن پنج، `ارتباط روشن و مؤثر` با وزن سه، `یادگیری و پذیرش بازخورد` با وزن سه و `آراستگی و رعایت استاندارد مصوب محیط کار` با وزن دو درصد از نتیجه کل هستند. `رعایت برنامه حضور و غیبت` با وزن پنج و `وقت‌شناسی و تأخیر غیرموجه` با وزن چهار درصد، بخش رفتاری سیستمی را می‌سازند؛ مرخصی، مأموریت، بیماری ثبت‌شده و روز غیرکاری اثر منفی و اضافه‌کاری اثر مثبت ندارند.
_Avoid_: قضاوت شخصیت یا ظاهر، سلیقه شخصی درباره پوشش، جریمه غیبت موجه، پاداش اضافه‌کاری، امتیاز رفتار بدون مشاهده کافی، یا معیار رفتاری پنهان

**نگاشت انتظار شغلی به امتیاز عملکرد**:
قاعده نسخه‌داری است که تحقق دقیق انتظار مصوب Job را نزدیک امتیاز هفتادوپنج و سطح `توانمند` قرار می‌دهد و برای عملکرد بالاتر از انتظار تا امتیاز صد فضا نگه می‌دارد. هر عامل سقف و منحنی خودش را دارد تا بیش‌عملکرد در یک عامل، ضعف بحرانی عامل دیگر را نامحدود جبران نکند.
_Avoid_: امتیاز صد برای صرف تحقق هدف عادی، توقف همه عوامل در هدف بدون فضای بیش‌عملکرد، رشد نامحدود مبلغ فروش، یا استفاده از یک فرمول نسبت ساده برای همه انواع عامل

**آستانه هفت‌سطحی عملکرد**:
طبقه‌بندی نسخه‌دار امتیاز نهایی است: کمتر از پنجاه `همراه`، از پنجاه تا کمتر از شصت `کوشا`، از شصت تا کمتر از هفتاد `شایسته`، از هفتاد تا کمتر از هشتاد `توانمند`، از هشتاد تا کمتر از نود `برتر`، از نود تا کمتر از نودوهفت `سرآمد` و از نودوهفت تا صد `الگو`. جزئیات شخصی فاصله از انتظار Job را صادقانه توضیح می‌دهد و نام مثبت جای معنای عددی را نمی‌گیرد.
_Avoid_: مرز هم‌پوشان، پنهان‌کردن پایین‌تربودن از انتظار، امتیاز بیش از صد، یا اعمال آستانه امروز به نتیجه تاریخی

**دروازه سطح بالای عملکرد**:
شرط مکمل امتیاز کل است که برای `برتر` یا بالاتر امتیاز رفتاری دست‌کم هفتاد، برای `سرآمد` یا بالاتر امتیاز وصول و کیفیت قرارداد دست‌کم هفتادوپنج و برای `الگو` امتیاز کل دست‌کم نودوهفت، رفتار دست‌کم نود، پوشش داده کافی، نبود تخلف جدی تأییدشده و نبود خانواده اصلی زیر شصت را لازم می‌کند. گزارش یا ادعای تأییدنشده اثری ندارد و داده ناکافی نتیجه تازه نمی‌سازد.
_Avoid_: خریدن سطح بالا با مبلغ فروش، اثر ادعای بررسی‌نشده، صفرکردن داده مفقود، صدور نتیجه با پوشش ناکافی، یا دورزدن حداقل یک خانواده با میانگین کل

**تخلف جدی قطعی عملکرد**:
تخلف دارای پرونده محرمانه، مدارک، رسیدگی، تصمیم و حق اعتراض مستقل است که فقط پس از قطعی‌شدن در نتیجه عملکرد اثر می‌گذارد. ارزیابی، مرجع پرونده و تصمیم نهایی را Snapshot می‌کند و سرور وجود تخلف قطعی را استخراج می‌کند؛ نبود چنین پرونده‌ای همواره به معنی `خیر` است. گزارش، ادعا یا پرونده در حال رسیدگی اثری بر امتیاز یا سطح ندارد.
_Avoid_: Checkbox دستی، انتخاب پیش‌فرض `بله`، یکی‌گرفتن اعتراض عملکرد با اعتراض انضباطی، اثر گزارش بررسی‌نشده، ثبت بدون مرجع تصمیم قطعی، یا اعلام به دست ارزیاب عادی

**نهایی‌سازی دوره عملکرد**:
مسیر رسمی نتیجه میان‌دوره‌ای یا پایان دوره است که پس از تثبیت و کنترل داده تا تاریخ برش، تکمیل ارزیابی انسانی و نظرسنجی، محاسبه قابل‌بازسازی سامانه و بررسی نقص داده، نتیجه را به Personnel نشان می‌دهد و هفت روز تقویمی برای اعتراض می‌دهد. HR Manager اعتراض را بررسی می‌کند؛ پس از پایان این مسیر نتیجه قفل، Badge جاری جایگزین و استفاده رسمی مجاز می‌شود، بی‌آنکه HR Processor یا Company Manager امتیاز را مستقیم بازنویسی کند.
_Avoid_: اثر رسمی پیش از فرصت اعتراض، استفاده از داده پس از تاریخ برش، ویرایش مستقیم امتیاز، تغییر بی‌دلیل Company Manager، پنهان‌کردن مسیر محاسبه از Personnel، یا بازنویسی نتیجه قفل‌شده

**ارزش خالص قرارداد قطعی فروشنده**:
مبلغ قرارداد کامل و امضاشده دارای فروشنده قطعی و تأیید مالی معتبر است که پس از اصلاحات محاسبه و با لغو معتبر کاهش یا حذف می‌شود. تغییر بعدی رویداد اصلاحی دوره خودش است و فقط اثبات نادرستی نتیجه تاریخی، مسیر رسمی اصلاح همان نتیجه را فعال می‌کند.
_Avoid_: شمردن پیش‌نویس یا قرارداد بدون تأیید مالی، مبلغ پیش از اصلاح، بازنویسی خاموش دوره قبلی، یا حفظ اعتبار فروش لغوشده

**تفکیک اعتبار فروش و مسئولیت وصول**:
ارزش قرارداد و وجه وصول‌شده برای آورده مالی به فروشنده قطعی هنگام تحقق فروش منتسب می‌شوند، درحالی‌که کیفیت و موعد پیگیری وصول به فروشنده قرارداد تعلق دارد مگر اینکه مسئولیت یک دریافتنی با شخص، تاریخ اثر و دلیل معتبر رسماً منتقل شده باشد. انتقال مسئولیت، اعتبار یا مسئولیت گذشته را بازنویسی نمی‌کند و هر پیگیری به مسئول همان زمان منتسب می‌شود.
_Avoid_: دادن اعتبار قرارداد به مسئول تازه مشتری، جریمه فروشنده قبلی برای پیگیری پس از انتقال، یکی‌گرفتن اعتبار وجه با کیفیت پیگیری، مسئول وصول بی‌بازه، یا انتقال بدون تاریخ اثر و دلیل

**فروشنده قطعی یکتای قرارداد**:
تنها فروشنده پاسخ‌گوی قرارداد در لحظه تحقق فروش است که تمام ارزش خالص همان قرارداد به او منتسب می‌شود و انتقال بعدی مسئولیت، اعتبار گذشته را جابه‌جا نمی‌کند. همکاری دیگران از مسیر قضاوت یا تقدیر جداگانه دیده می‌شود و مبلغ فروش میان چند Personnel تقسیم نمی‌گردد.
_Avoid_: چند فروشنده قطعی، تقسیم درصدی مبلغ، انتساب به مسئول جاری به‌جای فروشنده هنگام تحقق، یا بازنویسی اعتبار پس از انتقال

**دوره انتساب آورده مالی فروشنده**:
ارزش خالص قرارداد در دوره تحقق فروش قطعی و هر وجه در دوره دریافت واقعی خودش شمرده می‌شود؛ وجه قرارداد قدیمی نیز آورده دوره جاری فروشنده قطعی همان قرارداد است. نرخ وصول فقط تعهدهای سررسیدشده دوره را با حذف دوباره‌شماری وصول زودهنگام می‌سنجد و هر Financial Record معتبر دقیقاً یک بار شمرده می‌شود.
_Avoid_: شمردن همه اقساط در دوره قرارداد، دوباره‌شماری پرداخت، نسبت‌دادن وصول به دوره سررسید به‌جای دریافت برای شاخص مبلغ، یا ورود تعهد سررسیدنشد‌ه به مخرج نرخ وصول

**طبقه رخداد ناموفق فروش**:
تفکیک `لغو قرارداد` پیش از تحویل نهایی، `مرجوعی` کالای تحویل‌شده، `برگشت سفارش` به مرحله قبلی برای اصلاح و `اصلاح مالی` سند یا مبلغ است. واحد ثبت‌کننده دلیل و مسئولیت اولیه را از فروشنده، مشتری، موجودی یا تولید، Accounting، Logistics یا Guard، مدیریت، مشترک یا در حال بررسی انتخاب و مدیر Workspace مالک آن را تأیید می‌کند. فروشنده حق اعتراض دارد؛ HR Manager فقط اختلاف حل‌نشده را با مدارک تعیین تکلیف می‌کند و رخداد تا قطعی‌شدن بر امتیاز یا Badge اثر ندارد.
_Avoid_: یکی‌گرفتن لغو و مرجوعی، شمردن اصلاح مالی به‌عنوان لغو، رویداد بی‌دلیل یا بی‌مسئولیت، تأیید اولیه به دست HR، حذف حق اعتراض، اثر مورد اختلاف، یا جریمه فروشنده برای علت واحد دیگر

**نرخ لغو منتسب به فروشنده**:
درصد قراردادهای واجد سنجش فروشنده است که پیش از تحویل نهایی با علت قطعی قابل‌انتساب به همان فروشنده لغو شده‌اند. علت منتسب به مشتری، تولید یا موجودی، Accounting، Logistics، Guard یا مدیریت و علت مشترک یا در حال بررسی در صورت این نرخ قرار نمی‌گیرد.
_Avoid_: واحد شمار خام، لغو بی‌علت قطعی، ورود علت واحد دیگر، یکی‌گرفتن لغو با مرجوعی، یا مخرج نامرتبط با قراردادهای واجد سنجش فروشنده

**نرخ مرجوعی منتسب به فروشنده**:
درصد سفارش‌های تحویل‌شده فروشنده است که پس از تحویل با علت قطعی قابل‌انتساب به همان فروشنده مرجوع یا برگشت قطعی شده‌اند. علت منتسب به مشتری، تولید یا موجودی، Accounting، Logistics، Guard یا مدیریت و علت مشترک یا در حال بررسی در صورت این نرخ قرار نمی‌گیرد.
_Avoid_: واحد شمار خام، ورود سفارش تحویل‌نشده در مخرج، مرجوعی بی‌علت قطعی، ورود علت واحد دیگر، یا یکی‌گرفتن مرجوعی با لغو پیش از تحویل

**مشتری جدید معتبر فروشنده**:
Customer یکتایی است که نخستین قرارداد قطعی و معتبرش به فروشنده منتسب شده است. ساخت رکورد CRM، نام تکراری، ادغام رکورد یا قرارداد ناتمام مشتری جدید نمی‌سازد.
_Avoid_: امتیاز برای ثبت CRM، دوباره‌شماری مشتری ادغام‌شده، مشتری بدون فروش قطعی، یا شمارش دوباره در دوره بعد

**پروژه در حال پیگیری فروش**:
یک درخواست یا مذاکره واقعی مشتری است که به فروشنده مشخص سپرده شده و تا رسیدن به قرارداد یا بسته‌شدن بدون قرارداد، در Sales یا CRM با وضعیت، مسئول، زمان آخرین اقدام و علت نتیجه نگهداری می‌شود. نرخ تبدیل فقط پروژه‌هایی را می‌سنجد که در دوره به قرارداد رسیده یا بدون قرارداد بسته شده‌اند؛ پروژه باز وارد مخرج تبدیل نمی‌شود، اما عقب‌افتادن اقدام بعدی آن بیش از سه روز کاری در عامل پیگیری اثر دارد. علت خارج از کنترل فروشنده علیه او محاسبه نمی‌شود و باز نگه‌داشتن عمدی پروژه راه بهبود مصنوعی نرخ نیست.
_Avoid_: عنوان مبهم `فرصت` یا `بالقوه` در رابط کاربر، ورود پروژه باز به نرخ تبدیل، حذف پروژه بسته‌شده بدون قرارداد، پاداش بازماندن عمدی، جریمه نبود موجودی یا تصمیم مدیریت، یا پروژه بی‌مسئول، بی‌دلیل و بدون وضعیت

**اصلاح ورود دستی شاهد عملکرد**:
نسخه دلیل‌دار مقدار کمی دارای مدرک محاسبه ساختاریافته است که HR Processor تا پیش از نهایی‌شدن اصلاح می‌کند و مقدار قبلی و تازه را نگه می‌دارد. مدرک، نوع منبع و رکورد یا گزارش واقعی قابل بازکردن را مشخص می‌کند و متن آزاد جای آن را نمی‌گیرد. پس از نهایی‌شدن فقط نسخه اصلاحی نتیجه مجاز است؛ HR Manager منابع و هشدار اختلاف را هنگام نهایی‌سازی می‌بیند و استخراج خودکار رکورد مبدأ را جایگزین ورود دستی می‌کند.
_Avoid_: عدد بی‌منبع، شناسه ساختگی یا متن آزاد به‌جای مدرک، حذف مقدار قبلی، ویرایش مستقیم نتیجه نهایی، نهایی‌سازی بدون دیدن اختلاف، یا ازبین‌بردن مسیر اصلاح هنگام خودکارسازی

**استخراج خودکار عملکرد فروشنده**:
ساخت فقط‌خواندنی مقدار، شمار شاهد و مدرک محاسبه هر عامل سیستمی از رکوردهای معتبر Workspace مالک آن در Guard، Sales، Accounting و CRM است. هنگام بازکردن ارزیابی، تازه‌ترین داده محاسبه می‌شود و هنگام ثبت نتیجه پیشنهادی، ورودی‌ها تا تاریخ برش در Snapshot قفل می‌شوند. هر استخراج بازه، انتساب فروشنده، نسخه قاعده، رکوردهای ورودی و موارد کنارگذاشته‌شده را حفظ می‌کند؛ اصلاح فقط در Workspace مالک انجام و سپس پیش از انتشار محاسبه دوباره یا پس از انتشار نسخه اصلاحی ساخته می‌شود.
_Avoid_: ورود دستی عامل سیستمی، نتیجه زنده و تغییرپذیر پس از پیشنهاد، اتصال بدون lineage، انتساب از نام یا عنوان، اصلاح کپی در HR، ترکیب داده مورد اختلاف، یا ساخت مقدار از Workspace غیرمالک

**آمادگی داده خودکار ارزیابی فروشنده**:
شرط فعال‌شدن نسخه خودکار ارزیابی است: همه رخدادها و فیلدهای الزامی Sales، Accounting، CRM و Guard، قواعد انتساب، Collectorهای نسخه‌دار، شمار شاهد، مدارک قابل بازشدن و آزمون سازگاری انتهابه‌انتها باید پیش از انتشار همان نسخه آماده باشند. فعال‌سازی یکپارچه انجام می‌شود و تا پیش از آن نتیجه رسمی با ترکیب داده دستی و خودکار ساخته نمی‌شود؛ پس از فعال‌سازی نیز نبود داده معتبر هر خانواده اصلی، ثبت نتیجه رسمی را با دلیل و مسیر اصلاح در Workspace مالک متوقف می‌کند.
_Avoid_: فعال‌سازی نیمه‌کاره، نمایش `داده معتبر هنوز آماده نیست` در نخستین انتشار به علت پیش‌نیاز ساخته‌نشده، پرکردن دستی خلأ منبع خودکار، نادیده‌گرفتن انتساب یا lineage، یا انتشار نتیجه ناقص

**مانع واقعی داده خودکار عملکرد**:
نقص عملیاتی پس از فعال‌سازی کامل نسخه خودکار است که عنوان دقیق رکورد یا رخداد مفقود، Workspace مالک و مسیر مستقیم اصلاح را در ابتدای فرم نشان می‌دهد. ذخیره پیش‌نویس مجاز است، اما ثبت نتیجه رسمی و Override دستی داده سیستمی ممنوع می‌ماند؛ پس از اصلاح مبدأ، `محاسبه دوباره` Collectorها را تا همان تاریخ برش اجرا می‌کند و در صورت رفع همه موانع، مسیر ثبت را باز می‌سازد.
_Avoid_: پیام عمومی `داده آماده نیست`، مخفی‌کردن مالک اصلاح، بازماندن Modal روی پیام پشت آن، اسکرول‌نکردن به مانع، جایگزینی دستی، یا صفرکردن داده مفقود

**زمان کاری تحت کنترل فروشنده**:
مدت برحسب ساعت کاری است که انجام گام قرارداد واقعاً در اختیار فروشنده بوده است. تعطیلی، مرخصی یا مأموریت مصوب، انتظار مشتری و زمان متعلق به مدیریت، Accounting، تولید یا موجودی، Logistics یا Guard از آن حذف می‌شود و رکورد مبدأ باید آغاز، پایان و بازه‌های توقف را بازسازی کند.
_Avoid_: واحد عمومی `مقدار`، ساعت تقویمی، شمردن شب و تعطیلی، جریمه انتظار واحد دیگر، توقف بی‌دلیل، یا مدت بدون رخداد آغاز و پایان معتبر

**زمان نخستین ارسال کامل قرارداد فروشنده**:
فاصله ساعت کاری تحت کنترل فروشنده از رخداد صریح و تغییرناپذیر `شروع کار فروشنده` تا نخستین تحویل کامل و موفق قرارداد به Accounting است. ساخت رکورد، گرفتن قفل ویرایش یا ارسال ناقص آغاز یا پایان قطعی نمی‌سازد و ارسال ردشده زمان را متوقف نمی‌کند.
_Avoid_: شروع از `createdAt` صرف، پایان با اعلان، پذیرش ارسال ناقص، ساعت تقویمی، یا رخداد قابل بازنویسی

**زمان اصلاح فعال قرارداد فروشنده**:
میانگین ساعت کاری چرخه‌های اصلاح قطعی و منتسب به فروشنده است. هر چرخه با فعال‌شدن رسمی اصلاح برای فروشنده آغاز و با ذخیره و ارسال مجدد موفق پایان می‌یابد؛ انتظار ثبت‌شده خارج از کنترل فروشنده حذف می‌شود و چرخه مورد اختلاف تا تعیین تکلیف در محاسبه نمی‌آید.
_Avoid_: واحد عمومی `مقدار`، مجموعی که با تعداد اصلاح‌ها دوباره جریمه می‌کند، شروع از یادداشت آزاد، پایان پیش از ارسال مجدد، یا ورود اصلاح منتسب به واحد دیگر

**زمان امضا تا دریافت حسابداری**:
فاصله ساعت کاری تحت کنترل فروشنده از تغییر قطعی قرارداد به وضعیت امضاشده تا نخستین رخداد تغییرناپذیر و یکتای دریافت همان قرارداد در Accounting است. ساخت Financial Record یا اعلان خودکار به‌تنهایی دریافت حسابداری محسوب نمی‌شود.
_Avoid_: شروع پیش از امضای قطعی، پایان با ساخت رکورد نامرتبط، اعلان به‌جای دریافت، چند پایان برای یک نسخه قرارداد، یا ساعت تقویمی

**استخراج حضور و وقت‌شناسی برای عملکرد**:
تصویر فقط‌خواندنی Guard است که `رعایت برنامه حضور` را از نسبت روزهای کاری واجد سنجش بدون غیبت غیرموجه و `وقت‌شناسی` را از نسبت روزهای حضور واجد سنجش بدون تأخیر تعدیل‌شده می‌سازد. مرخصی، بیماری و مأموریت تأییدشده و روز غیرکاری از مخرج حذف می‌شوند، اضافه‌کاری امتیاز مثبت نمی‌سازد و نسخه برنامه، اصلاحات و شمار روزها در مدرک محاسبه حفظ می‌شود.
_Avoid_: ورود دستی درصد، شمردن غیبت موجه، پاداش اضافه‌کاری، استفاده از برنامه امروز برای گذشته، یا درصد بدون شمار روز و مرجع Guard

**هدف آغازین زمان گردش قرارداد فروشنده**:
حداکثر قابل‌قبول اولیه برای نخستین ارسال کامل قرارداد هشت ساعت کاری، برای اصلاح فعال قرارداد چهار ساعت کاری و برای فاصله امضا تا ارسال معتبر به Accounting چهار ساعت کاری است. Company Manager فقط برای دوره آینده می‌تواند نسخه تازه این هدف‌ها را تعیین کند و رابط، آغازین و قابل‌تغییربودن آن‌ها را صریح نشان می‌دهد.
_Avoid_: هدف عمومی `۱ مقدار`، ساعت تقویمی، تغییر هدف دوره آغازشده، پنهان‌کردن موقت‌بودن هدف آغازین، یا یک هدف مشترک برای هر سه مدت

**تاریخ تحویل ثبت‌شده فروشنده**:
تاریخ مورد انتظار تحویل است که فروشنده با توجه به اطلاعات معتبر ظرفیت در قرارداد ثبت می‌کند و تغییراتش در سابقه می‌ماند. دقت این تاریخ از وعده شفاهی جداست و تأخیر قطعی منتسب به مشتری، مدیریت، تولید یا موجودی، Accounting، Logistics یا Guard علیه فروشنده محاسبه نمی‌شود.
_Avoid_: اصطلاح مبهم `وعده`، تاریخ شفاهی، تغییر بی‌سابقه، جریمه علت واحد دیگر، یا فرض مسئولیت فروشنده بدون انتساب قطعی

**تقدیر رقابتی عملکرد**:
اعلام اختیاری و مثبت برندگان دستاوردهایی مانند رشد فروش، نظم پیگیری، کیفیت قرارداد یا عملکرد کلی در پایان دوره است، بدون جدول Badgeها، امتیاز دیگران یا معرفی افراد پایین‌تر. معرفی نام‌دار با اطلاع برنده انجام می‌شود و دسترسی شخصی Badgeها را گسترش نمی‌دهد.
_Avoid_: جدول رتبه عمومی، افشای امتیاز یا سطح همکاران، معرفی پایین‌ترین افراد، یا استفاده از تقدیر برای دورزدن محرمانگی Badge

**بانک آغازین گزاره‌های رفتاری**:
مجموعه قابل‌ویرایش دوازده گزاره مثبت و مشاهده‌پذیر در شش عامل مصوب است: رفتار محترمانه و آرام و پیگیری محترمانه اختلاف؛ اشتراک به‌موقع اطلاعات و همکاری منطقی در کار مشترک؛ پیگیری تعهد و اعلام زودهنگام تأخیر و پذیرش و اصلاح اشتباه؛ انتقال روشن و کامل اطلاعات و ثبت تصمیم یا تغییر مهم در مسیر مناسب؛ بررسی بازخورد و اصلاح روش و یادگیری و به‌کارگیری فرایند تازه؛ رعایت استاندارد مصوب پوشش، ایمنی و بهداشت و تمیزی و نظم فضای کار و ابزار مشترک. Supervisor و همکاران در نسخه آغازین از همین بانک مشترک استفاده می‌کنند، ولی سهم هر منبع جداگانه محاسبه می‌شود. هر نسخه فعال متن همان زمان را قفل می‌کند و `اطلاعات کافی برای پاسخ ندارم` پاسخ معتبر بی‌امتیاز باقی می‌ماند.
_Avoid_: آزمون شخصیت، سؤال پزشکی یا خانوادگی، قضاوت ظاهر و هویت، متن توهین‌آمیز، یا تغییر پاسخ تاریخی با ویرایش بانک

**قضاوت زمینه‌ای Supervisor فروشنده**:
دو گزاره محدود است: `برای پروژه‌های در حال پیگیری فروش، وضعیت و اقدام بعدی را روشن ثبت می‌کند و مشتری را بدون پاسخ رها نمی‌کند` با وزن سه درصد و `تاریخ تحویل را براساس اطلاعات موجود تعیین می‌کند و تغییرات مؤثر را به‌موقع ثبت و اعلام می‌کند` با وزن دو درصد از نتیجه کل. Supervisor آن‌ها را با همان مقیاس رفتاری پنج‌گزینه‌ای ثبت می‌کند و عدد موجود سامانه را تکرار یا جایگزین نمی‌کند؛ `پرونده پیچیده` به دلیل تعریف‌ناپذیری عامل ارزیابی نیست.
_Avoid_: امتیاز دوباره مبلغ فروش، قضاوت کلی و بی‌موضوع، عامل مبهم پرونده پیچیده، عدد آزاد بدون راهنمای رفتاری، جایگزینی داده سیستمی، یا وزن بیش از پنج درصد

**نسخه پاسخ نظرسنجی رفتاری**:
پاسخ creator-owned است که پاسخ‌دهنده تا پایان کمپین می‌تواند پیش‌نویس یا اصلاح و نهایی کند و فقط آخرین نسخه نهایی آن در تجمیع اثر دارد. پس از پایان، پاسخ قفل است و اصلاح فقط با درخواست خود پاسخ‌دهنده نسخه جانشین می‌سازد؛ HR Manager پاسخ را به‌جای او تغییر نمی‌دهد.
_Avoid_: اثر پیش‌نویس، چند نسخه هم‌زمان در امتیاز، ویرایش پس از پایان، اصلاح به نام HR، یا حذف سابقه نسخه قبلی

**مرز اصلاح داده خودکار عملکرد**:
قاعده‌ای است که مقدار استخراج‌شده از Sales، Accounting یا Guard را در ارزیابی غیرقابل‌ویرایش می‌کند و اصلاح را فقط در Workspace مالک داده و سپس محاسبه مجدد می‌پذیرد. شاهدی که هنوز در مبدأ قابل‌اصلاح نیست `مورد اختلاف` می‌شود و تا تعیین تکلیف اثر ندارد؛ Override دستی عدد سیستمی مجاز نیست.
_Avoid_: اصلاح کپی در HR، Override عدد مبدأ، امتیاز شاهد مورد اختلاف، یا واگذاری مالکیت حقیقت عملیاتی به ارزیابی

**مرحله نخست گسترش ارزیابی عملکرد**:
دوره‌ای است که همه Personnelهای فعال Badge با نام `همراه` دارند و می‌توانند در نظرسنجی رفتاری واقعی شرکت کنند، اما فقط Job فروشنده دارای ترکیب کامل رفتاری و عملکردی و نتیجه رسمی هفت‌سطحی است. داده رفتاری Jobهای دیگر فقط هنگامی در نتیجه آینده به کار می‌رود که با دوره رسمی آن Job منطبق و هنوز معتبر باشد.
_Avoid_: ساخت Badge امتیازدار بدون الگوی عملکردی Job، استفاده از نظرسنجی منقضی یا ناهم‌دوره، محدودکردن اصل نظرسنجی به فروشندگان، یا نمایش نام متفاوت برای همراه بدون نتیجه رسمی

**تعارض منافع ورود عملکرد**:
وضعیتی است که HR Processor یا هر عامل عملیاتی، موضوع داده یا نتیجه‌ای است که می‌تواند آماده، اصلاح یا تأیید کند؛ در این حالت عامل دیگری با اختیار معتبر کار را انجام می‌دهد و علت جایگزینی حسابرسی می‌شود. تعارض اعلام‌شده مرتبط نیز همین منع خودکنشی را دارد.
_Avoid_: ورود یا اصلاح داده خود، تأیید نتیجه خود، نادیده‌گرفتن تعارض اعلام‌شده، یا جایگزینی عامل بدون سابقه

**تحویل عملکرد به Personnel بدون User**:
تحویل احرازشده خلاصه شخصی از مسیر HR و ثبت اعتراض با رسید قابل‌پیگیری برای Personnel فعالی است که حساب User متصل ندارد. این تحویل هویت پاسخ‌دهنده یا متن خام را آشکار نمی‌کند و حساب عمومی یا مسیر OTP جداگانه نمی‌سازد.
_Avoid_: محروم‌کردن Personnel بدون حساب، تحویل بدون احراز هویت، اعتراض بی‌رسید، افشای نظر خام، یا ساخت دسترسی عمومی

**چرخه Badge رابطه استخدامی**:
Personnel فعال Badge جاری دارد؛ Personnel معلق آخرین Badge معتبر را بدون ارزیابی تازه نگه می‌دارد و Personnel پایان‌یافته فقط تاریخچه مجاز دارد. استخدام مجدد با Badge دارای نام `همراه` و وضعیت بدون نتیجه رسمی آغاز می‌شود و نتیجه رابطه قبلی منتقل نمی‌گردد؛ نتیجه میان‌دوره‌ایِ نهایی‌شده پس از خروج فقط شاهد تاریخی است و Badge جاری برای رابطه پایان‌یافته نمی‌سازد.
_Avoid_: ارزیابی تازه در تعلیق، Badge جاری برای رابطه پایان‌یافته، انتقال Badge به استخدام مجدد، حذف تاریخچه، یا انتقال نتیجه یک رابطه استخدامی به رابطه دیگر

**سیاست نگهداری ارزیابی هفت‌سطحی**:
سیاست نسخه‌دار مصوب Company Manager برای مدت نگهداری پیش‌نویس، پاسخ عددی، متن خام، نتیجه نهایی، اعتراض، تصمیم وابسته و حسابرسی است که متن خام را زودتر از نتیجه تجمیعی حذف یا ناشناس می‌کند. اعتراض، رسیدگی یا توقف حقوقی حذف مرتبط را متوقف می‌کند و اجرای حذف رسید غیرشخصی می‌سازد؛ حذف خودکار پیش از انتشار سیاست فعال نمی‌شود.
_Avoid_: نگهداری نامحدود بدون سیاست، یک مدت برای همه طبقات، حذف هنگام رسیدگی، حذف بدون رسید، نگهداری متن خام به‌اندازه نتیجه تجمیعی بدون ضرورت، یا ادعای انطباق قانونی از سیاست داخلی

**ترکیب مأموریت‌های عملکردی**:
ترکیب بخش‌های مستقل هر Job و Position هم‌زمان یا متوالی براساس حاصل روزهای مؤثر و درصد تخصیص کاری است. عنوان اصلی یا فرعی به‌تنهایی وزن نمی‌سازد، تخصیص مفقود حدس زده نمی‌شود و انتقال به Job تازه Badge Job قبلی را به آن منتقل نمی‌کند.
_Avoid_: وزن برابر دلخواه، غالب‌کردن مأموریت اصلی از روی عنوان، حدس تخصیص، دوباره‌شماری هم‌پوشانی، یا حمل Badge میان Jobها

**نمای مدیریتی همراه بدون نتیجه**:
نمای همان Badge با نام صرفاً `همراه` در فهرست مدیر مجاز است که وضعیت جداگانه `بدون نتیجه رسمی` و امکان فیلتر آن از همراه حاصل از ارزیابی را فراهم می‌کند. این افراد در توزیع سطح‌های ارزیابی‌شده شمرده نمی‌شوند و فقط با شمار جداگانه `در انتظار نخستین ارزیابی` گزارش می‌شوند.
_Avoid_: افزودن آغازین به نام Badge، Badge تصویری جداگانه ارزیابی‌نشده، شمردن حالت بدون نتیجه به‌عنوان عملکرد پایین، پنهان‌کردن وضعیت از مدیر مجاز، یا ورود به رتبه و پیامد رسمی

**سنگ سطح عملکرد**:
نگین کشیده هم‌شکل در قاب و سبک ثابت Badge است که برای هفت سطح به‌ترتیب از عقیق، کهربا، فیروزه، زمرد، یاقوت کبود، یاقوت سرخ و الماس استفاده می‌کند. تشخیص سطح فقط به رنگ وابسته نیست و نام فارسی سطح همیشه در تجربه قابل‌دسترسی همراه نشان باقی می‌ماند.
_Avoid_: تراش متفاوت میان سطح‌ها، تغییر قاب یا سبک Badge، معنای فقط‌رنگی، حذف نام سطح، یا استفاده از یک سنگ برای دو سطح

**مدیریت ساده نظرسنجی رفتاری**:
اختیار HR Manager و Company Manager برای ایجاد، ویرایش، مرتب‌سازی، حذف و فعال‌سازی پرسش‌ها در شش بخش ثابت عوامل رفتاری است؛ HR Processor فقط وضعیت پاسخ و نتیجه تجمیعی را می‌بیند. پرسش‌های هر بخش وزن برابر دارند، فرم فعال قفل است و تغییر آن نسخه تازه می‌سازد.
_Avoid_: گردش تأیید چندمرحله‌ای، تغییر فرم فعال، وزن پنهان سؤال، افزودن بخش خارج از عوامل مصوب، یا اختیار ویرایش برای HR Processor

**تجمیع نظرسنجی‌های رفتاری دوره**:
ترکیب همه نظرسنجی‌های معتبر و نامحدود یک دوره است که پاسخ‌های هر ترکیب پاسخ‌دهنده، Personnel هدف و عامل رفتاری را ابتدا میانگین می‌گیرد و سپس میان پاسخ‌دهندگان یکتا با وزن برابر ترکیب می‌کند. تکرار پاسخ یک شخص قدرت رأی او را افزایش نمی‌دهد، متن آزاد امتیاز ندارد و هر عامل دست‌کم سه پاسخ‌دهنده یکتای دارای پاسخ امتیازدار می‌خواهد؛ تا رسیدن همه عوامل لازم به این کف، ثبت نتیجه رسمی متوقف می‌ماند.
_Avoid_: محدودکردن تعداد نظرسنجی، وزن‌دادن به تعداد فرم‌های یک پاسخ‌دهنده، شمارش پاسخ تکراری، امتیاز متن آزاد، صفر یا بازتوزیع وزن کمبود پاسخ، یا نتیجه رسمی از کمتر از سه پاسخ‌دهنده معتبر در هر عامل

**دامنه انتخاب نظرسنجی رفتاری**:
انتخاب صریح سؤال‌ها، Personnelهای هدف، Personnelهای پاسخ‌دهنده و زمان شروع و پایان به دست HR Manager یا Company Manager است. ارتباط کاری فقط به‌صورت اطلاعات کمکی نشان داده می‌شود و نبود آن مانع یا نیازمند دلیل نیست. پاسخ به خود و پاسخ تکراری همان فرم ممنوع است؛ هر دو طرف باید فعال باشند و `اطلاعات کافی برای پاسخ ندارم` پاسخ بی‌امتیاز معتبر است، بی‌آنکه تعداد کمپین، هدف، اجرا یا مدت انتخابی محدود شود.
_Avoid_: خودارزیابی، پاسخ تکراری، الزام ارتباط کاری یا دلیل، اجبار به قضاوت بدون شناخت، هدف یا پاسخ‌دهنده غیرفعال، کمپین بی‌پایان، یا سقف تعداد و مدت انتخابی نظرسنجی

**دسترسی متن نظرسنجی رفتاری**:
مرز محرمانگی‌ای است که Personnel هدف، Supervisor و HR Processor فقط خلاصه بی‌نام و بررسی‌شده یا امتیاز تجمیعی و وضعیت تکمیل متناسب با مجوزشان را می‌بینند. HR Manager و Company Manager دارای مجوز مخصوص می‌توانند متن خام را با حسابرسی مشاهده کنند، اما هویت پاسخ‌دهنده فقط برای HR Manager، تنها در رسیدگی رسمی و با ثبت دلیل آشکار می‌شود. HR Manager می‌تواند نمایش مدیریتی متن توهین‌آمیز، حساس، شخصی یا ادعای اثبات‌نشده را با دلیل محدود کند؛ اصل حسابرسی‌شده حذف نمی‌شود، Company Manager فقط از وجود مورد تحت رسیدگی آگاه می‌شود و ادعا صرفاً از مسیر پرونده انضباطی اثر می‌گیرد.
_Avoid_: افشای متن خام یا هویت به هدف، نمایش هویت به Company Manager، آشکارسازی هویت خارج از رسیدگی رسمی، حذف اصل نظر، استفاده HR Manager از اختیار برای پاسخ‌های مربوط به خودش، دسترسی HR Processor به متن خام، مشاهده یا محدودسازی بی‌حسابرسی، یا تبدیل مستقیم ادعا به امتیاز

**هدف رسمی عملکرد فروشنده**:
مقدار انتظار یک دوره برای Job فروشنده یا Position مشخص است که Company Manager پیش از آغاز دوره در یک اقدام تعیین می‌کند و HR آن را بدون تأیید جداگانه می‌بیند. هدف دوره آغازشده تغییر نمی‌کند؛ هدف تازه فقط آینده‌اثر است و رخداد خارج از کنترل از مسیر استثنای مستند رسیدگی می‌شود.
_Avoid_: تأیید چندمرحله‌ای، هدف گذشته‌اثر، تغییر هدف پس از دیدن نتیجه، هدف Position بدون دامنه روشن، یا پنهان‌کردن استثنا در مقدار هدف

**مرحله دستی ارزیابی فروشنده**:
جریان رسمی آغازین است که Supervisor قضاوت‌های خود، HR Processor مقادیر کمی دارای منبع و پاسخ‌دهندگان نظرسنجی پاسخ محرمانه را ثبت می‌کنند؛ سامانه امتیاز و Badge را محاسبه و HR Manager نتیجه را نهایی می‌کند. Company Manager نتیجه را می‌بیند اما امتیاز یا Badge را مستقیم انتخاب نمی‌کند؛ خودکارسازی آینده فقط شیوه ورود داده کمی را عوض می‌کند نه معنای عامل و فرمول.
_Avoid_: انتخاب دستی Badge، عدد بی‌منبع، تغییر فرمول هنگام خودکارسازی، بازنویسی مستقیم امتیاز به دست مدیر، یا ثبت قضاوت Supervisor به نام HR

**کف نمونه عامل عملکرد**:
حداقل شاهد لازم برای نرخ معتبر است: سه پاسخ‌دهنده یکتای همکار، سه قرارداد قطعی برای کیفیت قرارداد، سه تعهد سررسیدشده برای وصول و پنج پروژه تعیین‌تکلیف‌شده برای تبدیل؛ برای خود نتیجه رسمی کف زمانی وجود ندارد. شمار شاهد خودکار را سامانه مالک داده تعیین می‌کند؛ در ورود دستی، همان مفهوم با نام رکورد کسب‌وکاری مانند `تعداد قراردادهای بررسی‌شده` نمایش داده می‌شود نه با عنوان عمومی `تعداد نمونه`. زیرعامل ناکافی صفر نمی‌شود و وزن آن فقط طبق قاعده ثابت در همان خانواده توزیع می‌شود؛ نبود داده معتبر کل یک خانواده اصلی، صدور نتیجه تازه را متوقف می‌کند.
_Avoid_: ورود دستی شمار شاهد خودکار، عنوان مبهم `تعداد نمونه`، صفرکردن کمبود داده، انتقال وزن به خانواده دیگر، نرخ از نمونه کوچک، ساخت کف زمانی برای نتیجه رسمی، صدور نتیجه بدون خانواده اصلی، یا جایگزینی Badge معتبر با خطای پوشش

**زیرعامل‌های عملکردی فروشنده**:
تقسیم مصوب هفتاد درصد عملکردی است: آورده فروش شامل ارزش خالص قرارداد نسبت به هدف با وزن چهارده و وجه وصول‌شده نسبت به هدف با وزن هشت؛ وصول شامل نرخ وصول با وزن پنج، رعایت موعد پیگیری با وزن شش و مطالبات بدون پیگیری با وزن چهار؛ سرعت شامل نخستین ارسال با وزن سه، اصلاح فعال با وزن دو و ارسال به Accounting با وزن سه؛ کیفیت شامل پذیرش نخستین ارسال با وزن چهار و اصلاح منتسب با وزن سه؛ لغو و برگشت شامل لغو منتسب با وزن سه و مرجوعی منتسب با وزن دو؛ مدیریت مشتری شامل مشتری جدید با وزن دو، نرخ پروژه‌های پیگیری‌شده منجر به قرارداد با وزن دو، پروژه‌های دارای پیگیری عقب‌افتاده با وزن یک و قضاوت Supervisor درباره کیفیت پیگیری پروژه‌ها با وزن سه؛ تعهد تحویل شامل دقت تاریخ تحویل ثبت‌شده با وزن دو، مشکل منتسب با وزن یک و قضاوت Supervisor با وزن دو درصد از نتیجه کل است. قضاوت مبهم درباره پرونده پیچیده عامل ارزیابی نیست.
_Avoid_: جمعی غیر از هفتاد درصد، دوباره‌شماری وجه وصول‌شده در نرخ وصول، وزن پنهان، شمار خام بدون هدف یا مخرج، عامل مبهم پرونده پیچیده، یا قضاوت انسانی به‌جای داده موجود

**کالیبراسیون هدف فروشنده**:
نمای دوازده ماه گذشته در همان فرم تعیین هدف است که Company Manager برای انتخاب هدف دوره بعد از آن کمک می‌گیرد، بی‌آنکه سامانه هدف را خودکار تعیین کند. هدف Position می‌تواند با دلیل متفاوت باشد و در نبود سابقه کافی، Company Manager مقدار آغازین را مستقیم تعیین می‌کند.
_Avoid_: هدف خودکار، کپی کور میان Positionها، تفاوت بی‌دلیل، پنهان‌کردن سابقه هنگام تصمیم، یا جلوگیری از آغاز صرفاً به‌علت نبود سابقه

**منحنی استاندارد امتیاز عامل عملکرد**:
سه نگاشت نسخه‌دار است: عامل `بیشتر بهتر` از صفر در مقدار صفر به هفتادوپنج در هدف و صد در صدوبیست درصد هدف می‌رسد؛ عامل `کمتر بهتر` از صد در مقدار صفر به هفتادوپنج در حداکثر قابل‌قبول و صفر در دو برابر آن می‌رسد؛ نرخ سقف‌دار در هدف هفتادوپنج و در صد درصد، صد می‌گیرد. مقدارهای بیرون دامنه در صفر یا صد متوقف می‌شوند و فرمول اختصاصی فقط برای ضرورت مصوب ساخته می‌شود.
_Avoid_: امتیاز صد برای هدف عادی، رشد نامحدود، امتیاز منفی یا بیش از صد، فرمول متفاوت بی‌دلیل، یا تغییر منحنی تاریخی

**اصلاح شاهد مبدأ عملکرد**:
تغییر قابل‌ردیابی رکورد عملیاتی پس از استخراج است که پیش از پایان اعتراض نتیجه پیشنهادی را دوباره محاسبه می‌کند و پس از نهایی‌شدن فقط نسخه اصلاحی مرتبط می‌سازد. شناسه مبدأ، زمان و عامل تغییر حفظ می‌شوند و هر تصمیم رسمی وابسته برای بازبینی علامت می‌خورد، حتی اگر اصلاح ثبت‌شده سطح را تغییر ندهد.
_Avoid_: ویرایش درجا نتیجه نهایی، اصلاح بی‌منبع، پنهان‌کردن تغییر کوچک، بی‌اثر دانستن اصلاح بر تصمیم وابسته، یا بازنویسی تاریخچه

**تکمیل ارزیابی عملکرد**:
شرط نهایی‌کردن ارزیابی است که برای همه شاخص‌های الگوی ثابت مقدار واقعی معتبر وجود داشته باشد. پیش‌نویس ناقص قابل ذخیره است، اما مقدار خالی صفر نیست و نتیجه یا Badge تولید نمی‌کند.
_Avoid_: نهایی‌کردن داده ناقص، تبدیل مقدار خالی به صفر، یا پنهان‌کردن شاخص تکمیل‌نشده

**پیش‌نویس ارزیابی عملکرد**:
ارزیابی نهایی‌نشده‌ای است که فقط پس از ورود دست‌کم یک مقدار و اقدام صریح `ذخیره پیش‌نویس` ساخته می‌شود و فقط سازنده دارای مجوز زنده می‌تواند ادامه دهد یا نهایی کند. بازکردن یا بستن فرم خالی هیچ رکوردی نمی‌سازد. سازنده می‌تواند از پیش‌نویس خودش انصراف دهد و HR Manager می‌تواند هر پیش‌نویس را با دلیل کنار بگذارد؛ پیش‌نویس کنارگذاشته‌شده از کار فعال خارج می‌شود، اما حداقل سابقه حسابرسی آن باقی می‌ماند. لغو مجوز، دسترسی و نهایی‌سازی را متوقف می‌کند و هیچ پیش‌نویسی بر امتیاز یا نشان جاری اثر ندارد. اگر پس از آغاز پیش‌نویس نتیجه نهایی جدیدتری ثبت شده باشد، سامانه پیش از تأیید نهایی هشدار می‌دهد و ارزیاب می‌تواند بازگردد یا نتیجه خود را به‌عنوان ارزیابی تازه نهایی کند؛ آخرین نتیجه نهایی نشان جاری را تعیین می‌کند.
_Avoid_: ساخت پیش‌نویس از کلیک یا بازدید، ذخیره پیش‌نویس کاملاً خالی، حذف سخت شواهد یا حسابرسی، انصراف مدیریتی بدون دلیل، ادامه پیش‌نویس کنارگذاشته‌شده، ادامه کار پس از لغو مجوز، انتقال پیش‌نویس به کاربر دیگر، ویرایش پیش‌نویس دیگری، نهایی‌سازی بی‌هشدار روی نتیجه جدیدتر، قفل‌کردن ارزیابی تازه به‌علت وجود پیش‌نویس، نمایش پیش‌نویس به‌عنوان نتیجه، یا تغییر نشان از روی داده نهایی‌نشده

**انصراف از پیش‌نویس ارزیابی عملکرد**:
بستن حسابرسی‌شده یک پیش‌نویس بدون نتیجه و بدون اثر بر Badge است که سازنده برای پیش‌نویس خودش و HR Manager برای هر پیش‌نویس با دلیل انجام می‌دهند. انصراف، پیش‌نویس را از فهرست کار فعال حذف و ادامه یا نهایی‌سازی آن را ممنوع می‌کند، اما سابقه ایجاد، عامل انصراف، زمان و دلیل مدیریتی را نگه می‌دارد.
_Avoid_: حذف سخت، بازگرداندن پیش‌نویس منصرف‌شده به کار فعال، تغییر Badge، انصراف از نتیجه پیشنهادی یا رسمی، یا انصراف مدیریتی بی‌دلیل

**ارزیابی ماهانه عملکرد**:
گروه گزارش یک‌ماهه برای ارزیابی‌های عملکرد است، نه محدودیت ثبت. مدیر منابع انسانی یا سرپرست مجاز می‌تواند در هر روز و حتی چند بار در یک روز ارزیابی تازه ثبت کند و همه نتیجه‌ها در ماه مربوط باقی می‌مانند.
_Avoid_: محدودکردن Personnel به یک ارزیابی در ماه، میانگین‌گیری پنهان نتایج ماه، یا یکی‌گرفتن ماه گزارش با هویت ارزیابی

**تاریخ ارزیابی عملکرد**:
تاریخ سنجش یک ارزیابی تازه است که به‌طور پیش‌فرض امروز است و ارزیاب می‌تواند آن را به تاریخ دیگری در همان ماه تغییر دهد. تاریخ آینده مجاز نیست و زمان واقعی ثبت و نهایی‌کردن جداگانه در سابقه باقی می‌ماند.
_Avoid_: تاریخ آینده، جایگزین‌کردن زمان ثبت با تاریخ انتخابی، یا حذف ترتیب چند ارزیابی یک روز

**کتابخانه معیارهای عملکرد**:
مجموعه معیارهای قابل‌استفاده مجدد با هویت پایدار است که الگوهای شغل و افزوده‌های جایگاه به نسخه مشخص آن‌ها ارجاع می‌دهند. استفاده مجدد هرگز محتوای معیارِ حاضر در یک تصویر ثابت تاریخی را قابل‌تغییر نمی‌کند.
_Avoid_: کپی‌های بی‌هویت از یک معیار، اشتراک رکورد قابل‌ویرایش میان الگوها، یا بازنویسی نسخه تاریخی هنگام استفاده مجدد

**شاخص عملکرد**:
مولفه امتیازآور یک الگوی ارزیابی عملکرد است که کد پایدار، عنوان، واحد، هدف، جهت مطلوب و وزن دارد. ارزیاب مقدار واقعی بازه را وارد می‌کند و سامانه امتیاز شاخص را با قاعده نسخه‌دار محاسبه می‌کند.
_Avoid_: درجه توصیفی یک تا پنج، وزن پنهان، شاخص بدون هدف یا جهت، مقدار واقعی بدون بازه، یا تغییر قاعده ارزیابی تاریخی

**مقدار واقعی شاخص عملکرد**:
عدد واحددار عملکرد همان Personnel در بازه سنجش است: نرخ و امتیاز انسانی فقط از صفر تا صد است، اما مبلغ، شمار و مدت‌زمان مقدار نامنفی با واحد واقعی خود هستند و سقف مصنوعی صد ندارند. در مرحله دستی عامل مجاز آن را همراه منبع ثبت می‌کند و در مرحله خودکار سامانه مالک داده همان حقیقت را فقط‌خواندنی فراهم می‌کند.
_Avoid_: محدودکردن مبلغ، شمار یا مدت به صد، پذیرفتن نرخ یا امتیاز بیرون صفر تا صد، واحد عمومی `مقدار`، مقدار فرضی، استفاده از عدد بازه دیگر، جایگزینی داده مفقود با صفر، یا Override دستی عدد خودکار

**دسته معیارهای عملکرد**:
گروه معنایی وزن‌داری در تصویر ثابت الگو است که معیارهای قضاوتی مرتبط و وزن‌های نسبی آن‌ها را بدون زیردسته تو‌در‌تو گرد می‌آورد. سهم نهایی هر معیار از وزن دسته و وزن نسبی معیار به‌طور شفاف به دست می‌آید.
_Avoid_: دسته‌بندی چندلایه، وزن پنهان، جمع سهمی غیر از صد درصد، یا استفاده از دسته صرفاً نمایشی در حالی که محاسبه دیگری اعمال می‌شود

**امتیاز نرمال‌شده عملکرد**:
مقدار صفر تا صد حاصل مقایسه مقدار واقعی هر شاخص با هدف و جهت مطلوب آن و سپس جمع سهم‌های وزن‌دار شاخص‌ها است. شاخص `بالاتر بهتر` از نسبت مقدار واقعی به هدف و شاخص `پایین‌تر بهتر` از نسبت هدف به مقدار واقعی استفاده می‌کند و امتیاز هر شاخص در صد متوقف می‌شود؛ برای شاخص `پایین‌تر بهتر` با هدف صفر، مقدار واقعی صفر امتیاز صد و هر مقدار بیشتر از صفر امتیاز صفر دارد. مقدار مفقود، هدف نامعتبر و وزن ناقص نتیجه معتبر تولید نمی‌کنند.
_Avoid_: نگاشت درجه یک تا پنج، امتیاز صفر برای داده مفقود، تقسیم بر صفر، جمع وزن غیر از صد درصد، یا گردکردن میانی

**نتیجه نهایی ارزیابی عملکرد**:
نسخه قفل‌شده تاریخ ارزیابی، مقادیر واقعی، امتیاز محاسبه‌شده، سطح پنج‌گانه، الگوی ارزیابی، هویت ارزیاب و زمان نهایی‌سازی است که پس از نمایش خلاصه نام Personnel، تاریخ، امتیاز و نشان و اقدام صریح `تأیید نهایی` ایجاد می‌شود. هر ارزیابی تازه نتیجه مستقل خود را دارد؛ همان ارزیاب می‌تواند پیش‌نویس را نهایی کند و صف بررسی جداگانه‌ای وجود ندارد.
_Avoid_: نهایی‌شدن با ذخیره خودکار، ثبت نهایی بدون دیدن خلاصه نتیجه، بازنویسی نتیجه قبلی با ارزیابی تازه، ویرایش درجا پس از نهایی‌شدن، نتیجه بدون الگوی ثابت، یا الزام تأییدکننده دوم

**ارزیابی تازه عملکرد**:
سنجش مستقل تازه‌ای برای Personnel است که در تاریخ انتخابی ایجاد می‌شود و نتیجه‌های قبلی همان روز یا ماه را تغییر نمی‌دهد. این اقدام با اصلاح نتیجه قبلی متفاوت است.
_Avoid_: استفاده از ارزیابی تازه برای تصحیح اشتباه قبلی، جایگزینی خاموش نتیجه، یا حذف ارزیابی‌های همان ماه

**نسخه اصلاحی ارزیابی عملکرد**:
نتیجه نهایی تازه‌ای است که مقادیر نتیجه قبلی را برای اصلاح آماده می‌کند، دلیل اصلاح را ثبت می‌کند و با حفظ پیوند به نسخه قبلی جای آن را به‌عنوان نتیجه مؤثر می‌گیرد. سرپرست فقط نتیجه‌ای را اصلاح می‌کند که خودش ثبت کرده است؛ دارنده مجوز `ارزیابی همه پرسنل` می‌تواند هر نتیجه در دامنه مجاز را با دلیل اصلاح کند.
_Avoid_: اصلاح نتیجه دیگری به‌دست سرپرست، ویرایش نتیجه نهایی درجا، حذف نسخه قبلی، اصلاح بدون دلیل، یا مؤثر ماندن هم‌زمان نتیجه اصلی و نسخه اصلاحی آن

**سطح پنج‌گانه عملکرد**:
نمایش فارسی امتیاز نهایی است: کمتر از ۶۰ `نیازمند بهبود فوری`، از ۶۰ تا کمتر از ۷۵ `نیازمند بهبود`، از ۷۵ تا کمتر از ۹۰ `مطابق انتظار`، از ۹۰ تا کمتر از ۱۰۰ `فراتر از انتظار` و دقیقاً ۱۰۰ `عملکرد برجسته` است.
_Avoid_: سطح ششم امتیازدار، هم‌پوشانی مرزها، برجسته نامیدن امتیاز کمتر از صد، یا یکی‌گرفتن وضعیت ارزیابی‌نشده با یک سطح عملکرد

**متن ساده ارزیابی عملکرد**:
زبان کوتاه و فارسیِ عنوان، توضیح، راهنما، دکمه و خطای مسیر روزمره ارزیابی است که اقدام بعدی را بدون اصطلاح فنی بیان می‌کند. هر معیار مبهم عنوان کوتاه دارد و `روش محاسبه` آن تعریف، واحد، بازه، منبع، مخرج، استثناهای خارج از کنترل و نمونه قابل‌فهم را آشکار می‌کند.
_Avoid_: هش، Snapshot، بازسازی آمادگی، تصاحب پرونده، شناسه فنی، عنوان بی‌تعریف، پنهان‌کردن مخرج یا استثنا، جمله طولانی، یا توضیحی که برای انجام کار لازم نیست

**سطح عملکرد جاری ساده**:
سطح پنج‌گانه آخرین نتیجه نهایی مؤثر Personnel در رابطه استخدامی فعال یا معلق است. هر نتیجه نهایی تازه، حتی در همان روز، مبنای نشان جاری می‌شود؛ شروع ماه تازه نشان قبلی را پاک نمی‌کند و پیش‌نویس‌ها و میانگین ماهانه اثری بر آن ندارند. نسخه اصلاحی مؤثر به‌جای نتیجه اصلی خودش استفاده می‌شود و رابطه استخدامی پایان‌یافته نشان جاری ندارد.
_Avoid_: پاک‌کردن یا صفرکردن نشان در شروع ماه، میانگین‌گیری از چند نتیجه، انتخاب بهترین نتیجه ماه، اثر دادن به پیش‌نویس، نگه‌داشتن نشان قدیمی پس از نتیجه نهایی تازه، یا نمایش نشان جاری برای رابطه پایان‌یافته

**سابقه ساده ارزیابی عملکرد**:
فهرست فقط‌خواندنی همه ارزیابی‌های نهایی و نسخه‌های اصلاحی Personnel است که تاریخ ارزیابی، زمان نهایی‌سازی، ارزیاب، مبنای اختیار، امتیاز و سطح را به ترتیب روشن نشان می‌دهد.
_Avoid_: حذف نتیجه قدیمی، پنهان‌کردن ارزیاب HR، یکی‌گرفتن تاریخ ارزیابی با زمان نهایی‌سازی، یا نمایش نسخه جایگزین‌شده به‌عنوان نتیجه جاری

**مرز اصطلاحات میراثی عملکرد**:
تعریف‌های تخصصی عملکرد از `کاربردپذیری معیار عملکرد` تا `بسته شواهد ارتقای ارزیابی عملکرد` در ادامه، مدل پیچیده پیشین ADR-0024 را مستند می‌کنند و در جریان ساده جاری ADR-0057 به کار نمی‌روند. این اصطلاحات فقط برای فهم داده و کد میراثی باقی مانده‌اند و نباید مبنای رابط، مجوز، ارزیابی تازه، محاسبه یا نشان جاری باشند.
_Avoid_: ترکیب صف بررسی، شاهد، مأموریت شغلی، انقضای Badge یا تجمیع تاریخی مدل پیشین با جریان ساده جاری

**کاربردپذیری معیار عملکرد**:
تصمیم نسخه‌دار درباره مرتبط‌بودن یک معیار با یک بخش ارزیابی مأموریت شغلی است که ابتدا از واقعیت‌های تصویر ثابت تعیین می‌شود؛ درخواست دلیل‌دار سرپرست برای نامرتبط‌بودن فقط با پذیرش منابع انسانی نهایی می‌شود.
_Avoid_: حذف بی‌دلیل معیار، اختیار یک‌جانبه سرپرست برای کم‌کردن مخرج، یا امتیاز صفر برای معیار نامرتبط

**قاعده کنترل‌شده کاربردپذیری عملکرد**:
شرط نسخه‌داری بر واقعیت‌های تصویر ثابت مانند Job، Position، واحد، محل، شیفت، نوع مأموریت، مسئولیت و تاریخ اثر است که کاربرد یک معیار را بدون کد آزاد تعیین می‌کند. داده مفقود مانع ساختاری است و هویت Personnel یا سرپرست، ویژگی شخصی محافظت‌شده، امتیاز قبلی و داده زنده امروز ورودی مجاز نیستند.
_Avoid_: شرط شخص‌محور، تبعیض بر ویژگی محافظت‌شده، فرمول اجرایی آزاد، وابستگی به امتیاز پیشین، یا تبدیل خطای داده به نامرتبط‌بودن

**الزام شاهد معیار عملکرد**:
قاعده نسخه‌داری است که نوع و کف شاهد لازم یا اختیاری برای قضاوت یک معیار را تعیین می‌کند. نبود شاهد الزامی مانع ارسال است و نبود شاهد اختیاری ثبت می‌شود، اما هیچ‌کدام خودکار امتیاز تولید یا کم نمی‌کنند.
_Avoid_: ارسال بدون شاهد الزامی، جریمه عددی خودکار برای نبود شاهد، شاهد بدون منشأ، یا تبدیل داده عملیاتی به قضاوت نهایی

**شاهد معیار عملکرد**:
پیوند صریح یک معیار به ثبت عملیاتی تغییرناپذیر، سند کنترل‌شده دارای نسخه و هش، یا مشاهده ساختاریافته سرپرست با تاریخ رخداد است. واقعیت شاهد باید با بخش مأموریت هم‌پوشانی یا با دوره نگاه‌به‌عقب مصوب آن معیار سازگار باشد؛ تاریخ ثبت آن می‌تواند دیرتر باشد.
_Avoid_: لینک آزاد به‌عنوان تنها شاهد الزامی، روایت کلی بدون رخداد، فایل بی‌نسخه، شاهد خارج از بازه بدون قاعده، یا امتیازسازی خودکار از شاهد

**وضعیت کیفیت شاهد عملکرد**:
ارزیابی صریح شاهد به‌عنوان `قابل اتکا`، `ناقص`، `مورد اختلاف`، `ناموجود` یا `باطل‌شده` است؛ فقط شاهد قابل اتکا کف الزامی را تأمین می‌کند و اصلاح، نسخه تازه‌ای در کنار سابقه قبلی می‌سازد. وضعیت‌های دیگر برای زمینه و حسابرسی می‌مانند و تا رفع مشکل یا تصمیم غیرقابل‌ارزیابی، ارسال را متوقف می‌کنند.
_Avoid_: حذف شاهد معیوب، پذیرش شاهد مورد اختلاف به‌عنوان کف الزامی، اصلاح درجا، یا تبدیل کیفیت شاهد به امتیاز

**ترکیب وزنی الگوی عملکرد**:
قاعده‌ای است که به‌طور پیش‌فرض هشتاد درصد امتیاز را به الگوی ارزیابی شغل و بیست درصد را به افزوده ارزیابی جایگاه می‌دهد، با این قید که سهم الگوی شغل هرگز کمتر از هفتاد درصد نشود. نبود افزوده، سهم الگوی شغل را به صد درصد می‌رساند و افزوده هیچ معیار پایه‌ای را حذف، جایگزین یا کم‌وزن نمی‌کند.
_Avoid_: افزوده غالب بر شغل، تغییر وزن معیار پایه از مسیر جایگاه، جمع وزنی غیر از صد درصد، یا سهم خالی برای افزوده ناموجود

**پوشش قابل‌ارزیابی الگوی عملکرد**:
نسبت وزن اصلی معیارهای واقعاً کاربردپذیر به وزن کامل تصویر ثابت الگو است؛ نتیجه فقط با پوشش کل حداقل هفتاد درصد و پوشش حداقل پنجاه درصدی هر دسته الزامی قابل محاسبه است. همه معیارهای کاربردپذیر باید پاسخ داده شوند و این کف، مجوز رهاکردن معیارهای قابل‌ارزیابی نیست.
_Avoid_: یکی‌گرفتن معیار بی‌پاسخ با نامرتبط، محاسبه زیر کف پوشش، یا پایین‌آوردن گزینشی مخرج برای بهبود امتیاز

**سهم مؤثر بخش مأموریت در ارزیابی**:
سهم یک بخش ارزیابی مأموریت شغلی در نتیجه مصوب است که از حاصل روزهای مؤثر و درصد تخصیص کاری ثبت‌شده آن بخش به دست می‌آید. اصلی یا فرعی بودن مأموریت به‌تنهایی وزن نمی‌سازد و تخصیص مفقود یا ناسازگار مانع پذیرش نتیجه است.
_Avoid_: وزن مساوی دلخواه، اولویت خودکار مأموریت اصلی، دوباره‌شماری مأموریت‌های هم‌زمان، یا حدس تخصیص مفقود

**چرخه نسخه سیاست عملکرد**:
مسیر تغییرناپذیر `پیش‌نویس`، `زمان‌بندی‌شده`، `فعال` و `بازنشسته` برای معیار، دسته، الگو، سیاست محاسبه و سیاست سطح‌بندی است. تنها پیش‌نویس ویرایش می‌شود؛ انتشار محتوا و تاریخ اثر را قفل می‌کند، فعال‌شدن نسخه تازه نسخه پیشین را اتمیک بازنشسته می‌کند و تغییر بعدی یا استفاده مجدد نیازمند نسخه تازه با پیوند نسب است. سیاست سطح‌بندی فقط به دست User دارای مجوز مؤثر و مستقل مدیریت آن، با دلیل، پیش‌نمایش کامل و تأیید صریح منتشر می‌شود؛ در هر زمان یک نسخه سازمانی فعال است، اثر عادی آن از ابتدای تاریخ انتخابی به وقت تهران آغاز می‌شود و فعال‌سازی به گذشته مجاز نیست.
_Avoid_: ویرایش نسخه منتشرشده، دو نسخه فعال هم‌دامنه در یک تاریخ، فعال‌سازی دوباره نسخه بازنشسته، حذف نسخه‌ای که در تاریخچه استفاده شده است، استنتاج اختیار از عنوان یا نقش مدیریتی، فعال‌سازی بدون پیش‌نمایش و دلیل، یا اثر گذشته‌نگر سیاست سطح‌بندی

**پیش‌نمایش اثر سیاست عملکرد**:
تصویر قطعی و پیش از فعال‌سازی سیاست محاسبه، تجمیع یا سطح‌بندی تازه از شمار افزایش، کاهش، بدون تغییر، انقضا، نیاز به ارزیابی جدید و خطاهای Badgeهای جاری است. فعال‌سازی اثرگذار نیازمند دلیل و تأیید صریح این پیش‌نمایش است و فقط هنگامی به‌صورت اتمیک انجام می‌شود که باز‌محاسبه کل جمعیت بدون خطا ممکن باشد؛ رخدادهای باز‌محاسبه بعدی آن را حفظ می‌کنند.
_Avoid_: فعال‌سازی کور یا جزئی، تغییر بی‌دلیل Badgeها، پیش‌نمایش نمونه‌ای به‌جای جمعیت کامل مجاز، نادیده‌گرفتن خطای یک رکورد، یا اعمال سیاست معیارهای آینده به نتایج بسته

**نسخه مؤثر بخش ارزیابی**:
نسخه معیارها، الگو و قواعدی است که در تاریخ شروع بخش مأموریت مؤثر بوده و هنگام ساخت تصویر ثابت همان بخش قفل می‌شود. نسخه امروز جای نسخه تاریخی مفقود را نمی‌گیرد و فعال‌شدن نسخه بعدی هیچ ارزیابی باز یا تاریخی را تغییر نمی‌دهد.
_Avoid_: انتخاب نسخه در زمان ارسال یا پذیرش، اعمال نسخه تازه به ارزیابی باز، یا ساخت گذشته با سیاست امروز

**سیاست تجمیع زنده عملکرد**:
سیاست نسخه‌داری است که حداکثر چهار نتیجه مصوب و منقضی‌نشده رابطه استخدامی جاری را از جدید به قدیم با وزن‌های پنجاه، سی، پانزده و پنج درصد ترکیب و وزن‌های موجود را به صد درصد نرمال می‌کند. هر نتیجه فقط پس از پذیرش اثر می‌گذارد و سیصدوشصت‌وپنج روز پس از پایان بازه سنجش منقضی می‌شود.
_Avoid_: تجمع مادام‌العمر، اثر پیش از پذیرش، وابستگی به بازه گزارش، نگه‌داشتن نتیجه منقضی، یا ترکیب نتیجه استخدام قبلی

**رخداد باز‌محاسبه سطح عملکرد**:
ثبت تغییرناپذیر محاسبه دوباره Badge در اثر پذیرش یا اصلاح نتیجه، تعلیق اثر نتیجه نامعتبر، فعال‌شدن سیاست تازه، انقضای روزانه یا تغییر رابطه استخدامی است که سطح پیشین و تازه، علت، نسخه سیاست و ورودی‌های مؤثر را نگه می‌دارد. عبور مقدار دقیق از هر آستانه بدون دوره انتظار، حاشیه پنهان یا حداقل تعداد ارزیابی، همان لحظه سطح را بالا یا پایین می‌برد. ترتیب تازگی بر پایان بازه سنجش است و نتیجه‌ای که پس از انقضا پذیرفته شود فقط تاریخی می‌ماند.
_Avoid_: باز‌محاسبه بی‌علت، ترتیب بر زمان پذیرش، پاک‌کردن سطح پیشین، تأخیر یا حاشیه پنهان در عبور از آستانه، جلوگیری از کاهش سطح، یا اثر نتیجه دیرپذیرفته و منقضی

**ردپای محاسبه عملکرد**:
شاهد محرمانه و تغییرناپذیر همه نسخه‌ها، درجات، نگاشت‌ها، وزن‌های اصلی و مؤثر، تصمیم‌های کاربردپذیری، شواهد، جمع‌های میانی، سهم مأموریت، نرمال‌سازی، ترتیب و انقضای نتایج، سیاست و مقادیر دقیق و نمایشی است که هر نتیجه و سطح را بازسازی می‌کند.
_Avoid_: عدد نهایی بدون مسیر، بازسازی از داده زنده، حذف دلیل نامرتبط‌بودن، یا افشای ردپا به Personnel و User عادی

**جایگزینی نتیجه مصوب عملکرد**:
رابطه تغییرناپذیری است که نتیجه اصلاحی را در محاسبه زنده جانشین نتیجه مصوب پیشین همان ارزیابی می‌کند، بی‌آنکه هیچ‌یک از تاریخچه حذف یا بازنویسی شوند. ارزیابی‌های عادی امتیازآور با بازه‌های هم‌پوشان مجاز نیستند.
_Avoid_: شمردن هم‌زمان اصل و اصلاح، حذف نتیجه قبلی، اصلاح درجا، یا دوباره‌شماری دو ارزیابی هم‌پوشان

**تصویر ثابت الگوی ارزیابی**:
ترکیب تغییرناپذیر نسخه مؤثر الگوی ارزیابی شغل و افزوده ارزیابی جایگاه سازمانی که معیارها و وزن‌های یک بخش ارزیابی مأموریت شغلی را تعیین می‌کند.
_Avoid_: محاسبه ارزیابی تاریخی با الگوهای امروز، گنجاندن هویت یا زمینه استخدامی در تصویر الگو، یا تغییر معیار و وزن پس از بازشدن ارزیابی

**تصویر ثابت زمینه ارزیابی عملکرد**:
شاهد تغییرناپذیر هویت Personnel و Employment Relationship، مأموریت‌های مؤثر، زمینه سازمانی، سرپرستان مسئول و نسخه قواعد ترکیب در زمان بازشدن ارزیابی است. این تصویر به تصویر ثابت الگوی هر بخش ارجاع می‌دهد، اما شواهد عملیاتی را در خود کپی نمی‌کند.
_Avoid_: بازسازی تاریخ از ساختار سازمانی امروز، یکی‌گرفتن زمینه استخدامی با الگوی معیارها، یا کپی‌کردن داده عملیاتی قابل‌تغییر داخل تصویر زمینه

**نسخه اصلاح‌شده زمینه ارزیابی**:
نسخه تغییرناپذیر و دلیل‌داری است که پس از اصلاح مجاز تاریخچه مأموریت، Position، زمینه سازمانی یا سرپرست، جایگزین عملیاتی تصویر ثابت پیشین می‌شود و هم‌زمان تصویر و ارسال‌های قبلی را به‌عنوان سابقه نگه می‌دارد. پیش از ارسال، وظیفه‌های متأثر به مسئول صحیح منتقل می‌شوند؛ پس از ارسال و پیش از پذیرش، ارسال قبلی برای تصمیم نامعتبر می‌شود و ارسال تازه لازم است؛ پس از نتیجه مصوب، اصلاح رسمی نتیجه قبلی را بازنویسی نمی‌کند و در صورت لزوم نتیجه جایگزین مرتبط صادر می‌شود.
_Avoid_: بازنویسی Snapshot یا نتیجه مصوب، حذف ارسال مبتنی بر زمینه پیشین، ادامه تصمیم بر پایه ارسال نامعتبر، اصلاح بی‌دلیل، از دست‌دادن پیوند نسخه قدیم و جدید، یا نمایش تاریخچه اصلاح‌شده به‌عنوان حقیقتی که همیشه وجود داشته است

**ثبت شواهد عملیاتی ارزیابی**:
ثبت نسخه‌دار و فقط‌خواندنی واقعیت‌هایی است که سامانه عملیاتی مالک آن‌ها فراهم کرده و منبع، تعریف، بازه سنجش، زمان استخراج، نسخه و وضعیت کیفیت داده را برای زمینه ارزیابی نگه می‌دارد.
_Avoid_: شاهد قابل‌ویرایش در صفحه ارزیابی، استفاده مستقیم از داده بازبینی‌نشده به‌عنوان امتیاز، عدد بدون منشأ، یا بازنویسی ثبت قبلی هنگام استخراج مجدد

**ارسال ارزیابی عملکرد سرپرست**:
قضاوت ثبت‌شده و تغییرناپذیر سرپرست مسئول در سطح معیارها است که از تصویر ثابت الگوی ارزیابی، شواهد مرتبط، توضیح زمینه‌ای و وزن‌دهی شفاف استفاده می‌کند و پیش از پذیرش منابع انسانی نتیجه مصوب نیست. پیش‌نویس آن تا لحظه ارسال قابل‌ویرایش و حذف منطقی است؛ ارسال، Snapshot، پاسخ‌ها، شواهد، محاسبه، نویسنده و زمان را قفل می‌کند و هر اصلاح بعدی به نسخه مرتبط تازه نیاز دارد.
_Avoid_: امتیاز عملیاتی خودکار، عدد نهایی مبهم، ویرایش پنهانی پس از ارسال، نگهداری اجباری همه ذخیره‌های عادی پیش‌نویس، یکی‌گرفتن ارسال با نتیجه مصوب، یا تصمیم خودکار جبران خدمت و استخدام

**چرخه ارسال و بررسی عملکرد**:
مسیر هر بخش ارزیابی مأموریت شغلی از `شروع‌نشده` به `پیش‌نویس`، `ارسال‌شده و در انتظار بررسی` و سپس `پذیرفته‌شده` یا `ردشده` است؛ `نامعتبرشده` خطای زمینه یا مسئولیت را نشان می‌دهد و `غیرقابل‌ارزیابی` نبود امکان معتبر قضاوت را. عقب‌افتادگی، تصاحب موقت کار بررسی و نیاز به اصلاح پرچم‌اند، نه وضعیت کسب‌وکاری. چند بررسی‌کننده مجاز می‌توانند کار را ببینند، اما نخستین تصمیم معتبر و اتمیک آن را تکمیل می‌کند و خودبررسی عامل دارای هر دو اختیار نیز از همین قواعد پیروی می‌کند.
_Avoid_: وضعیت‌سازی از بازشدن صفحه، قفل دائمی با تصاحب بررسی، تصمیم هم‌زمان متعارض، یکی‌گرفتن رد محتوایی با نامعتبرشدن زمینه، امتیاز صفر برای غیرقابل‌ارزیابی، یا منع خودبررسی مجاز

**پنجره ارسال ارزیابی عملکرد**:
بازه سیاست‌گذاری‌شده‌ای است که پیش‌نویس را از آغاز هر بخش مأموریت باز می‌کند، ارسال را پس از پایان همان بخش مجاز می‌سازد و به‌طور پیش‌فرض هفت روز تقویمی تهران برای ارسال فرصت می‌دهد. ارسال دیرهنگام تا بستن صریح پنجره مجاز است و هر تمدید، موعد تازه، دلیل و موعد منجمد قبلی را حفظ می‌کند.
_Avoid_: ارسال پیش از پایان بخش، انتظار تا پایان کل ارزیابی برای بخش پایان‌یافته، پذیرش یا رد خودکار در موعد، تغییر موعد منجمد با ویرایش سیاست، یا بستن بی‌دلیل پنجره

**مهلت بررسی ارسال عملکرد**:
مهلت منجمد و به‌طور پیش‌فرض سه روز کاری تهران از آماده‌شدن هر ارسال برای تصمیم Human Resources است؛ یادآوری‌های سه روز، یک روز و روز موعد، اعلان روزانه عقب‌افتادگی و خلاصه هفتگی عملیات آن را پشتیبانی می‌کنند. پایان مهلت تصمیم خودکار نمی‌سازد و بستن پرونده ناقص به تصمیم دلیل‌دار درباره هر بخش نیاز دارد.
_Avoid_: پذیرش یا رد زمان‌محور، پنهان‌کردن تأخیر، تغییر گذشته با سیاست تازه، یا بستن خاموش بخش بدون ارسال

**پذیرش ارسال ارزیابی عملکرد**:
تصمیم اتمیک Human Resources بر آخرین نسخه معتبر ارسال است که مجوز لحظه‌ای بررسی‌کننده، اعتبار ارزیابی و Snapshot، کامل‌بودن شاهد و کاربردپذیری، بازتولید محاسبه و نبود تصمیم هم‌زمان را دوباره کنترل می‌کند. پذیرش کد استاندارد و یادداشت اختیاری دارد؛ خودبررسی مجاز نشانگر حسابرسی اجباری می‌گیرد، اما توضیح اضافه یا تأیید شخص دوم نمی‌خواهد.
_Avoid_: پذیرش نسخه قدیمی یا نامعتبر، اعتماد به محاسبه client، الزام توضیح دستی برای پذیرش عادی یا خودبررسی، تصمیم دوم هم‌زمان، یا باطل‌کردن ارسال تاریخی صرفاً با تغییر بعدی مجوز Supervisor

**رد و ارسال مجدد ارزیابی عملکرد**:
تصمیم تغییرناپذیر Human Resources است که بدون ویرایش قضاوت Supervisor، دست‌کم یک دسته دلیل کنترل‌شده، توضیح فارسی و در صورت لزوم ارجاع به معیار یا شاهد مسئله‌دار را ثبت می‌کند. ارسال مجدد، نسخه تازه‌ای با پیوند به نسخه ردشده می‌سازد؛ محتوای پیشین می‌تواند در پیش‌نویس تازه کپی شود، اما Supervisor باید آن را بازبینی و صریحاً دوباره ارسال کند. فقط آخرین نسخه ارسال‌شده قابل تصمیم تازه است و تعداد رفت‌وبرگشت محدود نمی‌شود.
_Avoid_: بازنویسی نسخه ردشده، دلیل آزاد بدون دسته، ثبت مقدار جایگزین به دست بررسی‌کننده، بازگشایی همان ارسال، حذف نسخه‌های قبلی، یا پذیرش هم‌زمان دو نسخه

**اعلان چرخه ارزیابی عملکرد**:
پیام درون‌سامانه‌ای و حداقل‌ضروری درباره ایجاد وظیفه، موعد، عقب‌افتادگی، پذیرش، رد، نامعتبرشدن یا مانع ساختاری است که فقط به عامل دارای دسترسی همان مرحله می‌رسد. Supervisor اعلان وظیفه و تصمیم ارسال خودش، بررسی‌کننده اعلان پرونده آماده، و عملیات Human Resources اعلان مانع و تکرار رد را می‌گیرند. هر تغییر خلاصه شخصی—سطح تازه، افزایش، کاهش، باقی‌ماندن همان سطح با تازگی جدید، انقضا یا نیاز به ارزیابی جدید—به Personnel یا User مرتبط فقط اعلام می‌کند که خلاصه مصوب به‌روزرسانی و سطح مجاز قابل مشاهده شده است؛ متن اعلان امتیاز، مقدار تغییر، معیار یا علت را افشا نمی‌کند. تلاش و نتیجه تحویل هر اعلان حفظ می‌شود و ارسال مجدد، رخداد ناموفق پیشین را بازنویسی نمی‌کند.
_Avoid_: افشای امتیاز، مقدار تغییر یا دلیل رد به Personnel، اعلان وضعیت داخلی به فرد فاقد مجوز، حذف اعلان چون نام سطح ثابت مانده، استفاده اجباری از پیامک یا پیام‌رسان بیرونی، حذف تلاش ناموفق، یا تبدیل اعلان به مجوز مشاهده پرونده

**لغو ارزیابی عملکرد**:
بستن دلیل‌دار و تغییرناپذیر یک ارزیابی اشتباه پیش از صدور نتیجه مصوب است که همه پیش‌نویس‌ها، ارسال‌ها و تصمیم‌هایش را به‌عنوان سابقه نگه می‌دارد اما هیچ اثر پایین‌دستی باقی نمی‌گذارد. پس از نتیجه مصوب، لغو جای خود را به تعلیق اثر و اصلاح می‌دهد و پایان استخدام یا بایگانی Personnel به‌تنهایی دلیل لغو نیست.
_Avoid_: حذف ارزیابی، لغو نتیجه مصوب، لغو خودکار با پایان استخدام، یا باقی‌ماندن اثر Badge و تحلیل پس از لغو

**نتیجه مصوب ارزیابی عملکرد**:
تنها نتیجه معتبر یک ارزیابی عملکرد پرسنل است که پس از پذیرش صریح همه ارسال‌های لازم یا ثبت دلیل‌دار بخش‌های غیرقابل‌ارزیابی، بدون تأیید سلیقه‌ای اضافه و به‌صورت اتمیک از قواعد مصوب ساخته و به سابقه نتایج مصوب عملکرد افزوده می‌شود. اگر کف محاسبه برقرار نباشد، پرونده به‌جای امتیاز نتیجه `غیرقابل‌ارزیابی در این دوره` می‌گیرد. نتیجه امتیازدار سطح عملکرد جاری را بی‌درنگ طبق سیاست نسخه‌دار محاسبه می‌کند و برای رتبه‌بندی و تحلیل‌های مجاز نیز قابل استفاده است.
_Avoid_: نتیجه‌گیری از ارسال ردشده یا در انتظار بررسی، تأیید دستی امتیاز تجمیعی قطعی، چند نتیجه جاری برای یک ارزیابی، امتیازسازی زیر کف پوشش، یا تغییر خودکار حقوق، ترفیع، تنزل، انضباط و رابطه استخدامی

**تعلیق اثر نتیجه مصوب عملکرد**:
اقدام دلیل‌دار و مجوزداری است که وقتی خطای قطعی زمینه، اختیار، داده یا محاسبه اعتبار یک نتیجه پذیرفته‌شده را از بین برده، بدون حذف یا بازنویسی آن نتیجه، اثرش را فوراً از سطح جاری، رتبه و تحلیل کنار می‌گذارد. اگر نتیجه هنوز معتبر و صرفاً نیازمند اصلاح دقیق‌تر باشد، تا پذیرش نتیجه اصلاحی مؤثر می‌ماند؛ در هر دو حالت نسخه اصلاحی فقط پس از پذیرش اثر می‌گیرد و با رابطه جایگزینی به نتیجه پیشین متصل است.
_Avoid_: حذف نتیجه معیوب، اثر نسخه اصلاحی پیش از پذیرش، تعلیق بی‌دلیل یا بدون مجوز، کنارگذاشتن نتیجه معتبر صرفاً با بازشدن اصلاح، یا شمردن هم‌زمان اصل و جایگزین

**مرز زمانی افشای نتیجه عملکرد**:
قاعده‌ای است که نشان شخصی، نشان فهرست Personnel، تحلیل مدیریتی و خروجی را فقط از نتیجه مصوب پس از پذیرش صریح Human Resources می‌سازد. پیش‌نویس، ارسال در انتظار، ردشده، نامعتبر یا اصلاح‌نشده فقط در دامنه محرمانه Supervisor مربوط و فرایند داخلی مجاز باقی می‌ماند.
_Avoid_: افشای وضعیت داخلی به Personnel، اثر ارسال پیش از پذیرش، تحلیل از نتیجه ردشده یا نامعتبر، یا خروجی‌گرفتن از قضاوت هنوز مصوب‌نشده

**سابقه نتایج مصوب عملکرد**:
توالی تغییرناپذیر نتایج پذیرفته‌شده یک Personnel در Employment Relationshipهای او است که هر نتیجه بازه سنجش، تاریخ اثر، مدت اعتبار و سیاست محاسبه خود را حفظ می‌کند. سطح جاری فقط از نتایج هنوز معتبر و مجاز رابطه استخدامی جاری به دست می‌آید و هیچ نتیجه منقضی یا جایگزین‌شده‌ای از تاریخچه حذف نمی‌شود.
_Avoid_: میانگین‌گیری از ارسال تأییدنشده، امتیاز تجمعی همیشگی، حذف نتیجه قدیمی، یا انتقال پنهانی اثر یک استخدام به استخدام بعدی

**ارجاع پیامد عملکرد**:
تحویل دستی، دلیل‌دار، نسخه‌دار و تغییرناپذیر یک یا چند نتیجه مصوب عملکرد متعلق به دقیقاً یک Personnel و یک Employment Relationship به فرایند مستقلِ بازبینی جبران خدمت، پاداش اختیاری، بررسی ارتقا، بررسی برنامه بهبود عملکرد یا بررسی تنزل است. هر ارجاع دقیقاً یک نوع پیامد دارد، سطح جاری را فقط به‌عنوان زمینه نگه می‌دارد، مقصد و کنترل‌هایش را از سیاست مؤثر همان نوع می‌گیرد و یک بسته ثابت و حداقل‌ضروری از سابقه چنددوره‌ای مجاز، زمینه استخدامی و در موارد مالی Compensation Agreement، محدوده پرداخت و بودجه را تحت مالکیت نگهداری مقصد حفظ می‌کند؛ مبدأ فقط پیوند، هش و وضعیت تحویل را نگه می‌دارد و دوره طولانی‌تر مقصد به‌خودی‌خود نگهداری همه محتوای پرونده عملکرد را تمدید نمی‌کند. مقصد پیش از تصمیم، زمینه زنده را دوباره اعتبارسنجی می‌کند. ارجاع نه اختیار مشاهده جزئیات محرمانه تازه‌ای می‌دهد، نه تصمیم مقصد یا تاریخ اثر نهایی است و نه مستقیماً حقوق، پاداش، مأموریت شغلی، انضباط یا استخدام را تغییر می‌دهد. پایان Employment Relationship مسیرهای آینده‌اثر را متوقف می‌کند، اما پاداش کارکرد گذشته فقط با اجازه سیاست و کنترل حقوقی می‌تواند ادامه یابد؛ استخدام مجدد ارجاع تازه می‌خواهد. تعلیق یا جایگزینی نتیجه منبع، ارجاع در جریان را متوقف می‌کند و تصمیم تکمیل‌شده را بازنویسی نمی‌کند، بلکه بازبینی و اصلاح آینده‌اثر می‌طلبد.
_Avoid_: ساخت خودکار از امتیاز یا سطح، ترکیب چند نوع پیامد، انتقال ارجاع به استخدام مجدد، انتخاب دلخواه مقصد، گسترش دسترسی از راه ارجاع، کپی کل پرونده در مقصد، تمدید پنهانی نگهداری مبدأ از سیاست مقصد، جایگزینی پنهانی نتیجه منبع، اثر نامساعد گذشته‌نگر، خودتصمیمی ضمنی، یا تغییر مستقیم جبران خدمت و استخدام

**خودتصمیمی مدیریتی پیامد عملکرد**:
...
افشای هویت‌محور و محدود نام فارسی سطح عملکرد جاری، معنای کوتاه استاندارد، تازگی و وضعیت‌هایی مانند ارزیابی‌نشده یا نیازمند ارزیابی جدید به همان Personnel است. User مرتبط آن را فقط از پیوند معتبر Personnel–User و بدون مجوز Human Resources می‌بیند؛ Personnel بدون حساب پس از احراز هویت، همان خلاصه را از Human Resources یا Supervisor مرتبط در تحویلی حسابرسی‌شده دریافت می‌کند.
_Avoid_: نمایش امتیاز خام، رتبه، فاصله تا سطح بعدی، معیار، شاهد یا روایت Supervisor، ایجاد دسترسی عمومی یا OTP، تحویل بدون احراز هویت، یا یکی‌گرفتن خلاصه شخصی با مجوز مشاهده پرونده محرمانه

**نمایش سطح عملکرد در فهرست Personnel**:
افشای مستقل نام سطح جاری و تاریخ آخرین ارزیابی نهایی در فهرست اصلی Personnel، نمای محدود Personnel تحت مسئولیت Supervisor و گزارش مدیریتی مجاز است. امتیاز دقیق فقط در سابقه مجاز دیده می‌شود. هنگام تازه‌سازی، نشان معتبر قبلی تا رسیدن پاسخ تازه باقی می‌ماند؛ خطای دریافت فقط پیام کوتاه `به‌روزرسانی نشان انجام نشد` می‌دهد و نشان را حذف یا به `ارزیابی‌نشده` تبدیل نمی‌کند. نداشتن مجوز، نشان و هر جای‌نگهدار یا نشانه قفل را کاملاً حذف می‌کند و این سطح در فهرست‌های نامرتبط، جست‌وجوی عمومی User، گفت‌وگو، وظایف بین‌واحدی یا انتخاب‌گرهای Personnel ظاهر نمی‌شود.
_Avoid_: ناپدیدکردن نشان هنگام بازشدن کارت یا خطای موقت، تبدیل خطای دریافت به وضعیت کسب‌وکاری، نمایش امتیاز دقیق در فهرست، پنهان‌کردن تاریخ تازگی نشان، استنتاج مجوز از دسترسی عمومی فهرست، نمایش قفل برای داده محرمانه، افشا در انتخاب‌گرها یا سطوح همکاری، یا یکی‌گرفتن نشان سطح با اجازه مشاهده امتیاز و پرونده

**دسترسی تاریخی Supervisor به ارسال عملکرد**:
توانایی محدود یک Supervisor برای دیدن فقط بخش‌ها و ارسال‌های تغییرناپذیر متعلق به مسئولیت تاریخی خودش و نتیجه بررسی Human Resources درباره همان ارسال‌ها است، مشروط به فعال‌بودن User، Employment Relationship و مجوز عملیاتی او در زمان مشاهده. تغییر Supervisor یا مأموریت هدف، نویسندگی تاریخی را حذف نمی‌کند اما دسترسی به ارسال دیگران نمی‌سازد؛ لغو مجوز، تعلیق یا پایان استخدام دسترسی آینده را قطع می‌کند.
_Avoid_: دسترسی دائمی صرفاً از نویسندگی گذشته، مشاهده ارسال Supervisor دیگر، گسترش دسترسی با تغییر ساختار امروز، یا حذف هویت نویسنده پس از قطع دسترسی

**تحلیل تجمیعی عملکرد**:
نمای مجوزدار گروه‌ها، روندها و توزیع‌های عملکرد برای حداقل ده Personnel واجد شرایط است که هیچ نام، شناسه پایدار شخص، متن آزاد، زمان دقیق فردی، پیوند، راهنمای شناور، خروجی یا شمارش استنتاج‌پذیر از هویت Personnel فراهم نمی‌کند و به تحلیل نام‌دار راه عبور ندارد. فیلتر، مقایسه و جمع‌های مکمل نیز هرگاه نتیجه یک فرد یا گروه زیر حداقل جمعیت را آشکار کنند سرکوب می‌شوند. یک نسخه فقط هنگامی واقعاً ناشناس است که اتصال دوباره آن به داده نام‌دار ممکن نباشد و آزمون بازشناسایی هنگام تولید و پس از تغییر ساختار سازمانی نیز این مرز را تأیید کند؛ پنهان‌سازی نام یا استفاده از شناسه مستعار ناشناس‌سازی نیست. جمعیت ناهمگون فقط تعداد و درصد پنج سطح مصوب، مخرج واجدان شرایط و شمار سرکوب‌شده موارد خارج از محاسبه را نشان می‌دهد؛ میانگین یا میانه امتیاز فقط برای امضای مقایسه‌پذیری یکسان مجاز است و امتیاز کل شرکت یا خانواده‌های Job ناهم‌امضا ساخته نمی‌شود. روند، «جمعیت هر دوره» را همراه با ورود، خروج و نبود نتیجه از «جمعیت ثابت قابل‌مقایسه» دارای نتیجه معتبر در هر دو دوره جدا می‌کند و هیچ تغییر گروهی را علت یا بهبود فردی نمی‌نامد.
_Avoid_: ریزشکافی تا فرد، گروه کمتر از ده نفر، شناسه مستعار قابل اتصال، زمان فردی دقیق، استخراج مقدار فرد از تفاضل دو گروه، فرض دائمی ناشناس‌بودن پس از تغییر ساختار، میانگین امتیاز Jobهای ناهم‌امضا، پنهان‌کردن تغییر ترکیب جمعیت، نسبت‌دادن علت به تغییر توزیع، خروجی نام‌دار با مجوز تجمیعی، یا پنهان‌کردن نام در رابط در حالی که پاسخ آن را برمی‌گرداند

**تحلیل نام‌دار عملکرد**:
نمای مجوزدار سطح جاری، نتیجه مصوب نهایی، مقایسه و رتبه Personnelهای درون دامنه گزارش است که از جزئیات معیار، شواهد، روایت Supervisor و ردپای محاسبه جدا می‌ماند. هر نتیجه تاریخی با سطح و نسخه سیاست معتبر هنگام اثرگذاری خودش مقایسه می‌شود و بازه گزارش برای هر Employment Relationship فقط تازه‌ترین نتیجه مصوب معتبر را برمی‌گزیند که پایان بازه سنجش آن درون بازه انتخابی است؛ یک Personnel هرگز با چند نتیجه جمعیت را بزرگ‌تر نمی‌کند. افراد فقط بر پایه پنج سطح مصوب گروه‌بندی می‌شوند و همه افراد یک سطح هم‌سطح‌اند؛ امتیاز دقیق ترتیب درون سطح یا شماره رتبه نمی‌سازد. `سطح عملکرد جاری` در صورت مجازبودن حقیقت زنده و جداگانه‌ای است و تغییر بازه گزارش یا سیاست امروز، سطح تاریخی را بازطبقه‌بندی نمی‌کند.
_Avoid_: اعطای ضمنی از تحلیل تجمیعی، افشای خارج از دامنه گزارش، نمایش ارسال ردشده یا در انتظار، شمردن چند نتیجه یک فرد، رتبه دقیق درون یک سطح، ادعای برتری آماری از مرز سطح، طبقه‌بندی تاریخ با سیاست امروز، یکی‌گرفتن سطح جاری با سطح تاریخی، یا تبدیل دسترسی مدیریتی به مشاهده همه شواهد و روایت‌ها

**گروه همتای عملکرد**:
مجموعه Personnelهای دارای نتیجه مصوب معتبر است که Job یکسان یا عضو یک خانواده مقایسه‌ای Job نسخه‌دار و زمینه زمانی قابل‌مقایسه دارند. منابع انسانی، Jobهای واقعاً مشابه را با توجه به ماهیت کار، سطح مسئولیت و انتظارهای نقش در این خانواده قرار می‌دهد؛ یکسان‌بودن معیارها، وزن‌ها، نسخه الگو یا افزوده Position شرط عضویت نیست، زیرا مقایسه میان اعضای خانواده فقط بر پایه سطح مصوب نسبت به انتظارهای نقش انجام می‌شود و امتیاز دقیق میان امضاهای متفاوت مقایسه یا مرتب نمی‌شود. Workspace، شباهت عنوان یا مقیاس مشترک صفر تا صد به‌تنهایی همتایی نمی‌سازد. تحلیل نام‌دار فقط با دست‌کم پنج همتا ساخته می‌شود؛ گروه کوچک‌تر به‌جای رتبه، نمای فردیِ مجاز نسبت به انتظارهای Job را نشان می‌دهد. همه بخش‌های امتیازآور یک نتیجه باید متعلق به یک خانواده باشند؛ نتیجه چندخانواده‌ای به هیچ Job غالب حدسی نسبت داده نمی‌شود و فقط می‌تواند در توزیع تجمیعی سازمانیِ واجد حداقل جمعیت حضور یابد. Personnel فاقد نتیجه معتبر، دارای نتیجه منقضی یا تعلیق‌شده، یا دارای نتیجه `غیرقابل‌ارزیابی در این دوره` وارد گروه نمی‌شود و هیچ صفر یا مقدار تخمینی جای آن را نمی‌گیرد.
_Avoid_: رتبه‌بندی سه یا چهار همکار، یکی‌گرفتن Workspace با گروه همتا، تشخیص خودکار شباهت از نام Job، حدس Job غالب برای نتیجه چندخانواده‌ای، مقایسه عددی امضاهای متفاوت، ترکیب Jobهای ناهمگون در یک خانواده، افزایش مصنوعی جمعیت با چند دوره یک فرد، امتیازسازی برای داده ناکافی، یا پایین‌آوردن حداقل جمعیت برای تولید رتبه

**خانواده مقایسه‌ای Job**:
تعریف نسخه‌دار، دلیل‌دار و تاریخ‌دار منابع انسانی از Jobهایی است که با وجود تفاوت معیار، وزن، الگو یا Position، ماهیت کار، سطح مسئولیت و انتظارهای نقش آن‌ها برای مقایسه سطح مصوب به‌اندازه کافی مشابه است. فقط User دارای مجوز مؤثر و مستقل مدیریت خانواده‌های مقایسه‌ای می‌تواند نسخه آینده‌اثر را پس از دیدن پیش‌نمایش گروه‌های ساخته‌شده یا سرکوب‌شده فعال کند. تغییر عضویت فقط نسخه تازه می‌سازد و گزارش تاریخی از نسخه مؤثر همان زمان استفاده می‌کند.
_Avoid_: گروه‌بندی از روی شباهت متن عنوان، ترکیب سطح‌های مسئولیت نامتوازن، تغییر گذشته با ویرایش عضویت امروز، فعال‌سازی بدون دلیل و پیش‌نمایش، استنتاج اختیار از عنوان منابع انسانی، یا استفاده از خانواده برای مقایسه امتیاز خام

**امضای مقایسه‌پذیری عملکرد**:
شناسه نسخه‌داری است که یکسان‌بودن یا هم‌ارزی صریح معیارهای امتیازآور، وزن‌ها، مقیاس و قواعد محاسبه دو نتیجه عملکرد را برای مقایسه عدد دقیق ثابت می‌کند. عضویت در یک خانواده مقایسه‌ای Job برای مقایسه سطح مصوب کافی است، اما مجوز مقایسه امتیاز خام نمی‌سازد؛ هر اعلام هم‌ارزی عددی بخشی از سیاست تغییرناپذیر و قابل حسابرسی است.
_Avoid_: حدس هم‌ارزی عددی از عنوان یا خانواده Job، مقایسه دو افزوده Position ناسازگار با امتیاز خام، نادیده‌گرفتن تغییر وزن یا معیار، یا اعلام هم‌ارزی پنهان و بدون نسخه

**کالیبراسیون ارزیاب عملکرد**:
کنترل تشخیصی محرمانه منابع انسانی برای آشکارکردن الگوهای پایدار سخت‌گیری، آسان‌گیری، تمرکز افراطی بر یک درجه یا پراکندگی نامتعارف Supervisor در زمینه‌های دارای امضای مقایسه‌پذیری یکسان است. کنترل توزیعی فقط پس از دست‌کم ده بخش پذیرفته‌شده متعلق به حداقل پنج Personnel متفاوت و دو بازه ارزیابی اجرا می‌شود و پیش از آن فقط ناکافی‌بودن داده را اعلام می‌کند. مشاهده آن مجوز مؤثر و مستقل می‌خواهد که به‌طور پیش‌فرض فقط در پروفایل مدیر منابع انسانی قرار می‌گیرد؛ منابع انسانی می‌تواند هشدار را با یادداشت بررسی، تأیید یا رد و برای آموزش پیگیری کند. این کنترل Supervisorها را امتیازدهی یا رتبه‌بندی نمی‌کند و هیچ امتیاز، نتیجه مصوب یا پیامد استخدامی را خودکار تغییر نمی‌دهد؛ هر تغییر نتیجه فقط از مسیر رسمی اصلاح می‌گذرد.
_Avoid_: نتیجه‌گیری از نمونه کوچک، مقایسه زمینه‌های ناهم‌امضا، نرمال‌سازی پنهان امتیاز، تصحیح خودکار قضاوت Supervisor، رتبه‌بندی عمومی ارزیابان، افشای هشدار بدون مجوز مستقل، یا ساخت پیامد استخدامی از هشدار کالیبراسیون

**رخداد ممیزی عملکرد**:
ثبت تغییرناپذیر و حداقل‌گرای یک اقدام یا تغییر مهم در سابقه عملکرد است که موضوع، عامل و اختیار مؤثر او، زمان، علت، نسخه‌های منبع و سیاست و پیوند آن با رخدادهای پیشین را برای بازسازی حقیقت تاریخی حفظ می‌کند، بدون آنکه دسترسی به خود رخداد مجوز دیدن محتوای محرمانه منبع باشد.
_Avoid_: گزارش قابل‌ویرایش، اتکا به وضعیت جاری به‌جای تاریخچه، کپی‌کردن روایت یا شاهد محرمانه در ممیزی، استنباط اختیار از نقش زنده امروز، یا حذف رخداد همراه با رکورد عملیاتی

**سیاست نگهداری سوابق عملکرد**:
سیاست سازمانی نسخه‌دار و مصوب داخلی است که برای هر طبقه از سابقه عملکرد، مدت نگهداری، محدودسازی، ناشناس‌سازی، حذف، توقف حقوقی و حداقل گواه باقی‌مانده را تعیین می‌کند. هر شاهد به Employment Relationship خودش تعلق دارد و استخدام مجدد دوره رابطه پیشین را از نو آغاز نمی‌کند؛ فقط اختلاف، رسیدگی یا پیامد وابسته و دامنه‌دار می‌تواند حذف همان شاهد مرتبط را متوقف کند. متن منتشرشده، هویت نسخه، بازه اثر و هش آن مرجع پایدار بازسازی‌اند و تا وجود هر شاهد وابسته، همه اجزای لازم همان نسخه حفظ می‌شوند. این سیاست می‌تواند بدون تأیید بیرونی مشاور حقوقی فعال شود، اما نباید به‌عنوان نظر حقوقی یا اثبات انطباق با قانون معرفی شود؛ هیچ مدت حدسی یا یکسانی برای همه داده‌ها جایگزین نسخه مؤثر و مستند آن نمی‌شود.
_Avoid_: عدد ثابت در کد بدون مبنای مستند سیاست داخلی، ادعای انطباق حقوقی بدون اعتبارسنجی واجدصلاحیت، حذف نسخه‌ای که شاهد زنده به آن وابسته است، آغاز دوباره نگهداری سابقه قدیم صرفاً با استخدام مجدد، تمدید همه پرونده از یک وابستگی محدود، حذف هنگام اختلاف یا توقف حقوقی، نگهداری نامحدود همه محتوای محرمانه، یک مدت مشترک برای پیش‌نویس و نتیجه مصوب، یا حذف بدون گواه حداقلی

**توقف حقوقی سوابق عملکرد**:
مانع دلیل‌دار، دامنه‌دار و بازبینی‌شونده‌ای است که نگهداری سوابق عملکرد مرتبط را هنگام الزام حقوقی، اختلاف یا رسیدگی فعال بر برنامه عادی حذف مقدم می‌کند. ایجاد و رفع آن اختیارهای مستقل دارند و رفع توقف به‌خودی‌خود حذف را مجاز نمی‌کند، بلکه همه موانع باید دوباره سنجیده شوند.
_Avoid_: توقف نامحدود و بی‌دامنه، رفع بی‌دلیل، حذف هنگام توقف فعال، یا اجرای خودکار حذف‌های عقب‌افتاده بدون بازآزمایی

**محدودسازی شاهد عملکرد**:
وضعیت موقت و دامنه‌داری است که استفاده از شاهد مورد اختلاف یا منتظر تعیین تکلیف را در سطح جاری، رتبه، تحلیل، خروجی و ارجاع پیامد متوقف می‌کند، بی‌آنکه شاهد، ممیزی یا امکان بررسی مجاز آن را حذف یا بازنویسی کند. محدودسازی با توقف حقوقی یکی نیست و پس از تصمیم باید به‌طور صریح رفع، جانشین یا وارد برنامه حذف شود.
_Avoid_: حذف به‌جای محدودسازی، ادامه اثر داده مورد اختلاف، محدودکردن همه پرونده از یک شاهد نامرتبط، رفع ضمنی، یا نمایش داده محدودشده در خروجی زنده

**پرونده درخواست حریم خصوصی عملکرد**:
پرونده مستقل و حسابرسی‌شده درخواست دسترسی، اصلاح یا حذف Personnel درباره داده عملکرد خودش است که هویت و دامنه را تأیید، استفاده از شاهد مورد اختلاف را در صورت نیاز محدود و پاسخ را به بخش‌های مجاز تفکیک می‌کند. پاسخ می‌تواند سطح‌های مصوب تاریخی، تاریخ‌ها، وضعیت اصلاح، هدف پردازش و گروه‌های گیرنده را ارائه کند، اما ابتدا داده شخص ثالث و متن آزاد غیرضروری را حذف یا به خلاصه ساختاریافته تبدیل می‌کند و یک بخش محافظت‌شده مانع تحویل بقیه پاسخ نمی‌شود. اصلاح نسخه جانشین می‌سازد و حذف همچنان تابع سیاست نگهداری و توقف فعال است.
_Avoid_: یکی‌گرفتن درخواست رسمی با Badge شخصی، افشای هویت شاهد یا رتبه دیگران، رد کل پاسخ به‌دلیل یک بخش محافظت‌شده، ویرایش سابقه درجا، حذف هنگام توقف، یا پاسخ بدون احراز هویت و رسید

**گواه حذف سوابق عملکرد**:
رسید پایدار و غیرشخصی اجرای حذف یا ناشناس‌سازی مجاز است که دامنه، نسخه سیاست، دلیل، عامل و اختیار، زمان، شمار و اثر پاک‌سازی نسخه‌های وابسته را بدون نگهداری محتوای حذف‌شده اثبات می‌کند.
_Avoid_: امکان بازسازی محتوای حذف‌شده، نام یا امتیاز Personnel، مسیر یا token عملیاتی، خطای خام، یا اعلام تکمیل پیش از تعیین تکلیف همه نسخه‌های قابل‌بازیابی

**بازسازی سابقه عملکرد**:
نمای مجوزداری است که یا دانسته‌های ثبت‌شده در یک زمان برش را با نسخه‌های همان زمان نشان می‌دهد، یا سابقه معتبر فعلی را با اعمال پیوندهای جانشینی و تعلیق بازمی‌سازد؛ این دو نما هرگز به‌جای یکدیگر ارائه نمی‌شوند.
_Avoid_: تفسیر تاریخ با سیاست زنده امروز، پنهان‌کردن نسخه اصلاح‌شده، بازنویسی خروجی تاریخی، یا گزارش بدون زمان برش و حالت بازسازی

**خروجی محرمانه عملکرد**:
نسخه فارسی PDF یا Excel از دقیقاً همان ردیف‌ها و فیلدهایی است که درخواست‌کننده با مجوز مستقل خروجی و مجوزهای مشاهده همراه آن می‌تواند ببیند. تولید و تحویل موفق دو افشای جداگانه و حسابرسی‌شده‌اند؛ فایل، پیوند کوتاه‌عمر و تک‌منظوره دارد و پس از تحویل یا انقضای کوتاه خود از بین می‌رود، درحالی‌که رسید و هش آن طبق سیاست نگهداری باقی می‌ماند.
_Avoid_: گسترش دسترسی از راه خروجی، خروجی نام‌دار بدون مجوز تحلیل نام‌دار، پیوند پایدار یا قابل اشتراک، یکی‌گرفتن تولید با دانلود موفق، نگهداری بلندمدت فایل، دانلود بی‌حسابرسی، یا تولید فایل از داده‌های پنهان رابط

**سطح عملکرد نیازمند ارزیابی جدید**:
وضعیت Badge پس از پایان اعتبار همه نتایج مؤثر است که بدون نمایش سطح قدیمی به‌عنوان عملکرد جاری، نیاز به ارزیابی تازه را نشان می‌دهد. تاریخچه و سطح‌های پیشین برای اشخاص مجاز محفوظ می‌مانند.
_Avoid_: نگه‌داشتن دائمی سطح منقضی، امتیاز صفر، پایین‌ترین سطح عملکرد، یا حذف تاریخچه

**ارزیابی‌نشده**:
معنای نبود نتیجه مصوب امتیازدار برای Personnel است، خواه ارزیابی هنوز آغاز نشده، در جریان، در انتظار بررسی یا فقط دارای نتیجه `غیرقابل‌ارزیابی در این دوره` باشد. دلیل غیرقابل‌ارزیابی‌بودن در خلاصه شخصی افشا نمی‌شود؛ اگر سطح معتبر پیشین منقضی شده باشد، `نیازمند ارزیابی جدید` مقدم است. این وضعیت نه امتیاز صفر است و نه پایین‌ترین سطح عملکرد.
_Avoid_: یکی‌گرفتن نبود نتیجه امتیازدار با عملکرد ضعیف، افشای دلیل داخلی غیرقابل‌ارزیابی در خلاصه شخصی، نمایش سطح ساختگی، یا پنهان‌کردن تفاوت مراحل داخلی از اشخاص مجاز

**غیرقابل‌ارزیابی در این دوره**:
تصمیم دلیل‌دار منابع انسانی است که اعلام می‌کند مدت مؤثر یا شواهد معتبر یک ارزیابی عملکرد پرسنل برای تولید نتیجه کافی نیست. این تصمیم از مانع قابل‌رفع ساختار سازمانی و از نتیجه عملکرد پایین جداست.
_Avoid_: حذف بی‌دلیل از نوبت یا جامعه مشمول، امتیاز صفر، پایین‌ترین سطح عملکرد، یا استفاده برای دورزدن نقص قابل‌اصلاح

**مانع ساختاری ارزیابی عملکرد**:
وضعیت قابل‌رفعی است که در آن Employment Relationship، مأموریت مؤثر، تاریخ اثر، تخصیص کاری، Position یا Supervisor مسئول معتبر برای ساخت Snapshot ارزیابی وجود ندارد. تا اصلاح دلیل‌دار و آینده‌اثر ساختار، ارزیابی مسدود می‌ماند و گذشته از وضعیت زنده امروز حدس زده نمی‌شود.
_Avoid_: امتیاز صفر، `غیرقابل‌ارزیابی در این دوره`، بازسازی عقب‌اثر، حدس Supervisor از Department یا Position فعلی، یا آغاز ارزیابی با Snapshot ناقص

**شاهد تاریخی عملکرد بدون اثر**:
سند کاغذی یا بیرون‌سامانه‌ای پیش از cutover است که در صورت نیاز کسب‌وکاری، فقط با منشأ، تاریخ ورود، هش و سطح اطمینان در محدوده محرمانه و فقط‌خواندنی نگهداری می‌شود. این شاهد هیچ‌گاه به نتیجه مصوب، امتیاز، Badge، رتبه، تحلیل یا ارجاع پیامد تبدیل نمی‌شود.
_Avoid_: تبدیل به نتیجه مصوب با تأیید بعدی، امتیازسازی از ارزیابی جذب یا گزارش عملیاتی، نگهداری بدون منشأ، یا نمایش به Personnel

**کلید غیرنمایشی موضوع عملکرد**:
هویت مستعار و داخلی شاهد عملکرد است که پس از حذف مجاز هویت مستقیم Personnel، پیوستگی سوابق هنوز مشمول نگهداری یا توقف حقوقی را بدون نام، کد ملی، شناسه قابل‌نمایش یا پیوند عادی به پروفایل حفظ می‌کند. این کلید تا پایان مبنای نگهداری فقط در دامنه بسیار محدود می‌ماند و سپس مطابق سیاست حذف می‌شود.
_Avoid_: ناشناس‌سازی ادعایی، هش قابل‌حدس از کد ملی، پیوند عمومی به Personnel حذف‌شده، نگهداری نامحدود، یا حذف شاهد پیش از پایان مبنای نگهداری

**مرز نوشتن canonical ارزیابی عملکرد**:
قاعده‌ای است که از نخستین فعال‌سازی، هر سیاست، Snapshot، ارسال، تصمیم، نتیجه، بازمحاسبه و رخداد ممیزی عملکرد فقط در مدل معتبر جدید نوشته می‌شود. مسیرهای قدیمی حداکثر منبع فقط‌خواندنی شاهدند و هیچ dual-write، fallback اختیار یا نویسنده رقیبی وجود ندارد.
_Avoid_: نوشتن روی `Personnel` یا `User`، dual-write، بازگشت به writer قدیمی، ساخت مجوز از role زنده، یا نمایش Badge از ستون دستی

**موضوع عملکرد**:
ریشه پایدار هویتی سوابق عملکرد است که در زمان فعالیت به دقیقاً یک Personnel و یک Employment Relationship پیوند دارد و همه ارزیابی‌ها، Snapshotها، نتیجه‌ها و رخدادهای همان دوره استخدام را گرد می‌آورد. پس از حذف مجاز Personnel، پیوند مستقیم آن جدا و تا پایان مبنای نگهداری با کلید غیرنمایشی حفظ می‌شود.
_Avoid_: ریشه‌گرفتن از `User`، ترکیب دو Employment Relationship، انتقال Badge به استخدام مجدد، cascade delete شواهد تاریخی، یا نگهداری پیوند نمایشی پس از حذف هویت

**بازسازی آمادگی داده عملکرد**:
پردازش تکرارپذیر و قابل‌ادامه‌ای است که پیوند Personnel و Employment Relationship، کیفیت مأموریت، Supervisor، تاریخ اثر و تخصیص کاری را با هش و نسخه منبع طبقه‌بندی می‌کند و وضعیت `ارزیابی‌نشده` یا مانع ساختاری می‌سازد. این پردازش هیچ سرپرست، Snapshot، امتیاز، نتیجه مصوب یا سطح تاریخی را حدس نمی‌زند.
_Avoid_: score backfill، جعل تاریخچه، write-on-read، نادیده‌گرفتن drift، retry غیرایمن، یا امتیازسازی از داده جذب و گزارش عملیاتی

**cohort فعال‌سازی ارزیابی عملکرد**:
مجموعه نام‌دار و کنترل‌شده‌ای از Personnel، Employment Relationshipها، Supervisorها و بررسی‌کنندگان آماده است که پس از عبور از دروازه‌های داده، مجوز، محرمانگی، ممیزی و آمادگی عملیاتی، یک مرحله از rollout را آغاز می‌کنند. مانع ساختاری بیرون cohort پیشروی گروه آماده را متوقف نمی‌کند، اما نقص سراسری تمامیت یا محرمانگی همه cohortها را متوقف می‌کند.
_Avoid_: ورود Personnel دارای مانع، مقایسه جمعیت فعال با غیرفعال، rollout بدون مالک عملیاتی، یا نادیده‌گرفتن نقص سراسری

**توقف ایمن ارزیابی عملکرد**:
وضعیت fail-closed پس از نخستین canonical write است که در پی نقص بحرانی، ایجاد نوبت، ارسال، تصمیم، خروجی و ارجاع پیامد تا اصلاح روبه‌جلو و بازتطبیق کامل متوقف می‌شود، درحالی‌که همه شواهد موجود باقی می‌مانند. پس از این مرز، بازگشت به writer قدیمی یا بازیابی checkpointی که writeهای جدید را حذف کند مجاز نیست.
_Avoid_: ادامه نوشتن در نقص تمامیت یا افشا، حذف event، downgrade Schema، بازگرداندن legacy writer، یا ازست‌دادن ارسال یا تصمیم ثبت‌شده

**دروازه ارتقای ارزیابی عملکرد**:
تصمیم fail-closed و بدون اغماض فردی برای فعال‌کردن یک قابلیت یا گسترش cohort ارزیابی عملکرد است که فقط پس از گذشت هم‌زمانِ حداقل زمان سالم، حجم کافی عملیات واقعی، تطبیق بدون اختلاف، بودجه‌های مصوب، نبود نقص بحرانی یا سطح‌بالا و تأیید Human Resources، امنیت و حریم خصوصی و مالک سامانه باز می‌شود. شکست یک قابلیت دیرتر، قابلیت پذیرفته‌شده پیشین را فقط هنگامی متوقف می‌کند که نقصی سراسری در محرمانگی، اختیار، تمامیت، ممیزی، lineage، نگهداری یا محاسبه آشکار شده باشد.
_Avoid_: ارتقا فقط بر پایه گذشت زمان یا چند نمونه موفق، نادیده‌گرفتن جمعیت یا بار نماینده، waiver یک تأییدکننده، عبور با P0 یا P1 باز، یا خاموش‌کردن بی‌دلیل قابلیت سالم پیشین

**بسته شواهد ارتقای ارزیابی عملکرد**:
سابقه تغییرناپذیر و متصل به release است که commit و image، نسخه Schema و سیاست، profile و seed داده، محیط و عاملان، نتیجه و زمان همه gateها، سنجه‌های ظرفیت، شمار و هش مهاجرت و تطبیق، آزمون‌های امنیت و تجربه، رخدادها و تصمیم نهایی ارتقا یا توقف را گرد می‌آورد. تغییر مرتبط در کد، Schema، سیاست یا زیرساخت، اعتبار شاهد وابسته را پایان می‌دهد و تولید دوباره آن را لازم می‌کند.
_Avoid_: checklist بدون نتیجه، screenshot به‌جای شاهد بازتولیدپذیر، استفاده از شاهد release دیگر، پنهان‌کردن شکست یا رخداد، نگهداری secret یا محتوای محرمانه در بسته، یا پذیرش با مدرک منقضی

**نوبت ارزیابی تخصصی متقاضی**:
یک رخداد برنامه‌ریزی‌شده یا تکمیل‌شده در ارزیابی داخلی متقاضی است که به Candidate Recruitment Case تعلق دارد و از ارزیابی عملکرد پرسنل پس از استخدام مستقل می‌ماند.
_Avoid_: استفاده مجدد از مدل جذب برای عملکرد کارکنان، انتقال خودکار امتیاز یا نتیجه جذب به Personnel، یا یکی‌گرفتن ارزیاب جذب با سرپرست مسئول
...
An ADMIN-only irreversible operation, available from active or archived Personnel without a prior offboarding or archival condition, that removes the selected Personnel, linked Candidate applications and User, credentials, sessions, direct identity data, and every operational record or live-storage file no longer protected by an effective retention rule or hold. Evidence that must temporarily survive is restricted, stripped of direct identity, linked only by a non-display identifier, and erased when its governing period ends; this survival never preserves a usable account or restores ordinary access to the erased person. Immutable backups age out under their normal retention cycle rather than being selectively rewritten. The operation cannot erase the acting ADMIN or the last active ADMIN, is never bulk-operated, and requires an exact impact preview, mandatory reason, current-password verification, exact-name challenge, final confirmation, atomic execution, and a Permanent Person Erasure Receipt.
_Avoid_: requiring prior archival, treating Personnel erasure as ordinary deletion, preserving undisclosed or ordinarily accessible copies, destroying evidence still protected by retention or hold, keeping direct identity when a non-display link suffices, retaining protected evidence after its period ends, deleting oneself or the final administrator, executing without preview and reauthentication, or claiming immediate selective removal from disaster-recovery backups

**Permanent Person Erasure Receipt**:
The immutable non-personal evidence retained after an ADMIN irreversibly erases a person, containing the deleting administrator, time, mandatory reason, a non-reversible deletion fingerprint, and record/file counts by category without the erased person's identifiers, name, contact details, national code, documents, or other personal information.
_Avoid_: retaining an erased identity identifier or other personal information in the receipt, making the fingerprint usable to recover or locate the person, allowing deletion without accountable evidence, or making the receipt editable or deletable
...
Partner Seller permissions appear in this same surface under their owning workspaces: inquiry viewing, assigned response and seller/responder management under Sales; identity verification under Human Resources; commercial and credit terms under Accounting; and aggregate reporting under Business Intelligence. Partner authorization derives fixed, backend-owned resource scopes from these effective levels and never accepts a client-selected scope.
...
_Avoid_: connecting production-intended hardware before simulator acceptance, testing only the happy path, treating retries as new confirmations or exits, or tolerating missing evidence
...
The role-specific proof that each Internal Dispatch Pilot Cohort participant understands owned actions, handoffs, prohibited actions, audit reasons, and escalation and can perform the normal path plus every exception their authority permits in a production-like environment. Accounting and Guard supervisors jointly demonstrate biometric fallback, Manual Outage Exit, and Dispatch Dual Control; each workspace demonstrates its recovery and rejection paths; support diagnoses device, licence, projection, audit, and notification failures without exposing secrets; and drivers practice capture, retry, and fallback expectations. Failed assessment requires retraining, while a material workflow change or relevant incident requires targeted recertification.
