...
The Finance-owned review of required collateral and obligations that begins only after Candidate selection and Accepted Offer. Through separately permissioned task-scoped work in the Accounting destination, one Finance recorder registers receipt, custody details, and scans, while a distinct Finance verifier approves or returns the evidence and clearance required before Hire Conversion. Recording and verification require distinct Users; neither an Accounting workspace administrator nor global ADMIN may bypass this custody separation. Candidates who are rejected, withdrawn, or still under assessment are not asked to provide collateral.
_Avoid_: collecting collateral from every applicant, recorder self-verification, granting an administrator bypass, merging the two actions, performing Finance work through general Human Resources access, converting to Personnel before clearance, or treating offer acceptance as collateral approval
...
The Finance-recorded handover of a collateral original after a pre-activation cancellation or another authorized release, identifying the recipient and return date and preserving a description plus uploaded proof of handover. The return controls exist only when an original is held and a return is required, and a distinct Finance verifier confirms the completed handover. Neither an Accounting workspace administrator nor global ADMIN may confirm a return they recorded.
_Avoid_: showing return fields during ordinary receipt, recording return without a recipient or proof file, confusing original return with rejection for correction, self-confirmation, granting an administrator bypass, merging record and confirmation, or losing the instrument's custody history
...
The report-first, manifest-gated, idempotent creation of one missing review duty only for each latest valid submitted and unresolved contract version, preserving existing duties and ignoring stale, returned, approved, or superseded versions.
_Avoid_: applying without a reviewed manifest, creating duplicate duties, routing an obsolete version, mutating contract evidence, or granting Accounting Human Resources workspace access
