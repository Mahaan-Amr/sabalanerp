...
The owner-only CRM boundary applied specifically to a Partner Seller: a newly created Customer belongs to that Partner Seller, and the Partner Seller may view and use only their own Customers and related work. A possible duplicate owned by someone else reveals only the matched name, person type, city, and a masked match witness such as the final four phone digits, then offers a managed transfer request; full identifiers, address, owner, Projects, Contracts, and timeline remain hidden. Ordinary internal Sabalan sellers retain their explicitly scoped CRM access and are not restricted by this Partner-only boundary.
_Avoid_: applying the Partner restriction to ordinary internal sellers, exposing another owner's complete match witness or related activity, treating a missing department as company scope, silently transferring ownership, allowing a Partner Seller to use another owner's Customer in a Contract, or resolving a match by creating an unrestricted duplicate

**پاسخ‌دهنده قیمت فروشنده همکار**:
The one active internal Sabalan seller with effective edit-capable Sales access whom management assigns to a Partner Seller account and whom the system automatically uses for every newly submitted Case pricing duty from that Partner. The Partner cannot choose or shop among responders per Case. A global ADMIN or authorized Sales manager may transfer a pending duty to another eligible responder only with a recorded reason and preserved assignment history. The responder sees the shared technical Product Graph and only delivery facts materially needed for pricing, such as city or delivery method; they never see the Partner's Customer identity, telephone, exact address, customer-facing price, payment plan, margin or other retail economics. The authority never makes the responder the Contract Creator, Responsible Seller, Customer owner, or recipient of sales-performance credit.
_Avoid_: letting the Partner select a responder per Case, asking the responder to approve the Partner's retail price, exposing Customer identity, contact details, exact address or margin data, showing Delivery facts with no pricing purpose, silently reassigning pending work, deriving the responder from Contract responsibility, allowing an unassigned seller to quote, or crediting the responder for the Partner Seller's sale

**استعلام قیمت فروشنده همکار**:
The mandatory Case-scoped request created after a Partner Seller completes the shared seven-step Contract workflow and sends the unfinalized Case for Sabalan pricing. Each price-bearing Product row has an independent decision in that Case revision; every required row must be approved before the Partner may finalize or expose the customer Contract, and no approval is reusable in another Case. The approved quote package remains usable for 48 hours from the final required row approval; finalization within that window permanently snapshots the approved Sabalan prices for the resulting internal Sale Record.
...
A saved Partner Sale Case created when the Partner sends the completed seven-step wizard for Sabalan pricing. It owns one immutable Case number and current revision so every duty, decision, rejection and correction remains traceable, but it does not yet own a final customer-Contract number or internal Sabalan-record number and cannot be shared with the Customer. It creates no Sabalan debt, Accounting eligibility, final artifact, reservation, fulfillment authority or realized-sale effect until Partner finalization succeeds with every required approval still valid.
...
The single canonical Product, configuration, quantity, processing, tool, and Delivery plan owned by a Partner Sale Case and shared by both records. Before pricing submission, the ordinary Contract Product Graph and pricing engine handle all edits. After a row is approved, any edit that changes its Sabalan base-rate subject, canonical quantity, or system-calculated Sabalan amount invalidates only that row and its price-dependent descendants and requires a new responder decision; retail-rate-only, Customer-identity, retail-payment, or non-priced Delivery-detail changes do not invalidate Sabalan pricing. A child made from previously paid remainder carries zero new material charge, while its new cutting, tools, and finishing remain billable exactly once. Correction preserves stable row identity, replays source and remaining-stone children atomically, and cannot reduce quantity below effective reserved plus physically dispatched evidence.
...
The two-stage all-or-nothing persistence boundary. Pricing submission rechecks the active Partner Seller, Customer scope, shared Product Graph, Partner retail inputs and Delivery plan before creating exactly one Case identity, first revision, Case number, pricing duty and audit evidence. Later Partner finalization rechecks that same Case revision and every required still-valid row decision before atomically allocating both final linked-record identities and numbers, creating their snapshots and bindings, fixing the Sabalan debt and making the customer-safe Contract available. Replaying either command with the same identity returns its recorded result instead of creating another Case or record pair.
...
One of multiple creator-private, named recoverable Partner Contract attempts before pricing submission, identified to the Partner Seller by its Customer, Project and last meaningful change. Starting or resuming one attempt never discards another. Sending one valid completed wizard promotes exactly that attempt into its own numbered Case and Case-scoped pricing duty; no price decision can be loaded into another Draft or Case.
_Avoid_: limiting a Partner Seller to one recoverable Contract attempt, overwriting one Customer's work when another Contract begins, creating a reusable standalone inquiry, sharing a draft across Partner Sellers, loading another Case's quote, or allowing one draft's recovery state to leak into another
