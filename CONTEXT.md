...
**فروشنده همکار**:
An external independent sales collaborator with an ERP account and a Partner sales profile. The Partner Seller creates Sales Contracts for their own CRM Customers, remains both the Contract Creator and Responsible Seller, and receives the resulting sales-performance credit; the internal seller who answers or approves the required price inquiry does not receive ownership or sales credit. A Partner Seller may save and share a numbered Case before every inquiry is approved, but must bind a still-valid approval for every intended Contract Product before finalizing the Case and accepting debt to Sabalan.
_Avoid_: treating the Partner Seller as an internal Sabalan seller, confusing the Partner Seller with فروش همکاری or مشتری همکاری, attributing the sale to the internal price responder, allowing finalization before the required product prices are approved, treating a numbered unfinalized Case as Sabalan debt, or relying on a broad system Role alone to express the Partner boundary
...
The one active internal Sabalan seller with effective edit-capable Sales access who is assigned to answer and approve one Partner Seller's price inquiries. Assignment belongs to the inquiry as a whole; a reasoned reassignment transfers only its pending rows, immediately ends the former responder's mutation authority, and preserves the effective assignment and eligibility with every historical decision. A global ADMIN or a User with effective Sales workspace administration may answer any open inquiry through an atomic audited management takeover. Price-response authority never makes the responder the Contract Creator, Responsible Seller, Customer owner, or recipient of sales-performance credit.
_Avoid_: deriving the responder from Contract responsibility, allowing an unassigned seller to approve a price, losing assignment history, silently redirecting pending work, or crediting the responder for the Partner Seller's sale

**استعلام قیمت فروشنده همکار**:
The mandatory request through which a Partner Seller asks their assigned Price Responder for Sabalan's rate for one or more catalog pricing subjects: the selected catalog main Stone for Longitudinal, Slab, and Stair components, or the exact catalog Product for Cubic and prepared Products that have no separate Stone selector. The inquiry reuses the ordinary Contract configurator to offer only the dimensions applicable to that family; those dimensions are optional context, and any values supplied are preserved immutably in the inquiry history. Approval identity is the Partner Seller and catalog pricing subject rather than those dimensions, so changing any dimension or other configuration never forces re-inquiry. Changing the pricing subject requires its own valid approval, while the Partner Seller may request a voluntary successor with an optional note and no mandatory reason. Each approval is reusable across the Partner Seller's own Customers and Contracts without quantity cap and remains valid for exactly 48 hours, with independent outcomes across bulk and concurrent submissions.
_Avoid_: applying approval to a different catalog pricing subject or Partner Seller, pricing a generated consumed-stone display row as the inquiry subject, requiring dimensions merely to submit, treating recorded dimensions as approval restrictions, automatically requiring re-inquiry after dimension, quantity, operation, Customer, Project, Delivery, or retail-price changes, carrying one prepared Product's approval to another, demanding a reason for voluntary re-inquiry, starting validity before response, mutating a decided row, serializing unrelated inquiries, or accepting final Contract submission with expired approval

**واحد نرخ استعلام فروشنده همکار**:
The catalog-derived pricing basis attached to an approved inquiry independently of delivered pieces or order quantity. Longitudinal stone, Slab, and Stair-component approvals are priced in toman per square meter of the selected catalog main Stone; Cubic and prepared Products use their catalog selling unit such as piece, tonne, or square meter; independent Services require no inquiry and retain their own canonical unit and system rate. The Price Responder enters an amount but cannot change this unit.
_Avoid_: using linear meters, finished-piece count, requested-order quantity, or a generated consumed-stone row as the rate basis for cut stone, allowing the responder to choose a unit, forcing independent Services through stone inquiry, or conflating the approved material-rate unit with Delivery or presentation units

**شواهد تصمیم استعلام قیمت فروشنده همکار**:
The immutable, integrity-verifiable record of one inquiry-row definition, submission, assignment, authorized decision, and later lifecycle or usage events. It preserves actor and effective authority, database-recorded time, reason or note, wholesale amount where approved, validity, predecessor lineage, revisions, command identity, configuration and policy hashes, and independent per-row outcomes without manufacturing an expiry mutation.
_Avoid_: mutable approval evidence, anonymous decisions, relying on current assignment or policy to explain history, losing a row outcome inside a bulk command, treating derived expiry as a human decision, or recording only the latest state

**قیمت فروش سبلان به فروشنده همکار**:
The Partner Seller's final purchase amount from Sabalan for one Contract Product row, calculated by the ordinary Contract pricing engine exactly once. For Longitudinal, Slab, and Stair components, the still-valid approved rate owned by the selected catalog main Stone is multiplied by the canonical actual billable quantity consumed from that same Stone, then canonical mandatory charges where applicable, cutting, tools, finishing, and other Sabalan-supplied components are added. A generated `سنگ مصرفی` row only presents that material calculation and is neither the approval subject nor a second charge. Cubic and prepared Products apply the approved rate for the exact catalog Product through its canonical catalog selling unit. Geometry and quantity changes recalculate the amount without mandatory re-inquiry; selecting another catalog pricing subject requires its approval, and the successful Case snapshots the resulting amount without constraining the Partner's retail price.
_Avoid_: attaching the approved rate to a generated consumed-stone row, charging main material twice, using requested or consumed display rows as separate sales, carrying one catalog subject's approval to another, applying a cut-stone square-meter rule to a prepared Product with another catalog unit, keeping the total unchanged after billable geometry or operations change, requiring re-inquiry for a non-subject change, or repricing an existing Case after inquiry expiry

**مصرف تأیید قیمت فروشنده همکار**:
The append-only evidence that one Partner Sale Case used one still-valid approved inquiry row for the matching Partner Seller and catalog pricing subject. Use never consumes or mutates the approval, while the Case snapshots the approved rate and unit, historical inquiry dimensions, current canonical material-consumption or catalog-unit calculation, component calculation, final Sabalan amount, and usage link without exposing wholesale evidence to the Customer.
_Avoid_: decrementing approval quantity, requiring current dimensions to equal historical inquiry dimensions, applying approval to a different catalog pricing subject or Partner Seller, changing the catalog-owned unit, omitting canonical calculation evidence, pricing a consumed-stone display row independently, or exposing wholesale evidence to the Customer

**قیمت فروش فروشنده همکار به مشتری**:
The freely chosen price at which a Partner Seller sells an approved Product configuration to one of their own Customers. It defaults to Sabalan's approved selling price for convenience but may be higher or lower without additional price approval. Both prices and their difference are preserved for reporting, while the difference is not Sabalan income.
_Avoid_: forcing equality with the inquiry price, treating a higher Partner price as Sabalan revenue, preventing a below-inquiry Partner price, overwriting either price with the other, or storing only the difference without both source prices

**بازفروش مستقل فروشنده همکار**:
The commercial relationship in which a Partner Seller buys from Sabalan at Sabalan's approved inquiry price and independently resells to their own Customer at the Partner Customer Sale Price. The Partner Seller, not the end Customer, owes Sabalan; any resale profit or loss belongs to the Partner Seller and is not Sabalan revenue or loss. The end Customer remains the Partner Seller's customer and does not become Sabalan's accounting debtor merely because the sale is recorded in Sabalan ERP.
...
The access boundary for Partner resale economics: the Partner Seller sees the approved base Stone rate, final amounts for cutting, tools, finishing and other visible components, total Sabalan purchase amount, validity, note or reason, state, and their own Cases that used it, but not internal formulas, managerial rates, policy inputs, or cost evidence. The assigned Price Responder and explicitly authorized Sales audit users may inspect inquiry evidence and usage status without retail Customer economics or margin; Sabalan Accounting uses only Sabalan's receivable, and unrelated sellers see none of these prices.
_Avoid_: hiding the Partner Seller's understandable final price breakdown, exposing internal formula or policy construction, exposing retail economics or Customer detail to the Price Responder, using resale value as an Accounting receivable, or granting management-report visibility through ordinary Sales access
...
A saved Partner Sale Case that already owns its immutable Case, customer-Contract and internal-record identities and may be shared with the Customer, while one or more Sabalan price approvals are missing, pending, rejected or expired. It creates no Sabalan debt, Accounting eligibility, final artifact, reservation, fulfillment authority or realized-sale effect until Partner finalization succeeds with every required approval still valid.
_Avoid_: Contract Creation Draft, unnumbered browser recovery, committed sale, provisional Sabalan receivable, final PDF, hiding the Customer-facing draft status, or deleting and reusing its business numbers

**سند فروش سبلان به فروشنده همکار**:
The purpose-specific internal commercial record inside a Partner Sale Case. Before Partner finalization it may be an explicitly unpriced shell waiting for required inquiry approvals and creates no debt or Accounting effect; finalization atomically freezes every Product amount from valid Partner Seller Price Inquiry evidence, makes the Partner Seller Sabalan's debtor, and establishes the sole basis for Sabalan Accounting, receivable and collection. It is not an ordinary Sales Contract and is never exposed to the end Customer.
_Avoid_: modelling it as a second ordinary Sales Contract, treating an unpriced shell as a receivable, naming the end Customer as Sabalan's debtor, using the Partner Customer Sale Price for Sabalan Accounting, exposing the record or inquiry price to the end Customer, or creating it without the linked Partner Customer Sales Contract
...
The synchronized commercial commitment of a Partner Sale Case. Final wizard submission atomically creates both linked Draft records and their evidence; customer OTP remains earlier approval evidence, while the first successful existing authenticated signature or final print/PDF action commits the case and internal record, fixes the Partner Seller's commercial debt, and enters the Accounting queue exactly once, with preview and repeated download remaining non-transitioning and financial approval later creating the ordinary official Accounting Receivable.
_Avoid_: creating commercial debt from a Draft, OTP, preview, or repeated download, creating the official Accounting Receivable before financial approval, delaying commercial commitment until financial approval, creating duplicate cases on retry, allowing a late OTP to regress a committed case, committing only one linked record, adding a separate Partner print action, or losing the commitment transition evidence
...
The single canonical Product, configuration, quantity, processing, tool, and Delivery plan owned by a Partner Sale Case and shared by both records. Before Case creation, every geometry, quantity, cutting, tool, finishing, and Delivery change is handled by the ordinary Contract Product Graph and pricing engine, recalculating canonical consumption and the final Sabalan amount without mandatory re-inquiry; only changing the catalog pricing subject requires another approval, while a voluntary successor may be requested at any time. A child made from previously paid remainder carries zero new material charge, while its new cutting, tools, and finishing remain billable exactly once. A same-Stone Stair layer reuses that Stone's approval, while a layer selecting another catalog Stone requires its own valid approval. Correction preserves stable row identity, replays source and remaining-stone children atomically, and cannot reduce quantity below effective reserved plus physically dispatched evidence.
_Avoid_: reducing the Product step to a simple catalog picker, duplicating the graph, matching rows by position, requiring re-inquiry for length, width, thickness, quantity, cutting, tools, finishing, or Delivery alone, carrying approval to a different catalog pricing subject, charging previously paid remainder material again, accepting partial child replay, or allowing linked records to drift
...
The continued editing of an already-created Partner Sale Case using its snapshotted approval evidence. Inquiry expiry does not reprice that created Case Draft; any configuration that retains the same catalog pricing subject may change through canonical recalculation, while selecting a different pricing subject requires current approval. A pre-submission Contract Creation Draft never freezes validity: expired approval blocks final submission, and an approved successor rate must be presented for explicit Partner acceptance before replacing the Draft amount.
_Avoid_: treating recoverable draft state as price-lock evidence, submitting after expiry, silently replacing a Draft price, requiring re-inquiry for a dimension, quantity, or operation change, carrying approval to another catalog pricing subject, or repricing a created Case merely because its inquiry later expires
...
The monotonic case-owned lifecycle `پیش‌نویس`, `در انتظار تأیید مشتری`, `تأییدشده مشتری`, `قطعی‌شده`, or pre-commit `لغوشده`, followed only where the reviewed post-commit voiding workflow succeeds by terminal `باطل‌شده`. Customer signature, print, original commitment, voiding adjustments, and financial or physical settlement remain independent append-only facts, so `باطل‌شده` never erases or rewrites what was previously committed.
_Avoid_: giving either linked record an independent lifecycle, writing customer status directly, replacing signature evidence with print evidence, moving a committed Case into pre-commit cancellation or customer approval, treating voiding as deletion, creating a second commitment or receivable from replay, or cancelling only one linked record
...
The all-or-nothing final submission that rechecks the active Partner Seller, Customer scope, exact still-valid inquiry evidence, shared Product Graph, both price and payment truths, and Delivery plan before creating the case identity, first revision, both records, snapshots, row bindings, and audit together. An approved inquiry row remains reusable, each use leaves immutable binding evidence, and replaying the same submission returns the same case rather than creating another pair.
_Avoid_: partial pair creation, trusting client-time eligibility, consuming a reusable approval, accepting one invalid row while saving the rest, creating duplicate cases on retry, or exposing a number for a case that did not commit

**هویت نسخه‌شده طرفین پرونده فروش همکار**:
The pairing of live Partner Commercial Account and Customer identities for current authority with their case-revision snapshots for commercial evidence. Suspension blocks mutation and commitment without erasing the Draft, later inquiry expiry does not invalidate already snapshotted evidence, Customer transfer never moves the case automatically, and changed party facts enter only through an explicit Draft revision or formal post-commit correction.
_Avoid_: authorizing from a stale snapshot, silently refreshing commercial evidence from live records, mutating while the Partner is suspended, expiring a validly created Draft with its inquiry, transferring a case merely because Customer ownership changed, or overwriting committed party evidence
...
The immutable schema-versioned customer-output evidence minted by the ordinary send-for-confirmation action and bound to one case revision, customer Contract number, normalized recipient, expiry, allowlisted content, and canonical hash. Customer approval is independent evidence rather than a prerequisite for Partner finalization: no response does not block finalization, while explicit rejection blocks that same revision; a material customer-visible or recipient change invalidates the prior confirmation and requires the ordinary send action again.
_Avoid_: treating silence as rejection, treating approval as the source of Sabalan debt, finalizing an explicitly rejected revision, minting on first public view, rebuilding from live relations, changing the recipient under an existing session, preserving a pending session after material change, or making the snapshot an editable Product Graph

**نهایی‌سازی فروشنده همکار**:
The authenticated, explicit and audited decision by the Partner Seller that atomically revalidates every required inquiry approval, freezes the Sabalan purchase amount and Partner-to-Customer commercial revision, records any required below-cost acceptance, and creates the Partner's debt and downstream Accounting and fulfillment eligibility. Customer silence and SMS delivery do not block it, but an explicit rejection of the current customer revision does; after it succeeds, correction, cancellation and voiding use the formal Partner Case lifecycle.
_Avoid_: Case save, autosave, SMS acceptance, customer confirmation, finalization with an invalid inquiry, silent below-cost acceptance, creating debt before finalization, or directly editing finalized commercial evidence

**برنامه‌های پرداخت پرونده فروش همکار**:
The two independent payment truths in a Partner Sale Case. The Partner Seller records the Partner-to-Customer plan in the sales wizard, while Accounting determines and manages the Partner-to-Sabalan plan after Case creation and commercial commitment from the approved Sabalan amount. Absence of the Sabalan plan never blocks Case creation or Customer confirmation but blocks final financial approval; corrected Customer plans preserve historical receipt and allocation evidence.
...
The per-row lifecycle `در انتظار پاسخ`, `تأییدشده`, `ردشده`, `منقضی‌شده`, `جایگزین‌شده`, or `لغوشده`, with one linear predecessor-successor chain and no branching. A valid predecessor remains usable while its successor is pending and becomes unusable only when the successor is approved; existing Cases keep their snapshot, while unsubmitted Drafts visibly adopt the new rate only after Partner confirmation. Cancelling a submitted inquiry cancels only pending rows, decided rows remain immutable, and parent summaries derive from independent row outcomes.
_Avoid_: branching successor history, revoking a predecessor for a pending or rejected successor, silently changing a Draft price, repricing an existing Case, cancelling or editing a decided row, requiring one outcome for every row, losing cancellation or succession evidence, or treating partial response as an error

**مرز مهاجرت استعلام قدیمی فروشنده همکار**:
An inquiry approved under the former exact-full-configuration model remains immutable historical evidence for its existing Case usage but cannot authorize a new Contract under the Stone-and-dimensions base-rate model. No base rate is inferred by dividing or decomposing its historical total; a new sale requires a new inquiry, while existing Cases, numbers, snapshots, and audit history remain unchanged.
_Avoid_: guessing a base rate from a historical total, treating old configuration approval as new Stone-rate authority, deleting old inquiry evidence, repricing an existing Case, or forcing migration of unrelated Partner history

**فضای استعلام قیمت فروشنده همکار**:
The Partner Seller's independent workspace for creating and tracking any number of concurrently open price inquiries, paired with the same inquiry action inside the Product stage of Partner Contract creation. The Contract wizard can search and load only the Partner Seller's own approved, unexpired configurations; while it remains open, newly answered inquiries become available there without page reload or loss of current work. A newly submitted or pending row remains linked to its recoverable Product draft and does not block unrelated approved rows from entering the Contract.
_Avoid_: limiting the Partner Seller to one open inquiry, serializing unrelated inquiries, forcing inquiry creation to exist only inside or only outside the Contract wizard, requiring a manual refresh to discover a new answer, loading another Partner Seller's approval, treating a pending row as approved Product authority, or losing the configured Product while awaiting response

**اعلان استعلام و تأیید مشتری فروشنده همکار**:
The separation between in-app Partner inquiry notifications and the existing customer Contract confirmation channel. Submission notifies the assigned responder; approval or rejection notifies the Partner Seller; a six-hour expiry warning notifies the Partner; and reassignment notifies the prior responder, new responder, and Partner. Inquiry notifications never send SMS, while the final Customer receives the ordinary Sales Contract SMS template, link, code, confirmation experience, and resend behavior from the Partner customer-safe snapshot.
_Avoid_: sending inquiry SMS, adding a Partner-specific customer template or confirmation page, selecting the Partner debtor account as recipient, exposing wholesale or internal evidence in the message or link, changing a snapshotted recipient during resend, or advancing Sabalan debt on OTP approval
...
**گزارش فروشنده همکار**:
The owner-scoped reporting surface for a Partner Seller's retail sales, Sabalan purchases, resale profit or loss, Customers, Products, private customer-collection status, debt and payments to Sabalan, and delivery status. Authorized Sales managers and Admins may view aggregate Partner reporting; Price Responders and unrelated sellers never receive retail-price or margin reporting through their ordinary access.
_Avoid_: exposing another Partner Seller's data, mixing retail value into Sabalan revenue, hiding the Partner Seller's own purchase economics, granting margin access to a Price Responder, or weakening Accounting and management-report permission boundaries
...
The rule that inquiry response requires edit-capable effective Sales access and current assignment. The responder workspace supports searchable concurrent queues, per-row prices in the catalog-owned unit, grouped input, independent or bulk approval and rejection, mandatory rejection reason, optional approval note, and immutable history; a global ADMIN or effective Sales administrator may take over pending rows atomically with reasoned assignment evidence.
_Avoid_: an ownerless price decision, response by a view-only User, changing the catalog unit, forcing one price across selected rows, overwriting a decided row, serializing unrelated inquiries, changing assignment without history, or granting mutation through reporting access

**محدوده امکانات فروشنده همکار**:
The full ordinary Sales and CRM creation experience constrained to the Partner Seller's own Customers, required owned Customer Projects, inquiries, Drafts, Partner Sale Cases, customer payment plans, customer confirmation SMS and output, fulfillment status, reporting, and حساب من با سبلان. Contract creation retains the ordinary seven-stage workflow, searchable selection, complete Product configuration, multi-Delivery quantity allocation, multi-method and multi-installment Customer payment entry, validation, recovery, and digital confirmation while substituting Partner inquiry and retail-price rules. It excludes Product-catalog creation or base-price administration, other sellers' records, inquiry response, Accounting mutation, and authoritative inventory, loading, or Delivery mutation.
_Avoid_: replacing the workflow with a reduced Partner form, removing ordinary Product, Delivery, Payment, recovery, or customer-confirmation capability merely because price is inquiry-controlled, allowing a Contract without an owned Customer Project, granting catalog administration, granting cross-seller visibility, exposing internal prices, or replacing action-specific restrictions with one broad Role check

**داشبورد فروشنده همکار**:
The concise operational home for the Partner Seller, showing pending inquiries, valid and expiring prices, Contract Drafts, Cases awaiting Customer confirmation, upcoming Deliveries, Customer receivables, debt and due dates to Sabalan, and prior internal work awaiting transfer, with direct actions for new sale and new inquiry. It follows the ordinary Sales workspace hierarchy and uses only short labels or guidance that prevents a likely mistake.
_Avoid_: a setup guide after activation, repeating visible information in explanatory paragraphs, exposing internal policy or unrelated seller data, hiding urgent work behind decorative summaries, or diverging visually from the Sabalan Design System

**پیش‌نویس ایجاد پرونده فروش همکار**:
One of multiple creator-private, named recoverable Partner Contract attempts, identified to the Partner Seller by its Customer, Project, and last meaningful change. Starting or resuming one attempt never discards another, concurrently open price inquiries remain independent of Contract drafts, and newly approved inquiry rows may be loaded into any eligible writable draft without copying another draft's Customer, Delivery, Payment, or retail evidence.
_Avoid_: limiting a Partner Seller to one recoverable Contract attempt, overwriting one Customer's work when another Contract begins, treating an inquiry as the Contract draft, sharing a draft across Partner Sellers, or allowing one draft's recovery state to leak into another
...
The rule that an Active Partner Seller User cannot simultaneously hold a general internal operational persona or create, edit, approve, or complete internal work. Conversion removes incompatible access but preserves an immutable pre-conversion role-and-access snapshot plus narrow read and transfer-request access to prior responsibilities. Before the first Partner Customer, Inquiry, or Case, an audited reversal restores that exact snapshot; afterward the Partner persona is irreversible on that User and a later internal hire receives a separately linked identity.
...
A Cubic or prepared Product without a separate Stone selector uses its exact catalog Product as the approval identity and its immutable catalog selling unit as the approved-rate basis. Entered dimensions may be preserved as inquiry history and still affect canonical calculation where the ordinary Contract engine defines that effect, but changing them does not invalidate the approval. An independent Service uses the canonical system rate without Partner inquiry and remains separately identified from stone material, while Product-catalog creation and rate administration remain unavailable to the Partner Seller.
_Avoid_: blocking Cubic or prepared Products, treating dimensions as approval identity, changing the catalog unit in an inquiry, requiring a Stone inquiry for an independent Service, allowing the Partner Seller to create catalog Products or rates, or merging Service revenue into stone material
...
The rule that Partner suspension blocks use without pausing, extending, shortening, or revoking an approved inquiry's natural 48-hour clock. If the profile returns to Active, only approvals whose server-recorded expiry is still in the future may be used; every other row requires re-inquiry.
_Avoid_: freezing expiry during suspension, granting a fresh 48 hours after reactivation, invalidating a price early solely because of suspension, or allowing a suspended Partner to create or submit a Contract

**مرز هم‌زمانی استعلام قیمت فروشنده همکار**:
The first-valid-commit rule for competing response, reassignment, cancellation, suspension, successor approval, and Partner Sale Case submission. Every contender rechecks the current row, assignment, actor eligibility, Partner status, and approval time boundary at commitment; the first valid commitment is retained and later contenders fail with an explicit retryable conflict, while a successfully snapshotted case is never undone by a later lifecycle change. Retrying the same command identity with the same intent returns its recorded whole or per-row result, while reusing that identity for different intent is a conflict and never creates duplicate decisions or cases.
_Avoid_: last-write-wins mutation, validating only before the commitment boundary, allowing two terminal outcomes for one pending row, letting stale assignment authorize a response, treating retry as a fresh decision, accepting changed intent under an old command identity, or retroactively invalidating an already-created case
...
The formal audited successor boundary after Partner Sale Case commitment. Retail-only correction is Sales-approved and becomes effective after one Partner save and fresh Customer confirmation without Accounting or internal-commercial change; shared-graph, Sabalan-amount, Partner-to-Sabalan-term, or voiding correction additionally requires the ordinary Accounting processing and manager gates, freezes affected mutation while unresolved, and switches the effective revision atomically only after every required verification and confirmation succeeds.
_Avoid_: creating an independent customer-only revision, routing retail-only correction through Accounting, changing internal truth for a retail-only correction, making a successor effective before all gates pass, leaving affected work mutable during review, retaining old customer confirmation for changed visible content, changing the shared Product Graph in only one projection, requiring re-inquiry for quantity-only or Delivery-only change, reusing stale approval for changed price-bearing configuration, letting the Partner edit Sabalan terms, or rewriting conclusive physical or financial evidence
...
The three immutable, unique, never-reused searchable business identifiers allocated together when the first numbered Partner Sale Case save succeeds: one Partner Sale Case number, one customer Contract number used in draft/final customer output, SMS and public confirmation, and one internal Sabalan record number reserved for the internal record and later Accounting use. The end Customer searches and sees only the customer Contract number; the owning Partner may search its case and customer Contract numbers, Accounting may cross-search all three through its purpose-bound view, and scoped management or Admin may search all three without widening returned projections.
_Avoid_: requiring gapless allocation, exposing the case or internal number to the end Customer, treating internal-number allocation as debt creation, letting a Partner search the internal number, forcing Accounting to search by only one identifier, widening a projection because an identifier matched, reusing a cancelled number, or allocating visible numbers for a failed save
