...
فقط برای محصول سنگ طولی، وقتی عرض و طول کل صریح هستند اما تعداد خالی یا صفر است، هر دو حالت فرمان بهینه‌سازی تعداد هستند و تعداد واقعی صفر محسوب نمی‌شوند. سیستم بیشترین تعداد نوار فیزیکی را که با عرض درخواستی و خوراک اره فعال در عرض سنگ اصلی جا می‌شود محاسبه می‌کند، طول کل را بین آن نوارها تقسیم می‌کند، و تعداد و طول هر قطعه محاسبه‌شده را پیش از ذخیره نشان می‌دهد. ردیف ذخیره‌شده تعداد و طول فیزیکی محاسبه‌شده را نگه می‌دارد، در حالی که متر طول کل و متر مربع تجاری ثابت می‌مانند؛ برای نمونه `40cm` منبع، `7cm` عرض، `50m` طول کل و تعداد خالی یا صفر به `5 × 10m` تبدیل می‌شود. تعداد خالی یا صفر برای اسلب، پله و محصول آماده همین معنا را ندارد و از قواعد همان نوع محصول پیروی می‌کند. اگر اطلاعات موجود چیدمان معتبری نسازد، ذخیره با توضیح روشن علت متوقف می‌شود و سیستم هرگز به تعداد یک برنمی‌گردد.
_Avoid_: تبدیل تعداد خالی یا صفر سنگ طولی به یک، تعمیم این فرمان به انواع دیگر محصول، اجبار کاربر به نوشتن صفر برای بهینه‌سازی، ادامه ذخیره پس از شکست بهینه‌سازی، ساختن یک قطعه `50m`، محاسبه باقی‌مانده `33cm × 50m`، تغییر رفتار تعدادهای صریح، یا پنهان‌کردن تعداد و طول محاسبه‌شده از کاربر
