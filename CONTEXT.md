...
The mandatory pre-Contract request through which a Partner Seller asks their assigned Price Responder for Sabalan's selling price for one or more exactly configured Contract Product rows. A row's immutable identity freezes the Partner Seller, Product, family, dimensions and other configuration, unit, material rate, calculation-policy and currency context, and every price-affecting component; Customer, Contract, quantity, Delivery, and retail price are checked when it is used but do not change that identity. Each approved row is reusable by the same Partner Seller for their own Customers without a quantity or Contract-count cap and is valid from its recorded approval instant until, but not including, exactly 48 hours later. Approval has an optional note; rejection, cancellation, reassignment, and supersession require a Persian reason. Bulk decisions preserve independent row outcomes and allow valid rows to succeed even when another selected row is stale, unauthorized, or expired.
_Avoid_: treating a catalog base price as approval, starting validity while the Partner Seller waits for a response, approving only a Product name while price-affecting configuration remains changeable, making Customer, Contract, quantity, Delivery, or retail price part of approval identity, mutating a decided row, copying one Product's price across unrelated rows through a bulk action, rolling back valid bulk outcomes because another selected row conflicts, losing per-row history inside a bulk action, accepting a Contract without server-side approval evidence, permitting Contract creation with an unapproved or expired row, applying one Partner Seller's approval to another Partner Seller, imposing a quantity or Contract-count cap, or requiring all rows in one request to receive the same outcome
...
The immutable approved all-in wholesale unit price at which Sabalan offers one exactly configured Product to one Partner Seller during the row's 48-hour validity window. Its calculation inputs and component breakdown are frozen as evidence, but the approved amount is the legal monetary truth and is never recomputed later; it remains required and snapshotted evidence for Partner Contract creation without limiting the retail price.
_Avoid_: exposing a catalog price before inquiry approval, treating a component recalculation as authority over the approved amount, treating the price as the Partner Seller's mandatory customer price, rewriting the approved price after response, losing it when the Partner chooses a different customer price, or expiring an already-created Contract when the inquiry later expires

**مصرف تأیید قیمت فروشنده همکار**:
The append-only evidence that one Partner Sale Case used one still-valid approved inquiry row for an exact matching Product configuration. Use never consumes or mutates the approval: the same Partner Seller may reuse it for their own Customers, while each successfully created case permanently snapshots the approval identity, integrity evidence, wholesale amount, and usage link without exposing them through the retail public projection.
_Avoid_: decrementing approval quantity, marking a reusable approval as consumed, relying on a mutable pointer instead of a case snapshot, accepting a mismatched Product configuration, or serializing wholesale evidence to the end Customer
...
The access boundary for Partner resale economics: the Partner Seller sees their exact inquiry configuration, final Sabalan price, validity, note or reason, state, and their own cases that used it, but not Sabalan's internal component rates, formulas, or calculation-policy inputs. The assigned Price Responder and explicitly authorized Sales audit users may inspect internal inquiry evidence and usage status without retail price, Customer economics, or margin; Sabalan Accounting uses only Sabalan's receivable, management reporting requires separate authority, and unrelated sellers see none of these prices.
_Avoid_: exposing internal price construction to the Partner Seller, exposing retail economics or Customer detail to the Price Responder, using resale value as an Accounting receivable, hiding the Partner's own approved price and usage from them, or granting audit or management-report visibility through an ordinary Sales permission
...
The purpose-specific internal commercial record inside a Partner Sale Case in which the Partner Seller is Sabalan's debtor and every Product amount comes from the valid approved Partner Seller Price Inquiry. It is not an ordinary Sales Contract, is the sole basis for Sabalan Accounting, receivable, and collection for the case, and is never exposed to the end Customer.
_Avoid_: modelling it as a second ordinary Sales Contract, naming the end Customer as Sabalan's debtor, using the Partner Customer Sale Price for Sabalan Accounting, exposing the record or inquiry price to the end Customer, or creating it without the linked Partner Customer Sales Contract
...
The legal person or organization linked one-to-one to a Partner Seller User and used as Sabalan's debtor identity. An Admin must record its legal name, identifiers, phone, and address before activating the Partner profile. It is distinct from every end Customer owned by the Partner Seller and remains the party on the Sabalan-to-Partner Sale Record.
_Avoid_: using a login account as sufficient accounting identity, treating an owned end Customer as the Partner Seller's own debtor account, activating a Partner profile without required commercial identity, or changing the debtor to the delivery recipient
...
The single canonical Product, configuration, quantity, processing, tool, and Delivery plan owned by a Partner Sale Case and shared by both records. Correction preserves stable row identity, reuses the committed frozen wholesale unit price for quantity-only change, requires fresh inquiry only for changed price-bearing configuration, replays source and remaining-stone children atomically in deterministic order, and cannot reduce quantity below effective reserved plus physically dispatched evidence; Logistics releases reservations first and dispatch decreases only through verified return and posted correction.
_Avoid_: reducing the Partner Product step to a simple catalog picker, exposing catalog or current rates during correction, duplicating or independently editing the graph, matching rows by catalog identity or position, requiring re-inquiry for quantity-only or Delivery-only change, repricing unchanged configuration after inquiry expiry, auto-releasing Logistics evidence inside commercial correction, accepting partial child replay, reducing below effective physical evidence, allowing the two records to drift, or mixing Partner-owned service revenue into Sabalan's Product evidence
...
The continued editing of an already-created Partner Sale Case using its snapshotted approval evidence. Inquiry expiry does not reprice or expire the Draft: an existing row may change quantity, retail price, customer payment plan, or Delivery and may be removed, while a new row or price-affecting configuration change requires a currently valid exact inquiry approval.
_Avoid_: repricing a Draft after expiry, imposing a quantity cap on an approved configuration, requiring re-inquiry for an unchanged price identity, adding a row from expired evidence, or treating Product similarity as configuration equality
...
The immutable schema-versioned customer-output evidence minted by the ordinary send-for-confirmation action and bound to one case revision, customer Contract number, normalized recipient, expiry, allowlisted content, and canonical hash. Resend for the same revision and recipient keeps that snapshot, a material customer-visible or recipient change invalidates the pending session and requires the same ordinary send action again, and customer-safe live Delivery status remains outside the approved commercial hash.
_Avoid_: minting on first public view, rebuilding from live relations, changing the recipient under an existing session, mixing live operational status into approved content, preserving a pending session after material change, or making the snapshot an editable Product Graph

**برنامه‌های پرداخت پرونده فروش همکار**:
The two independent payment plans in a Partner Sale Case. A corrected Partner-to-Customer plan is an effective-dated successor for future installments: prior receipt, allocation, delay, and reversal events remain immutable under their historical plan while the current retail balance includes every effective collection; the Partner-to-Sabalan plan alone enters Sabalan Accounting and changes only through its authorized audited workflow.
_Avoid_: copying one plan into both commercial relationships, rewriting or auto-reallocating historical Customer receipts onto a successor plan, losing prior collections from the current balance, treating a private Customer receipt as a Sabalan Accounting receipt, making the end Customer's payment schedule a Sabalan receivable, or allowing an unaudited Sabalan-term exception
...
The per-row lifecycle `در انتظار پاسخ`, `تأییدشده`, `ردشده`, `منقضی‌شده`, `جایگزین‌شده`, or `لغوشده`, with one linear predecessor-successor chain and no branching. A valid predecessor remains usable while its single successor is pending and becomes unusable as `جایگزین‌شده` only when that successor is approved; a rejected or cancelled successor does not affect it. Cancelling a submitted inquiry cancels only pending rows, while decided rows remain immutable. The parent summary is `پیش‌نویس` before submission, `ارسال‌شده` while every submitted row is pending, `پاسخ جزئی` while pending and decided rows coexist, and `تکمیل‌شده` when no pending row remains.
_Avoid_: manually setting a parent status that disagrees with its rows, branching successor history, revoking a valid predecessor merely because a successor was requested or rejected, cancelling or editing a decided row, revoking an approval merely because its parent was cancelled, overwriting an expired row during re-inquiry, requiring one outcome for every row, losing cancellation or succession evidence, or treating partial response as an error

**اعلان استعلام و تأیید مشتری فروشنده همکار**:
The separation between Partner price-inquiry notifications and the existing customer Contract confirmation channel. The customer channel preserves the ordinary confirmation flow and also emits a customer-safe cancellation or voiding notice when a snapshot was sent or verified; enqueue failure never rolls back the business transition, but enqueue, delivery outcome, and retry remain auditable without exposing internal reason or evidence.
_Avoid_: adding a Partner-specific confirmation page, selecting the Partner Commercial Account as recipient, changing a session's snapshotted recipient during resend, silently cancelling a customer-visible Contract, blocking cancellation on SMS delivery, exposing inquiry, internal-record, financial, or physical reason, or advancing Sabalan debt on OTP approval
...
The audited Partner profile lifecycle `در انتظار تکمیل`, `فعال`, `معلق`, or `خاتمه‌یافته`, distinct from the User account's global login state. Pending permits only onboarding-status access, Active permits owner-scoped Partner work, Suspended preserves login and read-only history while freezing mutation, and Terminated blocks login and new activity while preserving committed Accounting, correction, and fulfillment; suspension and termination require reasoned actor evidence.
_Avoid_: treating the profile as an ordinary Role or `isActive` toggle, restoring old internal grants through generic reactivation, allowing incomplete setup to sell, letting a responder decide suspended work, pausing or resetting approval expiry, deleting history at termination, stopping committed downstream work, permitting new mutations while suspended, or changing lifecycle without an audit reason

**پاسخ‌گویی پاسخ‌دهنده منتسب**:
The rule that ordinary inquiry response requires both edit-capable effective Sales access and the current assignment. A global ADMIN or a User with effective Sales workspace administration may see every open inquiry and answer its pending rows; the response command atomically appends a reasoned assignment takeover to the actual actor before recording the decision. Closed rows remain immutable, and reporting, assignment, audit inspection, Partner lifecycle, and response authority remain separate actions.
_Avoid_: an ownerless or anonymous price decision, response by an unrelated view-only User, overwriting an already decided row, changing assignment outside the response transaction, granting mutation through reporting access, or losing the prior and effective assignment evidence

**محدوده امکانات فروشنده همکار**:
The ordinary Sales and CRM experience constrained to the Partner Seller's own Customers, Potential Projects, inquiries, Drafts, Partner Sale Cases, correction requests, customer payment plans, customer confirmation SMS and output, fulfillment status, reporting, and حساب من با سبلان, including the full non-price Product-configuration experience. It excludes catalog or base-price administration, other sellers' records and comparisons, Sabalan discount policy, inquiry response, user or permission administration, Accounting mutation, and inventory, loading, or Delivery mutation.
_Avoid_: removing ordinary Product configuration merely because price is inquiry-controlled, granting cross-seller visibility, letting the Partner mutate authoritative downstream work, exposing internal prices, or replacing action-specific restrictions with one broad Role check
...
The fixed non-extensible set of ordinary owner-scoped Partner actions made available only while the Partner profile is Active; it is re-evaluated with lifecycle and resource ownership on every action rather than administered as many discretionary grants. Internal action grants, generic owner-assignment authority, workspace administration, or overlapping permissions never expand this bundle or remove its owner-only boundary.
_Avoid_: manually widening one Partner Seller's product capabilities, treating activation as a permanent authorization snapshot, extending the bundle through a generic grant, or allowing a suspended or terminated profile to retain mutation authority
...
The separation in which HR owns Partner identity, final gated activation, suspension, and termination; CRM owns duplicate identity and Customer ownership transfer; Sales owns commercial terms, Price Responder assignment, and commercial case management; Accounting owns Partner credit and Partner-to-Sabalan payment terms; and Logistics owns fulfillment of the internal Sabalan record. Admin may perform each explicit action with its domain meaning and audit evidence but cannot collapse these authorities or bypass their invariants.
_Avoid_: letting one workspace silently complete another workspace's gate, granting HR pricing authority, granting Sales credit-term authority, granting Accounting retail mutation, granting Logistics commercial visibility, or treating Admin as an unaudited invariant bypass

**انحصار persona فروشنده همکار**:
The initial-version rule that an Active Partner Seller User cannot simultaneously hold an internal operational persona or active Sales, CRM, HR, Accounting, Logistics, or Admin authority. Conversion may be abandoned back to the internal persona only during restricted onboarding and before any Partner-owned Customer, Inquiry, or Case exists; after activation or first Partner evidence, termination preserves that User and a later internal hire receives a separately linked audited identity.
_Avoid_: combining external Partner ownership with internal decision authority, using generic User reactivation as conversion, restoring the same internal account after Partner evidence exists, activating before internal work is finished, cancelled, or reassigned, deleting grant history, silently transferring historical Customers or Projects, or retaining internal grants

**تعیین تکلیف سابقه فروشنده تبدیل‌شده**:
The pre-activation transfer of current operational responsibility for every historical internal Contract that may later require correction to an active internal Seller, without changing creator or realized-sales credit. A Contract that lacks a valid internal successor blocks Partner activation and never becomes visible or correctable in the Partner workspace.
_Avoid_: leaving future correction assigned to a Partner persona, rewriting historical credit, granting Partner access to an internal Contract, inferring a successor, or activating while operational responsibility is unresolved
...
The three-part prerequisite for an authorized actor to move a Partner profile from `در انتظار تکمیل` to `فعال`: HR-authorized identity and Commercial Account verification, configured commercial and Partner-to-Sabalan payment terms, and an active assigned Price Responder from Sales. An actor with the explicit Partner HR action permission may create the User, record and verify legal identity and contact evidence, suspend or terminate for identity, security, or end-of-relationship reasons, and perform final activation only after every gate passes. HR authority alone never grants pricing, commercial-term mutation, Sales reporting, or Price Responder assignment; a global Admin may complete all gates with each actual action audited.
_Avoid_: granting Partner administration from an HR Manager title alone, activating with an incomplete gate, letting HR set prices or Sales assignments without separate permission, letting Sales silently self-verify identity, or collapsing the distinct actions into an unaudited Admin override

**بسته فعال‌سازی فروشنده همکار**:
The immutable, versioned evidence package that binds one Partner Seller candidate to their current identity, commercial and credit terms, Price Responder, internal-persona disposition, release cohort, deployed release readiness, authorization decisions, and exact revisions before activation. The server compiles and revalidates the package; a browser or Admin never assembles gate booleans or evidence identifiers as authority.
_Avoid_: treating activation as a role toggle, trusting client-supplied readiness, reusing a package across Users, cohorts, releases, or revisions, collapsing owner decisions into one Admin assertion, or mutating a package after issuance

**انتشار آمادگی فروش همکار**:
The append-only binding of one independently verified `GO` release package to the deployed Partner release, schema, immutable image set, recovery and telemetry evidence, and accountable approvals. It permits later cohort preparation while every operational pause remains fail-closed, and is neither deployment authority nor Partner activation by itself.
...
The three immutable, unique, never-reused searchable business identifiers allocated only when final case creation succeeds: one Partner Sale Case number, one customer Contract number used in PDF, SMS and public confirmation, and one internal Sabalan record number used by Accounting. The end Customer searches and sees only the customer Contract number; the owning Partner may search its case and customer Contract numbers, Accounting may cross-search all three through its purpose-bound view, and scoped management or Admin may search all three without widening returned projections.
_Avoid_: requiring gapless allocation, exposing the case or internal number to the end Customer, letting a Partner search the internal number, forcing Accounting to search by only one identifier, widening a projection because an identifier matched, reusing a cancelled number, or allocating a visible number for a failed case

**عرضه افزایشی فروشنده همکار**:
The prospective-only, named-cohort rollout of the Partner Seller capability through dedicated Partner profile and release eligibility, never through inherited generic feature permission. Enrollment pause stops new cohort entry and activation without disturbing active Partners; a separate operational emergency pause freezes all uncommitted Partner and responder mutation while preserving read, support cancellation, internal remediation, and committed Accounting and fulfillment.
_Avoid_: using Role or workspace inheritance as cohort membership, conflating enrollment pause with an operational freeze, permitting Draft edit or commitment during emergency pause, turning off downstream obligations, auto-converting existing Users or Contracts, inferring Partner history from creator, ownership, names, prices, or JSON, backfilling invented wholesale evidence, or restricting ordinary Sales through Partner policies
