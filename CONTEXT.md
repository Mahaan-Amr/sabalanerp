...
Every tool, finishing, and operation group belongs to one stable contract-product row through an independent identity. Duplicate internal identities may be separated automatically only when operation, quantity, rate, and side intent remain unambiguous and unchanged; contradictory ownership blocks contract submission beside the affected row with `ساختار عملیات این محصول قابل تشخیص نیست؛ ابزارها و پرداخت‌ها را بازبینی و دوباره ذخیره کنید`.
_Avoid_: treating catalog identity or array position as operation ownership, changing commercial values while repairing identity, or accepting contradictory operation ownership as a valid contract graph
