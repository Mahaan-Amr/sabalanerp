...
وقتی عرض و طول کل محصول طولی صریح هستند اما تعداد صفر است، صفر فرمان بهینه‌سازی تعداد است و تعداد واقعی صفر محسوب نمی‌شود. سیستم بیشترین تعداد نوار فیزیکی را که با عرض درخواستی و خوراک اره فعال در عرض سنگ اصلی جا می‌شود محاسبه می‌کند، طول کل را بین آن نوارها تقسیم می‌کند، و تعداد و طول هر قطعه محاسبه‌شده را پیش از ذخیره نشان می‌دهد. ردیف ذخیره‌شده تعداد و طول فیزیکی محاسبه‌شده را نگه می‌دارد، در حالی که متر طول کل و متر مربع تجاری ثابت می‌مانند؛ برای نمونه `40cm` منبع، `7cm` عرض، `50m` طول کل و تعداد صفر به `5 × 10m` تبدیل می‌شود.
_Avoid_: تبدیل تعداد صفر به یک، ساختن یک قطعه `50m`، محاسبه باقی‌مانده `33cm × 50m`، تغییر رفتار تعدادهای صریح، یا پنهان‌کردن تعداد و طول محاسبه‌شده از کاربر
