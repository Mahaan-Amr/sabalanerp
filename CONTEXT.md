...
The Finance-owned review of required collateral and obligations that begins only after Candidate selection and Accepted Offer. Through separately permissioned task-scoped work in the Accounting destination, one Finance recorder registers receipt, custody details, and scans, while a distinct Finance verifier approves or returns the evidence and clearance required before Hire Conversion. Ordinary recording and verification require distinct Users; an Accounting workspace administrator or global ADMIN who currently holds both collateral-custody permissions may perform the two actions sequentially, and the second action records the actual actor, time, self-verification state, and `استفاده از اختیار مدیریتی` label without a manual explanation. Candidates who are rejected, withdrawn, or still under assessment are not asked to provide collateral.
_Avoid_: collecting collateral from every applicant, ordinary recorder self-verification, granting managerial self-verification without both permissions and Accounting administration, merging the two actions, omitting the override audit label, performing Finance work through general Human Resources access, converting to Personnel before clearance, or treating offer acceptance as collateral approval
...
The Finance-recorded handover of a collateral original after a pre-activation cancellation or another authorized release, identifying the recipient and return date and preserving a description plus uploaded proof of handover. The return controls exist only when an original is held and a return is required, and a distinct Finance verifier normally confirms the completed handover. An Accounting workspace administrator or global ADMIN holding both collateral-custody permissions may record and then confirm the return sequentially; confirmation remains a separate action and carries the audited managerial self-verification label.
_Avoid_: showing return fields during ordinary receipt, recording return without a recipient or proof file, confusing original return with rejection for correction, ordinary self-confirmation, granting the exception without both permissions and Accounting administration, merging record and confirmation, or losing the instrument's custody history
