...
**بازه سنجش عملکرد**:
فاصله صریح «از تاریخ» تا «تا تاریخ» در یک ارزیابی عملکرد پرسنل است که تعیین می‌کند قضاوت سرپرست و شواهد متعلق به چه زمانی‌اند. این بازه فقط درون یک Employment Relationship قرار می‌گیرد و از زمان ارسال، بررسی و اعتبار سطح جاری مستقل است.
_Avoid_: ارزیابی بدون مرز زمانی، سرایت بازه به استخدام مجدد، یا یکی‌گرفتن بازه سنجش با مهلت فرایند و بازه گزارش
...
پرونده ارزیابی یک Personnel در دقیقاً یک Employment Relationship و یک بازه سنجش عملکرد است که همه بخش‌های مأموریت شغلی مؤثر آن بازه را گرد هم می‌آورد و در نهایت حداکثر یک نتیجه مصوب دارد. ارزیابی می‌تواند مستقل یا عضو یک نوبت ارزیابی باشد؛ رابطه Planned هنوز عملکردی برای ارزیابی ندارد، مدت Suspended به‌خودی‌خود بخش عملکرد تولید نمی‌کند، و پایان رابطه استخدامی بازه را کوتاه می‌کند اما تکمیل ارزیابی کارکرد پیشین را متوقف یا باطل نمی‌کند. استخدام مجدد همیشه ارزیابی تازه‌ای می‌سازد و غیرفعال یا بایگانی‌شدن Personnel تاریخچه را حذف نمی‌کند. ارزیابی برای Personnel بدون User account نیز به همان شکل معتبر است، هرچند ارسال سرپرست به User واجد اختیار نیاز دارد.
_Avoid_: ارزیابی User account، ارزیابی کارکرد برای رابطه Planned یا مدت Suspended، باطل‌کردن کارکرد گذشته پس از پایان استخدام، نتیجه مستقل و متعارض برای هر مأموریت شغلی، الزام عضویت در نوبت، ترکیب دو دوره استخدامی در یک ارزیابی، انتقال نتیجه یا تصویر ثابت به استخدام مجدد، حذف تاریخچه Personnel غیرفعال یا بایگانی‌شده، یا حذف Personnel فاقد حساب کاربری
...
رابطه تاریخی و مؤثر در بازه میان یک بخش ارزیابی مأموریت شغلی و دقیقاً یک Personnel سرپرست است که از دوره مسئولیت سرپرست ثبت‌شده برای همان مأموریت به دست می‌آید. خط گزارش‌دهی Position نامزد معتبر را تعیین می‌کند اما خود به‌تنهایی اختیار ارزیابی نمی‌سازد؛ یک نامزد واجد شرایط هنگام ثبت مسئولیت قابل انتخاب خودکار است، چند نامزد به انتخاب صریح نیاز دارند و نبود انتخاب مانع ساختار سازمانی ارزیابی است. تغییر مؤثر سرپرست یا خط گزارش‌دهی، بدون تغییر ساختگی مأموریت Personnel، بخش تازه‌ای می‌سازد و از تاریخ اثر به انتخاب و اعتبارسنجی مسئول تازه نیاز دارد. مأموریت Primary، Secondary یا Acting سرپرست فقط در مدت Employment Relationship فعال او می‌تواند این مسئولیت را بسازد؛ دوره Planned یا Suspended مسئولیت فعال ایجاد نمی‌کند. هر سرپرست فقط بخش‌های مأموریتی متعلق به مسئولیت خودش را قضاوت می‌کند، خودارزیابی و مسئولیت هم‌زمان چند سرپرست برای یک بخش مجاز نیست و دسترسی فضای کاری، مجوز عمومی، نقش سیستمی یا ساختار سازمانی امروز هیچ مسئولیت تاریخی را ایجاد یا بازسازی نمی‌کند.
_Avoid_: استنتاج سرپرست تاریخی از ساختار امروز، ادامه بی‌صدای مسئولیت پس از تغییر خط گزارش‌دهی، شکستن مأموریت برای نمایش صرف تغییر سرپرست، انتخاب دلخواه میان چند متصدی، مسئول دانستن سرپرست برنامه‌ریزی‌شده یا معلق، چند ارسال رسمی هم‌ارز برای یک بخش، یکی‌گرفتن دسترسی با مسئولیت، اجازه خودارزیابی، یا گسترش مسئولیت یک سرپرست به بخش مأموریتی سرپرست دیگر

**اختیار ارسال ارزیابی عملکرد**:
اجازه زنده و محدود یک User برای ثبت ارسال ارزیابی عملکرد سرپرست در بخش‌هایی است که Personnel مرتبط با همان User مسئولیت ارزیابی عملکرد آن‌ها را دارد. این اختیار فقط با اجتماع مسئولیت ثبت‌شده همان بخش، Employment Relationship فعال سرپرست در زمان ارسال، User فعال و مرتبط با همان Personnel، و مجوز عملیاتی مؤثر همان ارزیابی برقرار است؛ انتقال بعدی سرپرست به Position دیگر مسئولیت تاریخی او را از بین نمی‌برد، اما پایان یا تعلیق استخدام اختیار ارسال را متوقف می‌کند. دسترسی فضای منابع انسانی، مجوز عمومی، ADMIN بودن یا Human Resources Broad-Manager Override به‌تنهایی شخص را به سرپرست ارسال‌کننده تبدیل نمی‌کند. نبود حساب فعال یا مجوز، مسئولیت تاریخی را تغییر نمی‌دهد و اجازه ثبت جانشین، تفویض پسینی یا ارسال نیابتی به منابع انسانی نمی‌دهد؛ فقط جانشینی مؤثر و ثبت‌شده در خود بازه، بخش مسئولیت خودش را می‌سازد.
_Avoid_: اختیار از workspace access، اختیار از نقش یا عنوان شغلی، ارسال در زمان استخدام پایان‌یافته یا معلق، ارسال برای Personnel نامرتبط، تفویض اختیار فرم پس از دوره، ثبت نیابتی بی‌نشان، تبدیل مدیر منابع انسانی یا ADMIN به سرپرست تاریخی، یا تغییر مسئولیت گذشته به‌علت وضعیت دسترسی امروز

**الگوی ارزیابی شغل**:
معیارها و انتظارهای استاندارد و نسخه‌داری که میان افراد انجام‌دهنده یک Job مشترک است.
_Avoid_: زمینه عملیاتی خاص Position، یک الگوی عمومی برای همه شرکت، یا تغییر ارزیابی باز با تغییر الگو

**افزوده ارزیابی جایگاه سازمانی**:
معیارهای نسخه‌دار مربوط به خط گزارش‌دهی، شیفت، محل یا مسئولیت خاص یک Position که به الگوی ارزیابی شغل آن افزوده می‌شود.
_Avoid_: تکرار کامل الگوی شغل، جایگزینی پنهانی معیارهای مشترک، یا معیار شخصی و جانبدارانه
...
**معیار قضاوتی عملکرد**:
تنها معیار امتیازآور ارزیابی عملکرد است که سرپرست برای آن یکی از پنج درجه توصیفیِ متکی به شاهد را انتخاب می‌کند. شاخص عملیاتی، متن توضیحی و کنترل بله/خیر به‌خودی‌خود معیار امتیازآور نیستند.
_Avoid_: امتیاز خودکار از KPI، عدد آزاد سرپرست، تبدیل متن به امتیاز، یا پنهان‌کردن یک کنترل الزامی داخل امتیاز مرکب
...
**درجه توصیفی عملکرد**:
یکی از پنج مرتبه معنایی مشترک `به‌طور جدی پایین‌تر از انتظار`، `پایین‌تر از انتظار`، `مطابق انتظار`، `بالاتر از انتظار` و `به‌طور استثنایی بالاتر از انتظار` است که برای هر معیار توضیح اختصاصی دارد و به مقدار محاسباتی ثابت یک تا پنج نگاشت می‌شود. نامرتبط بودن و نبود شاهد کافی خارج از این مقیاس‌اند.
_Avoid_: امتیاز آزاد یا کسری، معنای متفاوت یک درجه میان معیارها، یکی‌گرفتن نامرتبط با صفر، یا نمایش نگاشت عددی به‌عنوان قضاوت مستقل

**امتیاز نرمال‌شده عملکرد**:
مقدار محرمانه صفر تا صد است که درجه یک تا پنج را با فرمول خطی `((درجه − ۱) ÷ ۴) × ۱۰۰` به نقاط صفر، بیست‌وپنج، پنجاه، هفتادوپنج و صد تبدیل و مبنای ترکیب معیار، دسته، بخش، نتیجه و تاریخچه زنده می‌کند. صفر فقط پایین‌ترین قضاوت صریح است و نبود پاسخ، نامرتبط‌بودن یا نبود شاهد نیست.
_Avoid_: مقیاس متفاوت در یک واحد یا شغل، گردکردن میانی، امتیاز صفر برای داده مفقود، یا نمایش امتیاز به Personnel و User عادی
...
سیاست سازمانی، نسخه‌دار و تغییرناپذیری است که مقدار دقیق امتیاز محرمانه را به پنج سطح فارسی نگاشت می‌کند: `نیازمند بهبود فوری` برای صفر تا کمتر از بیست با معنای «عملکرد مصوب به‌طور جدی پایین‌تر از انتظارهای نقش بوده است»، `نیازمند بهبود` برای بیست تا کمتر از چهل با معنای «عملکرد مصوب در بخشی از انتظارهای نقش نیازمند بهبود است»، `مطابق انتظار` برای چهل تا کمتر از شصت با معنای «عملکرد مصوب با انتظارهای نقش هم‌خوان است»، `فراتر از انتظار` برای شصت تا کمتر از هشتاد با معنای «عملکرد مصوب در مجموع فراتر از انتظارهای نقش بوده است» و `عملکرد برجسته` برای هشتاد تا صد با معنای «عملکرد مصوب به‌شکلی پایدار و برجسته فراتر از انتظارهای نقش بوده است». مرزها بر مقدار دقیق محاسباتی اعمال می‌شوند، نه مقدار گرد‌شده نمایشی. نام، معنا یا آستانه تازه فقط در نسخه‌ای با تاریخ اثر منتشر می‌شود و پس از پیش‌نمایش و تأیید صریح، همه سطح‌های جاری را از نتایج هنوز معتبر باز‌محاسبه می‌کند؛ سطح تاریخی هر نتیجه و نسخه سیاست پیشین تغییر نمی‌کند.
...
**سطح عملکرد جاری**:
وضعیت زنده و فارسی Personnel در Employment Relationship فعال یا معلق است که پس از هر نتیجه مصوب معتبر، انقضا، تعلیق اثر نتیجه نامعتبر یا فعال‌شدن سیاست تازه بی‌درنگ از سابقه نتایج معتبر محاسبه می‌شود و می‌تواند بالا یا پایین برود. نتیجه معتبرِ در حال اصلاح تا پذیرش نتیجه جانشین مؤثر می‌ماند؛ پذیرش جانشین، تعلیق اثر و باز‌محاسبه یک تغییر اتمیک‌اند و هیچ حالت میانی به Personnel افشا نمی‌شود. انقضای یک نتیجه، وزن نتایج معتبر باقی‌مانده را دوباره نرمال می‌کند؛ انقضای همه نتایج مؤثر، سطح پیشین را با `نیازمند ارزیابی جدید` جایگزین می‌کند و خطای فنی هرگز اعتبار منقضی را تمدید نمی‌کند. خواندن سطح نیز اعتبار ورودی‌ها را کنترل می‌کند و اگر محاسبه معتبر موقتاً ممکن نباشد، به‌جای هر وضعیت کسب‌وکاری پیام عمومی `خلاصه عملکرد موقتاً در دسترس نیست` نشان می‌دهد و رخداد قابل‌تلاش‌مجدد عملیات منابع انسانی را می‌سازد. تعلیق استخدام به‌تنهایی سطح را پاک نمی‌کند و نتیجه طبق تاریخ عادی منقضی می‌شود؛ رابطه Planned سطح جاری ندارد، رابطه Ended فقط تاریخچه مجاز دارد و استخدام مجدد از `ارزیابی‌نشده` آغاز می‌شود. User مرتبط فقط Badge سطح، معنای کوتاه استاندارد، پایان تازه‌ترین بازه سنجش مؤثر و تاریخ بازبینی بعدی محاسبه را می‌بیند؛ این تاریخ زودترین انقضای نتیجه مؤثر یا اثر سیاست سطح‌بندی زمان‌بندی‌شده است و ثابت‌ماندن سطح تا آن تاریخ را تضمین نمی‌کند. امتیاز خام، رتبه، تعداد نتایج، فاصله تا سطح بعدی و جزئیات محرمانه به او نمایش داده نمی‌شود.
_Avoid_: محاسبه از بازه گزارش یا پایان نوبت، سطحی که فقط بالا می‌رود و سابقه خدمت را به‌جای عملکرد می‌سنجد، تغییر از داده یا ارسال تأییدنشده، Badge میانی اصلاح، تمدید نتیجه منقضی یا نمایش سطح stale هنگام خطا، تبدیل خطای فنی به سطح یک، ارزیابی‌نشده یا نیازمند ارزیابی جدید، پاک‌کردن سطح صرفاً با تعلیق استخدام، انتقال سطح به استخدام مجدد، نگه‌داشتن سطح جاری پس از پایان استخدام، نمایش نوار پیشرفت، امتیاز خام یا تعداد نتایج، یا تغییر خودکار جبران خدمت و استخدام
...
افشای مستقل سطح جاری و وضعیت‌های ارزیابی‌نشده یا نیازمند ارزیابی جدید در فهرست اصلی Personnel، نمای محدود Personnel تحت مسئولیت Supervisor و گزارش مدیریتی مجاز است. نداشتن مجوز، نشان و هر جای‌نگهدار یا نشانه قفل را کاملاً حذف می‌کند و این سطح در فهرست‌های نامرتبط، جست‌وجوی عمومی User، گفت‌وگو، وظایف بین‌واحدی یا انتخاب‌گرهای Personnel ظاهر نمی‌شود.
_Avoid_: استنتاج مجوز از دسترسی عمومی فهرست، نمایش قفل برای داده محرمانه، افشا در انتخاب‌گرها یا سطوح همکاری، یا یکی‌گرفتن نشان سطح با اجازه مشاهده امتیاز و پرونده
