...
The one active internal Sabalan seller assigned by an Admin to answer and approve one Partner Seller's price inquiries. Assignment belongs to the inquiry as a whole; a reasoned reassignment transfers only its pending rows, immediately ends the former responder's mutation authority, and preserves the effective assignment and eligibility with every historical decision. Price-response authority never makes the responder the Contract Creator, Responsible Seller, Customer owner, or recipient of sales-performance credit.
...
The rule that every inquiry action requires both its explicit action authority and its current resource relationship, including owner scope for the Partner Seller and effective assignment for the active eligible Price Responder. Even an Admin may answer only after an audited eligible assignment, including assignment to themselves; reporting, assignment, reassignment, audit inspection, Partner lifecycle, and response authority remain separate actions, and no title, override, or open queue creates anonymous response authority.
_Avoid_: an ownerless price decision, direct Admin response without assignment, response by an unrelated seller, inferring authority from a broad Sales Role, granting mutation through reporting access, or losing the assignment effective at decision time
...
The explicit scope of authorized Partner management: an Admin has company-wide oversight, while `PARTNER_SALES_MANAGE` defaults to the holder's department and reaches the whole company only through an explicit `COMPANY` scope. A `MANAGER` title, Sales workspace administration, Customer visibility, or Price Responder assignment alone never grants Partner resale economics, cross-Partner comparison, or management reporting.
