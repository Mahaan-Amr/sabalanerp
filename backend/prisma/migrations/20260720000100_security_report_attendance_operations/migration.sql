...
ALTER TABLE "security_instant_report_categories"
  ADD COLUMN "useReportTypes" BOOLEAN NOT NULL DEFAULT true,
  ADD COLUMN "useRelatedPersonnel" BOOLEAN NOT NULL DEFAULT true;
...
ALTER TABLE "security_shift_log_entries"
  ...
  ADD COLUMN "reportTypeNameSnapshot" TEXT;
...
CREATE INDEX "security_shift_log_entries_categoryId_idx" ON "security_shift_log_entries"("categoryId");
ALTER TABLE "security_shift_log_entries"
...
ALTER TABLE "attendance_records" DROP CONSTRAINT "attendance_records_securityPersonnelId_fkey";
...
SELECT
  'migrated-' || record."id",
  record."id",
  date_trunc('day', record."date") + record."entryTime"::time - interval '3 hours 30 minutes',
  CASE
    WHEN record."exitTime" IS NULL THEN NULL
    WHEN record."exitTime"::time < record."entryTime"::time
      THEN date_trunc('day', record."date") + interval '1 day' + record."exitTime"::time - interval '3 hours 30 minutes'
    ELSE date_trunc('day', record."date") + record."exitTime"::time - interval '3 hours 30 minutes'
  END,
  personnel."userId",
...
CREATE INDEX "attendance_intervals_attendanceRecordId_enteredAt_idx" ON "attendance_intervals"("attendanceRecordId", "enteredAt");
CREATE INDEX "attendance_intervals_status_enteredAt_idx" ON "attendance_intervals"("status", "enteredAt");
...
CREATE TABLE "attendance_interval_audits" (
  "id" TEXT NOT NULL,
...
CREATE INDEX "attendance_interval_audits_intervalId_createdAt_idx" ON "attendance_interval_audits"("intervalId", "createdAt");
...
ALTER TABLE "exception_requests" ADD COLUMN "personnelId" TEXT;
ALTER TABLE "exception_requests" ALTER COLUMN "employeeId" DROP NOT NULL;
ALTER TABLE "exception_requests" DROP CONSTRAINT "exception_requests_employeeId_fkey";
...
CREATE INDEX "exception_requests_personnelId_status_startDate_idx" ON "exception_requests"("personnelId", "status", "startDate");
ALTER TABLE "exception_requests"
...
ALTER TABLE "mission_assignments" DROP CONSTRAINT "mission_assignments_employeeId_fkey";
ALTER TABLE "mission_assignments"
...
CREATE INDEX "mission_assignments_personnelId_status_startDate_idx" ON "mission_assignments"("personnelId", "status", "startDate");
ALTER TABLE "mission_assignments"
  ADD CONSTRAINT "mission_assignments_personnelId_fkey"
  FOREIGN KEY ("personnelId") REFERENCES "personnel"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

CREATE TABLE "security_attendance_workflow_events" (
  "id" TEXT NOT NULL,
  "exceptionId" TEXT,
  "missionId" TEXT,
  "action" TEXT NOT NULL,
...
CREATE INDEX "security_attendance_workflow_events_exceptionId_createdAt_idx" ON "security_attendance_workflow_events"("exceptionId", "createdAt");
CREATE INDEX "security_attendance_workflow_events_missionId_createdAt_idx" ON "security_attendance_workflow_events"("missionId", "createdAt");
