-- Permit an immutable approved-pricing row to bind to a legacy contract item
-- whose relational productRowId was never populated, but only when the new
-- version carries the exact deterministic migration assignment and the
-- referenced graph-migration audit exists. Existing non-null identities remain
-- strict and can never be overridden by compatibility evidence.
