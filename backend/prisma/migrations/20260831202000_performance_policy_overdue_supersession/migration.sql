DROP INDEX IF EXISTS "performance_policy_activation_previews_policyVersionId_generatedAt_idx";
CREATE UNIQUE INDEX "performance_policy_activation_previews_policy_key"
  ON "performance_policy_activation_previews"("policyVersionId");
...
BEGIN
  ...
  IF TG_TABLE_NAME = 'performance_policy_versions' AND old_state = 'DRAFT' AND new_state = 'SCHEDULED' THEN
    ...
    END IF;
  END IF;
  IF old_state = 'SCHEDULED' AND new_state = 'ACTIVE'
    AND NEW."effectiveFrom" > CURRENT_TIMESTAMP THEN
