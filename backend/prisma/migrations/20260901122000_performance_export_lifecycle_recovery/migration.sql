...
BEGIN
  ...
  IF TG_TABLE_NAME = 'performance_export_receipts' AND NOT (
    (old_state = 'QUEUED' AND new_state IN ('RUNNING','FAILED','DELETED')) OR
    (old_state = 'RUNNING' AND new_state IN ('READY','FAILED','QUEUED','DELETED')) OR
    (old_state = 'READY' AND new_state IN ('DELIVERING','EXPIRED','DELETED')) OR
    (old_state = 'DELIVERING' AND new_state IN ('READY','DOWNLOADED','DELETED')) OR
    (old_state IN ('DOWNLOADED','FAILED','EXPIRED') AND new_state = 'DELETED')
