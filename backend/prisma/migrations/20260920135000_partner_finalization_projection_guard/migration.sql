-- Case revisions remain append-only except for the one atomic enrichment that
-- attaches the final linked projections. Commercial evidence and its integrity
-- hash cannot change, and the enrichment cannot be repeated.
DROP TRIGGER partner_append_only ON partner_case_revisions;
...
CREATE TRIGGER partner_append_only BEFORE UPDATE OR DELETE ON partner_case_revisions FOR EACH ROW
  EXECUTE FUNCTION partner_finalize_revision_projection();
