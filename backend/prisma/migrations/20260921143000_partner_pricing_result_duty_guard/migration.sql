-- A completed Partner pricing inquiry returns to its owning Partner through a
-- narrowly scoped Partner-domain duty. Keep the general Partner persona guard
-- fail-closed while allowing only that exact, ownership-verified assignment.
...
DECLARE
  ...
  partner_pricing_result_duty boolean := false;
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'sales_contract_edit_sessions' THEN
    ...
    IF NOT partner_technical_session AND TG_OP = 'UPDATE'
       AND to_jsonb(OLD)->>'purpose' = 'PARTNER_TECHNICAL'
       AND to_jsonb(OLD)->>'contractId' IS NULL
       AND to_jsonb(NEW)->>'contractId' IS NOT NULL THEN
      SELECT EXISTS (
        SELECT 1 FROM sales_contracts
        WHERE id = to_jsonb(NEW)->>'contractId'
          AND "partnerKind" = 'PARTNER_CUSTOMER'
          AND "responsibleSellerId" = subject_id
      ) INTO partner_technical_session;
    END IF;
  ELSIF TG_TABLE_NAME = 'hr_duties' THEN
    ...
    IF NEW."sourceType" = 'PARTNER_PRICING'
       AND NEW."sourceActionCode" = 'PARTNER_PRICE_RESULT'
       AND NEW."destinationWorkspaceCode" = 'SALES' THEN
      SELECT EXISTS (
        SELECT 1
        FROM partner_inquiries inquiry
        JOIN partner_profiles profile ON profile.id = inquiry."profileId"
        WHERE inquiry.id = NEW."sourceId"
          AND profile."userId" = subject_id
          AND profile.state::text = 'ACTIVE'
      ) INTO partner_pricing_result_duty;
    END IF;
  ...
  IF irreversible_at IS NOT NULL THEN
    ...
    IF partner_pricing_result_duty AND profile_state = 'ACTIVE' THEN RETURN NEW; END IF;
