...
const liveEnrollmentReady = () => process.env.BIOMETRIC_CONNECTOR_MODE === 'physical';
...
router.get(
  ...
  async (_req, res) => {
    ...
    const [latest, failedConnectorCommands, unhealthyConfirmations, expiredAuthorizations, unhealthyProjections, auditExceptions, activeOutages, smsAttention] = await Promise.all([
    ...
    const platform = { connector: failedConnectorCommands, confirmation: unhealthyConfirmations, authorization: expiredAuthorizations,
      projection: unhealthyProjections, auditIntegrity: auditExceptions, outage: activeOutages, sms: smsAttention };
    const diagnostics = physical ? { mode: 'PHYSICAL', availability: (latest?.resultSummary as any)?.availability || 'UNKNOWN', liveEnrollmentEnabled: liveEnrollmentReady(),
...
router.post('/diagnostics/result', ...access, async (req: AuthRequest, res) => {
  try {
    ...
    res.json({ success: true, data: { mode: 'PHYSICAL', availability: claimed.response.result.availability, liveEnrollmentEnabled: liveEnrollmentReady(),
