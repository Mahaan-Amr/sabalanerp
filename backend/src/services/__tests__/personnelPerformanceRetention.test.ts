...
assert.equal(evaluatePerformanceRetention({ ...base, dependencies: [{ closedAt: null, kind: 'DISPUTE' }] }).state, 'DEPENDENCY_OPEN');
assert.equal(evaluatePerformanceRetention({ ...base, dependencies: [{ closedAt: new Date('2026-01-01Z'), kind: 'CORRECTION' }] }).deleteAfter?.toISOString(), '2033-01-01T00:00:00.000Z');
assert.equal(evaluatePerformanceRetention({ ...base, dependencies: [{ closedAt: new Date('2026-01-01Z'), kind: 'PRIVACY_ACCESS' }] }).deleteAfter?.toISOString(), '2026-04-01T00:00:00.000Z', 'a closed access request preserves its scope for 90 days without restarting seven years');
