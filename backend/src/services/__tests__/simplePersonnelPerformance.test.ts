...
assert.equal(result.score, '75.500000000000000000');
assert.equal(result.level, 'CAPABLE');
assert.deepEqual(result.indicators.map(({ score }) => score), ['67.500000000000000000', '87.500000000000000000']);
...
assert.equal(almostOutstanding.level, 'CAPABLE');
...
assert.equal(precisionBoundary.level, 'CAPABLE');
...
assert.equal(canEvaluatePersonnel({ hasEvaluateAll: false, hasEvaluateDirectReports: false, hasEnterEvidence: true, isResponsibleSupervisor: false }), 'HR_PROCESSOR');
...
assert.equal(validateSimpleEvaluationDate('2026-08-20', new Date('2026-09-12T12:00:00.000Z')).valid, true,
  'dates in the same six-month Persian period remain valid');
