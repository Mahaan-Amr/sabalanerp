...
const storedCommandResult = (value: unknown, intentFingerprint: string) => resultJson({ intentFingerprint, value });
const commandResult = async (tx: Tx, scope: DispatchDocumentCommandScope, scopeId: string, idempotencyKey: string,
  command: string, intentFingerprint: string) => {
  ...
  if (row.command !== command || envelope.intentFingerprint !== intentFingerprint) {
    throw new DispatchDocumentConflictError('The idempotency key is already bound to another dispatch command intent.');
  }
  if (row.status === 'SUCCEEDED') return envelope.value;
...
export class PrismaDispatchDocumentRepository implements DispatchDocumentRepository {
  ...
  async recordRetrieval(input: Parameters<DispatchDocumentRepository['recordRetrieval']>[0]) {
    await runSerializableDispatchOperation(this.prisma, async (tx) => {
      ...
      if (prior) {
        const envelope = record(prior.result);
        if (prior.command !== 'RETRIEVE' || envelope.intentFingerprint !== input.intentFingerprint) {
          throw new DispatchDocumentConflictError('The retrieval attempt identity is already bound to another artifact intent.');
        }
        return;
      }
      const at = new Date();
      await tx.dispatchDocumentCommandResult.create({ data: { waybillId: input.waybillId, scope: 'WAYBILL', scopeId: input.waybillId,
        idempotencyKey, command: 'RETRIEVE', status: input.status, result: storedCommandResult({
          artifactId: input.artifact?.id ?? null, requestedArtifactId: input.requestedArtifactId,
          byteLength: input.artifact?.byteLength ?? null, sha256: input.artifact?.sha256 ?? null }, input.intentFingerprint),
      ...
      await appendAudit(tx, { aggregateType: 'ACCOUNTING_DISPATCH_WAYBILL', aggregateId: input.waybillId,
        ...
        payload: { artifactId: input.artifact?.id ?? null, requestedArtifactId: input.requestedArtifactId,
          artifactKind: input.artifact?.kind ?? null, sha256: input.artifact?.sha256 ?? null,
          failureCode: input.failureCode ?? null, correlationId: input.correlationId }, actorId: input.actorId, at });
    });
  }
  async isRequiredArtifactMetadataMissing(input: { waybillId: string; kinds: DispatchDocumentKind[] }) {
  ...
  async recordPrintHandoff(input: Parameters<DispatchDocumentRepository['recordPrintHandoff']>[0]) {
    await runSerializableDispatchOperation(this.prisma, async (tx) => {
      ...
      if (input.status === 'SUCCEEDED') {
        ...
        if (priorSuccess) {
          if (priorSuccess.command !== 'PRINT_HANDOFF') throw new DispatchDocumentConflictError('idempotency key was used for another command');
          const envelope = record(priorSuccess.result);
          if (envelope.intentFingerprint !== input.intentFingerprint) throw new DispatchDocumentConflictError('idempotency key intent conflict');
        } else await tx.dispatchDocumentCommandResult.create({ data: { waybillId: input.waybillId,
          scope: 'PRINT_HANDOFF', scopeId: input.waybillId, idempotencyKey: input.operationIdempotencyKey,
          command: 'PRINT_HANDOFF', status: 'SUCCEEDED', result: storedCommandResult({ handoffId: handoff.id, kinds: input.kinds,
            artifactIds: input.artifacts.map(item => item.id) }, input.intentFingerprint), actorId: input.actorId,
  ...
  async getCombinedReadModel(input: Parameters<DispatchDocumentRepository['getCombinedReadModel']>[0]) {
    const candidate = await this.prisma.accountingDispatchCandidate.findFirst({ where: input.authorizedWaybillId
      ? { id: input.candidateId, waybills: { some: { id: input.authorizedWaybillId } } }
      : { id: input.candidateId, waybills: { none: {} }, status: { in: ['PENDING', 'STALE_REQUIRES_SUCCESSOR', 'EVIDENCE_CONFLICT'] } }, include: {
