...
export interface AuthorizationEvidence<Action extends PartnerActionV2 = PartnerAction> {
  evaluatedAt: string;
...
export interface AuthorizationSource<Action extends PartnerActionV2 = PartnerAction> {
  read(actorId: string, root: AuthorizationRoot): Promise<AuthorizationEvidence<Action>>;
}
