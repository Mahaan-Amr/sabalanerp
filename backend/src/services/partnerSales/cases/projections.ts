...
export async function buildCaseProjections(input: { caseId: string; revision: number; integrityHash: string;
  ...
  try {
    ...
    const accounting = priced ? SabalanInternalRecordViewSchema.parse({ schemaVersion: 1, purpose: 'ACCOUNTING',
    ...
    const fulfillment = priced ? FulfillmentViewSchema.parse({ schemaVersion: 1, purpose: 'FULFILLMENT',
