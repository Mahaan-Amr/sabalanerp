...
type UseInput = { binding: Binding; partnerSellerId: string; configurationHash: string;
  caseId?: string; pricingCaseRevision?: number };
...
export async function resolveApprovalForUse(tx: Prisma.TransactionClient, input: UseInput): Promise<Result<ApprovedInquiry>> {
  ...
  const row = await tx.partnerInquiryRow.findFirst({ where: { id: input.binding.rowId, inquiryId: input.binding.inquiryId },
    select: { id: true, revision: true, configurationHash: true, predecessorId: true,
      ...
      inquiry: { select: { id: true, caseId: true, caseRevision: true, pricingReadyAt: true, pricingExpiresAt: true,
  ...
  const approval = ApprovedInquirySchema.safeParse({ schemaVersion: 1, approvalId: row.approval.id,
    ...
    ...(row.predecessor?.approval?.id ? { predecessorApprovalId: row.predecessor.approval.id,
      ...(row.approval.supersessionReason ? { supersessionReason: row.approval.supersessionReason } : {}) } : {}),
...
export async function bindApprovalUsage(tx: Prisma.TransactionClient, input: UseInput & {
  usageId?: string; caseId: string; pricingCaseRevision: number; caseRevision: number; productRowId: string;
...
export async function bindMaterialApprovalUsage(tx: Prisma.TransactionClient, input: UseInput & {
  caseId: string; pricingCaseRevision: number; caseRevision: number; pricingSubjectId: string;
