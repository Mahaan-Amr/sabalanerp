...
export const getPerformanceAnalytics = async (client: PrismaClient, input: {
  ...
  const selectedIds = input.personnelIds?.length ? new Set(input.personnelIds) : null;
  const selected = selectedIds ? population.filter(({ personnelId }) => selectedIds.has(personnelId)) : population;
...
export const getEvaluatorCalibration = async (client: PrismaClient, input: { actorUserId: string; evaluatorPersonnelId: string; keyring?: PerformanceVaultKey }) => {
  ...
  for (const submission of submissions) {
    ...
    for (const grade of grades) samples.push({
...
const renderExportArtifact = async (kind: 'XLSX' | 'PDF', rows: Array<Record<string, unknown>>, target: string) => {
  if (kind === 'XLSX') {
  ...
  await writeFile(target, await generatePdfBufferFromHtml({ htmlContent: html }));
...
export const processPerformanceExport = async (client: PrismaClient, exportId: string, keyring = performanceVaultKeyFromEnvironment()) => {
  const receipt = await runPerformanceSerializableTransaction(client, async (tx) => {
    ...
    return tx.performanceExportReceipt.update({ where: { id: queued.id }, data: { status: PerformanceExportStatus.RUNNING, attemptCount: { increment: 1 } } });
  });
  ...
  try {
    ...
    const artifactPath = path.join(exportRoot(), `${receipt.id}.${receipt.exportKind === 'PDF' ? 'pdf' : 'xlsx'}`);
    const mimeType = await renderExportArtifact(receipt.exportKind as 'XLSX' | 'PDF', rows, artifactPath);
    ...
    const ready = await client.performanceExportReceipt.update({ where: { id: receipt.id }, data: {
    ...
    await client.performanceExportReceipt.update({ where: { id: receipt.id }, data: { status: PerformanceExportStatus.FAILED, failureCode: code } });
    return null;
  }
};
...
export const claimPerformanceExportDownload = async (client: PrismaClient, input: { exportId: string; actorUserId: string; token: string }) => {
  ...
  return { artifactPath: receipt.artifactPath, mimeType: receipt.artifactMimeType ?? 'application/octet-stream', filename: `performance-${receipt.id}.${receipt.exportKind === 'PDF' ? 'pdf' : 'xlsx'}` };
};
...
export const completePerformanceExportDownload = async (client: PrismaClient, input: { exportId: string; actorUserId: string; delivered: boolean }) => {
  ...
  await deleteDownloadedPerformanceExport(client, receipt.id);
};

const deleteDownloadedPerformanceExport = async (client: PrismaClient, exportId: string) => {
...
export const cleanupExpiredPerformanceExports = async (client: PrismaClient, now = new Date()) => {
  const expired = await client.performanceExportReceipt.findMany({
    where: { status: { in: [PerformanceExportStatus.READY, PerformanceExportStatus.DELIVERING, PerformanceExportStatus.DOWNLOADED, PerformanceExportStatus.EXPIRED] }, expiresAt: { lte: now } },
  });
...
export const createPerformanceConsequenceHandoff = async (client: PrismaClient, input: {
  ...
  return runPerformanceSerializableTransaction(client, async (tx) => {
    ...
    const results = await tx.performanceAcceptedResult.findMany({ where: { id: { in: [...input.resultIds] } } });
    ...
    const snapshot = {
      ...
      consequenceType: input.consequenceType,
      policyCycleKey: input.policyCycleKey,
      selectedResults: results.map((result) => ({ id: result.id, version: result.version, levelCode: result.levelCode, levelPolicyVersionId: result.levelPolicyVersionId, acceptedAt: result.acceptedAt, expiresAt: result.expiresAt })),
...
export const listEligibleConsequenceResults = async (client: PrismaClient, personnelId: string) => {
...
export const getPerformanceConsequenceHandoff = async (client: PrismaClient, input: { handoffId: string; actorUserId: string; keyring?: PerformanceVaultKey }) => {
  ...
  const keyring = input.keyring ?? performanceVaultKeyFromEnvironment();
  const snapshot = await readPerformancePayload<Record<string, unknown>>(client, handoff.encryptedPayloadId, keyring);
  await auditDisclosure(client, {
  ...
  const { encryptedPayloadId: _payloadId, ...publicHandoff } = handoff;
  return { handoff: publicHandoff, snapshot };
};
...
export const createPerformanceCorrection = async (client: PrismaClient, input: { evaluationId: string; actorUserId: string; correctionKind: string; reason: string }) => {
  ...
  return runPerformanceSerializableTransaction(client, async (tx) => {
    ...
    return tx.performanceCorrection.create({ data: {
