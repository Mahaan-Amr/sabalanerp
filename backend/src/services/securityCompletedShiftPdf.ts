...
const compactDurationLabel = (minutes: number | null) => {
...
const styles = `
  <style>
    ...
    *{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact}
    ...
    .sheet{padding:1mm}
    .report-header{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:7px;padding:8px 11px;border:1px solid #dce5e8;border-radius:12px;background:#f7fafb;box-shadow:3px 3px 8px rgba(110,128,142,.13),-3px -3px 8px rgba(255,255,255,.9)}
    ...
    h1,h2,h3,p{margin:0}h1{font-size:17px;color:#111820}h2{font-size:14px;color:#111820}h3{margin:7px 0 4px;font-size:11px;color:#26323d}
    .meta,.muted{color:#64717d}.meta{margin-top:2px;font-size:9px}
    .shift{padding:8px;border:1px solid #dce5e8;border-radius:12px;background:#f7fafb;box-shadow:3px 3px 9px rgba(110,128,142,.13),-3px -3px 9px rgba(255,255,255,.92);break-inside:auto}
    ...
    .cards{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:7px;margin:7px 0}.card{min-height:50px;padding:6px 8px;border:1px solid #dfe7ea;border-radius:10px;background:#f3f7f8;box-shadow:2px 2px 5px rgba(110,128,142,.1),-2px -2px 5px #fff}.card span{display:block;color:#71808d;font-size:8.5px}.card strong{display:block;margin-top:2px;color:#17212b;font-size:12px;word-break:break-word}
    table{width:100%;margin:0 0 6px;border-collapse:separate;border-spacing:0;table-layout:fixed;overflow:hidden;border:1px solid #cfdadd;border-radius:8px;background:#fff}th,td{padding:4px 6px;border-left:1px solid #d9e1e4;border-bottom:1px solid #d9e1e4;vertical-align:top;text-align:right;word-break:break-word}tr:last-child>th,tr:last-child>td{border-bottom:0}th:last-child,td:last-child{border-left:0}th{background:#eaf2f1;color:#26343d;font-weight:700}thead th{background:#e2eeec;color:#173f3a}.summary-table th{width:13%}.summary-table td{width:37%}.timeline-table .event-col{width:18%}.timeline-table .people-col{width:17%}.timeline-table .description-col{width:44%}.timeline-table .status-col{width:9%}.timeline-table .time-col{width:12%}
...
const renderShift = (slot: any, options: RenderOptions) => {
  ...
  const displayTimeline = [
    ...timeline.filter((event: any) => event.kind === 'SHIFT_LOG'),
    ...patrolSessions.map((patrol: any) => ({
      id: `patrol-${patrol.id}`,
      kind: 'PATROL',
      linkedId: patrol.id,
      rowNumber: null,
      ...
      author: personName(patrol.personnel),
    })),
  ...
  const timelineRows = displayTimeline.map((event: any) => {
    ...
    const status = event.kind === 'SHIFT_LOG'
      ...
      : event.endedAt
        ? '<span class="badge active">تکمیل‌شده</span>'
        : '<span class="badge warning">ناتمام</span>';
    const timeOrDuration = event.kind === 'PATROL'
      ? compactDurationLabel(minutesBetween(event.startedAt, event.endedAt))
      : dateTime(event.createdAt);
    return `<tr><td>${event.rowNumber != null ? `ردیف ${event.rowNumber.toLocaleString('fa-IR')} · ` : ''}${escapeHtml(event.title)}${event.typeDescription ? `<div class="muted">${escapeHtml(event.typeDescription)}</div>` : ''}</td><td>${escapeHtml(people)}</td><td>${escapeHtml(event.description || '—')}</td><td>${status}</td><td>${timeOrDuration}</td></tr>${entry ? `<tr><td colspan="5">${evidenceHtml(entry, options.resolveAttachmentDataUri)}</td></tr>` : ''}`;
  }).join('');
  ...
  return `<section class="shift">
    ...
    <div class="section"><h3 class="section-title">خط زمانی شیفت</h3>${timelineRows ? `<table class="timeline-table"><colgroup><col class="event-col"/><col class="people-col"/><col class="description-col"/><col class="status-col"/><col class="time-col"/></colgroup><thead><tr><th>رویداد</th><th>نیرو / افراد مرتبط</th><th>شرح</th><th>وضعیت</th><th>زمان / مدت</th></tr></thead><tbody>${timelineRows}</tbody></table>` : '<div class="empty">رویداد عملیاتی ثبت نشده است.</div>'}</div>
  </section>`;
};
