...
export const renderSecurityAttendanceReportHtml = ({ baseStyles, title, totals, rows, showDateColumn }: RenderSecurityAttendanceReportOptions) => {
  const columnCount = showDateColumn ? 8 : 7;
  const detailRows = rows.map((row) => `<tr>${showDateColumn ? `<td>${escapeHtml(row.date)}</td>` : ''}<td class="employee-cell">${escapeHtml(row.employee)}</td><td>${escapeHtml(row.status)}</td><td>${escapeHtml(row.entryTime)}</td><td>${escapeHtml(row.exitTime)}</td><td>${escapeHtml(minutesLabel(row.delayMinutes))}</td><td>${escapeHtml(minutesLabel(row.overtimeMinutes, row.overtimePending))}</td><td class="notes-cell">${escapeHtml(row.notes)}</td></tr>`).join('');
  return `${baseStyles}<style>.cards{grid-template-columns:repeat(2,1fr)}.attendance-detail{font-size:9px}.attendance-detail thead{display:table-header-group}.attendance-detail tr{break-inside:avoid;page-break-inside:avoid}.attendance-detail th,.attendance-detail td{padding:5px 6px;vertical-align:middle}.attendance-detail .employee-cell{font-weight:700;color:#0f172a}.attendance-detail .notes-cell{white-space:pre-line;line-height:1.55}.attendance-count{margin:0 0 7px;color:#475569;font-size:9px}.minimal-header{margin-bottom:10px}</style><div class="sheet"><header class="header minimal-header"><h1>${escapeHtml(title)}</h1></header><div class="cards"><div class="card"><span>غایب</span><strong>${totals.absent.toLocaleString('fa-IR')}</strong></div><div class="card"><span>تأخیر</span><strong>${totals.late.toLocaleString('fa-IR')}</strong></div></div><p class="attendance-count">${rows.length.toLocaleString('fa-IR')} ردیف استثنای حضور</p><table class="attendance-detail"><thead><tr>${showDateColumn ? '<th>تاریخ</th>' : ''}<th>پرسنل</th><th>وضعیت</th><th>ورود</th><th>خروج</th><th>تأخیر</th><th>اضافه‌کار</th><th>یادداشت</th></tr></thead><tbody>${detailRows || `<tr><td colspan="${columnCount}">در بازه انتخاب‌شده غیبت، تأخیر یا اضافه‌کاری ثبت نشده است.</td></tr>`}</tbody></table></div>`;
};
