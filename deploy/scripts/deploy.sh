...
PERFORMANCE_RUNTIME_INFRASTRUCTURE_HASH="$(compose config | grep -v 'PERFORMANCE_RUNTIME_INFRASTRUCTURE_HASH:' | node -e "const c=require('node:crypto');let b='';process.stdin.on('data',d=>b+=d);process.stdin.on('end',()=>process.stdout.write(c.createHash('sha256').update(b).digest('hex')))")"
export PERFORMANCE_RUNTIME_INFRASTRUCTURE_HASH
