...
The earlier Partner workflow treated one approved catalog-subject rate as reusable across Customers and Contracts for 48 hours, allowed independent inquiries outside Contract creation, allocated all three Case and record identities at the first numbered save, and allowed Customer confirmation while Sabalan pricing was incomplete. That model no longer matches the required commercial conversation: the internal Sabalan seller must quote one numbered Partner Case without seeing the Partner's Customer or retail economics, pricing agreement must happen immediately after Product selection and before Delivery or payment planning, and no customer-facing Contract or Sabalan debt may exist until the Partner deliberately accepts the completed Sabalan price package and finishes the remaining wizard.
...
Partner Contract creation reuses the ordinary Sales workflow and canonical Product Graph, with one Case-scoped `استعلام قیمت` gate inserted immediately after `انتخاب محصولات`. When the Partner continues from a valid Product selection, the system creates one immutable Partner Sale Case number, current revision, audit evidence and one cross-workspace pricing duty for the responder assigned to that Partner account. It does not yet allocate the customer-Contract number or internal Sabalan-record number, create either final linked record, contact the Customer, create Sabalan debt, or make the Case eligible for Accounting or fulfillment. The pricing gate displays the responder's offer separately for every required Product subject and prevents navigation to `برنامه تحویل` until all required responses are current and the Partner explicitly accepts the package.

Every responder decision is bound to one Case revision and stable Product row and cannot be reused in another Case. At this pre-Delivery gate the responder sees only the technical Product graph needed for pricing; Customer identity, contact details, delivery plan, exact address, retail price, retail payment plan, margin and other retail economics remain hidden. The Partner cannot select a responder per Case. Authorized management may reassign pending work only with a reason and preserved history.
...
- Product-boundary pricing submission, row decisions, Partner package acceptance, correction, package expiry, re-request and finalization require append-only evidence, expected Case revision, command identity and idempotent atomic boundaries.
