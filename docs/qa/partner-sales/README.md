...
CI regression reference: run `33055122225` at `678359aa` failed before unit tests with `ENOENT ... apps/sabalan-inquiry/app` after successful installs/builds. The existing `inventory.test.mjs` requires the Inquiry routes, XLSX handler and server action; run `node scripts/run-partner-sales-tests.mjs unit` and `check-inventory` from a fresh checkout after the initialization above. This exercises the original failure path without replacing the full Inquiry inventory with a mock.
