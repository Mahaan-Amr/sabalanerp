...
Automatic inference remains intentionally limited to same-length longitudinal layouts whose per-source distribution is uniquely determined by capacity, generated-remainder identities and requested count. The current writer records the exact per-source distribution for new layouts. The bridge also safely upgrades an older two-dimensional draft when its ledger proves exactly one requested child piece per consumed source; canonical replay must reproduce every secondary remainder and cutting amount. Tests cover both that case and a proved 2/2/1 same-length distribution with two untouched original source units retained. Other layouts or incomplete/contradictory evidence are blocked, not optimized or guessed. Missing/cyclic dependency evidence likewise receives no invented reconstruction order.

## Two-axis creation regression (2026-09-13)

The reported creation draft used 160 source pieces of 10cm by 1.36m for 160 child pieces of 10cm by 1.26m. The frontend correctly calculated 16m of cross cutting, 320,000 toman, and 160 secondary pieces of 10cm by 0.1m, but it did not persist the per-source distribution. The write-only bridge then applied its legacy same-length inference boundary and rejected the valid draft as `unsupported-physical-layout` before insertion.

The allocation writer now uses the shared canonical packing engine and persists the exact number of physical segments placed on each consumed source. Recovery accepts that evidence, validates it against consumed identities and physical pieces, replays the same canonical two-dimensional packing policy, and maps every generated remainder by source ordinal, sequence, and exact geometry. A logical child piece that spans multiple source-length segments keeps its logical dimensions while recording those physical segments explicitly. Existing one-piece-per-source drafts such as the reported case are upgraded without guessing. Invalid distributions, missing lineage, price drift, inventory drift, and ambiguous layouts still abort the entire write.

Automated verification covered the exact 160-piece geometry and amount, shared frontend/canonical decomposition with multiple secondary geometries, a logical piece split into three physical segments, existing same-length recovery, graph serialization and projections, create/edit database transactions with rollback, persistence, migration, print consumers, full contract-creation tests, typechecks, production builds, architecture ownership, and design-system static/foundation checks. Visual and browser QA were deliberately not run at the user's request; the user-facing acceptance steps are supplied separately.
