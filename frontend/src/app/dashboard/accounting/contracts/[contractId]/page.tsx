...
} from "@/features/accounting/AccountingVoidWorkflowPanel";
...
const financialRecordKindLabels: Record<string, string> = {
  ...
  RECEIVABLE: "دریافتنی قرارداد",
  PAYMENT: "دریافت ثبت‌شده",
  TAX_RECORD: "پرونده مالیاتی",
};
const correctionPriorityLabels: Record<string, string> = {
  LOW: "کم",
  MEDIUM: "متوسط",
  HIGH: "زیاد",
  CRITICAL: "فوری",
  ...
  blockers
    .map((item) => {
      const dependencies = (item.details || []).map((detail: any) =>
        [detail.kind, detail.reference || detail.id, detail.status]
          .filter(Boolean)
          .join(" / "),
      );
...
export default function AccountingContractDetailPage(props: {
  ...
  const focusKind =
    searchParams.get("focus") === "receivable" ||
    searchParams.get("focus") === "check"
  ...
  const [voidReceivableTarget, setVoidReceivableTarget] = useState<{
    workflow: AccountingVoidWorkflowView;
    receivableId: string;
  } | null>(null);
  const [resolveTaxTarget, setResolveTaxTarget] = useState<{
    workflow: AccountingVoidWorkflowView;
    taxRecordId: string;
  } | null>(null);
  const [cancelVoidTarget, setCancelVoidTarget] =
    useState<AccountingVoidWorkflowView | null>(null);
  ...
  const [flagCloseTarget, setFlagCloseTarget] = useState<{
    item: any;
    mode: "resolve" | "cancel";
  } | null>(null);
  const [correctionModalOpen, setCorrectionModalOpen] = useState(false);
  const [salesPdfVariant, setSalesPdfVariant] =
    useState<SalesPdfVariant>("accounting");
  const [customPrintSettings, setCustomPrintSettings] =
    useState<CustomPrintSettings>(defaultCustomPrintSettings);
  const [collectionFocusState, setCollectionFocusState] = useState<
    "focused" | "missing" | null
  >(null);
  ...
  const loadDetail = useCallback(async () => {
    try {
      ...
      if (profileResponse.data.success)
        setUserRole(profileResponse.data.data.role || "USER");
    } catch (error) {
  ...
  const execute = async (action: any) => {
    try {
      ...
      setActionError(userFacingError(error, "اقدام حسابداری انجام نشد"));
    ...
    try {
      ...
      setActionError(
        blockers.length
          ...
          : userFacingError(error, "اقدام مدیریت وضعیت قرارداد انجام نشد"),
      );
    } finally {
      setActionLoading(false);
    }
  ...
  const decideLifecycle = async (
    ...
    try {
      ...
      await accountingAPI.decideContractLifecycleRequest(requestId, {
        decision,
        reason:
          decision === "REJECT" ? "درخواست مدیریت وضعیت رد شد" : undefined,
      });
      await loadDetail();
    } catch (error: any) {
      const blockers = error?.response?.data?.blockers || [];
      setActionError(
        blockers.length
          ? `اقدام متوقف شد: ${formatLifecycleBlockers(blockers)}`
          : userFacingError(error, "تصمیم درخواست ثبت نشد"),
      );
    } finally {
      setActionLoading(false);
    }
  ...
  const approveFinancialInvoice = (
    payload: FinancialInvoiceApprovalPayload,
  ) => {
    execute({
  ...
  const deleteDraftRecord = async (values: Record<string, string | number>) => {
    ...
    const applied = await execute({
      ...
      note: String(
        values.note || "Deleted draft from accounting contract detail",
      ),
    });
    if (applied) setDeleteTarget(null);
  };

  const voidAccountingRecord = async (
  ...
  const startAccountingVoidCase = async (
    values: Record<string, string | number>,
  ) => {
    ...
    const applied = await execute({
      ...
      effectiveAt: PersianCalendar.toGregorian(
        String(values.effectiveAt),
      ).toISOString(),
      ...(reasonKind === "DUPLICATE_ISSUE"
        ? { retainedRecordId: String(values.retainedRecordId || "") }
        : {}),
    });
    if (applied) setStartVoidTarget(null);
  };

  const voidAccountingReceivable = async (
    values: Record<string, string | number>,
  ) => {
  ...
  const cancelAccountingVoidCase = async (
    values: Record<string, string | number>,
  ) => {
    if (!cancelVoidTarget) return;
    const applied = await execute({
      kind: "CANCEL_ACCOUNTING_VOID_CASE",
      voidCaseId: cancelVoidTarget.id,
  ...
  const closeFlag = async (values: Record<string, string | number>) => {
    ...
    const applied = await execute({
      kind:
        flagCloseTarget.mode === "resolve"
          ? "RESOLVE_CONTRACT_FLAG"
          : "CANCEL_CONTRACT_FLAG",
  ...
  const requestCorrection = async (values: Record<string, string | number>) => {
    ...
    try {
      setActionError(null);
      setActionLoading(true);
      await accountingAPI.createCorrectionRequest(
        params.contractId,
        {
      ...
      const response = (error as any)?.response?.data;
      setActionError(
        response?.message || "ثبت درخواست اصلاح انجام نشد. دوباره تلاش کنید.",
      );
    } finally {
      setActionLoading(false);
    }
  ...
  const openSalesContractPdf = async (tryPrint = false) => {
    ...
    try {
      const pdfParams = {
        fresh: true,
        variant: salesPdfVariant,
        ...buildCustomPrintParams(),
      };
      if (!tryPrint) {
        const response = await accountingAPI.downloadSalesContractPdf(
          params.contractId,
          pdfParams,
        );
        downloadBlobResponse(
          response,
          `sales_contract_${params.contractId}_${salesPdfVariant}.pdf`,
        );
        return;
      }

      const response = await accountingAPI.getSalesContractPdf(
        params.contractId,
        pdfParams,
      );
      const url = response.data?.data?.url;
      if (!response.data?.success || !url)
        throw new Error("Sales contract PDF url was not returned");
      openPdfUrl(url, tryPrint);
    } catch (error) {
      console.error("Sales contract PDF failed:", error);
      setActionError(
        tryPrint ? "پرینت قرارداد انجام نشد" : "دانلود PDF قرارداد انجام نشد",
      );
    } finally {
  ...
  const canUseAccountingAction = (kind: string) =>
    accountingActionAvailability(contract, kind)?.visible === true;
  const canExecuteAccountingAction = (kind: string) =>
  ...
  const replacementRecord = replacementWorkflow?.replacementRecordId
    ? (data.financialRecords || []).find(
        (record: any) => record.id === replacementWorkflow.replacementRecordId,
      )
    : null;
  const reviewCaseForFlag = (flag: any) =>
    (data.financialEvidenceReviewCases || []).find(
      (reviewCase: any) => reviewCase.id === flag.id,
    );
  ...
  return (
    <ErpPage
      ...
      metrics={[
        {
          label: "مبلغ قرارداد",
          value: money(contract.accounting.totalContractAmount),
          icon: FaBalanceScale,
          tone: "primary",
        },
        {
          label: "صورتحساب شده",
          value: money(contract.accounting.invoicedAmount),
          icon: FaFileInvoice,
          tone: "info",
        },
        {
          label: "دریافت شده",
          value: money(contract.accounting.receivedAmount),
          icon: FaReceipt,
          tone: "success",
        },
        {
          label: "مانده",
          value: money(contract.accounting.remainingAmount),
          icon: FaMoneyCheckAlt,
          tone:
            contract.accounting.receivableStatus === "OVERDUE"
              ? "danger"
              : "warning",
        },
      ]}
    >
      ...
      <div hidden={detailSection !== "summary"}>
        <ErpSection title="مدیریت وضعیت قرارداد">
          {contract.isInactive && (
            <ErpInlineState
              kind="stale"
              title={`قرارداد غیرفعال است${contract.inactiveReason ? ` — ${contract.inactiveReason}` : ""}`}
              className="mb-3"
            />
          )}
            ...
            {contract.isInactive && (
              <ErpButton
                ...
                onClick={() =>
                  setLifecycleTarget({
                    action: "REACTIVATE",
                    mode: userRole === "ADMIN" ? "DIRECT" : "REQUEST",
                  })
                }
              />
            )}
          ...
          {lifecycle && (
            <div className="mt-3 space-y-2">
              {[
                ...(contract.isInactive
                  ? []
                  : lifecycle.deactivationEligibility?.blockers || []),
                ...(["DRAFT", "CANCELLED"].includes(contract.status)
                  ? lifecycle.deleteEligibility?.blockers || []
                  : []),
              ].map((blocker: any) => (
                <ErpInlineState
                  key={blocker.code}
                  kind="stale"
                  title={`${blocker.label}: ${Number(blocker.count).toLocaleString("fa-IR")}`}
                />
              ))}
            </div>
          )}
          {(data.lifecycleRequests || [])
            .filter((request: any) => request.status === "PENDING")
            .map((request: any) => {
              const canDecide =
                userRole === "ADMIN" ||
                (userRole === "MANAGER" && request.kind === "DEACTIVATE");
              return (
                <div
                  key={request.id}
                  className="mt-3 rounded-[var(--sds-radius-lg)] border border-[var(--sds-border-default)] p-3"
                >
                  ...
                  {canDecide && (
                    <div className="mt-3 flex gap-2">
                      <ErpButton
                        ...
                        tone="success"
                        onClick={() =>
                          void decideLifecycle(request.id, "APPROVE")
                        }
                      ...
                      <ErpButton
                        ...
                        variant="outline"
                        onClick={() =>
                          void decideLifecycle(request.id, "REJECT")
                        }
                        disabled={actionLoading}
                      />
                    </div>
        ...
        <ErpTwoColumn
          main={
            <>
                  ...
                  <ErpSummaryGrid
                    ...
                    items={[
                      ...
                      },
                      {
                        ...
                        value:
                          invoiceStatusLabels[
                            contract.accounting.invoiceStatus
                          ] ||
                          operationalStatusLabel(
                            contract.accounting.invoiceStatus,
                          ),
              ...
              {detailSection === "financial" && replacementWorkflow && (
                <ErpSection title="جایگزینی رکورد مالی">
                  <div className="space-y-4">
                    <ErpSummaryGrid
                      ...
                      items={[
                        {
                          label: "مبلغ رکورد قبلی",
                          value: replacementWorkflow.oldAmount
                            ? money(replacementWorkflow.oldAmount)
                            : "-",
                        },
                        {
                          label: "مبلغ اصلاح‌شده",
                          value: money(replacementWorkflow.correctedAmount),
                        },
                        {
                          label: "اثر مبلغی",
                          value: replacementWorkflow.amountChanged
                            ? "دارد"
                            : "ندارد",
                        },
                      ]}
                      ...
                      <div className="space-y-3">
                        <div className="grid gap-3 lg:grid-cols-4">
                          <CompactQueueItem
                            ...
                            meta={
                              replacementWorkflow.sourceRecordStatus ===
                              "VOIDED"
                                ? "انجام شده"
                                : "نیازمند شاهد خارجی"
                            }
                            status={
                              <StatusBadge
                                status={replacementWorkflow.sourceRecordStatus}
                              />
                            }
                          />
                          <CompactQueueItem
                            ...
                            status={
                              <StatusBadge
                                status={
                                  replacementWorkflow.replacementRecordStatus ||
                                  "PENDING"
                                }
                              />
                            }
                          ...
                          <CompactQueueItem
                            ...
                            meta={
                              replacementWorkflow.replacementFinanciallyApprovedAt
                                ? dateFa(
                                    replacementWorkflow.replacementFinanciallyApprovedAt,
                                  )
                                : "در انتظار"
                          ...
                          <CompactQueueItem
                            ...
                            title="۴. بستن اصلاح"
                            meta={
                              replacementWorkflow.canResolve
                                ? "آماده بستن"
                                : "ناتمام"
                            }
                            status={
                        ...
                        <div className="flex flex-wrap gap-2">
                          <ErpButton
                            ...
                            tone="danger"
                            disabled={
                              !replacementWorkflow.canVoidSource ||
                              actionLoading
                            }
                            onClick={() => {
                              const sourceRecord = (
                                ...
                                (record: any) =>
                                  record.id ===
                                  replacementWorkflow.sourceRecordId,
                              );
                              if (sourceRecord)
                          ...
                          <ErpButton
                            ...
                            tone="success"
                            disabled={
                              !replacementWorkflow.canResolve || actionLoading
                            }
                            onClick={() =>
                              setResolveTarget({
                                id: replacementWorkflow.correctionRequestId,
                              })
                            }
                          />
                        </div>
              ...
              {detailSection === "items" && (
                <ErpSection
                  title="اقلام قرارداد"
                  description="این اطلاعات از قرارداد فروش خوانده می‌شود و در رکوردهای حسابداری به صورت Snapshot نگهداری می‌شود."
                >
                  <div className="space-y-3">
                    {(source.items || []).map((item: any) => (
                      <CompactQueueItem
                        ...
                        meta={`مقدار: ${
                          item.quantityPresentation?.status === "RECONCILED"
                            ? `${item.quantityPresentation.quantity} متر`
                            : item.quantityPresentation?.status ===
                                "REVIEW_REQUIRED"
                              ? "نیازمند بررسی مقدار"
                              : item.quantity
                        } · قیمت واحد: ${money(item.unitPrice)}`}
                    ...
                    {(!source.items || source.items.length === 0) && (
                      <p className="text-sm text-[var(--sds-text-secondary)] dark:text-[var(--sds-text-muted)]">
                        قلمی برای قرارداد ثبت نشده است.
                      </p>
                    )}
                  </div>
                </ErpSection>
                    ...
                    <div className="space-y-3">
                      {(data.financialRecords || []).map((record: any) => (
                        <CompactQueueItem
                          ...
                          icon={FaFileInvoice}
                          title={
                            financialRecordKindLabels[record.kind] ||
                            "رکورد مالی"
                          }
                          meta={[
                            `ایجاد: ${dateFa(record.createdAt)}`,
                            record.systemInvoiceNumber
                              ? `شماره فاکتور سیستمی: ${record.systemInvoiceNumber}`
                              : null,
                            record.systemInvoiceDate
                              ? `تاریخ فاکتور سیستمی: ${dateFa(record.systemInvoiceDate)}`
                              : null,
                          ]
                            .filter(Boolean)
                            .join(" · ")}
                          ...
                          footer={
                            record.kind === "INVOICE_CANDIDATE" ? (
                              <div className="space-y-3">
                                ...
                                !["ISSUED", "POSTED", "VOIDED"].includes(
                                  record.status,
                                ) ? (
                                  <div className="rounded-lg border border-[var(--sds-warning-border)] bg-[var(--sds-warning-surface)] p-3 text-sm text-[var(--sds-warning)] dark:border-[var(--sds-warning-border)] dark:bg-[var(--sds-warning-surface)] dark:text-[var(--sds-warning)]">
                                    ابتدا درخواست‌های اصلاح و پرچم‌های
                                    مسدودکننده باز را بررسی و ببندید؛ سپس تایید
                                    مالی انجام می‌شود.
                                  </div>
                                ) : (
                                  <FinancialInvoiceApprovalForm
                                ...
                                {["ISSUED", "POSTED"].includes(record.status) &&
                                  !(data.voidWorkflows || []).some(
                                    (workflow: AccountingVoidWorkflowView) =>
                                      workflow.sourceRecordId === record.id &&
                      ...
                      {(!data.financialRecords ||
                        ...
                        <p className="text-sm text-[var(--sds-text-secondary)] dark:text-[var(--sds-text-muted)]">
                          هنوز رکورد مالی برای این قرارداد ایجاد نشده است.
                        </p>
                      )}
                    </div>
                  </ErpSection>
              ...
              {detailSection === "financial" &&
                (data.voidWorkflows || []).length > 0 && (
                  <div id="financial-void-cases" className="scroll-mt-24">
                    <ErpSection
                      title="پرونده‌های ابطال مالی"
                      description="هر مرحله را به‌ترتیب انجام دهید؛ فقط اقدام بعدی فعال است."
                    >
                      <AccountingVoidWorkflowPanel
                        ...
                        onVoidReceivable={(workflow, receivableId) =>
                          setVoidReceivableTarget({ workflow, receivableId })
                        }
                        onVoidRecord={(workflow, recordId) =>
                          setVoidTarget({
                            ...workflow,
                ...
                <div id="collections" className="scroll-mt-6">
                  <ErpSection title="دریافتنی‌ها و دریافت‌ها">
                    {collectionFocusState === "missing" && (
                      <ErpInlineState
                        kind="stale"
                        title="رکورد پیوندشده دیگر در این پرونده در دسترس نیست؛ اطلاعات فعلی قرارداد نمایش داده می‌شود."
                      />
                    )}
                    {collectionFocusState === "focused" && (
                      <ErpInlineState
                        kind="success"
                        title="رکورد پیوندشده در وضعیت فعلی پرونده پیدا و متمرکز شد."
                      />
                    )}
                      ...
                      {(data.paymentEvents || []).map((item: any) => (
                        ...
                        >
                          <CompactQueueItem
                            ...
                            status={
                              <StatusBadge
                                status={item.checkStatus || item.status}
                              />
                            }
                          />
                        </div>
                  ...
                  <div className="space-y-2">
                    {canUseAccountingAction("CREATE_INVOICE") && (
                      <ErpButton
                        ...
                        tone="info"
                        disabled={
                          !canExecuteAccountingAction("CREATE_INVOICE") ||
                          !canCreateRecords ||
                          actionLoading
                        }
                        title={
                          accountingActionReason("CREATE_INVOICE") ||
                          contract.accounting.eligibilityReason
                        }
                        onClick={() =>
                          execute({
                            kind: "CREATE_INVOICE",
                    ...
                    {canUseAccountingAction("CREATE_RECEIVABLE") && (
                      <ErpButton
                        label="ایجاد دریافتنی"
                        icon={FaReceipt}
                        tone="success"
                        disabled={
                          !canExecuteAccountingAction("CREATE_RECEIVABLE") ||
                          !canCreateRecords ||
                          contract.accounting.invoiceStatus !== "ISSUED" ||
                          actionLoading
                        }
                        title={
                          accountingActionReason("CREATE_RECEIVABLE") ||
                          contract.accounting.eligibilityReason ||
                          (contract.accounting.invoiceStatus !== "ISSUED"
                            ? "ابتدا صورتحساب را تایید مالی کنید"
                            : undefined)
                        }
                        onClick={() =>
                          execute({
                            ...
                            contractId: contract.contractId,
                            amount:
                              contract.accounting.remainingAmount ||
                              contract.accounting.totalContractAmount,
                            dueDate: new Date(
                    ...
                    {canUseAccountingAction("FLAG_CONTRACT") && (
                      <ErpButton
                        label="پرچم حسابداری"
                        icon={FaFlag}
                        tone="warning"
                        disabled={
                          !canExecuteAccountingAction("FLAG_CONTRACT") ||
                          actionLoading
                        }
                    ...
                    {canUseAccountingAction("CREATE_CORRECTION_REQUEST") && (
                      <ErpButton
                        label="درخواست اصلاح"
                        icon={FaExclamationTriangle}
                        tone="danger"
                        disabled={
                          !canExecuteAccountingAction(
                            "CREATE_CORRECTION_REQUEST",
                          ) || actionLoading
                        }
                        title={accountingActionReason(
                          "CREATE_CORRECTION_REQUEST",
                        )}
                  ...
                  <div className="space-y-3">
                    {(data.tax || []).map((item: any) => (
                      <CompactQueueItem
                        ...
                        icon={FaBalanceScale}
                        title={
                          taxStatusLabels[item.submissionStatus] ||
                          operationalStatusLabel(item.submissionStatus)
                        }
                        meta={
                          item.missingFields?.length
                            ? `کسری: ${item.missingFields.join("، ")}`
                            : item.trackingCode || "بدون کد پیگیری"
                        }
                    ...
                    {(!data.tax || data.tax.length === 0) && (
                      <p className="text-sm text-[var(--sds-text-secondary)] dark:text-[var(--sds-text-muted)]">
                        پرونده مالیاتی هنوز ایجاد نشده است.
                      </p>
                    )}
                  </div>
                </ErpSection>
                    ...
                    <div className="space-y-3">
                        ...
                        return (
                          <CompactQueueItem
                            ...
                            footer={
                              item.status === "OPEN" ? (
                                <div className="flex flex-wrap gap-2">
                                  {isFinancialEvidenceReviewCase(item) &&
                                  financialReview ? (
                                    <ErpButton
                                      label="باز کردن پرونده بررسی"
                                      href={financialEvidenceCaseHref(
                                        contract.contractId,
                                        financialReview.id,
                                      )}
                                      tone="danger"
                      ...
                      {(data.correctionRequests || []).map((item: any) => (
                        <CompactQueueItem
                          ...
                          meta={`اولویت: ${correctionPriorityLabels[item.priority] || "ثبت‌شده"}`}
                          status={<StatusBadge status={item.status} />}
                          footer={
                            [
                              ...
                              <div className="flex flex-wrap gap-2">
                                ...
                                {item.status === "SALES_EDITED" && (
                                  <ErpButton
                                    ...
                                    disabled={
                                      actionLoading ||
                                      (replacementWorkflow?.correctionRequestId ===
                                        item.id &&
                                        !replacementWorkflow.canResolve)
                                    }
        ...
        fields={[
          {
            ...
            label: "یادداشت حذف",
            type: "textarea",
            defaultValue: "Deleted draft from accounting contract detail",
          },
        ]}
        ...
        title={
          lifecycleTarget?.action === "DELETE"
            ...
            : lifecycleTarget?.action === "DEACTIVATE"
              ? "غیرفعال‌سازی قرارداد"
              : "فعال‌سازی مجدد قرارداد"
        }
        description={
          lifecycleTarget?.action === "DELETE"
            ? "حذف دائمی برگشت‌پذیر نیست و فقط در نبود وابستگی مسدودکننده انجام می‌شود."
            : undefined
        }
        fields={[
          { id: "reason", label: "دلیل", type: "textarea", required: true },
        ]}
        submitLabel={
          lifecycleTarget?.mode === "REQUEST"
            ? "ثبت درخواست"
            : lifecycleTarget?.action === "DELETE"
              ? "حذف دائمی"
              : "ثبت اقدام"
      ...
      <AccountingActionModal
        ...
        fields={[
          ...
          {
            id: "retainedRecordId",
            label: "فاکتور معتبر باقی‌مانده",
            type: "select",
            visibleWhen: { fieldId: "reasonKind", equals: "DUPLICATE_ISSUE" },
            requiredWhen: { fieldId: "reasonKind", equals: "DUPLICATE_ISSUE" },
            options: (data.financialRecords || [])
              .filter(
                (record: any) =>
                  record.id !== startVoidTarget?.id &&
                  ["ISSUED", "POSTED"].includes(record.status),
              )
              .map((record: any) => ({
                label: record.systemInvoiceNumber
                  ? `فاکتور ${record.systemInvoiceNumber}`
                  : record.id,
                value: record.id,
              })),
          },
        ...
        fields={[
          {
            ...
            type: "text",
            required: true,
            defaultValue: "نیازمند بررسی حسابداری",
          },
          {
            id: "category",
            label: "دسته",
      ...
      <AccountingActionModal
        ...
        submitLabel={
          flagCloseTarget?.mode === "resolve" ? "بستن پرچم" : "لغو پرچم"
        }
        busy={actionLoading}
        error={actionError}
