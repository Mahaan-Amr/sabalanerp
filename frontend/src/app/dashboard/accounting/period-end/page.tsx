  ...
  return (
    ...
    >
      ...
      {tab === 'reports' && <div className="space-y-4"><ErpSection title="ساخت snapshot رسمی" description="PDF و Excel باید از همین dataset منجمد و یک اثر انگشت واحد ساخته شوند.">
        ...
        </div><div className="mt-4 flex justify-end"><ErpButton label={saving ? 'در حال ساخت…' : 'ساخت نسخه رسمی منجمد'} disabled={saving || !report.fiscalYearId || !report.from || !report.to || !report.cutoffAt || (['FINANCIAL_STATEMENT', 'CASH_FLOW'].includes(report.reportKind) && !report.mappingVersionId) || (report.reportKind === 'LEGAL_BOOK' && !report.statutoryFormatId)} onClick={() => act(() => accountingAPI.createOfficialReportSnapshot({ request: { ...report, mappingVersionId: report.mappingVersionId || undefined, statutoryFormatId: report.statutoryFormatId || undefined, legalBookKind: report.reportKind === 'LEGAL_BOOK' ? 'JOURNAL' : undefined, bookId: book.id, columns: 8, level: 'SUBSIDIARY' } }), 'نسخه رسمی با مجموعه‌داده و اثر انگشت واحد ساخته شد.')} /></div>
