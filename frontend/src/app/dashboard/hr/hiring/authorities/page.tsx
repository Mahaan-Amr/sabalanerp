...
export default function HiringAuthoritiesPage() {
  ...
  return <ErpPage eyebrow="منابع انسانی · تنظیمات استخدام" title="اختیارهای تأیید استخدام" description="دسترسی عمومی Admin یا Workspace جایگزین اختیار کسب‌وکاری تأیید نیست." backHref="/dashboard/hr/hiring">
    {message && <p className="rounded-xl bg-emerald-50 p-3 text-emerald-700">{message}</p>}{error && <p className="rounded-xl bg-rose-50 p-3 text-rose-700">{error}</p>}
    <ErpSection title="واگذاری اختیار"><ErpCard className="grid gap-3 p-4 md:grid-cols-3"><select className={field} value={userId} onChange={(e) => setUserId(e.target.value)}><option value="">انتخاب کاربر</option>{users.map((u) => <option key={u.id} value={u.id}>{u.firstName} {u.lastName} · {u.username}</option>)}</select><select className={field} value={authority} onChange={(e) => setAuthority(e.target.value)}>{authorities.map((a) => <option key={a}>{a}</option>)}</select><ErpButton label="ثبت اختیار" disabled={!userId} onClick={save} /></ErpCard></ErpSection>
    <ErpSection title="اختیارهای فعال"><div className="grid gap-2 md:grid-cols-2">{rows.map((row) => { const user = users.find((u) => u.id === row.userId); return <ErpCard key={row.id} className="flex justify-between p-3 text-sm"><span>{user ? `${user.firstName} ${user.lastName}` : row.userId}</span><b>{row.authority}</b></ErpCard>; })}</div></ErpSection>
  </ErpPage>;
}
