  ...
  return <ErpPage eyebrow="منابع انسانی · کنترل انتقال" title="مهاجرت و تطبیق داده‌های موجود" description="پیش‌نمایش خواندنی و اجرای قابل تکرار؛ هیچ تاریخ یا رابطه ناشناخته‌ای ساخته نمی‌شود." actions={[{ label: 'اجرای دوباره پیش‌نمایش', icon: FaSync, onClick: load, tone: 'neutral' }]} backHref="/dashboard/hr">
    ...
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
      ...
      <ErpSection title="طبقه‌بندی استثناهای قدیمی" description="در این فاز تبدیل نمی‌شوند؛ فقط برای تصمیم فاز مرخصی گزارش می‌شوند.">
        <div className="space-y-3">{data?.exceptions?.map((item: any) => <div key={item.type} className="flex items-center justify-between rounded-xl border border-slate-200 p-3 dark:border-slate-700"><span>{item.type}</span><ErpBadge tone="neutral">{item.count.toLocaleString('fa-IR')}</ErpBadge></div>)}</div>
      </ErpSection>
    </div>
    <ErpSection title="اجرای کنترل‌شده" description="فقط پرسنل فعال را در تاریخ مبنا وارد رابطه استخدامی فعال می‌کند. تاریخ استخدام ناشناخته باقی می‌ماند و تخصیص اصلی باید بعداً توسط HR تکمیل شود.">
