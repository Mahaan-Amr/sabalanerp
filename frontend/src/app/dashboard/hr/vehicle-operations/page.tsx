...
const driverInitial = { personnelId: '', licenceNumber: '', licenceClass: '', licenceExpiresAt: '', effectiveFrom: today(), reason: 'تأیید صلاحیت رانندگی داخلی' };
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [driverResponse, vehicleResponse, personnelResponse] = await Promise.all([
        dispatchMasterDataAPI.getInternalDrivers(),
        ...
        <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(300px,380px)_minmax(0,1fr)]">
          {canManage && <ErpSection title="تعریف راننده داخلی" description="فقط پرسنل فعال انتخاب می‌شوند.">
            <form className="space-y-4" onSubmit={(event) => {
              ...
              <label className={field}>شماره گواهینامه<ErpInput required value={driverForm.licenceNumber} onChange={(event) => setDriverForm({ ...driverForm, licenceNumber: event.target.value })} /></label>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <label className={field}>پایه گواهینامه<ErpInput value={driverForm.licenceClass} onChange={(event) => setDriverForm({ ...driverForm, licenceClass: event.target.value })} /></label>
                <label className={field}>اعتبار گواهینامه<ErpInput type="date" value={driverForm.licenceExpiresAt} onChange={(event) => setDriverForm({ ...driverForm, licenceExpiresAt: event.target.value })} /></label>
              </div>
          ...
          <ErpSection title="رانندگان داخلی">
            {canManage && <form className="mb-5 grid grid-cols-1 gap-3 rounded-xl border border-[var(--sds-border-subtle)] p-3 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void run(() => dispatchMasterDataAPI.updateInternalDrivingProfile(profileForm.driverId, profileForm), 'مشخصات رانندگی به‌روزرسانی شد.').then(() => setProfileForm(profileInitial)); }}>
              ...
              <label className={field}>پایه گواهینامه<ErpInput value={profileForm.licenceClass} onChange={(event) => setProfileForm({ ...profileForm, licenceClass: event.target.value })} /></label>
      ...
      {section === 'vehicles' && (
        <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(300px,380px)_minmax(0,1fr)]">
          ...
          <ErpSection title="ناوگان شرکت">
            {canManage && <form className="mb-5 grid grid-cols-1 gap-3 rounded-xl border border-[var(--sds-border-subtle)] p-3 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void run(() => dispatchMasterDataAPI.changeCompanyVehiclePlate(plateForm.vehicleId, plateForm), 'پلاک جدید ثبت شد.').then(() => setPlateForm(plateInitial)); }}>
      ...
      {section === 'assignments' && canManage && (
        <ErpSection title="تخصیص فعال راننده و خودرو" description="تخصیص جدید، تخصیص فعال قبلی هر دو طرف را در همان زمان می‌بندد و سابقه را نگه می‌دارد.">
          <form className="grid grid-cols-1 gap-4 md:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void run(() => dispatchMasterDataAPI.assignCompanyVehicle(assignmentForm), 'تخصیص خودرو ثبت شد.').then(() => setAssignmentForm(assignmentInitial)); }}>
            ...
            <label className={field}>خودرو<ErpSelect required value={assignmentForm.vehicleId} onChange={(event) => setAssignmentForm({ ...assignmentForm, vehicleId: event.target.value })}><option value="">انتخاب کنید</option>{vehicles.filter((vehicle) => vehicle.status === 'IN_SERVICE').map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.fleetCode} · {vehicle.plates[0]?.plate || vehicle.vehicleType}</option>)}</ErpSelect></label>
