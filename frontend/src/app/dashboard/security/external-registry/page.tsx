...
const driverInitial = { firstName: '', lastName: '', nationalCode: '', phone: '', notes: '', reason: 'ثبت پیش‌نویس راننده متفرقه' };
    ...
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
      {canManage && <ErpSection title="ثبت راننده متفرقه">
        <form className="grid grid-cols-1 gap-3 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void run(() => dispatchMasterDataAPI.createExternalDriver(driverForm), 'راننده متفرقه ثبت شد.').then(() => setDriverForm(driverInitial)); }}>
          ...
          <label className={`${field} sm:col-span-2`}>دلیل<ErpInput required value={driverForm.reason} onChange={(event) => setDriverForm({ ...driverForm, reason: event.target.value })} /></label>
