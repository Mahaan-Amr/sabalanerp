        ...
        <>
          ...
          <ErpSection>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm sds-text-muted">{slot.plan?.title || 'برنامه شیفت گارد'}</p>
              </div>
            ...
            <dl className="mt-5 grid gap-4 border-t border-[var(--sds-border-subtle)] pt-4 text-sm dark:border-[var(--sds-border-subtle)] sm:grid-cols-3">
              <div><dt className="text-xs sds-text-muted">بازه برنامه</dt><dd className="mt-1 font-semibold">{dateTime(slot.startsAt)} تا {dateTime(slot.endsAt)}</dd></div>
              <div><dt className="text-xs sds-text-muted">نیروی برنامه‌ریزی‌شده</dt><dd className="mt-1 font-semibold">{personName(slot.plannedPersonnel)}</dd></div>
              <div><dt className="text-xs sds-text-muted">جایگزین</dt><dd className="mt-1 font-semibold">{personName(slot.replacementPersonnel)}</dd></div>
            </dl>
          </ErpSection>
          ...
          {slot.noShiftConfirmedAt && (
            <ErpSection title="تصمیم مدیر">
              ...
              <p className="mt-2 text-sm sds-text-secondary ">{slot.noShiftConfirmReason}</p>
            </ErpSection>
          )}

          <ErpSection title="حضور ثبت‌شده">
            {slot.attendance?.length ? slot.attendance.map((attendance: any) => (
              <p key={attendance.id} className="border-b border-[var(--sds-border-subtle)] py-3 text-sm last:border-0 dark:border-[var(--sds-border-subtle)]">
