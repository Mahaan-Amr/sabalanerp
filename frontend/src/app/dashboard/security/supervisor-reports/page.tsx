...
export default function SecuritySupervisorReportsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'دریافت گزارش‌های شیفت ناموفق بود.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const createReport = async () => {
    ...
    try {
      ...
      setMessage('گزارش شیفت ثبت شد.');
      await loadData();
    } catch (err: any) {
      setError(err.response?.data?.error || 'ثبت گزارش شیفت ناموفق بود.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <ErpPage
      eyebrow="حراست"
      title="گزارش شیفت"
      description="گزارش هر شیفت حراست با خلاصه، رخدادها و پیگیری‌ها."
    ...
    >
      ...
      <ErpSegmentedControl<SupervisorSection>
        ...
        options={[
          { value: 'create', label: 'ثبت گزارش شیفت', icon: FaPlus },
          { value: 'reports', label: 'گزارش‌های شیفت', icon: FaList },
      ...
      {activeSection === 'reports' && (
        <ErpSection title="گزارش‌های شیفت">
          {reports.length === 0 ? (
            <ErpEmptyState icon={FaClipboardCheck} title="گزارش شیفت ثبت نشده است" />
          ) : (
