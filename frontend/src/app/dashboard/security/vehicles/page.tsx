...
export default function SecurityVehiclesPage() {
  ...
  const [canApproveDispatchConfirmation, setCanApproveDispatchConfirmation] = useState(false);
  ...
  useEffect(() => {
    dispatchConfirmationAPI.getCapabilities()
      .then((response) => setCanApproveDispatchConfirmation(Boolean(response.data.data.canApproveGuardConfirmation)))
  ...
  return (
    <ErpWorkspacePage className="guard-workspace" title="تردد خودروها" primaryAction={canEditQueue ? { label: 'ثبت راننده یا خودرو', icon: FaPlus, href: '/dashboard/security/external-registry', variant: 'solid' } : undefined} secondaryActions={[...(canEditQueue ? [{ label: 'ثبت متفرقه‌های جدید', icon: FaUserShield, href: '/dashboard/security/external-registry' }] : []), { label: 'به‌روزرسانی', icon: FaRedo, onClick: loadData }]}>
      ...
      {activeSection === 'queue' && <ErpSection title="پذیرش صف جاری" description="پذیرش، هویت راننده، خودرو، پلاک و مدارک آماده‌بودن را برای همین مراجعه ثابت می‌کند.">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-[minmax(0,1fr)_auto]">
            ...
            <ErpButton label="ثبت پذیرش" icon={FaPlus} variant="solid" disabled={dispatchTimelineStale || saving || (admissionSource === 'INTERNAL' ? !internalDriverId : !externalDriverId || !externalVehicleId)} onClick={admitQueueTurn} className="self-end" />
          </div>
          ...
          {!canonicalQueue.length ? <ErpEmptyState icon={FaClock} title="مراجعه جاری در صف وجود ندارد" /> : <div className="space-y-3">{canonicalQueue.map((turn) => {
            ...
            return <ErpCard key={turn.id} className="p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-semibold sds-text-primary">{driverName} · {snapshot.plate?.plate || 'بدون پلاک'}</p><p className="mt-1 text-sm sds-text-muted">{snapshot.vehicle?.vehicleType || 'خودرو'} · پذیرش {new Date(turn.admittedAt).toLocaleString('fa-IR')}</p></div><ErpBadge tone={turn.status === 'AVAILABLE_FOR_LOADING' ? 'success' : turn.status === 'RESERVED_FOR_LOADING' ? 'warning' : 'neutral'}>{canonicalQueueStatusLabel[turn.status] || turn.status}</ErpBadge></div>{canEditQueue && <div className="mt-3 flex flex-wrap gap-2">{turn.status === 'WAITING_AT_GATE' && <ErpButton label="آماده بارگیری" icon={FaCheck} variant="solid" disabled={dispatchTimelineStale || saving} onClick={() => runQueueCommand(() => securityAPI.makeCanonicalQueueTurnAvailable(turn.id), 'راننده برای بارگیری آماده شد.')} />}{turn.status === 'AVAILABLE_FOR_LOADING' && <ErpButton label="بازگشت به انتظار" icon={FaRedo} variant="soft" disabled={dispatchTimelineStale || saving || !queueReason.trim()} onClick={() => runQueueCommand(() => securityAPI.returnCanonicalQueueTurnToWaiting(turn.id, queueReason.trim()), 'مراجعه به انتظار گیت بازگشت.', true)} />} {['WAITING_AT_GATE', 'AVAILABLE_FOR_LOADING', 'RESERVED_FOR_LOADING'].includes(turn.status) && <><ErpButton label="خروج بدون بارگیری" tone="warning" variant="soft" disabled={dispatchTimelineStale || saving || !queueReason.trim()} onClick={() => runQueueCommand(() => securityAPI.closeCanonicalQueueTurnWithoutLoading(turn.id, queueReason.trim()), 'مراجعه بدون بارگیری بسته شد.', true)} /><ErpButton label="ابطال پذیرش" tone="danger" variant="outline" disabled={dispatchTimelineStale || saving || !queueReason.trim()} onClick={() => setVoidTarget(turn)} /></>}</div>}</ErpCard>;
          })}</div>}
      ...
      <ErpSheet open={Boolean(voidTarget)} onClose={() => { if (!saving) setVoidTarget(null); }} title="تأیید ابطال پذیرش" presentation="modal" dismissible={!saving} footer={<div className="flex justify-end gap-2"><ErpButton label="انصراف" variant="ghost" disabled={saving} onClick={() => setVoidTarget(null)} /><ErpButton label="تأیید ابطال" tone="danger" variant="solid" disabled={dispatchTimelineStale || saving || !queueReason.trim()} onClick={() => void runQueueCommand(() => securityAPI.voidCanonicalQueueTurn(voidTarget.id, queueReason.trim()), 'پذیرش با حفظ سابقه باطل شد.', true).then(() => setVoidTarget(null))} /></div>}><p className="text-sm leading-7 sds-text-muted">این مراجعه از صف جاری خارج می‌شود و برای حفظ زنجیره حسابرسی قابل حذف یا بازگردانی نیست. دلیل ثبت‌شده همراه سابقه باقی می‌ماند.</p></ErpSheet>
      <ErpSheet open={Boolean(exitTarget)} onClose={() => { if (!saving) setExitTarget(null); }} title="تأیید خروج فیزیکی" presentation="modal" dismissible={!saving} footer={<div className="flex justify-end gap-2"><ErpButton label="انصراف" variant="ghost" disabled={saving} onClick={() => setExitTarget(null)} /><ErpButton label="ثبت خروج فیزیکی" tone="success" variant="solid" disabled={dispatchTimelineStale || saving} onClick={() => void recordAuthorizedExit(exitTarget.id)} /></div>}><p className="text-sm leading-7 sds-text-muted">این فرمان مجوز یک‌بارمصرف را مصرف می‌کند و عبور محموله از گیت را به‌صورت تغییرناپذیر ثبت می‌کند.</p></ErpSheet>
          ...
          <div className="grid grid-cols-1 gap-3 xl:grid-cols-2">
            {authorizedExits.map((authorization) => {
              ...
              return <ErpCard key={authorization.id} className="p-4">
                ...
                {canEditQueue && <div className="mt-3"><ErpButton label="ثبت خروج" icon={FaCheck} onClick={() => setExitTarget(authorization)} disabled={dispatchTimelineStale || saving} variant="solid" tone="success" /></div>}
              </ErpCard>;
            })}
          </div>
        )}
      ...
      {canApproveDispatchConfirmation && <ErpSection title="تأیید خروج راننده در گارد" description="تأیید مسیر جایگزین با احراز هویت تازه و لغو مجوز فقط در مالکیت گارد انجام می‌شود.">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-4"><ErpInput aria-label="شناسه نشست تأیید" value={confirmationSessionId} onChange={(event) => setConfirmationSessionId(event.target.value)} /><ErpInput aria-label="رمز عبور تازه گارد" type="password" value={guardPassword} onChange={(event) => setGuardPassword(event.target.value)} /><ErpInput aria-label="دلیل تأیید جایگزین یا لغو مجوز" value={queueReason} onChange={(event) => setQueueReason(event.target.value)} /><ErpButton label="تأیید گارد" disabled={dispatchTimelineStale || saving || !confirmationSessionId.trim() || !guardPassword} onClick={async () => { setSaving(true); setError(''); try { await dispatchConfirmationAPI.approveByGuard(confirmationSessionId.trim(), { password: guardPassword, reason: queueReason.trim() || undefined }); setMessage('تأیید گارد و مجوز خروج ثبت شد.'); setGuardPassword(''); } catch (err: any) { setError(err.response?.data?.error || 'تأیید گارد انجام نشد.'); } finally { setSaving(false); } }} /></div>
        <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2"><ErpInput aria-label="شناسه مجوز خروج برای لغو" value={authorizationToRevoke} onChange={(event) => setAuthorizationToRevoke(event.target.value)} /><ErpButton label="لغو مجوز توسط گارد" tone="danger" variant="outline" disabled={dispatchTimelineStale || saving || !authorizationToRevoke.trim() || !queueReason.trim()} onClick={async () => { setSaving(true); setError(''); try { await dispatchConfirmationAPI.revokeAuthorizationAsGuard(authorizationToRevoke.trim(), queueReason.trim()); setMessage('مجوز خروج توسط گارد لغو شد.'); setAuthorizationToRevoke(''); } catch (err: any) { setError(err.response?.data?.error || 'لغو مجوز انجام نشد.'); } finally { setSaving(false); } }} /></div>
      </ErpSection>}
      <RoleAwareDispatchCases workspace="security" onStaleChange={setDispatchTimelineStale} />
    </ErpWorkspacePage>
  );
}
