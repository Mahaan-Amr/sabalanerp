      ...
      <ErpListPage
        ...
        rowActions={(user) => {
          ...
          return [
            ...
            { label: 'حذف', onClick: () => handleDeleteUser(user), icon: FaTrash, tone: 'danger', disabled: disableDelete, title: user.id === currentUserId ? 'حذف حساب فعلی مجاز نیست' : currentUserRole !== 'ADMIN' ? 'فقط مدیر سیستم می‌تواند حساب را حذف کند' : 'حذف حساب و دسترسی‌های وابسته' },
          ];
        }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm">
          <ErpSection className="w-full max-w-md">
            ...
            <p className="text-sm leading-6 text-slate-600 dark:text-slate-300">
              آیا مطمئن هستید که می‌خواهید حساب کاربر{' '}
              <span className="font-semibold text-slate-900 dark:text-white">
                {userToDelete.firstName} {userToDelete.lastName}
              </span>{' '}
              را حذف کنید؟ حساب، پروفایل، مجوزها و اختیارهای جذب او حذف می‌شوند. پرونده پرسنلی و سوابق ممیزی یا عملیاتی حذف نمی‌شوند. این عمل قابل بازگشت نیست.
            </p>
