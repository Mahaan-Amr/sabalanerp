...
export default function WorkScheduleEditor({ value, onChange, readOnly = false }: { value: WorkScheduleValue; onChange: (value: WorkScheduleValue) => void; readOnly?: boolean }) {
  ...
  return <ErpSection title="ساعت کاری" description={readOnly ? "برنامه هفتگی ثبت‌شده برای این شخص" : "برنامه هفتگی پرسنل؛ برای حذف برنامه همه روزها را پاک کنید."}>
    <div className="space-y-5">
      <label className="block"><span className="mb-2 block text-sm font-medium">تاریخ اجرا</span><HrPersianCalendar value={value.effectiveDate} onChange={(effectiveDate) => onChange({ ...value, effectiveDate })} disablePastDates disabled={readOnly} /></label>
      <div><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><span className="text-sm font-medium">روزهای کاری</span>{!readOnly && <div className="flex flex-wrap gap-2"><ErpButton label="روزهای کاری" variant="outline" onClick={() => setDays([0, 1, 2, 3, 4, 5])} /><ErpButton label="انتخاب همه" variant="outline" onClick={() => setDays([0, 1, 2, 3, 4, 5, 6])} /><ErpButton label="پاک‌کردن ساعت کاری" tone="danger" variant="ghost" onClick={() => onChange({ ...value, days: [] })} /></div>}</div>
        <div className="grid grid-cols-4 gap-2 sm:grid-cols-7">{PERSIAN_WEEKDAYS.map((day, weekday) => <ErpPressable key={day} type="button" disabled={readOnly} aria-pressed={selected.has(weekday)} onClick={() => toggleDay(weekday)} tone={selected.has(weekday) ? 'primary' : 'neutral'} variant={selected.has(weekday) ? 'solid' : 'outline'} className="min-h-11 px-2 text-sm">{day}</ErpPressable>)}</div></div>
      ...
      {value.days.length > 0 && <>{!readOnly && <BulkTimes initialStart={bulkStart} initialEnd={bulkEnd} onApply={applyBulk} />}<div className="space-y-3"><p className="text-sm font-bold text-primary">جزئیات هر روز</p>{value.days.map((day) => <ErpCard key={day.weekday} className="grid items-end gap-3 p-3 sm:grid-cols-[120px_1fr_1fr]"><p className="pb-3 font-semibold">{PERSIAN_WEEKDAYS[day.weekday]}</p><label><span className="mb-1 block text-xs text-secondary">از</span><PersianTimePicker ariaLabel={`زمان شروع ${PERSIAN_WEEKDAYS[day.weekday]}`} value={day.startTime} onChange={(next) => updateDay(day.weekday, 'startTime', next)} disabled={readOnly} /></label><label><span className="mb-1 block text-xs text-secondary">تا</span><PersianTimePicker ariaLabel={`زمان پایان ${PERSIAN_WEEKDAYS[day.weekday]}`} value={day.endTime} onChange={(next) => updateDay(day.weekday, 'endTime', next)} disabled={readOnly} /></label><p className="col-span-full text-left text-xs text-secondary" dir="ltr">{formatTime12(day.startTime)} – {formatTime12(day.endTime)}</p></ErpCard>)}</div></>}
    </div>
  </ErpSection>;
}
