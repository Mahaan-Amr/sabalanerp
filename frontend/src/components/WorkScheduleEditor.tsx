...
export default function WorkScheduleEditor({ value, onChange }: { value: WorkScheduleValue; onChange: (value: WorkScheduleValue) => void }) {
  ...
  return <div className="space-y-5 rounded-xl border border-[var(--sds-border-default)] bg-[var(--sds-surface-subtle)] p-4 dark:border-[var(--sds-border-strong)] dark:bg-[var(--sds-surface-raised)]">
    ...
    <div><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><span className="text-sm font-medium">روزهای کاری</span><div className="flex flex-wrap gap-2"><ErpPressable type="button" className="sds-action px-3 py-2 text-xs" onClick={() => setDays([0, 1, 2, 3, 4, 5])}>روزهای کاری</ErpPressable><ErpPressable type="button" className="sds-action px-3 py-2 text-xs" onClick={() => setDays([0, 1, 2, 3, 4, 5, 6])}>انتخاب همه</ErpPressable><ErpPressable type="button" className="sds-action px-3 py-2 text-xs text-[var(--sds-danger)]" onClick={() => onChange({ ...value, days: [] })}>پاک‌کردن ساعت کاری</ErpPressable></div></div>
      <div className="grid grid-cols-4 gap-2 sm:grid-cols-7">{PERSIAN_WEEKDAYS.map((day, weekday) => <ErpPressable key={day} type="button" onClick={() => toggleDay(weekday)} className={`rounded-lg border px-2 py-3 text-sm transition ${selected.has(weekday) ? 'border-[var(--sds-border-strong)] bg-[var(--sds-accent)] text-[var(--sds-text-inverse)]' : 'border-[var(--sds-border-default)] bg-[var(--sds-surface-raised)] text-secondary dark:border-[var(--sds-border-strong)] dark:bg-[var(--sds-surface-raised)]'}`}>{day}</ErpPressable>)}</div></div>
    {value.days.length > 0 && <><BulkTimes initialStart={bulkStart} initialEnd={bulkEnd} onApply={applyBulk} /><div className="space-y-3"><p className="text-sm font-bold text-primary">جزئیات هر روز</p>{value.days.map((day) => <div key={day.weekday} className="grid items-end gap-3 rounded-lg border border-[var(--sds-border-default)] bg-[var(--sds-surface-raised)] p-3 sm:grid-cols-[120px_1fr_1fr] dark:border-[var(--sds-border-strong)] dark:bg-[var(--sds-surface-raised)]"><p className="pb-3 font-semibold">{PERSIAN_WEEKDAYS[day.weekday]}</p><label><span className="mb-1 block text-xs text-secondary">از</span><PersianTimePicker value={day.startTime} onChange={(next) => updateDay(day.weekday, 'startTime', next)} /></label><label><span className="mb-1 block text-xs text-secondary">تا</span><PersianTimePicker value={day.endTime} onChange={(next) => updateDay(day.weekday, 'endTime', next)} /></label><p className="col-span-full text-left text-xs text-secondary" dir="ltr">{formatTime12(day.startTime)} – {formatTime12(day.endTime)}</p></div>)}</div></>}
  </div>;
}
...
function BulkTimes({ initialStart, initialEnd, onApply }: { initialStart: string; initialEnd: string; onApply: (start: string, end: string) => void }) {
  ...
  return <div className="grid items-end gap-3 rounded-lg border border-[var(--sds-border-strong)] bg-[var(--sds-accent-surface)] p-3 sm:grid-cols-[1fr_1fr_auto] dark:border-[var(--sds-border-strong)] dark:bg-[var(--sds-accent-surface)]"><label><span className="mb-1 block text-xs font-medium">از برای روزهای انتخاب‌شده</span><PersianTimePicker value={startTime} onChange={setStartTime} /></label><label><span className="mb-1 block text-xs font-medium">تا برای روزهای انتخاب‌شده</span><PersianTimePicker value={endTime} onChange={setEndTime} /></label><ErpPressable type="button" className="sds-action sds-tone-primary sds-action-solid min-h-12 px-4" disabled={!startTime || !endTime} onClick={() => onApply(startTime, endTime)}>اعمال برای روزهای انتخاب‌شده</ErpPressable></div>;
}
