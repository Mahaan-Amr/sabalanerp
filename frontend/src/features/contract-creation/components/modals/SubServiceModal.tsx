...
// Sub-service/tool selection and configuration
...
interface SubServiceModalProps {
  ...
  setSelectedSubServices: React.Dispatch<React.SetStateAction<SubService[]>>;
  subServiceMeterValues: Record<string, number>;
  setSubServiceMeterValues: React.Dispatch<React.SetStateAction<Record<string, number>>>;
  subServiceCalculationBases: Record<string, 'length' | 'squareMeters'>;
  setSubServiceCalculationBases: React.Dispatch<React.SetStateAction<Record<string, 'length' | 'squareMeters'>>>;
  // Errors
...
export const SubServiceModal: React.FC<SubServiceModalProps> = ({
  ...
  if (!isOpen || productIndex === null) return null;
  ...
  // Modal content extracted from page.tsx lines 14016-14356
  // Content is in subservice_modal_fixed.txt - needs to be inserted here
  return (
