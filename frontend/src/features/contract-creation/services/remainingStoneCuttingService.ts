...
export const calculateRemainingChildCuttingBreakdown = ({
  ...
  return ([
    ...
    return meters > 0
      ? [{ type, meters, rate: safeRate, cost: Number(calculatePricing({
          policyVersion: 'pricing-v1', roundingPolicyVersion: 'rounding-v2',
          lines: [{ lineId: `${row.id}:${type}`, quantity: canonicalMeters, rateToman: parseCanonicalDecimal(String(safeRate)) }]
        }).totalAmountToman) }]
      : [];
  });
};
