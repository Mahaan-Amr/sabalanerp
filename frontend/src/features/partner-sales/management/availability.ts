...
export function actionPresentation(actions: readonly ActionAvailabilityV2[], action: PartnerActionV2, now: number) {
  ...
  return { enabled: value.enabled && !expired,
    reason: expired ? 'مهلت دسترسی به این اقدام پایان یافته است؛ وضعیت را تازه کنید.' : value.disabledReason ? getPartnerSalesErrorMessage(value.disabledReason) : (!value.enabled ? 'این اقدام فعلاً در دسترس نیست.' : undefined) };
}
