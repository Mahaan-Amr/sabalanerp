...
import { ErpEmptyState, ErpField, ErpInlineState, ErpInput, ErpLoading, ErpSelect, ErpToolbar } from '@/components/erp';
...
const initialFrom = () => { const value = new Date(); value.setDate(value.getDate() - 30); return tehranDate(value); };
...
export function PartnerReportRuntime() {
  ...
  if (pending && !report) return <><div className="grid gap-4 sm:grid-cols-2"><ErpField label="از"><ErpInput type="date" value={from} onChange={event => setFrom(event.target.value)} /></ErpField><ErpField label="تا"><ErpInput type="date" value={to} onChange={event => setTo(event.target.value)} /></ErpField></div>{filters}<ErpLoading /></>;
  if (!report) return <ErpInlineState kind="error" title={error || 'گزارش در دسترس نیست.'} action={{ label: 'تلاش مجدد', onClick: () => void load() }} />;
  return <div className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><ErpField label="از"><ErpInput type="date" value={from} onChange={event => setFrom(event.target.value)} /></ErpField><ErpField label="تا"><ErpInput type="date" value={to} onChange={event => setTo(event.target.value)} /></ErpField></div>{filters}
