...
export const hrAPI = {
  ...
  getFoundationDependencies: (entityType: string, id: string, params?: { purpose?: 'deletion' }) => api.get(`/hr/foundation/${entityType}/${id}/dependencies`, { params }),
  getFoundationDetail: (entityType: string, id: string) => api.get(`/hr/foundation/${entityType}/${id}/detail`),
  ...
  updateAssignmentPerformanceContext: (id: string, data: any) => api.post(`/hr/assignments/${id}/performance-context`, data),
...
export const accountingAPI = {
  ...
  reverseBankMatch: (id: string, reason: string) => api.post(`/accounting/ledger/treasury/bank-matches/${id}/reverse`, { reason }),
  recordInternalTreasuryTransfer: (data: any) => api.post('/accounting/ledger/treasury/internal-transfers', data),
  createReceivableCheck: (data: any) => api.post('/accounting/ledger/treasury/checks', data),
  transitionReceivableCheck: (id: string, data: any) => api.post(`/accounting/ledger/treasury/checks/${id}/events`, data),
  recordCashCount: (data: any) => api.post('/accounting/ledger/treasury/cash-counts', data),
  issuePettyCashAdvance: (data: any) => api.post('/accounting/ledger/treasury/petty-cash-advances', data),
  settlePettyCashAdvance: (id: string, data: any) => api.post(`/accounting/ledger/treasury/petty-cash-advances/${id}/settle`, data),
  configureTaxChannel: (data: any) => api.post('/accounting/ledger/tax-channels', data),
  createTaxRule: (data: any) => api.post('/accounting/ledger/tax-rules', data),
  bindTaxOutboxChannel: (id: string, channelId: string) => api.post(`/accounting/ledger/tax-outbox/${id}/channel`, { channelId }),
  reconcileVatPeriod: (data: any) => api.post('/accounting/ledger/tax/vat-reconciliations', data),
  getSupplyChainWorkspace: () => api.get('/accounting/supply-chain/workspace'),
  getSupplierAging: (id: string, asOf?: string) => api.get(`/accounting/supply-chain/suppliers/${id}/aging`, { params: { asOf } }),
  createSupplyChainPostingRule: (data: any) => api.post('/accounting/supply-chain/posting-rules', data),
  approveSupplierRelationship: (data: any) => api.post('/accounting/supply-chain/supplier-relationships', data),
  createSupplierInvoice: (data: any, idempotencyKey: string) => api.post('/accounting/supply-chain/supplier-invoices', data, { headers: { 'Idempotency-Key': idempotencyKey } }),
  createSupplierInvoiceCorrection: (id: string, data: any, idempotencyKey: string) => api.post(`/accounting/supply-chain/supplier-invoices/${id}/corrections`, data, { headers: { 'Idempotency-Key': idempotencyKey } }),
  createSupplierPayment: (data: any, idempotencyKey: string) => api.post('/accounting/supply-chain/supplier-payments', data, { headers: { 'Idempotency-Key': idempotencyKey } }),
  reverseSupplierAllocation: (id: string, data: any, idempotencyKey: string) => api.post(`/accounting/supply-chain/allocations/${id}/reversal`, data, { headers: { 'Idempotency-Key': idempotencyKey } }),
  recordAccountingInventoryMovement: (data: any) => api.post('/accounting/supply-chain/inventory/movements', data),
  reserveAccountingInventory: (data: any, idempotencyKey: string) => api.post('/accounting/supply-chain/inventory/reservations', data, { headers: { 'Idempotency-Key': idempotencyKey } }),
  releaseAccountingInventoryReservation: (id: string, data: any) => api.post(`/accounting/supply-chain/inventory/reservations/${id}/release`, data),
  consumeAccountingInventory: (data: any, idempotencyKey: string) => api.post('/accounting/supply-chain/inventory/consumptions', data, { headers: { 'Idempotency-Key': idempotencyKey } }),
  completeAccountingProduction: (data: any, idempotencyKey: string) => api.post('/accounting/supply-chain/production', data, { headers: { 'Idempotency-Key': idempotencyKey } }),
  issuePayableCheck: (data: any, idempotencyKey: string) => api.post('/accounting/supply-chain/checks', data, { headers: { 'Idempotency-Key': idempotencyKey } }),
  assignCustomerCheck: (data: any, idempotencyKey: string) => api.post('/accounting/supply-chain/checks/assigned-customer', data, { headers: { 'Idempotency-Key': idempotencyKey } }),
  ...
  getPeriodEndOverview: (bookId: string) => api.get('/accounting/period-end/overview', { params: { bookId } }),
  getTAccount: (params: { accountId: string; from: string; to: string; cutoffAt: string }) => api.get('/accounting/period-end/t-accounts', { params }),
  submitPeriodEndEvidence: (data: any) => api.post('/accounting/period-end/evidence', data),
  ...
  createPayrollSettlementVoucher: (id: string, data: any) => api.post(`/accounting/period-end/payroll-obligations/${id}/settlement-voucher`, data),
  ...
  recordTaxSubmissionAttempt: (id: string, data: any) => api.post(`/accounting/period-end/tax-obligations/${id}/attempts`, data),
  reconcileTaxObligation: (id: string, data: any) => api.post(`/accounting/period-end/tax-obligations/${id}/reconcile`, data),
  advanceCloseRun: (data: any) => api.post('/accounting/period-end/close-runs', data),
  finalizeCloseRun: (id: string, data: { confirmed: boolean; reason: string }) => api.post(`/accounting/period-end/close-runs/${id}/finalize`, data),
  createYearEndTransition: (id: string, data: { retainedResultAccountId: string; openingFiscalYearId: string; openingPeriodId: string; closingPeriodId?: string }) => api.post(`/accounting/period-end/close-runs/${id}/year-end-transition`, data),
  reopenCloseRun: (id: string, data: { confirmed: boolean; reason: string }) => api.post(`/accounting/period-end/close-runs/${id}/reopen`, data),
  createRestatementCase: (data: any) => api.post('/accounting/period-end/restatement-cases', data),
  archiveAccountingEvidence: (data: any) => api.post('/accounting/period-end/archive-evidence', data),
...
export const personnelPerformanceAPI = {
  ...
  analytics: (input?: { personnelIds?: string[]; reportingFrom?: string; reportingTo?: string }) => api.post('/hr/personnel-performance/analytics', input ?? {}),
  ranking: (input?: { personnelIds?: string[]; reportingFrom?: string; reportingTo?: string }) => api.post('/hr/personnel-performance/ranking', input ?? {}),
  ...
  consequenceHandoff: (handoffId: string) => api.get(`/hr/personnel-performance/consequence-handoffs/${encodeURIComponent(handoffId)}`),
  ...
  readinessRun: (runId: string) => api.get(`/hr/personnel-performance/readiness/${encodeURIComponent(runId)}`),
  retryReadiness: (runId: string, batchSize = 100) => api.post(`/hr/personnel-performance/readiness/${encodeURIComponent(runId)}/retry`, { batchSize }),
  supervisorSections: () => api.get('/hr/personnel-performance/supervisor/sections'),
  supervisorSection: (sectionId: string) => api.get(`/hr/personnel-performance/supervisor/sections/${encodeURIComponent(sectionId)}`),
  saveSupervisorDraft: (sectionId: string, payload: unknown) => api.put(`/hr/personnel-performance/supervisor/sections/${encodeURIComponent(sectionId)}/draft`, payload),
  submitSupervisorSection: (sectionId: string, idempotencyKey: string) => api.post(
    `/hr/personnel-performance/supervisor/sections/${encodeURIComponent(sectionId)}/submit`, undefined,
    { headers: { 'x-idempotency-key': idempotencyKey } },
  ),
  reviews: () => api.get('/hr/personnel-performance/reviews'),
  lifecycleSections: () => api.get('/hr/personnel-performance/lifecycle/sections'),
  review: (submissionId: string) => api.get(`/hr/personnel-performance/reviews/${encodeURIComponent(submissionId)}`),
  claimReview: (submissionId: string) => api.post(`/hr/personnel-performance/reviews/${encodeURIComponent(submissionId)}/claim`),
  decideReview: (submissionId: string, input: { decision: 'ACCEPTED' | 'REJECTED' | 'NOT_EVALUABLE'; reason: string; reasonCategory?: string; criterionVersionId?: string; evidenceReferenceId?: string }, idempotencyKey: string) => api.post(
    `/hr/personnel-performance/reviews/${encodeURIComponent(submissionId)}/decision`, input,
    { headers: { 'x-idempotency-key': idempotencyKey } },
  ),
  extendSection: (sectionId: string, input: { dueAt: string; reason: string }) => api.post(`/hr/personnel-performance/sections/${encodeURIComponent(sectionId)}/extend`, input),
  markSectionNotEvaluable: (sectionId: string, input: { reasonCategory: string; reason: string }, idempotencyKey: string) => api.post(
    `/hr/personnel-performance/sections/${encodeURIComponent(sectionId)}/not-evaluable`, input,
    { headers: { 'x-idempotency-key': idempotencyKey } },
  ),
