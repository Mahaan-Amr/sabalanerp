...
test("public login remains accessible at mobile width and 200% zoom in both themes", async ({
  page,
}) => {
...
test("login errors are Persian and successful login starts protected providers without 401 races", async ({
  page,
}) => {
  await page.goto("/login");
  await page
    .getByRole("textbox", { name: "ایمیل، نام کاربری یا شماره تماس" })
    .fill("invalid-e2e-user");
  await page.locator('input[type="password"]').fill("invalid-e2e-password");
  await page.getByRole("button", { name: "ورود", exact: true }).click();
  const loginError = page
    .getByRole("alert")
    .filter({ hasText: "نام کاربری یا رمز عبور نادرست است." });
  ...
  await page
    .getByRole("textbox", { name: "ایمیل، نام کاربری یا شماره تماس" })
    .fill(process.env.DESIGN_SYSTEM_E2E_ADMIN_USERNAME || "admin");
  await page
    .locator('input[type="password"]')
    .fill(process.env.DESIGN_SYSTEM_E2E_ADMIN_PASSWORD || "admin123");
  await page.getByRole("button", { name: "ورود", exact: true }).click();
...
test("Administration security actions use a pending-safe canonical dialog", async ({
  page,
}) => {
  ...
  await expect(
    dialog.getByRole("button", { name: "تأیید و ثبت" }),
  ).toBeDisabled();
  await assertNoSeriousAxeViolations(page);
...
test("HR work schedule and Persian time selection are RTL, keyboard, mobile, and zoom safe", async ({
  page,
}) => {
  await loginAsAdmin(page);
  ...
  if ((await saturday.getAttribute("aria-pressed")) !== "true")
...
test("Hiring lifecycle journey remains keyboard, reduced-motion, mobile, zoom, and theme safe", async ({
  page,
}) => {
  await loginAsAdmin(page);
  let finalRejectionPayload: Record<string, unknown> | undefined;
  await page.route(
    "**/api/hr-hiring/applications/*/final-rejection",
    async (route) => {
  ...
  await expect(
    page.getByRole("heading", { name: "جذب و پرونده‌های متقاضیان" }),
  ...
  await expect(page.getByText("مسیر جذب و شروع همکاری")).toBeVisible({
    timeout: 15_000,
  });
  await page
    .getByRole("button", { name: "رد نهایی پرونده", exact: true })
    .click();
  ...
  await rejectionDialog
    .getByRole("textbox", { name: "دلیل رد نهایی پرونده" })
    .fill("آزمون گذار کامل چرخه جذب");
  await rejectionDialog
    .getByRole("button", { name: "بستن پرونده و دسترسی" })
    .click();
  await expect(
    page
      .getByRole("status")
      .filter({ hasText: "رد نهایی ثبت و پرونده بسته شد." }),
  ).toBeVisible();
  expect(finalRejectionPayload?.reason).toBe("آزمون گذار کامل چرخه جذب");
  const phases = page
    .getByRole("list", { name: "مراحل جذب" })
    .first()
    .getByRole("button");
  ...
  await assertMinimumTargetSize(
    page.getByRole("list", { name: "مراحل جذب" }).last().getByRole("button"),
  );
  await setViewportAndZoom(page, { width: 780, height: 844 }, 2);
  await assertNoHorizontalOverflow(page);
});

test("User creation exposes canonical non-color permission state and responsive validation", async ({
  page,
}) => {
  await loginAsAdmin(page);
  ...
  await page.route("**/api/users", async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        data: {
          id: "created-user-1",
          permissionSummary: { workspacePermissions: 3 },
        },
      }),
    });
  });
  ...
  await expect(
    page.getByRole("heading", { name: "ایجاد کاربر جدید" }),
  ).toBeVisible({ timeout: 15_000 });
  ...
  await submit.click();
  await expect(
    page.getByRole("alert").filter({ hasText: "نام الزامی است" }),
  ).toBeVisible();
  ...
  releaseCreate?.();
  await expect(
    page.getByRole("status").filter({ hasText: "کاربر با موفقیت ایجاد شد" }),
  ).toBeVisible();
  await setViewportAndZoom(page, { width: 390, height: 844 });
  for (const theme of ["light", "dark"] as const) {
    await setTheme(page, theme);
...
test("Logistics Loading create and detail routes retain canonical responsive interaction", async ({
  page,
}) => {
  await loginAsAdmin(page);
  const loadingFixture = {
    id: "loading-1",
    loadingNumber: "LD-TEST-1",
    status: "DRAFT",
  ...
  await page.route("**/api/logistics/**", async (route) => {
    ...
    if (pathname.endsWith("/logistics/customers"))
      return reply([
        {
          id: "customer-1",
          customerName: "مشتری نمونه",
          companyName: "شرکت نمونه",
          loadableProjectCount: 1,
        },
      ]);
    if (pathname.endsWith("/logistics/drivers")) return reply([]);
    if (pathname.endsWith("/logistics/customers/customer-1/projects"))
      return reply([
        {
          id: "project-1",
    ...
    if (pathname.endsWith("/logistics/projects/project-1/draft"))
      return reply(loadingFixture, { resumed: false });
    if (pathname.endsWith("/logistics/projects/project-1/remaining"))
      return reply({ groups: [] });
    if (pathname.endsWith("/logistics/loadings/loading-1"))
      return reply(loadingFixture);
    return route.continue();
  });
  await page.goto("/dashboard/logistics/loadings/new");
  await expect(page.getByRole("heading", { name: "بارگیری جدید" })).toBeVisible(
    { timeout: 15_000 },
  );
  ...
  await expect(
    page.getByRole("heading", { name: "شماره بارگیری ثبت‌شده" }),
  ).toBeVisible({ timeout: 15_000 });
  await assertNoSeriousAxeViolations(page);
  await assertNoHorizontalOverflow(page);
  await setViewportAndZoom(page, { width: 780, height: 844 }, 2);
