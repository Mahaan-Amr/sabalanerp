...
test('Guard admits a canonical visit and advances it while legacy history remains read-only', async ({ page }) => {
  ...
  await expect(workspace.getByRole('heading', { name: 'رانندگان حاضر', exact: true })).toBeVisible();
  await workspace.getByRole('button', { name: 'پذیرش راننده', exact: true }).last().click();
  const admission = page.getByRole('dialog', { name: 'پذیرش راننده و خودرو' });
  await admission.getByRole('combobox', { name: 'راننده و خودروی آماده' }).selectOption('internal-1');
  await admission.getByRole('button', { name: 'ثبت پذیرش', exact: true }).click();
  await expect.poll(() => admissions).toBe(1);
  await expect(workspace.getByText(/راننده داخلی/)).toBeVisible();
  await workspace.getByRole('button', { name: 'ورود برای بارگیری', exact: true }).click();
  ...
  const closeWithoutLoading = page.getByRole('dialog', { name: 'خروج بدون بارگیری' });
  await closeWithoutLoading.getByRole('textbox', { name: 'دلیل' }).fill('پایان مراجعه آزمایشی');
  await closeWithoutLoading.getByRole('button', { name: 'تأیید و ثبت', exact: true }).click();
  await workspace.getByRole('button', { name: 'پذیرش راننده', exact: true }).last().click();
  const externalAdmission = page.getByRole('dialog', { name: 'پذیرش راننده و خودرو' });
  await externalAdmission.getByRole('button', { name: 'راننده متفرقه', exact: true }).click();
  await externalAdmission.getByRole('combobox', { name: 'راننده' }).selectOption('external-driver-1');
  await externalAdmission.getByRole('combobox', { name: 'خودرو' }).selectOption('external-vehicle-1');
  await externalAdmission.getByRole('button', { name: 'ثبت پذیرش', exact: true }).click();
  ...
  await confirmation.getByRole('textbox', { name: 'دلیل ابطال' }).fill('ثبت اشتباه برای آزمون');
  expect(voids).toBe(0);
  await confirmation.getByRole('button', { name: 'تأیید ابطال', exact: true }).click();
  await expect.poll(() => voids).toBe(1);
  await workspace.getByRole('button', { name: 'خروج مجاز', exact: true }).click();
  ...
  await workspace.getByRole('button', { name: 'پذیرش راننده', exact: true }).first().click();
  ...
  await workspace.getByRole('button', { name: 'سوابق قدیمی', exact: true }).click();
...
test('view-only Guard sees redacted queue state without mutation controls', async ({ page }) => {
  ...
  await expect(workspace.getByRole('button', { name: 'ورود برای بارگیری', exact: true })).toHaveCount(0);
  await expect(workspace.getByRole('button', { name: 'ابطال پذیرش', exact: true })).toHaveCount(0);
  await expect(workspace.getByRole('textbox', { name: 'دلیل' })).toHaveCount(0);
