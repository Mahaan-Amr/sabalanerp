...
import { checkTechnicalForms } from './technical-browser.mjs';
...
try {
  for (const theme of ['light', 'dark']) for (const width of [390, 1440]) {
    ...
    await checkTechnicalForms(page, output, theme, width);
    if (process.env.PARTNER330_TECHNICAL_ONLY === '1') {
      evidence.push({ theme, width, technicalPreview: true });
      await page.close();
      continue;
    }
    ...
    await page.getByRole('button', { name: 'بعدی', exact: true }).click();
    await page.getByRole('button', { name: 'بعدی', exact: true }).click();
    assert.equal(await page.getByRole('heading', { name: 'انتخاب محصولات', exact: true }).last().evaluate(element => element === document.activeElement), true);
    ...
    await page.getByRole('button', { name: 'بعدی', exact: true }).click();
    await page.getByRole('heading', { name: 'برنامه تحویل', exact: true }).last().waitFor();
    await page.getByRole('button', { name: 'بعدی', exact: true }).click();
    await page.getByRole('button', { name: 'بعدی', exact: true }).click();
    ...
    await page.getByRole('button', { name: 'استعلام جدید', exact: true }).click();
    const bulkDialog = page.getByRole('dialog');
    await bulkDialog.getByText('ردشده', { exact: true }).waitFor();
    assert.equal(await bulkDialog.getByRole('checkbox', { name: 'سنگ طولی آزمایشی' }).isChecked(), true);
    await page.keyboard.press('Escape');
    await bulkDialog.waitFor({ state: 'detached' });
    await page.getByRole('button', { name: 'استعلام مجدد', exact: true }).click();
    const dialog = page.getByRole('dialog');
    await dialog.getByRole('textbox', { name: 'یادداشت' }).fill('تغییر مشخصات سنگ');
